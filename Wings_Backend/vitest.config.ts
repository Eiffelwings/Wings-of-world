...
export default defineConfig({
  resolve: {
    // Keep this ordered: the base `mechanical-wings/plugin-sdk` alias is a prefix match.
    alias: [
      {
        find: "mechanical-wings/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      ...pluginSdkSubpaths.map((subpath) => ({
        find: `mechanical-wings/plugin-sdk/${subpath}`,
        replacement: path.join(repoRoot, "src", "plugin-sdk", `${subpath}.ts`),
      })),
      {
        find: "mechanical-wings/plugin-sdk",
