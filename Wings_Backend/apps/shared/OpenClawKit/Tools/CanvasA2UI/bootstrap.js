...
import "@a2ui/lit/ui";
import { themeContext } from "@mechanical-wings/a2ui-theme-context";
...
class WingsA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  themeProvider = new ContextProvider(this, {
    ...
    initialValue: mechanicalWingsTheme,
  });
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--mechanical-wings-a2ui-inset-top, 0px)
        var(--mechanical-wings-a2ui-inset-right, 0px)
        var(--mechanical-wings-a2ui-inset-bottom, 0px)
        var(--mechanical-wings-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--mechanical-wings-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--mechanical-wings-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--mechanical-wings-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--mechanical-wings-a2ui-empty-top, var(--mechanical-wings-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  connectedCallback() {
    ...
    globalThis.mechanicalWingsA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["mechanical-wings:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__mechanicalWingsLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.mechanicalWingsCanvasA2UIAction ??
      globalThis.mechanicalWingsCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.mechanicalWingsCanvasA2UIAction) {
...
if (!customElements.get("mechanical-wings-a2ui-host")) {
  customElements.define("mechanical-wings-a2ui-host", WingsA2UIHost);
}
