...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> Wings🪽ChatHistoryPayload
{
    Wings🪽ChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> Wings🪽ChatSessionEntry {
    Wings🪽ChatSessionEntry(
        key: key,
        kind: nil,
        displayName: nil,
        surface: nil,
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> Wings🪽ChatSessionEntry
{
    Wings🪽ChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> Wings🪽ChatModelChoice {
    Wings🪽ChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [Wings🪽ChatHistoryPayload],
    sessionsResponses: [Wings🪽ChatSessionsListResponse] = [],
    modelResponses: [[Wings🪽ChatModelChoice]] = [],
    ...
    -> (TestChatTransport, Wings🪽ChatViewModel)
{
...
private func sendUserMessage(_ vm: Wings🪽ChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, Wings🪽ChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [Wings🪽ChatHistoryPayload]
    private let sessionsResponses: [Wings🪽ChatSessionsListResponse]
    private let modelResponses: [[Wings🪽ChatModelChoice]]
    ...
    private let stream: AsyncStream<Wings🪽ChatTransportEvent>
    private let continuation: AsyncStream<Wings🪽ChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<Wings🪽ChatTransportEvent>.Continuation!
    ...
    func events() -> AsyncStream<Wings🪽ChatTransportEvent> {
        self.stream
    }

    func setActiveSessionKey(_: String) async throws {}

    func requestHistory(sessionKey: String) async throws -> Wings🪽ChatHistoryPayload {
        ...
        return self.historyResponses.last ?? Wings🪽ChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [Wings🪽ChatAttachmentPayload]) async throws -> Wings🪽ChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return Wings🪽ChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> Wings🪽ChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? Wings🪽ChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func listModels() async throws -> [Wings🪽ChatModelChoice] {
    ...
    func emit(_ evt: Wings🪽ChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = Wings🪽ChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = Wings🪽ChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: Wings🪽ChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = Wings🪽ChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: Wings🪽ChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(Wings🪽ChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == Wings🪽ChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = Wings🪽ChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: Wings🪽ChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = Wings🪽ChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = Wings🪽ChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = Wings🪽ChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { Wings🪽ChatViewModel(sessionKey: "main", transport: transport) }
