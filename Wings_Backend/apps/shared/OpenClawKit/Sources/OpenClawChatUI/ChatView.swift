...
@MainActor
public struct Wings🪽ChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    private var visibleMessages: [Wings🪽ChatMessage] {
        let base: [Wings🪽ChatMessage]
    ...
    private func mergeToolResults(in messages: [Wings🪽ChatMessage]) -> [Wings🪽ChatMessage] {
    ...
    private func isToolResultMessage(_ message: Wings🪽ChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: Wings🪽ChatMessage) -> Bool {
    ...
    private func primaryText(in message: Wings🪽ChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: Wings🪽ChatMessage) -> Bool {
    ...
    private func toolCalls(in message: Wings🪽ChatMessage) -> [Wings🪽ChatMessageContent] {
    ...
    private func inlineToolResults(in message: Wings🪽ChatMessage) -> [Wings🪽ChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: Wings🪽ChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: Wings🪽ChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(Wings🪽ChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
