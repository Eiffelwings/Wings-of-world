...
enum Wings🪽ChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("Wings🪽ChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("Wings🪽ChatTheme.onboardingAssistantBubble"),
...
enum Wings🪽PlatformImageFactory {
    static func image(_ image: Wings🪽PlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
