...
private let chatUILogger = Logger(subsystem: "ai.mechanical-wings", category: "Wings🪽ChatUI")

@MainActor
@Observable
public final class Wings🪽ChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [Wings🪽ChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [Wings🪽ChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [Wings🪽PendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [Wings🪽ChatPendingToolCall] = []
    public private(set) var sessions: [Wings🪽ChatSessionEntry] = []
    private let transport: any Wings🪽ChatTransport
    private var sessionDefaults: Wings🪽ChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: Wings🪽ChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any Wings🪽ChatTransport,
    ...
    public var sessionChoices: [Wings🪽ChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [Wings🪽ChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: Wings🪽PendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [Wings🪽ChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: Wings🪽ChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: Wings🪽ChatMessage) -> Wings🪽ChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> Wings🪽ChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return Wings🪽ChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: Wings🪽ChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: Wings🪽ChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: Wings🪽ChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [Wings🪽ChatMessage],
        incoming: [Wings🪽ChatMessage]) -> [Wings🪽ChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [Wings🪽ChatMessage]) -> [Wings🪽ChatMessage] {
        var result: [Wings🪽ChatMessage] = []
    ...
    private static func dedupeKey(for message: Wings🪽ChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [Wings🪽ChatMessageContent] = [
            Wings🪽ChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> Wings🪽ChatAttachmentPayload in
            Wings🪽ChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> Wings🪽ChatSessionEntry {
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = Wings🪽ChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: Wings🪽ChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: Wings🪽ChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: Wings🪽AgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = Wings🪽ChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> Wings🪽PlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
