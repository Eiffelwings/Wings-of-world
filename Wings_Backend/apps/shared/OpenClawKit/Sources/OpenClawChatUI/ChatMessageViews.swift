...
struct ChatMessageBubble: View {
    let message: Wings🪽ChatMessage
    let style: Wings🪽ChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? Wings🪽ChatTheme.userText : Wings🪽ChatTheme.assistantText
    ...
    private var inlineAttachments: [Wings🪽ChatMessageContent] {
    ...
    private var toolCalls: [Wings🪽ChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [Wings🪽ChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? Wings🪽ChatTheme.userBubble
        }
        if self.style == .onboarding {
            return Wings🪽ChatTheme.onboardingAssistantBubble
        }
        return Wings🪽ChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return Wings🪽ChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: Wings🪽ChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? Wings🪽ChatTheme.userText : Wings🪽ChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: Wings🪽ChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(Wings🪽ChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [Wings🪽ChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: Wings🪽ChatTheme.assistantText)
            }
        }
    }
}
