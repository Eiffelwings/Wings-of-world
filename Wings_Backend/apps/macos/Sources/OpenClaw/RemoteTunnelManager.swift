...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.mechanical-wings", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous Wings🪽 run already has an SSH listener on the expected port (common after restarts),
