...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: Wings🪽Protocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.mechanical-wings", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: Wings🪽Kit.AnyCodable]()) {
                $0[$1.key] = Wings🪽Kit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: Wings🪽Protocol.AnyCodable?) -> [String: Wings🪽Protocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: Wings🪽Protocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: Wings🪽Kit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: Wings🪽Protocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("mechanical-wings.control.heartbeat")
    static let controlAgentEvent = Notification.Name("mechanical-wings.control.agent")
}
