...
enum DebugActions {
    private static let verboseDefaultsKey = "mechanical-wings.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "mechanical-wings.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = Wings🪽Paths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Wings🪽", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = Wings🪽Paths.configURL
        guard
