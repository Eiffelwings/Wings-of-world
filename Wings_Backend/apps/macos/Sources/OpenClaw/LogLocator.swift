...
enum LogLocator {
    private static var logDir: URL {
        ...
        return URL(fileURLWithPath: "/tmp/mechanical-wings")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("mechanical-wings-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("mechanical-wings-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/mechanical-wings/ (rolling or stdout), or nil if none exist.
