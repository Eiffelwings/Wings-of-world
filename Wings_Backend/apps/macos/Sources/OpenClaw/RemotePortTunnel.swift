...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.mechanical-wings", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        let sshKey = Wings🪽ConfigFile.hostKey(sshHost)
        let urlKey = Wings🪽ConfigFile.hostKey(host)
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.mechanical-wings.remote.tunnel.port", qos: .utility)
            do {
