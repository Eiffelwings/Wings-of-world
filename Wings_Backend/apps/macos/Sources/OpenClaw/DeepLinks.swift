...
private let deepLinkLogger = Logger(subsystem: "ai.mechanical-wings", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Wings🪽 is paused", message: "Unpause Wings🪽 to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Wings🪽 agent?", message: body) else { return }
        }
