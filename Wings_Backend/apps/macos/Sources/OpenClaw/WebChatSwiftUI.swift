...
private let webChatSwiftLogger = Logger(subsystem: "ai.mechanical-wings", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "mechanical-wings.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: Wings🪽ChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> Wings🪽ChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(Wings🪽ChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            Wings🪽ChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return Wings🪽ChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    func sendMessage(
        ...
        attachments: [Wings🪽ChatAttachmentPayload]) async throws -> Wings🪽ChatSendResponse
    {
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> Wings🪽ChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                Wings🪽GatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: Wings🪽Protocol.ModelChoice) -> Wings🪽ChatModelChoice {
        Wings🪽ChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<Wings🪽ChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any Wings🪽ChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = Wings🪽ChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: Wings🪽ChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<Wings🪽ChatView>) -> NSViewController
    {
