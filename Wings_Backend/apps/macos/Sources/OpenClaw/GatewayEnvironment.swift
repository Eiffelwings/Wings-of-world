...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.mechanical-wings", category: "gateway.env")
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = Wings🪽ConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.mechanical-wingsExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "mechanical-wings CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "mechanical-wings@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "mechanical-wings@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "mechanical-wings@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "mechanical-wings@\(target)"])
            }

        statusHandler("Installing mechanical-wings@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed mechanical-wings@\(target)")
        } else {
