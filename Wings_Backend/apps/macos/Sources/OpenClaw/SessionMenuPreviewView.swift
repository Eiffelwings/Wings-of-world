...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.mechanical-wings", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> Wings🪽SessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: Wings🪽SessionPreviewEntry,
    ...
    private static func cache(payload: Wings🪽SessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: Wings🪽ChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [Wings🪽Kit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [Wings🪽Kit.AnyCodable]) -> [Wings🪽ChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(Wings🪽ChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: Wings🪽ChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: Wings🪽ChatMessage) -> Bool {
    ...
    private static func toolNames(for message: Wings🪽ChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: Wings🪽ChatMessage) -> String? {
