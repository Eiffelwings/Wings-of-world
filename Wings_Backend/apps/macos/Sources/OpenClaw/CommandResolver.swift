...
enum CommandResolver {
    private static let projectRootDefaultsKey = "mechanical-wings.gatewayProjectRootPath"
    private static let helperName = "mechanical-wings"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let mechanical-wingsEntry = root.appendingPathComponent("mechanical-wings.mjs").path
        if FileManager().isReadableFile(atPath: mechanical-wingsEntry) { return mechanical-wingsEntry }
        let binEntry = root.appendingPathComponent("bin/mechanical-wings.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/mechanical-wings")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let mechanical-wingsPaths = self.mechanical-wingsManagedPaths(home: home)
        if !mechanical-wingsPaths.isEmpty {
            extras.insert(contentsOf: mechanical-wingsPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + mechanical-wingsPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func mechanical-wingsManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".mechanical-wings"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func mechanical-wingsExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectWings🪽Executable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("mechanical-wings.mjs").path,
            root.appendingPathComponent("bin/mechanical-wings.js").path,
        ]
    ...
    static func hasAnyWings🪽Invoker(searchPaths: [String]? = nil) -> Bool {
        if self.mechanical-wingsExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func mechanical-wingsNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        let root = self.projectRoot()
        if let mechanical-wingsPath = self.projectWings🪽Executable(projectRoot: root) {
            return [mechanical-wingsPath, subcommand] + extraArgs
        }
        if let mechanical-wingsPath = self.mechanical-wingsExecutable(searchPaths: searchPaths) {
            return [mechanical-wingsPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "mechanical-wings", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            mechanical-wings entrypoint missing (looked for dist/index.js or mechanical-wings.mjs); run pnpm build.
            """
    ...
    static func mechanical-wingsCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
        searchPaths: [String]? = nil) -> [String]
    {
        self.mechanical-wingsNodeCommand(
            subcommand: subcommand,
            extraArgs: extraArgs,
            defaults: defaults,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real mechanical-wings CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/mechanical-wings"
        ...
        \(cliSection)
        \(projectSection)
        if command -v mechanical-wings >/dev/null 2>&1; then
          CLI="$(command -v mechanical-wings)"
          mechanical-wings \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/mechanical-wings.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/mechanical-wings.mjs"
            node "$PRJ/mechanical-wings.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/mechanical-wings.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/mechanical-wings.js"
            node "$PRJ/bin/mechanical-wings.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent mechanical-wings"
          pnpm --silent mechanical-wings \(quotedArgs);
        else
          echo "mechanical-wings CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? Wings🪽ConfigFile.loadDict()
