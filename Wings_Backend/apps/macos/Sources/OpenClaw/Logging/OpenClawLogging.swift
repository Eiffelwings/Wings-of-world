...
enum Wings🪽Logging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = Wings🪽OSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = Wings🪽FileLogHandler(label: label)
    ...
    static func parseLabel(_ label: String) -> (String, String) {
        guard let range = label.range(of: labelSeparator) else {
            return ("ai.mechanical-wings", label)
        }
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        Wings🪽Logging.bootstrapIfNeeded()
        let label = Wings🪽Logging.makeLabel(subsystem: subsystem, category: category)
...
struct Wings🪽OSLogHandler: AppLogLevelBackedHandler {
...
struct Wings🪽FileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = Wings🪽Logging.parseLabel(self.label)
