...
struct MacGatewayChatTransportMappingTests {
    @Test func `snapshot maps to health`() {
        let snapshot = Snapshot(
            presence: [],
            health: Wings🪽Protocol.AnyCodable(["ok": Wings🪽Protocol.AnyCodable(false)]),
    ...
    @Test func `health event maps to health`() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: Wings🪽Protocol.AnyCodable(["ok": Wings🪽Protocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func `chat event maps to chat`() {
        let payload = Wings🪽Protocol.AnyCodable([
            "runId": Wings🪽Protocol.AnyCodable("run-1"),
            "sessionKey": Wings🪽Protocol.AnyCodable("main"),
            "state": Wings🪽Protocol.AnyCodable("final"),
        ])
    ...
    @Test func `unknown event maps to nil`() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: Wings🪽Protocol.AnyCodable(["a": Wings🪽Protocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
