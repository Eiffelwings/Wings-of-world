...
struct VoiceWakeRuntimeTests {
    @Test func `trims after trigger keeps post speech`() {
        let triggers = ["claude", "mechanical-wings"]
    ...
    @Test func `has content after trigger false when only trigger`() {
        let triggers = ["mechanical-wings"]
        let text = "hey mechanical-wings"
    ...
    @Test func `trims after chinese trigger keeps post speech`() {
        let triggers = ["小爪", "mechanical-wings"]
    ...
    @Test func `gate requires gap between trigger and command`() {
        let transcript = "hey mechanical-wings do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["mechanical-wings"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "mechanical-wings", start: 0.2, duration: 0.1, range: transcript.range(of: "mechanical-wings")),
