...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    private func voiceWakeChangedEvent(payload: Wings🪽Protocol.AnyCodable) -> EventFrame {
        EventFrame(
            type: "event",
    ...
    @Test func `applies voice wake changed event to app state`() async {
        ...
        let evt = self.voiceWakeChangedEvent(payload: Wings🪽Protocol.AnyCodable(["triggers": [
            "mechanical-wings",
            "computer",
        ]]))
        ...
        #expect(updated == ["mechanical-wings", "computer"])
    ...
    @Test func `ignores voice wake changed event with invalid payload`() async {
        let previous = await applyTriggersAndCapturePrevious(["before"])
        let evt = self.voiceWakeChangedEvent(payload: Wings🪽Protocol.AnyCodable(["unexpected": 123]))
