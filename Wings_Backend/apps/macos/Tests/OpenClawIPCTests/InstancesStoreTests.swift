...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: Wings🪽Protocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: Wings🪽Protocol.AnyCodable] = [
            "presence": .init([Wings🪽Protocol.AnyCodable(entry)]),
        ]
        let payload = Wings🪽Protocol.AnyCodable(payloadMap)
