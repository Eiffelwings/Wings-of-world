...
private const val NOTIFICATION_CHANNEL_BASE_ID = "mechanical-wings.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Wings🪽 Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Wings🪽 Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Wings🪽 Active")
      }
