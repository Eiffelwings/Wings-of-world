...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this Wings🪽 Android gateway connection failure.

    Please:
    - pick one route only: same machine, same LAN, Tailscale, or public URL
    - classify this as pairing/auth, TLS trust, wrong advertised route, wrong address/port, or gateway down
    - quote the exact app status/error below
    - tell me whether `mechanical-wings devices list` should show a pending pairing request
    - if more signal is needed, ask for `mechanical-wings qr --json`, `mechanical-wings devices list`, and `mechanical-wings nodes status`
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Wings🪽 gateway diagnostics", report))
