  ...
  fun refreshNodeCanvasCapability_sendsObjectParamsAndUpdatesScopedUrl() = runBlocking {
    ...
    val server =
      startGatewayServer(json) { webSocket, id, method, frame ->
        when (method) {
          "connect" -> {
            webSocket.send(connectResponseFrame(id, canvasHostUrl = "http://127.0.0.1/__mechanical-wings__/cap/old-cap"))
          }
    ...
    try {
      ...
      assertEquals(
        "http://127.0.0.1:${server.port}/__mechanical-wings__/cap/new-cap",
    ...
    session.connect(
      ...
      options =
        GatewayConnectOptions(
          ...
          client =
            GatewayClientInfo(
              id = "mechanical-wings-android-test",
