...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<Wings🪽TalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.mechanical-wings", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = Wings🪽TalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> Wings🪽TalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return Wings🪽TalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> Wings🪽TalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> Wings🪽TalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return Wings🪽TalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> Wings🪽TalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = Wings🪽TalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: Wings🪽TalkPTTStopPayload) {
