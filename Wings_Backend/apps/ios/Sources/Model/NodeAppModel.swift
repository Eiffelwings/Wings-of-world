...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.mechanical-wings.ios", category: "DeepLink")
    private let pushWakeLogger = Logger(subsystem: "ai.mechanical-wings.ios", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "ai.mechanical-wings.ios", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "ai.mechanical-wings.ios", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.mechanical-wings.ios", category: "WatchReply")
    enum CameraHUDKind {
        case photo
        case recording
        case success
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = Wings🪽CanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = Wings🪽CanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = Wings🪽CanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = Wings🪽CanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = Wings🪽CanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: Wings🪽LocationMode) async -> Bool {
        ...
        self.gatewayHealthMonitor.start(
            check: { [weak self] in
                ...
                do {
                    ...
                    guard let decoded = try? JSONDecoder().decode(Wings🪽GatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: Wings🪽NodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: Wings🪽NodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: Wings🪽NodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(Wings🪽LocationGetParams.self, from: req.paramsJSON)) ??
            Wings🪽LocationGetParams()
        ...
        let payload = Wings🪽LocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case Wings🪽CanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(Wings🪽CanvasPresentParams.self, from: req.paramsJSON)) ??
                Wings🪽CanvasPresentParams()
        ...
        case Wings🪽CanvasCommand.hide.rawValue:
            self.screen.showDefaultCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case Wings🪽CanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(Wings🪽CanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case Wings🪽CanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(Wings🪽CanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case Wings🪽CanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(Wings🪽CanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case Wings🪽CanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.mechanical-wingsA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing mechanical-wingsA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

        case Wings🪽CanvasA2UICommand.push.rawValue, Wings🪽CanvasA2UICommand.pushJSONL.rawValue:
            let messages: [Wings🪽Kit.AnyCodable]
            if command == Wings🪽CanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(Wings🪽CanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try Wings🪽CanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(Wings🪽CanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try Wings🪽CanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.mechanical-wingsA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case Wings🪽CameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case Wings🪽CameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(Wings🪽CameraSnapParams.self, from: req.paramsJSON)) ??
                Wings🪽CameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case Wings🪽CameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(Wings🪽CameraClipParams.self, from: req.paramsJSON)) ??
                Wings🪽CameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(Wings🪽ScreenRecordParams.self, from: req.paramsJSON)) ??
            Wings🪽ScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(Wings🪽SystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: Wings🪽NodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard finalStatus == .authorized || finalStatus == .provisional || finalStatus == .ephemeral else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: Wings🪽NodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: Wings🪽NodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(Wings🪽ChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: Wings🪽NodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = Wings🪽ChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case Wings🪽DeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case Wings🪽DeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(Wings🪽PhotosLatestParams.self, from: req.paramsJSON)) ??
            Wings🪽PhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case Wings🪽ContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(Wings🪽ContactsSearchParams.self, from: req.paramsJSON)) ??
                Wings🪽ContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case Wings🪽ContactsCommand.add.rawValue:
            let params = try Self.decodeParams(Wings🪽ContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case Wings🪽CalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(Wings🪽CalendarEventsParams.self, from: req.paramsJSON)) ??
                Wings🪽CalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case Wings🪽CalendarCommand.add.rawValue:
            let params = try Self.decodeParams(Wings🪽CalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case Wings🪽RemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(Wings🪽RemindersListParams.self, from: req.paramsJSON)) ??
                Wings🪽RemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case Wings🪽RemindersCommand.add.rawValue:
            let params = try Self.decodeParams(Wings🪽RemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case Wings🪽MotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(Wings🪽MotionActivityParams.self, from: req.paramsJSON)) ??
                Wings🪽MotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case Wings🪽MotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(Wings🪽PedometerParams.self, from: req.paramsJSON)) ??
                Wings🪽PedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case Wings🪽TalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case Wings🪽TalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case Wings🪽TalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case Wings🪽TalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
private extension NodeAppModel {
    ...
    func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([Wings🪽LocationCommand.get.rawValue]) { [weak self] req in
        ...
        register([Wings🪽ScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([Wings🪽SystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([Wings🪽ChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            Wings🪽WatchCommand.status.rawValue,
            Wings🪽WatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([Wings🪽PhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            Wings🪽RemindersCommand.list.rawValue,
            Wings🪽RemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            Wings🪽TalkCommand.pttStart.rawValue,
            Wings🪽TalkCommand.pttStop.rawValue,
            Wings🪽TalkCommand.pttCancel.rawValue,
            Wings🪽TalkCommand.pttOnce.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case Wings🪽WatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = Wings🪽WatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case Wings🪽WatchCommand.notify.rawValue:
            let params = try Self.decodeParams(Wings🪽WatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = Wings🪽WatchNotifyPayload(
    ...
    func locationMode() -> Wings🪽LocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return Wings🪽LocationMode(rawValue: raw) ?? .off
    }
...
private extension NodeAppModel {
                            ...
                            if let requestId, !requestId.isEmpty {
                                self.gatewayStatusText =
                                    "Pairing required (requestId: \(requestId)). "
                                        + "Approve on gateway and return to Wings🪽."
                            } else {
                                self.gatewayStatusText =
                                    "Pairing required. Approve on gateway and return to Wings🪽."
                            }
                        }
    ...
    func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "mechanical-wings-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "Wings🪽 Share Self-Test",
            url: URL(string: "https://mechanical-wings.ai/share-self-test"),
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.mechanical-wingsPushKind(userInfo)
        let receivedMessage =
    ...
    private static func mechanical-wingsPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        if let payload = userInfo["mechanical-wings"] as? [String: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        }
        if let payload = userInfo["mechanical-wings"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
