...
final class ContactsService: ContactsServicing {
    ...
    func search(params: Wings🪽ContactsSearchParams) async throws -> Wings🪽ContactsSearchPayload {
        ...
        return Wings🪽ContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: Wings🪽ContactsAddParams) async throws -> Wings🪽ContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return Wings🪽ContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return Wings🪽ContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> Wings🪽ContactPayload {
        Wings🪽ContactPayload(
