...
@MainActor
final class Wings🪽AppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.mechanical-wings.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.mechanical-wings.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.mechanical-wings.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "mechanical-wings.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "mechanical-wings.watch.promptId"
    static let sessionKeyKey = "mechanical-wings.watch.sessionKey"
    static let actionPrimaryIDKey = "mechanical-wings.watch.action.primary.id"
    static let actionPrimaryLabelKey = "mechanical-wings.watch.action.primary.label"
    static let actionSecondaryIDKey = "mechanical-wings.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "mechanical-wings.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "mechanical-wings.watch.action.primary"
    static let actionSecondaryIdentifier = "mechanical-wings.watch.action.secondary"
    static let actionIdentifierPrefix = "mechanical-wings.watch.action."
    static let actionIDKeyPrefix = "mechanical-wings.watch.action.id."
    static let actionLabelKeyPrefix = "mechanical-wings.watch.action.label."
    static let categoryPrefix = "mechanical-wings.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: Wings🪽WatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> Wings🪽WatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return Wings🪽WatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Wings🪽" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [Wings🪽WatchAction]) -> [UNNotificationAction] {
...
@main
struct Wings🪽App: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(Wings🪽AppDelegate.self) private var appDelegate
...
extension Wings🪽App {
    private static func installUncaughtExceptionLogger() {
        NSLog("Wings🪽: installing uncaught exception handler")
