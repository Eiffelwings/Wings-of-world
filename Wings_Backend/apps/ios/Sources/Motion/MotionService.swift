...
final class MotionService: MotionServicing {
    func activities(params: Wings🪽MotionActivityParams) async throws -> Wings🪽MotionActivityPayload {
        ...
        let mapped: [Wings🪽MotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return Wings🪽MotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: Wings🪽PedometerParams) async throws -> Wings🪽PedometerPayload {
        ...
        let payload: Wings🪽PedometerPayload = try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = Wings🪽PedometerPayload(
