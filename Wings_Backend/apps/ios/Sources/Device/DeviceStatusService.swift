...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> Wings🪽DeviceStatusPayload {
        ...
        return Wings🪽DeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> Wings🪽DeviceInfoPayload {
        ...
        return Wings🪽DeviceInfoPayload(
    ...
    private func batteryStatus() -> Wings🪽BatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: Wings🪽BatteryState = switch device.batteryState {
        ...
        return Wings🪽BatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> Wings🪽ThermalStatusPayload {
        let state: Wings🪽ThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return Wings🪽ThermalStatusPayload(state: state)
    }

    private func storageStatus() -> Wings🪽StorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return Wings🪽StorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
