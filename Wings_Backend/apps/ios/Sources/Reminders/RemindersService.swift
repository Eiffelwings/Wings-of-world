...
final class RemindersService: RemindersServicing {
    func list(params: Wings🪽RemindersListParams) async throws -> Wings🪽RemindersListPayload {
        ...
        let payload: [Wings🪽ReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return Wings🪽RemindersListPayload(reminders: payload)
    }
    ...
    func add(params: Wings🪽RemindersAddParams) async throws -> Wings🪽RemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = Wings🪽ReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return Wings🪽RemindersAddPayload(reminder: payload)
    }
