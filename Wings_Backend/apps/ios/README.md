# Wings🪽 iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an Wings🪽 Gateway as a `role: node`.
...
- Beta release uses canonical `ai.mechanical-wings.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `Wings🪽PushTransport=relay`, `Wings🪽PushDistribution=official`, and `Wings🪽PushAPNsEnvironment=production`.
...
- `apps/ios/Sources/Wings🪽.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `Wings🪽PushTransport=direct` and `Wings🪽PushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- Debug builds default to `Wings🪽PushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `mechanical-wings.json`. `OPENCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Wings🪽 official builds and to ensure a
