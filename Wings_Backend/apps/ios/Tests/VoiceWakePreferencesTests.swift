...
@Suite struct VoiceWakePreferencesTests {
    @Test func sanitizeTriggerWordsTrimsAndDropsEmpty() {
        #expect(VoiceWakePreferences.sanitizeTriggerWords([" mechanical-wings ", "", " \nclaude\t"]) == ["mechanical-wings", "claude"])
    }
    ...
    @Test func displayStringUsesSanitizedWords() {
        #expect(VoiceWakePreferences.displayString(for: ["", " "]) == "mechanical-wings, claude")
    }
