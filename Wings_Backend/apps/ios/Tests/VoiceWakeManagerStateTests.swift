...
@Suite(.serialized) struct VoiceWakeManagerStateTests {
    ...
    @Test @MainActor func handleRecognitionCallbackDispatchesCommand() async {
        let manager = VoiceWakeManager()
        manager.triggerWords = ["mechanical-wings"]
        manager.isEnabled = true
        ...
        let transcript = "mechanical-wings hello"
        let triggerRange = transcript.range(of: "mechanical-wings")!
        let helloRange = transcript.range(of: "hello")!
        let segments = [
            WakeWordSegment(text: "mechanical-wings", start: 0.0, duration: 0.2, range: triggerRange),
