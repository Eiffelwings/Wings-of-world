 ...
 * `mechanical-wings update`. Use it to verify the completion installation flow
...
const CLI_NAME = "mechanical-wings";
...
\`mechanical-wings update\`, \`mechanical-wings doctor\`, and \`mechanical-wings onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Wings🪽 Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`mechanical-wings completion --install\` later to enable.`));
    return;
  }
