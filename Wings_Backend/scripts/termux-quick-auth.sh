...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/mechanical-wings/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Wings🪽 Re-Auth" -c "After getting key, run: ssh $SERVER '~/mechanical-wings/scripts/mobile-reauth.sh'" --id mechanical-wings-auth
        ;;
    *)
