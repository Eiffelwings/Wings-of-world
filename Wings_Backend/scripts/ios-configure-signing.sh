...
normalize_bundle_id() {
  ...
  if [[ -z "$raw" ]]; then
    raw="ai.mechanical-wings.ios.test.local"
  fi
  printf '%s\n' "$raw"
}
...
if [[ -z "${bundle_base}" ]]; then
  bundle_base="ai.mechanical-wings.ios.test.${bundle_suffix}"
fi
...
tmp_file="$(mktemp "${TMPDIR:-/tmp}/mechanical-wings-ios-signing.XXXXXX")"
cat >"${tmp_file}" <<EOF
