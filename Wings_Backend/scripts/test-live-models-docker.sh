...
if [[ -n "${OPENCLAW_DOCKER_AUTH_DIRS:-}" ]]; then
  ...
  while IFS= read -r auth_dir; do
    ...
    {
      mechanical-wings_live_collect_auth_dirs_from_csv "${OPENCLAW_LIVE_PROVIDERS:-}"
      mechanical-wings_live_collect_auth_dirs_from_csv "${OPENCLAW_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
