...
const EXPECTED_REPOSITORY_URL = "https://github.com/mechanical-wings/mechanical-wings";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "mechanical-wings") {
    errors.push(`package.json name must be "mechanical-wings"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.["mechanical-wings"] !== "mechanical-wings.mjs") {
    errors.push(
      `package.json bin.mechanical-wings must be "mechanical-wings.mjs"; found "${pkg.bin?.["mechanical-wings"] ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`mechanical-wings-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `mechanical-wings-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
