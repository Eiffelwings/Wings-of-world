...
#
# Suggested cron: */30 * * * * /home/admin/mechanical-wings/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Wings🪽 notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., mechanical-wings-alerts)
...
STATE_FILE="$HOME/.mechanical-wings/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Wings🪽 if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use mechanical-wings
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Wings🪽 to $NOTIFY_PHONE..."
            mechanical-wings send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Wings🪽 is down. Run: ssh l36 '~/mechanical-wings/scripts/mobile-reauth.sh'" "urgent"
    exit 1
