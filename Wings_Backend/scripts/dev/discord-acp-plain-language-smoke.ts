...
type DriverMode = "token" | "webhook" | "mechanical-wings";
...
function resolveStateDir(): string {
  ...
  return path.join(home, ".mechanical-wings");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver mechanical-wings] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) Wings🪽 spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|mechanical-wings> Driver transport mode (default: token)\n" +
    ...
    "  --mechanical-wings-bin <path>        Wings🪽 CLI binary for driver=mechanical-wings (default: mechanical-wings)\n" +
...
function parseArgs(): Args {
  ...
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "mechanical-wings"
        ? "mechanical-wings"
  ...
  const threadBindingsPath =
    ...
    defaultBindingsPath;
  const mechanicalWingsBin =
    resolveArg("--mechanical-wings-bin") || process.env.OPENCLAW_DISCORD_SMOKE_OPENCLAW_BIN || "mechanical-wings";
...
async function mechanicalWingsCliJson<T>(params: { mechanicalWingsBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.mechanicalWingsBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`mechanical-wings ${params.args.join(" ")} returned empty stdout`);
  }
...
async function readMessagesWithOpenclaw(params: {
  ...
  const response = await mechanicalWingsCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    mechanicalWingsBin: params.mechanicalWingsBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "mechanical-wings") {
    return await readMessagesWithOpenclaw({
      mechanicalWingsBin: params.args.mechanicalWingsBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `mechanical-wings-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await mechanicalWingsCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
        ...
        mechanicalWingsBin: args.mechanicalWingsBin,
        args: [
          "message",
          "send",
          "--channel",
      ...
      if (!sentMessageId) {
        throw new Error("mechanical-wings message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "mechanical-wings"
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from Wings🪽.`,
        diagnostics: {
          bindingCandidates: [
            {
