...
const warningFilterKey = Symbol.for("mechanical-wings.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadWingsPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadWingsPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@mechanical-wings/build-smoke-plugin",
      type: "module",
      "mechanical-wings": {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'mechanical-wings/plugin-sdk';",
