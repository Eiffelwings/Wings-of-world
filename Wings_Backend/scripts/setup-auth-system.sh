#!/bin/bash
# Setup Wings🪽 Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'mechanical-wings-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Wings🪽 message: Send warning via Wings🪽 itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/mechanical-wings-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/mechanical-wings-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/mechanical-wings-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now mechanical-wings-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status mechanical-wings-auth-monitor.timer"
echo ""
echo "Quick commands:"
