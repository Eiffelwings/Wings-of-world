#!/usr/bin/env bash
# ClawDock - Docker helpers for Wings🪽
# Inspired by Simon Willison's "Running Wings🪽 in Docker"
# https://til.simonwillison.net/llms/mechanical-wings-docker
#
# Installation:
...
# Common paths to check for Wings🪽
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/mechanical-wings"
  "${HOME}/workspace/mechanical-wings"
  "${HOME}/projects/mechanical-wings"
  "${HOME}/dev/mechanical-wings"
  "${HOME}/code/mechanical-wings"
  "${HOME}/src/mechanical-wings"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🪽 Found Wings🪽 at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/mechanical-wings"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Wings🪽 not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/mechanical-wings/mechanical-wings.git ~/mechanical-wings"
    echo "  cd ~/mechanical-wings && ./scripts/docker/setup.sh"
    echo ""
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d mechanical-wings-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart mechanical-wings-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f mechanical-wings-gateway
}
...
clawdock-workspace() {
  cd ~/.mechanical-wings/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec mechanical-wings-gateway \
    bash -c 'echo "alias mechanical-wings=\"./mechanical-wings.mjs\"" > /tmp/.bashrc_mechanical-wings && bash --rcfile /tmp/.bashrc_mechanical-wings'
}
...
clawdock-cli() {
  _clawdock_compose run --rm mechanical-wings-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build mechanical-wings-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "OPENCLAW_GATEWAY_TOKEN=$token" mechanical-wings-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" mechanical-wings-gateway \
    bash -c './mechanical-wings.mjs config set gateway.remote.token "$TOKEN" && ./mechanical-wings.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec mechanical-wings-gateway \
    bash -c "./mechanical-wings.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart mechanical-wings-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm mechanical-wings-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec mechanical-wings-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'mechanical-wings config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🪽 ClawDock - Docker Helpers for Wings🪽${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (mechanical-wings alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to mechanical-wings project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.mechanical-wings)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'mechanical-wings channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'mechanical-wings status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.wings-of-world.ai${_CLR_RESET}"
  echo ""
}
