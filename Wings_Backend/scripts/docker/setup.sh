...
IMAGE_NAME="${OPENCLAW_IMAGE:-mechanical-wings:local}"
...
read_config_gateway_token() {
  local config_path="$OPENCLAW_CONFIG_DIR/mechanical-wings.json"
...
run_prestart_cli() {
  # During setup, avoid the shared-network mechanical-wings-cli service because it
  ...
  run_prestart_gateway --entrypoint node mechanical-wings-gateway \
    dist/index.js "$@"
}
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" mechanical-wings-cli "$@"
}
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$HOME/.mechanical-wings}"
OPENCLAW_WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-$HOME/.mechanical-wings/workspace}"
...
if [[ -z "${OPENCLAW_GATEWAY_TOKEN:-}" ]]; then
  ...
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    ...
    echo "Reusing gateway token from $OPENCLAW_CONFIG_DIR/mechanical-wings.json"
  else
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${OPENCLAW_CONFIG_DIR}:/home/node/.mechanical-wings"
    gateway_workspace_mount="${OPENCLAW_WORKSPACE_DIR}:/home/node/.mechanical-wings/workspace"
...
if [[ "$IMAGE_NAME" == "mechanical-wings:local" ]]; then
...
# After fixing the config dir, only the Wings🪽 metadata subdirectory
# (.mechanical-wings/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh mechanical-wings-gateway -c \
  'find /home/node/.mechanical-wings -xdev -exec chown node:node {} +; \
   [ -d /home/node/.mechanical-wings/workspace/.mechanical-wings ] && chown -R node:node /home/node/.mechanical-wings/workspace/.mechanical-wings || true'
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm mechanical-wings-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm mechanical-wings-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm mechanical-wings-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.wings-of-world.ai/channels"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d mechanical-wings-gateway
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  ...
  if [[ -f "$ROOT_DIR/Dockerfile.sandbox" ]]; then
    echo "Building sandbox image: mechanical-wings-sandbox:bookworm-slim"
    docker build \
      -t "mechanical-wings-sandbox:bookworm-slim" \
  ...
  if ! docker compose "${COMPOSE_ARGS[@]}" run --rm --entrypoint docker mechanical-wings-gateway --version >/dev/null 2>&1; then
    ...
    echo "  or use a local build (OPENCLAW_IMAGE=mechanical-wings:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Wings🪽 config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.wings-of-world.ai/gateway/sandboxing"
    ...
    docker compose "${BASE_COMPOSE_ARGS[@]}" up -d --force-recreate mechanical-wings-gateway
  fi
else
...
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f mechanical-wings-gateway"
echo "  ${COMPOSE_HINT} exec mechanical-wings-gateway node dist/index.js health --token \"$OPENCLAW_GATEWAY_TOKEN\""
