...
function collectPluginSourceEntries(packageJson) {
  const extensionConfig = packageJson?.["mechanical-wings"] ?? packageJson?.openclaw;
  let packageEntries = Array.isArray(extensionConfig?.extensions)
    ? extensionConfig.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof extensionConfig?.setupEntry === "string" && extensionConfig.setupEntry.trim().length > 0
      ? extensionConfig.setupEntry
      : undefined;
  if (setupEntry) {
...
export function collectBundledPluginBuildEntries(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = fs.existsSync(path.join(pluginDir, "mechanical-wings.plugin.json"))
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasPackageJson, sourceEntries } of entries) {
    artifacts.add(`dist/extensions/${id}/mechanical-wings.plugin.json`);
    if (hasPackageJson) {
