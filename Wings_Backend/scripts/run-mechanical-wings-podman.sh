#!/usr/bin/env bash
# Rootless Wings🪽 in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-mechanical-wings-podman.sh launch        # Start gateway
#   ./scripts/run-mechanical-wings-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   mechanical-wings --container mechanical-wings dashboard --no-open
#   mechanical-wings --container mechanical-wings channels login
#
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-mechanical-wings-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.mechanical-wings}"
...
CONTAINER_NAME="${OPENCLAW_PODMAN_CONTAINER:-mechanical-wings}"
OPENCLAW_IMAGE="${OPENCLAW_PODMAN_IMAGE:-${OPENCLAW_IMAGE:-mechanical-wings:local}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v mechanical-wings >/dev/null 2>&1; then
    echo "Warning: mechanical-wings not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    OPENCLAW_CONTAINER="" OPENCLAW_CONFIG_DIR="$config_dir" \
      mechanical-wings config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    OPENCLAW_CONTAINER="" OPENCLAW_CONFIG_DIR="$config_dir" \
      mechanical-wings config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  OPENCLAW_CONTAINER="" OPENCLAW_CONFIG_DIR="$config_dir" \
    mechanical-wings config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
CONFIG_JSON="$CONFIG_DIR/mechanical-wings.json"
...
if [[ "$RUN_SETUP" == true ]]; then
  ...
  podman run --pull="$PODMAN_PULL" --rm -it \
    ...
    -e NPM_CONFIG_CACHE=/home/node/.mechanical-wings/.npm \
    -e OPENCLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.mechanical-wings:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.mechanical-wings/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$OPENCLAW_IMAGE" \
...
echo "Host CLI: mechanical-wings --container $CONTAINER_NAME dashboard --no-open"
...
if [[ "$PLATFORM_NAME" == "Darwin" ]]; then
  ...
  echo "  OPENCLAW_CONTAINER=$CONTAINER_NAME mechanical-wings dashboard --no-open"
  echo "  One-time setup:"
  echo "    OPENCLAW_CONTAINER=$CONTAINER_NAME mechanical-wings config set gateway.controlUi.allowedOrigins '[\"http://127.0.0.1:18789\",\"http://localhost:18789\",\"http://127.0.0.1:28889\",\"http://localhost:28889\"]' --strict-json"
