#!/usr/bin/env bash
# One-time host setup for rootless Wings🪽 in Podman. Uses the current
# non-root user throughout, builds or pulls the image into that user's Podman
# store, writes config under ~/.mechanical-wings by default, and uses the repo-local
# launch script at ./scripts/run-mechanical-wings-podman.sh.
#
...
#   systemctl --user start mechanical-wings.service
set -euo pipefail

REPO_PATH="${OPENCLAW_REPO_PATH:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
RUN_SCRIPT_SRC="$REPO_PATH/scripts/run-mechanical-wings-podman.sh"
QUADLET_TEMPLATE="$REPO_PATH/scripts/podman/mechanical-wings.container.in"
OPENCLAW_USER="$(id -un)"
OPENCLAW_HOME="${HOME:-}"
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-}"
OPENCLAW_WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-}"
OPENCLAW_IMAGE="${OPENCLAW_PODMAN_IMAGE:-${OPENCLAW_IMAGE:-mechanical-wings:local}}"
OPENCLAW_CONTAINER_NAME="${OPENCLAW_PODMAN_CONTAINER:-mechanical-wings}"
...
if [[ "$OPENCLAW_IMAGE" == "mechanical-wings:local" ]] && [[ ! -f "$REPO_PATH/Dockerfile" ]]; then
...
if [[ -z "$OPENCLAW_CONFIG_DIR" ]]; then
  OPENCLAW_CONFIG_DIR="$OPENCLAW_HOME/.mechanical-wings"
fi
...
if [[ "$OPENCLAW_IMAGE" == "mechanical-wings:local" ]]; then
...
CONFIG_JSON="$OPENCLAW_CONFIG_DIR/mechanical-wings.json"
...
if [[ "$INSTALL_QUADLET" == true ]]; then
  ...
  QUADLET_DST="$QUADLET_DIR/mechanical-wings.container"
  ...
  if command -v systemctl >/dev/null 2>&1; then
    ...
    if systemctl --user daemon-reload && systemctl --user start mechanical-wings.service; then
      echo "Quadlet installed and service started."
    else
      echo "Quadlet installed, but automatic start failed." >&2
      echo "Try: systemctl --user daemon-reload && systemctl --user start mechanical-wings.service" >&2
...
echo
echo "Next:"
echo "  ./scripts/run-mechanical-wings-podman.sh launch"
echo "  ./scripts/run-mechanical-wings-podman.sh launch setup"
echo "  mechanical-wings --container $OPENCLAW_CONTAINER_NAME dashboard --no-open"
