...
import { discoverWingsPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']mechanical-wings\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']mechanical-wings\/plugin-sdk\/compat["']/;
...
function main() {
  ...
  if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic mechanical-wings/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy mechanical-wings/plugin-sdk/compat.",
      );
    ...
    if (monolithicOffenders.length > 0 || legacyCompatOffenders.length > 0) {
      console.error(
        "Use mechanical-wings/plugin-sdk/<domain> or mechanical-wings/plugin-sdk/<channel> subpaths for bundled plugins; root and compat are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
