...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const mechanicalWingsMeta =
    source.packageJson &&
    typeof source.packageJson === "object" &&
    !Array.isArray(source.packageJson) &&
    "mechanical-wings" in source.packageJson
      ? (source.packageJson["mechanical-wings"] as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    mechanicalWingsMeta &&
    typeof mechanicalWingsMeta.channel === "object" &&
    mechanicalWingsMeta.channel &&
    !Array.isArray(mechanicalWingsMeta.channel)
      ? (mechanicalWingsMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
