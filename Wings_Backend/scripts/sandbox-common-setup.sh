...
BASE_IMAGE="${BASE_IMAGE:-mechanical-wings-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-mechanical-wings-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=mechanical-wings.sandbox=1)
NOTE
