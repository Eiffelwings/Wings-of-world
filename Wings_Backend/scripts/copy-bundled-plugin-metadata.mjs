...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "mechanical-wings.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "mechanical-wings.plugin.json");
    ...
    if (packageJson["mechanical-wings"] && "extensions" in packageJson["mechanical-wings"]) {
      packageJson["mechanical-wings"] = {
        ...packageJson["mechanical-wings"],
        extensions: rewritePackageExtensions(packageJson["mechanical-wings"].extensions),
        ...(typeof packageJson["mechanical-wings"].setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson["mechanical-wings"].setupEntry) }
          : {}),
      };
    }
