...
CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-$HOME/.mechanical-wings}"
WORKSPACE_DIR="${OPENCLAW_WORKSPACE_DIR:-$HOME/.mechanical-wings/workspace}"
PROFILE_FILE="${OPENCLAW_PROFILE_FILE:-$HOME/.profile}"
CLI_TOOLS_DIR="${OPENCLAW_DOCKER_CLI_TOOLS_DIR:-$HOME/.cache/mechanical-wings/docker-cli-tools}"
...
if [[ -n "${OPENCLAW_DOCKER_AUTH_DIRS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(mechanical-wings_live_collect_auth_dirs)
else
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(mechanical-wings_live_collect_auth_dirs_from_csv "$CLI_PROVIDER")
fi
AUTH_DIRS_CSV="$(mechanical-wings_live_join_csv "${AUTH_DIRS[@]}")"
...
docker run --rm -t \
  ...
  -v "$ROOT_DIR":/src:ro \
  -v "$CONFIG_DIR":/home/node/.mechanical-wings \
  -v "$WORKSPACE_DIR":/home/node/.mechanical-wings/workspace \
