...
const RUNTIME_SECURITY_OVERRIDES = {
...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson["mechanical-wings"]?.bundle?.stageRuntimeDependencies === true;
}
...
function sanitizeBundledManifestForRuntimeInstall(pluginDir) {
  ...
  if (packageJson.peerDependencies?.["mechanical-wings"]) {
    const nextPeerDependencies = { ...packageJson.peerDependencies };
    delete nextPeerDependencies["mechanical-wings"];
  ...
  if (packageJson.peerDependenciesMeta?.["mechanical-wings"]) {
    const nextPeerDependenciesMeta = { ...packageJson.peerDependenciesMeta };
    delete nextPeerDependenciesMeta["mechanical-wings"];
  ...
  if (packageJson.devDependencies?.["mechanical-wings"]) {
    const nextDevDependencies = { ...packageJson.devDependencies };
    delete nextDevDependencies["mechanical-wings"];
  ...
  if (shouldStageRuntimeDeps(packageJson) && hasRuntimeDeps(packageJson)) {
    const nextOverrides = {
      ...packageJson.overrides,
      ...RUNTIME_SECURITY_OVERRIDES,
    };
    if (JSON.stringify(packageJson.overrides ?? {}) !== JSON.stringify(nextOverrides)) {
...
function resolveRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".mechanical-wings-runtime-deps-stamp.json");
}
  ...
  if (platform === "win32") {
    ...
    throw new Error(
      `failed to resolve a toolchain-local npm next to ${execPath}. ` +
        ...
        "Wings🪽 refuses to shell out to bare npm on Windows; install a Node.js toolchain that bundles npm or run with a matching Node installation.",
    );
  }
