...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/mechanical-wings-parallels-linux.XXXXXX)"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/mechanical-wings-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  ...
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/mechanical-wings-install.sh
  guest_exec /usr/bin/env OPENCLAW_NO_ONBOARD=1 bash /tmp/mechanical-wings-install.sh "${version_args[@]}" --no-onboard
  guest_exec mechanical-wings --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec mechanical-wings --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" mechanical-wings onboard \
    --non-interactive \
    --mode local \
...
verify_local_turn() {
  guest_exec /usr/bin/env "OPENAI_API_KEY=$OPENAI_API_KEY_VALUE" mechanical-wings agent \
    --local \
    --agent main \
    --message ping \
    --json
