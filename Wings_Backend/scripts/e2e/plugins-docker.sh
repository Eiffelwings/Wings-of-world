...
IMAGE_NAME="mechanical-wings-plugins-e2e"
...
home_dir=$(mktemp -d "/tmp/mechanical-wings-plugins-e2e.XXXXXX")
...
JS
  cat > "$dir/mechanical-wings.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/mechanical-wings.plugin.json" "$id"
const fs = require("node:fs");
...
mkdir -p "$HOME/.mechanical-wings/extensions/demo-plugin"

cat > "$HOME/.mechanical-wings/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.mechanical-wings/extensions/demo-plugin/mechanical-wings.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
pack_dir="$(mktemp -d "/tmp/mechanical-wings-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@mechanical-wings/demo-plugin-tgz",
  "version": "0.0.1",
  "mechanical-wings": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/mechanical-wings.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
dir_plugin="$(mktemp -d "/tmp/mechanical-wings-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@mechanical-wings/demo-plugin-dir",
  "version": "0.0.1",
  "mechanical-wings": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/mechanical-wings.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
file_pack_dir="$(mktemp -d "/tmp/mechanical-wings-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@mechanical-wings/demo-plugin-file",
  "version": "0.0.1",
  "mechanical-wings": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/mechanical-wings.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
bundle_root="$HOME/.mechanical-wings/extensions/claude-bundle-e2e"
...
const configPath = path.join(process.env.HOME, ".mechanical-wings", "mechanical-wings.json");
...
gateway_log="/tmp/mechanical-wings-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/mechanical-wings-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@mechanical-wings/slash-install-plugin",
  "version": "0.0.1",
  "mechanical-wings": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/mechanical-wings.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "mechanical-wings/fixture-marketplace"
    }
  }
}
JSON
