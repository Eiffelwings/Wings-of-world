...
IMAGE_NAME="mechanical-wings-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/mechanical-wings-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/mechanical-wings-bin/trash
  ...
  run_wizard_cmd() {
    ...
    set_isolated_mechanical-wings_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/mechanical-wings-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/mechanical-wings-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/mechanical-wings-e2e-$1.XXXXXX"
  }

  set_isolated_mechanical-wings_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export OPENCLAW_HOME="$home_dir"
    export OPENCLAW_STATE_DIR="$home_dir/.mechanical-wings"
    export OPENCLAW_CONFIG_PATH="$OPENCLAW_STATE_DIR/mechanical-wings.json"
