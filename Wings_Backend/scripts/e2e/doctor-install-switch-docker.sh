...
IMAGE_NAME="mechanical-wings-doctor-install-switch-e2e"
...
docker run --rm -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0 "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/mechanical-wings-bin:$PATH"
  mkdir -p /tmp/mechanical-wings-bin

  cat > /tmp/mechanical-wings-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/mechanical-wings-bin/systemctl

  cat > /tmp/mechanical-wings-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/mechanical-wings-bin/loginctl
  ...
  npm install -g --prefix /tmp/npm-prefix "/app/$pkg_tgz"
	  ...
	  npm_bin="/tmp/npm-prefix/bin/mechanical-wings"
	  npm_root="/tmp/npm-prefix/lib/node_modules/mechanical-wings"
	  ...
	  git_cli="/app/mechanical-wings.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/mechanical-wings-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/mechanical-wings-gateway.service"
    if [ ! -f "$unit_path" ]; then
