...
INSTALL_URL="https://wings-of-world.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_OPENCLAW_BIN="/opt/homebrew/bin/mechanical-wings"
GUEST_OPENCLAW_ENTRY="/opt/homebrew/lib/node_modules/mechanical-wings/mechanical-wings.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/mechanical-wings-parallels-smoke.XXXXXX)"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://wings-of-world.ai/install.sh
...
send -- "cat >/tmp/mechanical-wings-prl.sh <<'__OPENCLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__OPENCLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/mechanical-wings-prl.sh; rc=\$?; rm -f /tmp/mechanical-wings-prl.sh; printf '__OPENCLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1
...
curl -fsSL $install_url_q -o /tmp/mechanical-wings-install.sh
bash /tmp/mechanical-wings-install.sh${version_arg_q}
...
start_server() {
  ...
  ) >/tmp/mechanical-wings-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_bundle_permissions() {
  ...
  cmd="$(cat <<EOF
root=\$($npm_q root -g); check_path() { local path="\$1"; [ -e "\$path" ] || return 0; local perm perm_oct; perm=\$(/usr/bin/stat -f '%OLp' "\$path"); perm_oct=\$((8#\$perm)); if (( perm_oct & 0002 )); then echo "world-writable install artifact: \$path (\$perm)" >&2; exit 1; fi; }; check_path "\$root/mechanical-wings"; check_path "\$root/mechanical-wings/extensions"; if [ -d "\$root/mechanical-wings/extensions" ]; then while IFS= read -r -d '' extension_dir; do check_path "\$extension_dir"; done < <(/usr/bin/find "\$root/mechanical-wings/extensions" -mindepth 1 -maxdepth 1 -type d -print0); fi
EOF
)"
...
resolve_dashboard_url() {
  ...
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from mechanical-wings dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL "\$dashboard_http_url" >/tmp/mechanical-wings-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>Wings🪽 Control</title>' /tmp/mechanical-wings-dashboard-smoke.html >/dev/null; then
      if grep -F '<mechanical-wings-app></mechanical-wings-app>' /tmp/mechanical-wings-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>Wings🪽 Control</title>' /tmp/mechanical-wings-dashboard-smoke.html >/dev/null
grep -F '<mechanical-wings-app></mechanical-wings-app>' /tmp/mechanical-wings-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/mechanical-wings-discord-token <<'__OPENCLAW_TOKEN__'
$DISCORD_TOKEN_VALUE
__OPENCLAW_TOKEN__
cat >/tmp/mechanical-wings-discord-guilds.json <<'__OPENCLAW_GUILDS__'
$guilds_json
__OPENCLAW_GUILDS__
token="\$(tr -d '\n' </tmp/mechanical-wings-discord-token)"
guilds_json="\$(cat /tmp/mechanical-wings-discord-guilds.json)"
...
rm -f /tmp/mechanical-wings-discord-token /tmp/mechanical-wings-discord-guilds.json
EOF
)"
  prlctl exec "$VM_NAME" --current-user /usr/bin/env \
    PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "mechanical-wings-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "mechanical-wings-main-upgrade.tgz"
