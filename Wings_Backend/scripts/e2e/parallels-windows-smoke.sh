...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-mechanical-wings-native-e2e-2026-03-12"
MODE="both"
OPENAI_API_KEY_ENV="OPENAI_API_KEY"
INSTALL_URL="https://wings-of-world.ai/install.ps1"
HOST_PORT="18426"
...
MINGIT_ZIP_NAME=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/mechanical-wings-parallels-windows.XXXXXX)"
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/mechanical-wings-parallels-build.lock"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-mechanical-wings-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --openai-api-key-env <var> Host env var name for OpenAI API key.
                             Default: OPENAI_API_KEY
  --install-url <url>        Installer URL for latest release. Default: https://wings-of-world.ai/install.ps1
  ...
  --target-package-spec <npm-spec>
                             ...
                             Example: mechanical-wings@2026.3.13-beta.1
...
\$mechanical-wings = Join-Path \$env:APPDATA 'npm\mechanical-wings.cmd'
\$args = $args_literal
if ('${env_name_q}' -ne '') {
  Set-Item -Path ('Env:' + '${env_name_q}') -Value '${env_value_q}'
}
# mechanical-wings.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$mechanical-wings @args 2>&1
if (\$null -ne \$output) {
...
resolve_latest_version() {
  ...
  npm view mechanical-wings version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "mechanical-wings-parallels-smoke",
...
ensure_guest_git() {
  ...
  guest_exec cmd.exe /d /s /c "if exist \"%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\" rmdir /s /q \"%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "if not exist \"%LOCALAPPDATA%\\Wings🪽\\deps\" mkdir \"%LOCALAPPDATA%\\Wings🪽\\deps\""
  guest_exec cmd.exe /d /s /c "mkdir \"%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "curl.exe -fsSL \"$mingit_url\" -o \"%TEMP%\\$MINGIT_ZIP_NAME\""
  guest_exec cmd.exe /d /s /c "tar.exe -xf \"%TEMP%\\$MINGIT_ZIP_NAME\" -C \"%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\""
  guest_exec cmd.exe /d /s /c "del /q \"%TEMP%\\$MINGIT_ZIP_NAME\" & set \"PATH=%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\\usr\\bin;%PATH%\" && git.exe --version"
}
...
pack_main_tgz() {
  ...
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/mechanical-wings-main-$short_head.tgz"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/mechanical-wings-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
& (Join-Path \$env:APPDATA 'npm\mechanical-wings.cmd') --version
EOF
)"
...
install_main_tgz() {
  ...
  run_windows_retry "main tgz install" 2 \
    guest_exec cmd.exe /d /s /c "set \"PATH=%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\\cmd;%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\\mingw64\\bin;%LOCALAPPDATA%\\Wings🪽\\deps\\portable-git\\usr\\bin;%PATH%\" && curl.exe -fsSL \"$tgz_url\" -o \"%TEMP%\\$temp_name\" && npm.cmd install -g \"%TEMP%\\$temp_name\" --no-fund --no-audit && \"%APPDATA%\\npm\\mechanical-wings.cmd\" --version"
}

verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_mechanical-wings "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  ...
  runner_name="mechanical-wings-onboard-$RANDOM-$RANDOM.ps1"
  log_name="mechanical-wings-onboard-$RANDOM-$RANDOM.log"
  done_name="mechanical-wings-onboard-$RANDOM-$RANDOM.done"
...
try {
  \$mechanical-wings = Join-Path \$env:APPDATA 'npm\mechanical-wings.cmd'
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice openai-api-key --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --accept-risk --json > "{1}" 2>&1' -f \$mechanical-wings, \$log)
...
verify_gateway() {
  guest_run_mechanical-wings "" "" gateway status --deep --require-rpc
}

show_gateway_status_compat() {
  if guest_run_mechanical-wings "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_mechanical-wings "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_mechanical-wings "" "" gateway status --deep
}

verify_turn() {
  guest_run_mechanical-wings "" "" agent --agent main --message "Reply with exact ASCII text OK only." --json
}
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "mechanical-wings-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "mechanical-wings-main-upgrade.tgz" || return $?
