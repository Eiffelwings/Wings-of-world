...
# Build and bundle Wings🪽 into a minimal .app we can open.
# Outputs to dist/Wings🪽.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Wings🪽.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Wings🪽"
BUNDLE_ID="${BUNDLE_ID:-ai.mechanical-wings.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/mechanical-wings/mechanical-wings/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Wings🪽/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :Wings🪽BuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :Wings🪽GitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Wings🪽"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Wings🪽"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Wings🪽"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Wings🪽" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Wings🪽/Resources/Wings🪽.icns" "$APP_ROOT/Contents/Resources/Wings🪽.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Wings🪽/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying Wings🪽Kit resources"
OPENCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/Wings🪽Kit_Wings🪽Kit.bundle"
if [ -d "$OPENCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/Wings🪽Kit_Wings🪽Kit.bundle"
  cp -R "$OPENCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/Wings🪽Kit_Wings🪽Kit.bundle"
else
  echo "WARN: Wings🪽Kit resource bundle not found at $OPENCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Wings🪽"
killall -q Wings🪽 2>/dev/null || true
