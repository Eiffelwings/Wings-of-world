...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("mechanical-wings/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("mechanical-wings/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced mechanical-wings/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced mechanical-wings/plugin-sdk/<subpath> must exist in the public package exports.",
  );
