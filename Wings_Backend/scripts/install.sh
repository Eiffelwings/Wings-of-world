...
# Wings🪽 Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one Wings🪽."
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🪽 Wings🪽 Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://wings-of-world.ai/install.ps1 | iex"
        exit 1
    fi
...
show_footer_links() {
    local faq_url="https://docs.wings-of-world.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_mechanical-wings_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.mechanical-wings-* "$npm_root"/mechanical-wings 2>/dev/null || true
}

extract_mechanical-wings_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_mechanical-wings_bin_conflict() {
    local bin_path="$1"
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/mechanical-wings" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/mechanical-wings"|"/usr/local/bin/mechanical-wings")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/mechanical-wings/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale mechanical-wings symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing mechanical-wings binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing Wings🪽 package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_mechanical-wings_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "Wings🪽 npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*mechanical-wings" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_mechanical-wings_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_mechanical-wings_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "Wings🪽 npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an mechanical-wings binary already exists"
            if [[ -n "$conflict" ]]; then
...
GIT_DIR_DEFAULT="${HOME}/mechanical-wings"
...
print_usage() {
    cat <<EOF
Wings🪽 installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/mechanical-wings)
...
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Wings🪽 checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Wings🪽 source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_mechanical-wings_checkout() {
    local dir="$1"
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"mechanical-wings"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://wings-of-world.ai/install.sh | bash"
}
...
ensure_mechanical-wings_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/mechanical-wings" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/mechanical-wings" ]]; then
        ln -sf "$npm_root/mechanical-wings/dist/entry.js" "${npm_bin}/mechanical-wings"
        ui_info "Created mechanical-wings bin link at ${npm_bin}/mechanical-wings"
    fi
    return 0
}

# Check for existing Wings🪽 installation
check_existing_mechanical-wings() {
    if [[ -n "$(type -P mechanical-wings 2>/dev/null || true)" ]]; then
        ui_info "Existing Wings🪽 installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make mechanical-wings show as \"command not found\" in new terminals."
...
warn_mechanical-wings_not_found() {
    ui_warn "Installed, but mechanical-wings is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t mechanical-wings 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named mechanical-wings; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_mechanical-wings_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P mechanical-wings 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/mechanical-wings" ]]; then
        echo "${npm_bin}/mechanical-wings"
        return 0
    fi
...
install_mechanical-wings_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/mechanical-wings/mechanical-wings.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing Wings🪽 from git checkout: ${repo_dir}"
    else
        ui_info "Installing Wings🪽 from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning Wings🪽" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building Wings🪽" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/mechanical-wings" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/mechanical-wings"
    ui_success "Wings🪽 wrapper installed to \$HOME/.local/bin/mechanical-wings"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view mechanical-wings dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    local value="$2"
    if [[ "${value,,}" == "main" ]]; then
        echo "github:mechanical-wings/mechanical-wings#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_mechanical-wings() {
    local package_name="mechanical-wings"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
    ...
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing Wings🪽 v${resolved_version}"
    else
        ui_info "Installing Wings🪽 (${OPENCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${OPENCLAW_VERSION}")"

    if ! install_mechanical-wings_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_mechanical-wings_paths
        install_mechanical-wings_npm "${install_spec}"
    fi

    if [[ "${OPENCLAW_VERSION}" == "latest" && "${package_name}" == "mechanical-wings" ]]; then
        if ! resolve_mechanical-wings_bin &> /dev/null; then
            ui_warn "npm install mechanical-wings@latest failed; retrying mechanical-wings@next"
            cleanup_npm_mechanical-wings_paths
            install_mechanical-wings_npm "mechanical-wings@next"
        fi
    fi

    ensure_mechanical-wings_bin_link || true
...
run_doctor() {
    ...
    if [[ -z "$claw" ]]; then
        claw="$(resolve_mechanical-wings_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (mechanical-wings not on PATH yet)"
        warn_mechanical-wings_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    ...
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.mechanical-wings/workspace-${profile}"
    else
        echo "${HOME}/.mechanical-wings/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${OPENCLAW_CONFIG_PATH:-$HOME/.mechanical-wings/mechanical-wings.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run mechanical-wings onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but mechanical-wings not on PATH; skipping onboarding"
        warn_mechanical-wings_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run mechanical-wings onboard to retry"
        return
    }
}
...
if ! declare -F extract_mechanical-wings_semver >/dev/null 2>&1; then
# Inline fallback when version-parse.sh could not be sourced (for example, stdin install).
extract_mechanical-wings_semver() {
    local raw="${1:-}"
    local parsed=""
    parsed="$(
        printf '%s\n' "$raw" \
...
resolve_mechanical-wings_version() {
    local version=""
    local raw_version_output=""
    local claw="${OPENCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v mechanical-wings &> /dev/null; then
        claw="$(command -v mechanical-wings)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_mechanical-wings_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/mechanical-wings/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/mechanical-wings/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: mechanical-wings not on PATH yet"
        warn_mechanical-wings_not_found
        return 1
    fi

    run_quiet_step "Checking Wings🪽 version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: mechanical-wings gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_mechanical-wings_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found Wings🪽 checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
    ...
    if check_existing_mechanical-wings; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing Wings🪽"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g mechanical-wings &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g mechanical-wings 2>/dev/null || true
        ...
        install_mechanical-wings_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/mechanical-wings" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/mechanical-wings"
    ...
    OPENCLAW_BIN="$(resolve_mechanical-wings_bin || true)"
    ...
    installed_version=$(resolve_mechanical-wings_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🪽 Wings🪽 installed successfully (${installed_version})!"
    else
        ui_celebrate "🪽 Wings🪽 installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/mechanical-wings"
        ui_kv "Update command" "mechanical-wings update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running mechanical-wings doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
            ...
            ui_info "No TTY; run mechanical-wings doctor and mechanical-wings plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run mechanical-wings onboard later"
        else
            local config_path="${OPENCLAW_CONFIG_PATH:-$HOME/.mechanical-wings/mechanical-wings.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (mechanical-wings not on PATH yet)"
                    warn_mechanical-wings_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run mechanical-wings onboard to finish setup"
            return 0
        fi
    fi

    if command -v mechanical-wings &> /dev/null; then
        local claw="${OPENCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_mechanical-wings_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (mechanical-wings daemon restart)"
            else
                ...
                if OPENCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ...
                    ui_warn "Gateway restart failed; try: mechanical-wings daemon restart"
                fi
            fi
        fi
    fi
