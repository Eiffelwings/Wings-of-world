...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson["mechanical-wings"]) ? packageJson["mechanical-wings"] : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left["mechanical-wings"]?.channel?.id) || left.name;
    const rightId = trimString(right["mechanical-wings"]?.channel?.id) || right.name;
