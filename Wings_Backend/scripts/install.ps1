# Wings🪽 Installer for Windows (PowerShell)
# Usage: iwr -useb https://wings-of-world.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://wings-of-world.ai/install.ps1))) -NoOnboard

param(
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\mechanical-wings",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🪽 Wings🪽 Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one Wings🪽.$NC" -Level info
    Write-Host ""
}
...
function Install-Wings🪽Git {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing Wings🪽 from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/mechanical-wings/mechanical-wings.git $RepoDir 2>&1
    } elseif ($Update) {
    ...
    @"
@echo off
node "%~dp0..\mechanical-wings\dist\entry.js" %*
"@ | Out-File -FilePath "$wrapperDir\mechanical-wings.cmd" -Encoding ASCII -Force
...
function Resolve-PackageInstallSpec {
    ...
    if ([string]::IsNullOrWhiteSpace($trimmed)) {
        return "mechanical-wings@latest"
    }
    if ($trimmed.ToLowerInvariant() -eq "main") {
        return "github:mechanical-wings/mechanical-wings#main"
    }
    if (Test-ExplicitPackageInstallSpec -Target $trimmed) {
        return $trimmed
    }
    return "mechanical-wings@$trimmed"
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Wings🪽 from git to $GitDir" -Level info
        } else {
            Install-Wings🪽Git -RepoDir $GitDir -Update:(-not $NoGitUpdate)
        }
    } else {
        # npm method
        if (!(Ensure-Git)) {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install Wings🪽 via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            if (!(Install-Wings🪽Npm -Target $Tag)) {
                exit 1
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'mechanical-wings onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🪽 Wings🪽 installed successfully!" -Level success
}
