...
const extensionRestartMetadataFiles = new Set(["mechanical-wings.plugin.json", "package.json"]);
...
const logRunner = (message, deps) => {
  ...
  deps.stderr.write(`[mechanical-wings] ${message}\n`);
};

const runWings = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["mechanical-wings.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
const syncRuntimeArtifacts = (deps) => {
  try {
    copyPluginSdkRootAlias({ cwd: deps.cwd, env: deps.env });
    copyBundledPluginMetadata({ cwd: deps.cwd, env: deps.env });
  } catch (error) {
    logRunner(
