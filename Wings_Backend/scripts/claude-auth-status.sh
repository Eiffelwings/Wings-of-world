...
# Checks both Claude Code and Wings🪽 auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
OPENCLAW_AUTH="$HOME/.mechanical-wings/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    mechanical-wings models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    mechanical-wings_status=$(check_mechanical-wings_auth 2>/dev/null || true)

    claude_expires=0
    mechanical-wings_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        mechanical-wings_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        mechanical-wings_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$OPENCLAW_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$mechanical-wings_status" \
        --arg be "$mechanical-wings_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            mechanical-wings: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$mechanical-wings_status" == EXPIRED* ]] || [[ "$mechanical-wings_status" == MISSING* ]]; then
        echo "OPENCLAW_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$mechanical-wings_status" == EXPIRING* ]]; then
...
echo ""
echo "Wings🪽 Auth (~/.mechanical-wings/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'mechanical-wings doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active mechanical-wings >/dev/null 2>&1; then
    echo -e "Wings🪽 service: ${GREEN}running${NC}"
else
    echo -e "Wings🪽 service: ${RED}NOT running${NC}"
fi
