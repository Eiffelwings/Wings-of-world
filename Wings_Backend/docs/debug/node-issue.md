...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Wings🪽
...
Running Wings🪽 via Node with `tsx` fails at startup with:

```
[mechanical-wings] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `mechanical-wings status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch mechanical-wings.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node mechanical-wings.mjs status` works on Node 25.
