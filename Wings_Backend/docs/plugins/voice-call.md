...
read_when:
  - You want to place an outbound voice call from Wings🪽
...
Voice calls for Wings🪽 via a plugin. Supports outbound notifications and
...
- Use `mechanical-wings voicecall ...` or the `voice_call` tool
...
```bash
mechanical-wings plugins install ./extensions/voice-call
...
mechanical-wings voicecall start --to "+15555550123"   # alias for call
...
mechanical-wings voicecall latency                     # summarize turn latency from logs
