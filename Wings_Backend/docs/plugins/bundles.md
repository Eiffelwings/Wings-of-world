---
summary: "Install and use Codex, Claude, and Cursor bundles as Wings🪽 plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Wings🪽 maps bundle content into native features
...
Wings🪽 can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Wings🪽 maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Wings🪽 plugins. Native plugins run
...
of requiring authors to rewrite them as native Wings🪽 plugins, Wings🪽
...
## What Wings🪽 maps from bundles

Not every bundle feature runs in Wings🪽 today. Here is what works and what
...
| ------------- | ---------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Wings🪽 skills                                                    | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                           | Claude, Cursor |
| Hook packs    | Wings🪽-style `HOOK.md` + `handler.ts` layouts                                                      | Codex          |
...
These are recognized and shown in diagnostics, but Wings🪽 does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Wings🪽 best when they use skill roots and Wings🪽-style
...
Wings🪽 checks for native plugin format first:

1. `mechanical-wings.plugin.json` or valid `package.json` with `mechanical-wings.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Wings🪽 uses the native path. This prevents
...
- Wings🪽 does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `mechanical-wings plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. Wings🪽 does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Wings🪽 hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
