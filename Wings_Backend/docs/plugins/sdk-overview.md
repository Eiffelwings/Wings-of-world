...
read_when:
  ...
  - You want a reference for all registration methods on Wings🪽PluginApi
...
The `register(api)` callback receives an `Wings🪽PluginApi` object with these
methods:
...
- User config still wins. Wings🪽 merges `agents.defaults.cliBackends.<id>` over the
...
| `api.config`             | `Wings🪽Config`          | Current config snapshot                                   |
...
<Warning>
  Never import your own plugin through `mechanical-wings/plugin-sdk/<your-plugin>`
