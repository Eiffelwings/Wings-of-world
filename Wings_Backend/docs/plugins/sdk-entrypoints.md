...
**Import:** `mechanical-wings/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `Wings🪽PluginConfigSchema \| () => Wings🪽PluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: Wings🪽PluginApi) => void`                               | Yes      | —                   |

- `id` must match your `mechanical-wings.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `mechanical-wings/plugin-sdk/core`
...
| `registerFull` | `(api: Wings🪽PluginApi) => void`                               | No       | —                   |
...
Wings🪽 loads this instead of the full entry when a channel is disabled,
...
Wings🪽 classifies loaded plugins by their registration behavior:
...
Use `mechanical-wings plugins inspect <id>` to see a plugin's shape.
