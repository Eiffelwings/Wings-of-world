...
read_when:
  ...
  - You maintain an external Wings🪽 plugin
---

# Plugin SDK Migration

Wings🪽 has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`mechanical-wings/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`mechanical-wings/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`mechanical-wings/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "mechanical-wings/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "mechanical-wings/plugin-sdk/compat";
    ...
    import { runEmbeddedPiAgent } from "mechanical-wings/extension-api";
...
```bash
OPENCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 mechanical-wings gateway run
OPENCLAW_SUPPRESS_EXTENSION_API_WARNING=1 mechanical-wings gateway run
```
