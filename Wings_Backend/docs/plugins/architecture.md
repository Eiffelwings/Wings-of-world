...
read_when:
  - Building or debugging native Wings🪽 plugins
...
This page covers the internal architecture of the Wings🪽 plugin system.

## Public capability model

Capabilities are the public **native plugin** model inside Wings🪽. Every
native Wings🪽 plugin registers against one or more capability types:
...
Wings🪽 classifies every loaded plugin into a shape based on its actual
...
Use `mechanical-wings plugins inspect <id>` to see a plugin's shape and capability
...
When you run `mechanical-wings doctor` or `mechanical-wings plugins inspect <id>`, you may see
one of these labels:
...
signals also appear in `mechanical-wings status --all` and `mechanical-wings plugins doctor`.

## Architecture overview

Wings🪽's plugin system has four layers:

1. **Manifest + discovery**
   Wings🪽 finds candidate plugins from configured paths, workspace roots,
   global extension roots, and bundled extensions. Discovery reads native
   `mechanical-wings.plugin.json` manifests plus supported bundle manifests first.
2. **Enablement + validation**
   Core decides whether a discovered plugin is enabled, disabled, blocked, or
   selected for an exclusive slot such as memory.
3. **Runtime loading**
   Native Wings🪽 plugins are loaded in-process via jiti and register
...
4. **Surface consumption**
   The rest of Wings🪽 reads the registry to expose tools, channels, provider
...
That split lets Wings🪽 validate config, explain missing/disabled plugins, and
...
normal chat actions. Wings🪽 keeps one shared `message` tool in core, and
...
Wings🪽 treats a native plugin as the ownership boundary for a **company** or a
**feature**, not as a grab bag of unrelated integrations.

That means:

- a company plugin should usually own all of that company's Wings🪽-facing
  surfaces
...
So if Wings🪽 adds a new domain such as video, the first question is not
...
A company plugin should feel cohesive from the outside. If Wings🪽 has shared
contracts for models, speech, media understanding, and web search, a vendor can
own all of its surfaces in one place:

```ts
import type { Wings🪽PluginDefinition } from "mechanical-wings/plugin-sdk";
import {
  buildOpenAISpeechProvider,
  createPluginBackedWebSearchProvider,
  describeImageWithModel,
  transcribeOpenAiCompatibleAudio,
} from "mechanical-wings/plugin-sdk";

const plugin: Wings🪽PluginDefinition = {
  id: "exampleai",
  name: "ExampleAI",
  register(api) {
...
Wings🪽 already treats image/audio/video understanding as one shared
...
If Wings🪽 adds a new domain later, such as video generation, use the same
...
`Wings🪽PluginApi`. That contract defines the supported registration points and
...
2. **contract tests**
   ...
   Wings🪽 can assert ownership explicitly. Today this is used for model
   providers, speech providers, web search providers, and bundled registration
   ownership.

The practical effect is that Wings🪽 knows, up front, which plugin owns which
...
- ad hoc runtime objects that are not part of `Wings🪽PluginApi` or
  `api.runtime`
...
Native Wings🪽 plugins run **in-process** with the Gateway. They are not
...
Compatible bundles are safer by default because Wings🪽 currently treats them
...
name: `@mechanical-wings/<id>` by default, or an approved typed suffix such as
...
Wings🪽 exports capabilities, not implementation convenience.
...
At startup, Wings🪽 does roughly this:
...
The manifest is the control-plane source of truth. Wings🪽 uses it to:
...
Wings🪽 keeps short in-process caches for:
...
Wings🪽 still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Wings🪽 calls hooks in this rough order.
...
| --  | _(built-in model lookup)_     | Wings🪽 tries the normal registry/catalog path first                                    | _(not a plugin hook)_                                                                |
...
that still runs on Wings🪽's normal inference loop.
...
- OpenRouter uses `catalog` plus `resolveDynamicModel` and
  ...
  model ids before Wings🪽's static catalog updates; it also uses
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Wings🪽 adds those
...
- If Wings🪽 adds a new capability such as video generation later, define the
...
- Wings🪽 only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Wings🪽 plugin runtime helpers",
    count: 5,
  },
});
```
...
Use SDK subpaths instead of the monolithic `mechanical-wings/plugin-sdk` import when
authoring plugins:

- `mechanical-wings/plugin-sdk/plugin-entry` for plugin registration primitives.
- `mechanical-wings/plugin-sdk/core` for the generic shared plugin-facing contract.
- Stable channel primitives such as `mechanical-wings/plugin-sdk/channel-setup`,
  `mechanical-wings/plugin-sdk/channel-pairing`,
  `mechanical-wings/plugin-sdk/channel-contract`,
  `mechanical-wings/plugin-sdk/channel-feedback`,
  `mechanical-wings/plugin-sdk/channel-inbound`,
  `mechanical-wings/plugin-sdk/channel-lifecycle`,
  `mechanical-wings/plugin-sdk/channel-reply-pipeline`,
  `mechanical-wings/plugin-sdk/command-auth`,
  `mechanical-wings/plugin-sdk/secret-input`, and
  `mechanical-wings/plugin-sdk/webhook-ingress` for shared setup/auth/reply/webhook
...
- Domain subpaths such as `mechanical-wings/plugin-sdk/channel-config-helpers`,
  `mechanical-wings/plugin-sdk/allow-from`,
  `mechanical-wings/plugin-sdk/channel-config-schema`,
  `mechanical-wings/plugin-sdk/channel-policy`,
  `mechanical-wings/plugin-sdk/config-runtime`,
  `mechanical-wings/plugin-sdk/infra-runtime`,
  `mechanical-wings/plugin-sdk/agent-runtime`,
  `mechanical-wings/plugin-sdk/lazy-runtime`,
  `mechanical-wings/plugin-sdk/reply-history`,
  `mechanical-wings/plugin-sdk/routing`,
  `mechanical-wings/plugin-sdk/status-helpers`,
  `mechanical-wings/plugin-sdk/runtime-store`, and
  `mechanical-wings/plugin-sdk/directory-runtime` for shared runtime/config helpers.
- `mechanical-wings/plugin-sdk/channel-runtime` remains only as a compatibility shim.
...
- Bundled extension internals remain private. External plugins should use only
  `mechanical-wings/plugin-sdk/*` subpaths. Wings🪽 core/test code may use the repo
...
- Avoid the root `mechanical-wings/plugin-sdk` barrel for new code.
- Prefer the narrow stable primitives first. The newer setup/pairing/reply/
  ...
  Target parsing/matching belongs on `mechanical-wings/plugin-sdk/channel-targets`.
  Message action gates and reaction message-id helpers belong on
  `mechanical-wings/plugin-sdk/channel-actions`.
- Bundled extension-specific helper barrels are not stable by default. If a
  ...
  `mechanical-wings/plugin-sdk/<extension>`.
...
`mechanical-wings/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Wings🪽 writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Wings🪽's
...
- if both `catalog` and `discovery` are registered, Wings🪽 uses `catalog`
...
- Read-only command paths such as `mechanical-wings status`, `mechanical-wings status --all`,
  `mechanical-wings channels status`, `mechanical-wings channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `mechanical-wings.extensions`:
...
Security guardrail: every `mechanical-wings.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `mechanical-wings plugins install` installs plugin dependencies with
`npm install --ignore-scripts` (no lifecycle scripts). Keep plugin dependency
trees "pure JS/TS" and avoid packages that require `postinstall` builds.

Optional: `mechanical-wings.setupEntry` can point at a lightweight setup-only module.
When Wings🪽 needs setup surfaces for a disabled channel plugin, or
...
Optional: `mechanical-wings.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Wings🪽 load the
...
Channel plugins can advertise setup/discovery metadata via `mechanical-wings.channel` and
install hints via `mechanical-wings.install`. This keeps the core catalog data-free.
...
Wings🪽 can also merge **external channel catalogs** (for example, an MPM
...
contain `{ "entries": [ { "name": "@scope/pkg", "mechanical-wings": { "channel": {...}, "install": {...} } } ] }`.
...
```ts
import { delegateCompactionToRuntime } from "mechanical-wings/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `Wings🪽PluginApi` and/or `api.runtime` with the smallest useful
...
This is how Wings🪽 stays opinionated without becoming hardcoded to one
