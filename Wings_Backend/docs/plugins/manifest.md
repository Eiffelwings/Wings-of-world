...
read_when:
  - You are building an Wings🪽 plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (mechanical-wings.plugin.json)

This page is for the **native Wings🪽 plugin manifest** only.
...
Wings🪽 auto-detects those bundle layouts too, but they are not validated
against the `mechanical-wings.plugin.json` schema described here.

For compatible bundles, Wings🪽 currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches Wings🪽 runtime expectations.

Every native Wings🪽 plugin **must** ship a `mechanical-wings.plugin.json` file in the
**plugin root**. Wings🪽 uses this manifest to validate configuration
...
`mechanical-wings.plugin.json` is the metadata Wings🪽 reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that Wings🪽 can inspect without loading plugin code.                                      |
...
Wings🪽 reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, Wings🪽 falls back to `choiceId`.                                        |
...
Use `contracts` only for static capability ownership metadata that Wings🪽 can
...
`imageGenerationProviders` are deprecated. Use `mechanical-wings doctor --fix` to move
...
| File                   | Use it for                                                                                                         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `mechanical-wings.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs           |
| `package.json`         | npm metadata, dependency installation, and the `mechanical-wings` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Wings🪽 must know it before loading plugin code, put it in `mechanical-wings.plugin.json`
...
- The manifest is **required for native Wings🪽 plugins**, including local filesystem loads.
