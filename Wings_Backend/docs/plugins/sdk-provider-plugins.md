...
summary: "Step-by-step guide to building a model provider plugin for Wings🪽"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to Wings🪽
...
(LLM) to Wings🪽. By the end you will have a provider with a model catalog,
API key auth, and dynamic model resolution.

<Info>
  If you have not built any Wings🪽 plugin before, read
...
<Steps>
  <Step title="Package and manifest">
    ...
    ```json package.json
    {
      "name": "@myorg/mechanical-wings-acme-ai",
      "version": "1.0.0",
      "type": "module",
      "mechanical-wings": {
        "extensions": ["./index.ts"],
    ...
    ```json mechanical-wings.plugin.json
    {
      "id": "acme-ai",
      "name": "Acme AI",
    ...
    The manifest declares `providerAuthEnvVars` so Wings🪽 can detect
  ...
  <Step title="Register the provider">
    ...
    ```typescript index.ts
    import { definePluginEntry } from "mechanical-wings/plugin-sdk/plugin-entry";
    import { createProviderApiKeyAuthMethod } from "mechanical-wings/plugin-sdk/provider-auth";
    ...
    `mechanical-wings onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "mechanical-wings/plugin-sdk/provider-entry";
    ...
    `mechanical-wings/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      Wings🪽 calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    Wings🪽 classifies this as a **hybrid-capability** plugin. This is the
...
```
extensions/acme-ai/
├── package.json              # mechanical-wings.providers metadata
├── mechanical-wings.plugin.json      # Manifest with providerAuthEnvVars
