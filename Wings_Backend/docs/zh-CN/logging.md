...
你可以在 `~/.mechanical-wings/mechanical-wings.json` 中覆盖此设置：
...
所有日志配置都在 `~/.mechanical-wings/mechanical-wings.json` 的 `logging` 下。
...
- Wings🪽 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `mechanical-wings plugins enable diagnostics-otel` 启用插件。
...
- `mechanical-wings.tokens`（计数器，属性：`mechanical-wings.token`、`mechanical-wings.channel`、`mechanical-wings.provider`、`mechanical-wings.model`）
- `mechanical-wings.cost.usd`（计数器，属性：`mechanical-wings.channel`、`mechanical-wings.provider`、`mechanical-wings.model`）
- `mechanical-wings.run.duration_ms`（直方图，属性：`mechanical-wings.channel`、`mechanical-wings.provider`、`mechanical-wings.model`）
- `mechanical-wings.context.tokens`（直方图，属性：`mechanical-wings.context`、`mechanical-wings.channel`、`mechanical-wings.provider`、`mechanical-wings.model`）

消息流：

- `mechanical-wings.webhook.received`（计数器，属性：`mechanical-wings.channel`、`mechanical-wings.webhook`）
- `mechanical-wings.webhook.error`（计数器，属性：`mechanical-wings.channel`、`mechanical-wings.webhook`）
- `mechanical-wings.webhook.duration_ms`（直方图，属性：`mechanical-wings.channel`、`mechanical-wings.webhook`）
- `mechanical-wings.message.queued`（计数器，属性：`mechanical-wings.channel`、`mechanical-wings.source`）
- `mechanical-wings.message.processed`（计数器，属性：`mechanical-wings.channel`、`mechanical-wings.outcome`）
- `mechanical-wings.message.duration_ms`（直方图，属性：`mechanical-wings.channel`、`mechanical-wings.outcome`）

队列 + 会话：

- `mechanical-wings.queue.lane.enqueue`（计数器，属性：`mechanical-wings.lane`）
- `mechanical-wings.queue.lane.dequeue`（计数器，属性：`mechanical-wings.lane`）
- `mechanical-wings.queue.depth`（直方图，属性：`mechanical-wings.lane` 或 `mechanical-wings.channel=heartbeat`）
- `mechanical-wings.queue.wait_ms`（直方图，属性：`mechanical-wings.lane`）
- `mechanical-wings.session.state`（计数器，属性：`mechanical-wings.state`、`mechanical-wings.reason`）
- `mechanical-wings.session.stuck`（计数器，属性：`mechanical-wings.state`）
- `mechanical-wings.session.stuck_age_ms`（直方图，属性：`mechanical-wings.state`）
- `mechanical-wings.run.attempt`（计数器，属性：`mechanical-wings.attempt`）
...
- `mechanical-wings.webhook.error`
  - `mechanical-wings.channel`、`mechanical-wings.webhook`、`mechanical-wings.chatId`、`mechanical-wings.error`
- `mechanical-wings.message.processed`
  - `mechanical-wings.channel`、`mechanical-wings.outcome`、`mechanical-wings.chatId`、`mechanical-wings.messageId`、`mechanical-wings.sessionKey`、`mechanical-wings.sessionId`、`mechanical-wings.reason`
- `mechanical-wings.session.stuck`
  - `mechanical-wings.state`、`mechanical-wings.ageMs`、`mechanical-wings.queueDepth`、`mechanical-wings.sessionKey`、`mechanical-wings.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `mechanical-wings doctor`。
