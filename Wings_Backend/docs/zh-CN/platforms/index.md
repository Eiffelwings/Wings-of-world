...
Wings🪽 核心使用 TypeScript 编写。**Node 是推荐的运行时**。
...
- 服务状态：`mechanical-wings gateway status`

## Gateway 网关服务安装（CLI）

使用以下任一方式（均受支持）：

- 向导（推荐）：`mechanical-wings onboard --install-daemon`
- 直接安装：`mechanical-wings gateway install`
- 配置流程：`mechanical-wings configure` → 选择 **Gateway 服务**
- 修复/迁移：`mechanical-wings doctor`（会提供安装或修复服务的选项）

服务目标取决于操作系统：

- macOS：LaunchAgent（`ai.mechanical-wings.gateway` 或 `ai.mechanical-wings.<profile>`；旧版为 `com.mechanical-wings.*`）
- Linux/WSL2：systemd 用户服务（`mechanical-wings-gateway[-<profile>].service`）
