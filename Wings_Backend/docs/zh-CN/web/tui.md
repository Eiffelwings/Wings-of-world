...
- 开启投递：
  - `/deliver on`
  - 或设置面板
  - 或使用 `mechanical-wings tui --deliver` 启动
...
- 确认智能体可以运行：`mechanical-wings status` 和 `mechanical-wings models status`。
...
- 选择器中没有智能体：检查 `mechanical-wings agents list` 和你的路由配置。
