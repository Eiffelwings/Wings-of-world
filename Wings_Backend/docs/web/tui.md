...
```bash
mechanical-wings tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `mechanical-wings tui --deliver`
...
- If your terminal uses a light background and auto-detection is wrong, set `OPENCLAW_THEME=light` before launching `mechanical-wings tui`.
...
- Check the Gateway logs: `mechanical-wings logs --follow`.
- Confirm the agent can run: `mechanical-wings status` and `mechanical-wings models status`.
...
- No agents in picker: check `mechanical-wings agents list` and your routing config.
