...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/mechanical-wings`)
...
If the page fails to load, start the Gateway first: `mechanical-wings gateway`.
...
created. Re-run `mechanical-wings devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `mechanical-wings devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.mechanical-wings/mechanical-wings.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop mechanical-wings`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Wings🪽
...
Wings🪽 **blocks** Control UI connections without device identity.
