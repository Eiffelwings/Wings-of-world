...
Wings🪽 can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere Wings🪽 can send audio.
...
When `messages.tts.provider` is unset, Wings🪽 picks the first configured
speech provider in registry auto-select order.

## Config

TTS config lives under `messages.tts` in `mechanical-wings.json`.
...
- If `provider` is **unset**, Wings🪽 uses the first configured speech provider in registry auto-select order.
...
`~/.mechanical-wings/settings/tts.json`, override with `OPENCLAW_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, Wings🪽 retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so Wings🪽 registers
