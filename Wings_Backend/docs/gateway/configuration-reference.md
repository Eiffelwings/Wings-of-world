...
summary: "Complete reference for every Wings🪽 config key, defaults, and channel settings"
read_when:
...
Every field available in `~/.mechanical-wings/mechanical-wings.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Wings🪽 uses safe defaults when omitted.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // authDir: "~/.mechanical-wings/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- Legacy single-account Baileys auth dir is migrated by `mechanical-wings doctor` into `whatsapp/default`.
...
- In multi-account setups (2+ account ids), set an explicit default (`channels.telegram.defaultAccount` or `channels.telegram.accounts.default`) to avoid fallback routing; `mechanical-wings doctor` warns when this is missing or invalid.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["mechanical-wings-dm"] },
      guilds: {
        "123456789012345678": {
          slug: "friends-of-mechanical-wings",
          requireMention: false,
...
- Wings🪽 additionally attempts voice receive recovery by leaving/rejoining a voice session after repeated decrypt failures.
...
Mattermost ships as a plugin: `mechanical-wings plugins install @mechanical-wings/mattermost`.
...
- `commands.callbackUrl` must resolve to the Wings🪽 gateway endpoint and be reachable from the Mattermost server.
...
Wings🪽 spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
- If you add a non-default account via `mechanical-wings channels add` (or channel onboarding) while still on a single-account top-level channel config, Wings🪽 moves account-scoped top-level single-account values into `channels.<channel>.accounts.default` first so the original account keeps working.
- Existing channel-only bindings (no `accountId`) keep matching the default account; account-scoped bindings remain optional.
- `mechanical-wings doctor --fix` also repairs mixed shapes by moving account-scoped top-level single-account values into `accounts.default` when named accounts exist but `default` is missing.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@mechanical-wings", "mechanical-wings"] } }],
  },
}
```
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@mechanical-wings"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `mechanical-wings.json`). For gateway `chat.send` clients, persistent `/config set|unset` writes also require `operator.admin`; read-only `/config show` stays available to normal write-scoped operator clients.
...
Default: `~/.mechanical-wings/workspace`.
...
Optional repository root shown in the system prompt's Runtime line. If unset, Wings🪽 auto-detects by walking upward from the workspace.

```json5
{
  agents: { defaults: { repoRoot: "~/Projects/mechanical-wings" } },
}
```
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Wings🪽 assumes `anthropic` (deprecated).
...
- `timeoutSeconds`: maximum seconds allowed for a single compaction operation before Wings🪽 aborts it. Default: `900`.
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.mechanical-wings/sandboxes",
        docker: {
          image: "mechanical-wings-sandbox:bookworm-slim",
          containerPrefix: "mechanical-wings-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "mechanical-wings-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        ssh: {
          ...
          command: "ssh",
          workspaceRoot: "/tmp/mechanical-wings-sandboxes",
        ...
        browser: {
          enabled: false,
          image: "mechanical-wings-sandbox-browser:bookworm-slim",
          network: "mechanical-wings-sandbox-browser",
          cdpPort: 9222,
...
- `identityData` / `certificateData` / `knownHostsData`: inline contents or SecretRefs that Wings🪽 materializes into temp files at runtime
...
- `none`: per-scope sandbox workspace under `~/.mechanical-wings/sandboxes`
...
In `remote` mode, host-local edits made outside Wings🪽 are not synced into the sandbox automatically after the seed step.
...
**Sandboxed browser** (`sandbox.browser.enabled`): Chromium + CDP in a container. noVNC URL injected into system prompt. Does not require `browser.enabled` in `mechanical-wings.json`.
noVNC observer access uses VNC auth by default and Wings🪽 emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `mechanical-wings-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.mechanical-wings/workspace",
        agentDir: "~/.mechanical-wings/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@mechanical-wings"] },
        sandbox: { mode: "off" },
        runtime: {
          type: "acp",
          acp: {
            ...
            backend: "acpx",
            mode: "persistent",
            cwd: "/workspace/mechanical-wings",
          },
        },
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.mechanical-wings/workspace-home" },
      { id: "work", workspace: "~/.mechanical-wings/workspace-work" },
    ],
  },
  bindings: [
...
For `type: "acp"` entries, Wings🪽 resolves by exact conversation identity (`match.channel` + account + `match.peer.id`) and does not use the route binding tier order above.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.mechanical-wings/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.mechanical-wings/workspace-family",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.mechanical-wings/workspace-public",
...
```json5
{
  session: {
    ...
    store: "~/.mechanical-wings/agents/{agentId}/sessions/sessions.json",
...
- **`parentForkMaxTokens`**: max parent-session `totalTokens` allowed when creating a forked thread session (default `100000`).
  - If parent `totalTokens` is above this value, Wings🪽 starts a fresh thread session instead of inheriting parent transcript history.
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.mechanical-wings/settings/tts.json",
      elevenlabs: {
...
- When `openai.baseUrl` points to a non-OpenAI endpoint, Wings🪽 treats it as an OpenAI-compatible TTS server and relaxes model/voice validation.
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:mechanical-wings`   | All built-in tools (excludes provider plugins)                                           |
...
- Files are materialized into the child workspace at `.mechanical-wings/attachments/<uuid>/` with a `.manifest.json`.
...
Wings🪽 uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.mechanical-wings/agents/<agentId>/agent/models.json`.
...
- `models.providers.*.models.*.compat.supportsDeveloperRole`: optional compatibility hint. For `api: "openai-completions"` with a non-empty non-native `baseUrl` (host not `api.openai.com`), Wings🪽 forces this to `false` at runtime. Empty/omitted `baseUrl` keeps default OpenAI behavior.
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Use `opencode/...` refs for the Zen catalog or `opencode-go/...` refs for the Go catalog. Shortcut: `mechanical-wings onboard --auth-choice opencode-zen` or `mechanical-wings onboard --auth-choice opencode-go`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `mechanical-wings onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `mechanical-wings onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `mechanical-wings onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `mechanical-wings onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `mechanical-wings onboard --auth-choice minimax-api`.
...
- Loaded from `~/.mechanical-wings/extensions`, `<workspace>/.mechanical-wings/extensions`, plus `plugins.load.paths`.
- Discovery accepts native Wings🪽 plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native Wings🪽 plugin schema when available).
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; Wings🪽 applies those as sanitized agent settings, not as raw Wings🪽 config patches.
- `plugins.slots.memory`: pick the active memory plugin id, or `"none"` to disable memory plugins.
- `plugins.slots.contextEngine`: pick the active context engine plugin id; defaults to `"legacy"` unless you install and select another engine.
- `plugins.installs`: CLI-managed install metadata used by `mechanical-wings plugins update`.
...
```json5
{
  browser: {
    ...
    profiles: {
      mechanical-wings: { cdpPort: 18800, color: "#FF4500" },
...
```bash
OPENCLAW_CONFIG_PATH=~/.mechanical-wings/a.json \
OPENCLAW_STATE_DIR=~/.mechanical-wings-a \
mechanical-wings gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.mechanical-wings-dev` + port `19001`), `--profile <name>` (uses `~/.mechanical-wings-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.mechanical-wings/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-mechanical-wings-token: <token>`.
...
{
  canvasHost: {
    root: "~/.mechanical-wings/workspace/canvas",
    liveReload: true,
...
- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__mechanical-wings__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__mechanical-wings__/a2ui/`
...
- Also serves A2UI at `/__mechanical-wings__/a2ui/`.
...
- Hostname defaults to `mechanical-wings`. Override with `OPENCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.mechanical-wings/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `mechanical-wings dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.mechanical-wings/.env` (neither overrides existing vars).
...
- `secrets apply` targets supported `mechanical-wings.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.mechanical-wings/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/mechanical-wings-vault-resolver",
...
- Legacy OAuth imports from `~/.mechanical-wings/credentials/oauth.json`.
...
- Default log file: `/tmp/mechanical-wings/mechanical-wings-YYYY-MM-DD.log`.
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one Wings🪽.`).
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `mechanical-wings doctor --fix` can strip unknown keys).
...
```json5
// ~/.mechanical-wings/mechanical-wings.json
{
  gateway: { port: 18789 },
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `mechanical-wings.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
