...
Wings🪽 supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Wings🪽 runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Wings🪽 validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/Wings🪽 QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/mechanical-wings"],
...
- Wings🪽 resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `mechanical-wings.json` refs)
...
When reload-time activation fails after a healthy state, Wings🪽 enters degraded secrets state.
...
- Strict command paths (for example `mechanical-wings memory` remote-memory paths and `mechanical-wings qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `mechanical-wings status`, `mechanical-wings status --all`, `mechanical-wings channels status`, `mechanical-wings channels resolve`, `mechanical-wings security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `mechanical-wings secrets reload`.
...
```bash
mechanical-wings secrets audit --check
mechanical-wings secrets configure
mechanical-wings secrets audit --check
```
...
- plaintext values at rest (`mechanical-wings.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `mechanical-wings.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `mechanical-wings secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `mechanical-wings.json` plus `auth-profiles.json` for one agent scope
...
- `mechanical-wings secrets configure --providers-only`
- `mechanical-wings secrets configure --skip-provider-setup`
- `mechanical-wings secrets configure --agent <id>`
...
```bash
mechanical-wings secrets apply --from /tmp/mechanical-wings-secrets-plan.json
mechanical-wings secrets apply --from /tmp/mechanical-wings-secrets-plan.json --allow-exec
mechanical-wings secrets apply --from /tmp/mechanical-wings-secrets-plan.json --dry-run
mechanical-wings secrets apply --from /tmp/mechanical-wings-secrets-plan.json --dry-run --allow-exec
```
...
Wings🪽 intentionally does not write rollback backups containing historical plaintext secret values.
