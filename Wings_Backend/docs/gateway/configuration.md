...
read_when:
  - Setting up Wings🪽 for the first time
...
Wings🪽 reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.mechanical-wings/mechanical-wings.json`.

If the file is missing, Wings🪽 uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `mechanical-wings onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    mechanical-wings onboard       # full onboarding flow
    mechanical-wings configure     # config wizard
    ```
  </Tab>
  <Tab title="CLI (one-liners)">
    ```bash
    mechanical-wings config get agents.defaults.workspace
    mechanical-wings config set agents.defaults.heartbeat.every "2h"
    mechanical-wings config unset plugins.entries.brave.config.webSearch.apiKey
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.mechanical-wings/mechanical-wings.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Wings🪽 only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`mechanical-wings doctor`, `mechanical-wings logs`, `mechanical-wings health`, `mechanical-wings status`)
- Run `mechanical-wings doctor` to see exact issues
- Run `mechanical-wings doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `mechanical-wings.json`.
    ...
    ```bash
    mechanical-wings config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
...
The Gateway watches `~/.mechanical-wings/mechanical-wings.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `mechanical-wings config set` for single keys.
    </Warning>
    ...
    ```bash
    mechanical-wings gateway call config.get --params '{}'  # capture payload.hash
    mechanical-wings gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.mechanical-wings/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    mechanical-wings gateway call config.patch --params '{
...
Wings🪽 reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.mechanical-wings/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Wings🪽 runs your login shell and imports only the missing keys:
