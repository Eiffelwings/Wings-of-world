...
Wings🪽 supports OAuth and API keys for model providers. For always-on gateway
...
2. Put it on the **gateway host** (the machine running `mechanical-wings gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.mechanical-wings/.env` so the daemon can read it:

```bash
cat >> ~/.mechanical-wings/.env <<'EOF'
<PROVIDER>_API_KEY=...
EOF
```
...
API keys for daemon use: `mechanical-wings onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.mechanical-wings/.env`, systemd/launchd).
...
```bash
mechanical-wings models auth paste-token --provider anthropic
mechanical-wings models auth paste-token --provider openrouter
```
...
```bash
mechanical-wings models status --check
```
...
- Wings🪽 retries with the next key only for rate-limit errors (for example
...
Run `mechanical-wings models status` to confirm which profile is expiring. If the profile
