---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for Wings🪽 agents"
read_when:
...
OpenShell is a managed sandbox backend for Wings🪽. Instead of running Docker
containers locally, Wings🪽 delegates sandbox lifecycle to the `openshell` CLI,
...
- Wings🪽 Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, Wings🪽 creates an OpenShell
...
- You edit files locally outside Wings🪽 and want those changes visible in the
...
- When the sandbox is first created, Wings🪽 seeds the remote workspace from
...
- Wings🪽 does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside Wings🪽 after the initial seed,
the remote sandbox does **not** see those changes. Use
`mechanical-wings sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"mechanical-wings"`  | Sandbox source for first-time create                  |
...
1. Wings🪽 calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Wings🪽 calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `mechanical-wings sandbox` commands
