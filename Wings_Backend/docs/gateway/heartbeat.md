...
- During heartbeat runs, Wings🪽 treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `mechanical-wings sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Wings🪽 skips the heartbeat run entirely (no model call).
...
```bash
mechanical-wings system event --text "Check for urgent follow-ups" --mode now
```
