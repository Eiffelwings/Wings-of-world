...
read_when:
  - Running Wings🪽 behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of Wings🪽
...
- You run Wings🪽 behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. Wings🪽 checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. Wings🪽 extracts the user identity from the configured header
...
- Wings🪽 can stay on loopback HTTP behind the proxy.
...
If Wings🪽 itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://mechanical-wings.example.com
    to: http://mechanical-wings-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
mechanical-wings.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy mechanical-wings:18789 {
...
location / {
    ...
    proxy_pass http://mechanical-wings:18789;
...
`mechanical-wings security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update Wings🪽 config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `mechanical-wings security audit` and review findings
