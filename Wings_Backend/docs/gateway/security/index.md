...
> Wings🪽 is **not** a hostile multi-tenant security boundary for multiple adversarial users sharing one agent/gateway.
...
Wings🪽 security guidance assumes a **personal assistant** deployment: one trusted operator boundary, potentially many agents.
...
## Quick check: `mechanical-wings security audit`
...
```bash
mechanical-wings security audit
mechanical-wings security audit --deep
mechanical-wings security audit --fix
mechanical-wings security audit --json
```
...
Wings🪽 is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
Wings🪽 assumes the host and config boundary are trusted:

- If someone can modify Gateway host state/config (`~/.mechanical-wings`, including `mechanical-wings.json`), treat them as a trusted operator.
- Running one Gateway for multiple mutually untrusted/adversarial operators is **not a recommended setup**.
- For mixed-trust teams, split trust boundaries with separate gateways (or at minimum separate OS users/hosts).
- Wings🪽 can run multiple gateway instances on one machine, but recommended operations favor clean trust-boundary separation.
- Recommended default: one user per machine/host (or VPS), one gateway for that user, and one or more agents in that gateway.
- If multiple users want Wings🪽, use one VPS/host per user.
...
Wings🪽 is designed as a personal assistant security model: one trusted operator boundary, potentially many agents.
...
4. Claim is not listed in [Out of Scope](https://github.com/mechanical-wings/mechanical-wings/blob/main/SECURITY.md#out-of-scope).
...
If you run `--deep`, Wings🪽 also attempts a best-effort live Gateway probe.

## Credential storage map

Use this when auditing access or deciding what to back up:

- **WhatsApp**: `~/.mechanical-wings/credentials/whatsapp/<accountId>/creds.json`
- **Telegram bot token**: config/env or `channels.telegram.tokenFile` (regular file only; symlinks rejected)
- **Discord bot token**: config/env or SecretRef (env/file/exec providers)
- **Slack tokens**: config/env (`channels.slack.*`)
- **Pairing allowlists**:
  - `~/.mechanical-wings/credentials/<channel>-allowFrom.json` (default account)
  - `~/.mechanical-wings/credentials/<channel>-<accountId>-allowFrom.json` (non-default accounts)
- **Model auth profiles**: `~/.mechanical-wings/agents/<agentId>/agent/auth-profiles.json`
- **File-backed secrets payload (optional)**: `~/.mechanical-wings/secrets.json`
- **Legacy OAuth import**: `~/.mechanical-wings/credentials/oauth.json`
...
| ------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------- | -------- |
| `fs.state_dir.perms_world_writable`                           | critical      | Other users/processes can modify full Wings🪽 state                                 | filesystem perms on `~/.mechanical-wings`                                                                    | yes      |
| `fs.config.perms_writable`                                    | critical      | Others can change auth/tool policy/config                                            | filesystem perms on `~/.mechanical-wings/mechanical-wings.json`                                                      | yes      |
...
| `tools.exec.auto_allow_skills_enabled`                        | warn          | Exec approvals trust skill bins implicitly                                           | `~/.mechanical-wings/exec-approvals.json`                                                                    | no       |
...
`mechanical-wings security audit` warns when this setting is enabled.

## Insecure or dangerous flags summary

`mechanical-wings security audit` includes `config.insecure_or_dangerous_flags` when
...
Complete `dangerous*` / `dangerously*` config keys defined in Wings🪽 config
schema:
...
- Wings🪽 gateway is local/loopback first. If you terminate TLS at a reverse proxy, set HSTS on the proxy-facing HTTPS domain there.
- If the gateway itself terminates HTTPS, you can set `gateway.http.securityHeaders.strictTransportSecurity` to emit the HSTS header from Wings🪽 responses.
...
Wings🪽 stores session transcripts on disk under `~/.mechanical-wings/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.mechanical-wings` (see the audit section below). If you need
...
- Approval mode binds exact request context and, when possible, one concrete local script/file operand. If Wings🪽 cannot identify exactly one direct local file for an interpreter/runtime command, approval-backed execution is denied rather than promising full semantic coverage.
- If you don’t want remote execution, set security to **deny** and remove node pairing for that Mac.

## Dynamic skills (watcher / remote nodes)

Wings🪽 can refresh the skills list mid-session:
...
- If you install plugins (`mechanical-wings plugins install <package>`), treat it like running untrusted code:
  - The install path is `~/.mechanical-wings/extensions/<pluginId>/` (or `$OPENCLAW_STATE_DIR/extensions/<pluginId>/`).
  - Wings🪽 uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
```bash
mechanical-wings pairing list <channel>
mechanical-wings pairing approve <channel> <code>
```
...
By default, Wings🪽 routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
Wings🪽 has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.allowFrom` / `channels.slack.allowFrom`; legacy: `channels.discord.dm.allowFrom`, `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to the account-scoped pairing allowlist store under `~/.mechanical-wings/credentials/` (`<channel>-allowFrom.json` for default account, `<channel>-<accountId>-allowFrom.json` for non-default accounts), merged with config allowlists.
...
- “Paste the full contents of ~/.mechanical-wings or your logs.”

## Unsafe external content bypass flags

Wings🪽 includes explicit bypass flags that disable external-content safety wrapping:
...
- `~/.mechanical-wings/mechanical-wings.json`: `600` (user read/write only)
- `~/.mechanical-wings`: `700` (user only)

`mechanical-wings doctor` can warn and offer to tighten these permissions.
...
- Canvas host: `/__mechanical-wings__/canvas/` and `/__mechanical-wings__/a2ui/` (arbitrary HTML/JS; treat as untrusted content)
...
If you run Wings🪽 with Docker on a VPS, remember that published container ports
...
The Gateway broadcasts its presence via mDNS (`_mechanical-wings-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
Doctor can generate one for you: `mechanical-wings doctor --generate-gateway-token`.
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), Wings🪽
accepts Tailscale Serve identity headers (`tailscale-user-login`) for Control
UI/WebSocket authentication. Wings🪽 verifies the identity by resolving the
...
- Wings🪽 will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.mechanical-wings/` (or `$OPENCLAW_STATE_DIR/`) may contain secrets or private data:

- `mechanical-wings.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `mechanical-wings status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@mechanical-wings", "@mybot"] }
      }
    ]
  }
}
...
- Keep filesystem roots narrow: avoid broad roots like your home directory for agent workspaces/sandbox workspaces. Broad roots can expose sensitive local files (for example state/config under `~/.mechanical-wings`) to filesystem tools.
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.mechanical-wings/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `mechanical-wings` profile).
...
Wings🪽’s browser network policy defaults to the trusted-operator model: private/internal destinations are allowed unless you explicitly disable them.
    ...
    list: [
      {
        ...
        // Session tools can reveal sensitive data from transcripts. By default Wings🪽 limits these tools
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `mechanical-wings gateway` process.
...
1. Check Gateway logs: `/tmp/mechanical-wings/mechanical-wings-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.mechanical-wings/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).
4. Re-run `mechanical-wings security audit --deep` and confirm critical findings are resolved.

### Collect for a report

- Timestamp, gateway host OS + Wings🪽 version
...
Found a vulnerability in Wings🪽? Please report responsibly:

1. Email: [security@wings-of-world.ai](mailto:security@wings-of-world.ai)
