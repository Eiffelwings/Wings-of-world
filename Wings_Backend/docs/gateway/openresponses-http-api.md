...
Wings🪽’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`mechanical-wings agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "mechanical-wings"`, `model: "mechanical-wings/default"`, `model: "mechanical-wings/<agentId>"`, or `x-mechanical-wings-agent-id`
- use `x-mechanical-wings-model` when you want to override the selected agent's backend model
- use `x-mechanical-wings-session-key` for explicit session routing
- use `x-mechanical-wings-message-channel` when you want a non-default synthetic ingress channel context
...
For the canonical explanation of how agent-target models, `mechanical-wings/default`, embeddings pass-through, and backend model overrides fit together, see [OpenAI Chat Completions](/gateway/openai-http-api#agent-first-model-contract) and [Model list and agent routing](/gateway/openai-http-api#model-list-and-agent-routing).
...
- `previous_response_id`: Wings🪽 reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
curl -sS http://127.0.0.1:18789/v1/responses \
  ...
  -H 'x-mechanical-wings-agent-id: main' \
  -d '{
    "model": "mechanical-wings",
    "input": "hi"
  }'
```
