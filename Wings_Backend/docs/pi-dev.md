...
This guide summarizes a sane workflow for working on the pi integration in Wings🪽.
...
- Trigger the agent directly:
  - `pnpm mechanical-wings agent --message "Hello" --thinking low`
...
State lives under the Wings🪽 state directory. Default is `~/.mechanical-wings`. If `OPENCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `mechanical-wings.json` for config
