...
<Steps>
  ...
  <Step title="Configure">
    ```bash
    mechanical-wings configure --section web
    ```
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Wings🪽 plugin SDK" });
    ```
...
If no `provider` is set, Wings🪽 checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  Wings🪽 resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
<Tabs>
  <Tab title="Config file">
    Run `mechanical-wings configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.mechanical-wings/.env`.
