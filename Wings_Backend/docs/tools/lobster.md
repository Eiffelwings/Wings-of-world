---
title: Lobster
summary: "Typed workflow runtime for Wings🪽 with resumable approval gates."
read_when:
...
Lobster is a workflow shell that lets Wings🪽 run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Wings🪽 runs one Lobster tool call and gets a structured result.
...
Wings🪽 launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | mechanical-wings.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
mechanical-wings.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Wings🪽 tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Wings🪽 Gateway (see the [Lobster repo](https://github.com/mechanical-wings/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Wings🪽 keeps core tools enabled. To restrict core
...
→ mechanical-wings calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ mechanical-wings calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls Wings🪽 tools that do.
