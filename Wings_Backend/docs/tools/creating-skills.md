...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.mechanical-wings/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so Wings🪽 picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    mechanical-wings agent --message "give me a greeting"
    ```
...
| `metadata.mechanical-wings.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.mechanical-wings.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.mechanical-wings.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `mechanical-wings agent --message "..."` to test before sharing
...
| `~/.mechanical-wings/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with Wings🪽) | Lowest     | Global                |
