...
read_when:
  - Running Wings🪽 Gateway in WSL2 while Chrome lives on Windows
...
- Wings🪽 Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/mechanical-wings/mechanical-wings/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Wings🪽 and Chrome are on the same machine
...
- Wings🪽 points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Wings🪽 is not the problem yet.
...
Fix that before touching Wings🪽 config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Wings🪽 at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting Wings🪽 to succeed
...
```bash
mechanical-wings browser open https://example.com --browser-profile remote
mechanical-wings browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `mechanical-wings browser tabs` returns the target
...
3. Wings🪽 config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Wings🪽 config or Control UI auth
