---
summary: "ClawHub guide: public registry, native Wings🪽 install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **Wings🪽 skills and plugins**.

- Use native `mechanical-wings` commands to search/install/update skills and install
...
```bash
mechanical-wings plugins install clawhub:<package>
...
Native `mechanical-wings` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for Wings🪽 skills.
...
4. Users browse, download, and install skills in Wings🪽.
...
If you want to add new capabilities to your Wings🪽 agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `mechanical-wings skills install <skill-slug>`
3. Start a new Wings🪽 session so it picks up the new skill.
...
## How it fits into Wings🪽

Native `mechanical-wings skills install` installs into the active workspace `skills/`
directory. `mechanical-wings plugins install clawhub:...` records a normal managed
...
current working directory. If an Wings🪽 workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). Wings🪽 loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.mechanical-wings/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches Wings🪽 how to perform a
...
Interested in becoming a moderator? Ask in the Wings🪽 Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to Wings🪽 workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/mechanical-wings/skills` and `~/.mechanical-wings/skills`). This is designed to find older skill installs without extra flags.
