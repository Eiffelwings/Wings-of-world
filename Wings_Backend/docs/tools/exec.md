...
- `gateway`/`node` approvals are controlled by `~/.mechanical-wings/exec-approvals.json`.
...
- Wings🪽 sets `OPENCLAW_SHELL=exec` in the spawned command environment (including PTY and sandbox execution) so shell/profile rules can detect exec-tool context.
...
- `host=sandbox`: runs `sh -lc` (login shell) inside the container, so `/etc/profile` may reset `PATH`.
  Wings🪽 prepends `env.PATH` after profile sourcing via an internal env var (no shell interpolation);
...
`mechanical-wings security audit` warns when interpreter/runtime `safeBins` entries are missing explicit profiles, and `mechanical-wings doctor --fix` can scaffold missing custom `safeBinProfiles` entries.
`mechanical-wings security audit` and `mechanical-wings doctor` also warn when you explicitly add broad-behavior bins such as `jq` back into `safeBins`.
