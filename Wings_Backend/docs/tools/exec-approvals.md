...
- **gateway host** → `mechanical-wings` process on the gateway machine
...
- For shell scripts and direct interpreter/runtime file invocations, Wings🪽 also tries to bind
...
`~/.mechanical-wings/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.mechanical-wings/exec-approvals.sock",
...
When `tools.exec.strictInlineEval=true`, Wings🪽 treats inline code-eval forms as approval-only even if the interpreter binary itself is allowlisted.
...
- allowlist entries live in host-local `~/.mechanical-wings/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `mechanical-wings approvals allowlist ...`).
- `mechanical-wings security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `mechanical-wings doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Wings🪽 still rejects the `env` builtin in safe-bin
...
`~/.mechanical-wings/exec-approvals.json` directly.

CLI: `mechanical-wings approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
...
These clients are opt-in. If a channel does not have exec approvals enabled, Wings🪽 does not treat
...
topics, Wings🪽 preserves the topic for the approval prompt and the post-approval follow-up.
