...
`mechanical-wings agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    mechanical-wings agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    mechanical-wings agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    mechanical-wings agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    mechanical-wings agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    mechanical-wings agent --agent ops --message "Generate report" \
...
mechanical-wings agent --to +15555550123 --message "Trace logs" --verbose on --json

# Turn with thinking level
mechanical-wings agent --session-id 1234 --message "Summarize inbox" --thinking medium

# Deliver to a different channel than the session
mechanical-wings agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
