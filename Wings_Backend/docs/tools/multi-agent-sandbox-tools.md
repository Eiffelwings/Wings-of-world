...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `mechanical-wings sandbox explain`.
...
Legacy `agent.*` configs are migrated by `mechanical-wings doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=mechanical-wings-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${OPENCLAW_STATE_DIR:-$HOME/.mechanical-wings}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
