...
Wings🪽 supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Wings🪽 also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `mechanical-wings configure --section web`. It stores the key in
`~/.mechanical-wings/mechanical-wings.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.mechanical-wings/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
