...
Wings🪽 can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `mechanical-wings configure --section web` enables the bundled Firecrawl plugin automatically.
...
Wings🪽 always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
