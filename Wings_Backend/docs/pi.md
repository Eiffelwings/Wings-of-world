...
summary: "Architecture of Wings🪽's embedded Pi agent integration and session lifecycle"
read_when:
  - Understanding Pi SDK integration design in Wings🪽
  - Modifying agent session lifecycle, tooling, or provider wiring for Pi
---

# Pi Integration Architecture

This document describes how Wings🪽 integrates with [pi-coding-agent](https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent) and its sibling packages (`pi-ai`, `pi-agent-core`, `pi-tui`) to power its AI agent capabilities.

## Overview

Wings🪽 uses the pi SDK to embed an AI coding agent into its messaging gateway architecture. Instead of spawning pi as a subprocess or using RPC mode, Wings🪽 directly imports and instantiates pi's `AgentSession` via `createAgentSession()`. This embedded approach provides:
...
| `pi-tui`          | Terminal UI components (used in Wings🪽's local TUI mode)                                             |
...
├── pi-tools.ts                    # createWings🪽CodingTools()
...
├── mechanical-wings-tools.ts              # Wings🪽-specific tools
...
Image injection is prompt-local: Wings🪽 loads image refs from the current prompt and
...
2. **Custom Replacements**: Wings🪽 replaces bash with `exec`/`process`, customizes read/edit/write for sandbox
3. **Wings🪽 Tools**: messaging, browser, canvas, sessions, cron, gateway, etc.
...
This ensures Wings🪽's policy filtering, sandbox integration, and extended toolset remain consistent across providers.

## System Prompt Construction

The system prompt is built in `buildAgentSystemPrompt()` (`system-prompt.ts`). It assembles a full prompt with sections including Tooling, Tool Call Style, Safety guardrails, Wings🪽 CLI reference, Skills, Docs, Workspace, Sandbox, Messaging, Reply Tags, Voice, Silent Replies, Heartbeats, Runtime metadata, plus Memory and Reactions when enabled, and optional context files and extra system prompt content. Sections are trimmed for minimal prompt mode used by subagents.
...
Wings🪽 wraps this with `guardSessionManager()` for tool result safety.
...
Wings🪽 maintains an auth profile store with multiple API keys per provider:
...
Wings🪽 loads custom pi extensions for specialized behavior:
...
Wings🪽 also has a local TUI mode that uses pi-tui components directly:
...
| Aspect          | Pi CLI                  | Wings🪽 Embedded                                                                              |
| --------------- | ----------------------- | ---------------------------------------------------------------------------------------------- |
| Invocation      | `pi` command / RPC      | SDK via `createAgentSession()`                                                                 |
| Tools           | Default coding tools    | Custom Wings🪽 tool suite                                                                     |
| System prompt   | AGENTS.md + prompts     | Dynamic per-channel/context                                                                    |
| Session storage | `~/.pi/agent/sessions/` | `~/.mechanical-wings/agents/<agentId>/sessions/` (or `$OPENCLAW_STATE_DIR/agents/<agentId>/sessions/`) |
