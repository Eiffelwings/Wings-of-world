---
summary: "CLI reference for `mechanical-wings message` (send + channel actions)"
read_when:
...
```
mechanical-wings message <subcommand> [flags]
```
...
- On cache miss, Wings🪽 will attempt a live directory lookup when the provider supports it.
...
- `mechanical-wings message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Wings🪽 does not force a `default` account SecretRef scope
...
```
mechanical-wings message send --channel discord \
...
```
mechanical-wings message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
mechanical-wings message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
mechanical-wings message poll --channel msteams \
...
```
mechanical-wings message react --channel slack \
...
```
mechanical-wings message react --channel signal \
...
```
mechanical-wings message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
mechanical-wings message send --channel telegram --target @mychat \
