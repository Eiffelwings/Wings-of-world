---
summary: "Wings🪽 Gateway CLI (`mechanical-wings gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is Wings🪽’s WebSocket server (channels, nodes, sessions, hooks).

Subcommands in this page live under `mechanical-wings gateway …`.
...
- By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.mechanical-wings/mechanical-wings.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
...
```bash
mechanical-wings gateway health --url ws://127.0.0.1:18789
```
...
mechanical-wings gateway status --require-rpc
```
...
mechanical-wings gateway probe --json
```
...
```bash
mechanical-wings gateway probe --ssh user@gateway-host
```
...
```bash
mechanical-wings gateway call status
mechanical-wings gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
`gateway discover` scans for Gateway beacons (`_mechanical-wings-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `mechanical-wings.internal.`) and set up split DNS + a DNS server; see [/gateway/bonjour](/gateway/bonjour)
...
```bash
mechanical-wings gateway discover
```
...
```bash
mechanical-wings gateway discover --timeout 4000
mechanical-wings gateway discover --json | jq '.beacons[].wsUrl'
```
