---
summary: "CLI reference for `mechanical-wings onboard` (interactive onboarding)"
read_when:
...
```bash
mechanical-wings onboard
mechanical-wings onboard --flow quickstart
mechanical-wings onboard --flow manual
mechanical-wings onboard --mode remote --remote-url wss://gateway-host:18789
```
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `mechanical-wings gateway run`.
...
- Fastest first chat: `mechanical-wings dashboard` (Control UI, no channel setup).
