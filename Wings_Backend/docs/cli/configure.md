---
summary: "CLI reference for `mechanical-wings configure` (interactive configuration prompts)"
read_when:
...
Tip: `mechanical-wings config` without a subcommand opens the same wizard. Use
`mechanical-wings config get|set|unset` for non-interactive edits.
...
```bash
mechanical-wings configure
mechanical-wings configure --section model --section channels
```
