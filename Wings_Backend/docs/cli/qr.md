---
summary: "CLI reference for `mechanical-wings qr` (generate iOS pairing QR + setup code)"
read_when:
...
```bash
mechanical-wings qr
mechanical-wings qr --setup-code-only
mechanical-wings qr --json
mechanical-wings qr --remote
mechanical-wings qr --url wss://gateway.example/ws
```
