...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Wings🪽 Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`mechanical-wings acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Wings🪽 gateway or agent instead.                                                                                                                                     |
...
```bash
mechanical-wings acp

# Remote Gateway
mechanical-wings acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
mechanical-wings acp --url wss://gateway-host:18789 --token-file ~/.mechanical-wings/gateway.token

# Attach to an existing session key
mechanical-wings acp --session agent:main:main

# Attach by label (must already exist)
mechanical-wings acp --session-label "support inbox"

# Reset the session key before the first prompt
mechanical-wings acp --session agent:main:main --reset-session
```
...
```bash
mechanical-wings acp client

# Point the spawned bridge at a remote Gateway
mechanical-wings acp client --server-args --url wss://gateway-host:18789 --token-file ~/.mechanical-wings/gateway.token

# Override the server command (default: mechanical-wings)
mechanical-wings acp client --server "node" --server-args mechanical-wings.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an Wings🪽 Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `mechanical-wings acp` over stdio.

Example config (persisted):

```bash
mechanical-wings config set gateway.remote.url wss://gateway-host:18789
mechanical-wings config set gateway.remote.token <token>
```
...
```bash
mechanical-wings acp --session agent:main:main
mechanical-wings acp --session agent:design:main
mechanical-wings acp --session agent:qa:bug-123
```
...
Wings🪽 bot over ACP, use `acpx` with its built-in `mechanical-wings` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx mechanical-wings` at `mechanical-wings acp`.
3. Target the Wings🪽 session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Wings🪽 ACP session
acpx mechanical-wings exec "Summarize the active Wings🪽 session state."

# Persistent named session for follow-up turns
acpx mechanical-wings sessions ensure --name codex-bridge
acpx mechanical-wings -s codex-bridge --cwd /path/to/repo \
  "Ask my Wings🪽 work agent for recent context relevant to this repo."
```

If you want `acpx mechanical-wings` to target a specific Gateway and session key every
time, override the `mechanical-wings` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "mechanical-wings": {
      "command": "env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 mechanical-wings acp --url ws://127.0.0.1:18789 --token-file ~/.mechanical-wings/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Wings🪽 checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env OPENCLAW_HIDE_BANNER=1 OPENCLAW_SUPPRESS_NOTES=1 node mechanical-wings.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Wings🪽 agent without scraping a terminal.
...
In Zed, open the Agent panel and select “Wings🪽 ACP” to start a thread.
...
- `mechanical-wings acp client` sets `OPENCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `mechanical-wings`).
