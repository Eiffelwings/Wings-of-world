---
summary: "Deploy Wings🪽 Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Wings🪽 on a Kubernetes cluster
  - You want to test Wings🪽 in a Kubernetes environment
title: "Kubernetes"
---

# Wings🪽 on Kubernetes

A minimal starting point for running Wings🪽 on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Wings🪽 is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/mechanical-wings 18789:18789 -n mechanical-wings
...
```bash
kubectl get secret mechanical-wings-secrets -n mechanical-wings -o jsonpath='{.data.OPENCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: mechanical-wings (configurable via OPENCLAW_NAMESPACE)
├── Deployment/mechanical-wings        # Single pod, init container + gateway
├── Service/mechanical-wings           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/mechanical-wings-config  # mechanical-wings.json + AGENTS.md
└── Secret/mechanical-wings-secrets    # Gateway token + API keys
```
...
Edit `mechanical-wings.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret mechanical-wings-secrets -n mechanical-wings \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/mechanical-wings -n mechanical-wings
```
...
```yaml
image: ghcr.io/mechanical-wings/mechanical-wings:latest # or pin to a specific version from https://github.com/mechanical-wings/mechanical-wings/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # mechanical-wings.json + AGENTS.md
