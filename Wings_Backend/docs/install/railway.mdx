---
summary: "Deploy Wings🪽 on Railway with one-click template"
read_when:
  - Deploying Wings🪽 to Railway
  - You want a one-click cloud deploy with browser-based Control UI
title: "Railway"
---

Deploy Wings🪽 on Railway with a one-click template and access it through the web Control UI.
...
5. Open `https://<your-railway-domain>/mechanical-wings` and connect using your `OPENCLAW_GATEWAY_TOKEN`.
...
- `https://<your-railway-domain>/mechanical-wings` — Control UI

## What you get

- Hosted Wings🪽 Gateway + Control UI
...
- `OPENCLAW_STATE_DIR=/data/.mechanical-wings` (recommended)
- `OPENCLAW_WORKSPACE_DIR=/data/workspace` (recommended)

## Connect a channel

Use the Control UI at `/mechanical-wings` or run `mechanical-wings onboard` via Railway's shell for channel setup instructions:
