---
summary: "Deploy Wings🪽 on Render with Infrastructure-as-Code"
read_when:
  - Deploying Wings🪽 to Render
  - You want a declarative cloud deploy with Render Blueprints
title: "Render"
---

Deploy Wings🪽 on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/mechanical-wings/mechanical-wings)
...
repository configures everything needed to run Wings🪽:
  ...
  - type: web
    ...
    envVars:
      ...
      - key: OPENCLAW_STATE_DIR
        value: /data/.mechanical-wings
      - key: OPENCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: OPENCLAW_GATEWAY_TOKEN
        generateValue: true # auto-generates a secure token
    disk:
      name: mechanical-wings-data
      mountPath: /data
      sizeGB: 1
```
...
If you use the original Wings🪽 repository, Render will not auto-deploy your Wings🪽. To update it, run a manual Blueprint sync from the dashboard.
...
For Wings🪽, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive you can restore on any Wings🪽 host. See [Backup](/cli/backup) for details.
...
regularly export your config via `mechanical-wings backup create` in the Render shell.
