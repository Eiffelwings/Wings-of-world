---
summary: "Run Wings🪽 Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Wings🪽 running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# Wings🪽 on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent Wings🪽 Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Wings🪽 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Wings🪽 Gateway in Docker
- Persist `~/.mechanical-wings` + `~/.mechanical-wings/workspace` on the host (survives restarts/rebuilds)
...
<Steps>
  ...
  <Step title="Clone the Wings🪽 repository">
    ```bash
  ...
  <Step title="GCP-specific launch notes">
    ...
    ```bash
    docker compose run --rm mechanical-wings-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
