---
summary: "Optional Docker-based setup and onboarding for Wings🪽"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Wings🪽 on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export OPENCLAW_IMAGE="ghcr.io/mechanical-wings/mechanical-wings:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/mechanical-wings/mechanical-wings/pkgs/container/mechanical-wings).
  ...
  <Step title="Complete onboarding">
    ...
    `mechanical-wings-gateway` directly. `mechanical-wings-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm mechanical-wings-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm mechanical-wings-cli channels login

    # Telegram
    docker compose run --rm mechanical-wings-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm mechanical-wings-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t mechanical-wings:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node mechanical-wings-gateway \
...
docker compose up -d mechanical-wings-gateway
```
...
<Note>
Because `mechanical-wings-cli` shares `mechanical-wings-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d mechanical-wings-gateway`, run onboarding
and setup-time config writes through `mechanical-wings-gateway` with
...
```bash
docker compose exec mechanical-wings-gateway node dist/index.js health --token "$OPENCLAW_GATEWAY_TOKEN"
```
...
Docker Compose bind-mounts `OPENCLAW_CONFIG_DIR` to `/home/node/.mechanical-wings` and
`OPENCLAW_WORKSPACE_DIR` to `/home/node/.mechanical-wings/workspace`, so those paths
...
and rolling file logs under `/tmp/mechanical-wings/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/mechanical-wings/mechanical-wings/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/mechanical-wings/mechanical-wings/blob/main/scripts/shell-helpers/README.md).
...
<AccordionGroup>
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm mechanical-wings-cli gateway probe
    docker compose run -T --rm mechanical-wings-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `mechanical-wings-cli` uses `network_mode: "service:mechanical-wings-gateway"` so CLI
    ...
    `no-new-privileges` on `mechanical-wings-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.mechanical-wings`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/mechanical-wings-config /path/to/mechanical-wings-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export OPENCLAW_HOME_VOLUME="mechanical-wings_home"`
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/mechanical-wings/mechanical-wings/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Wings🪽 runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm mechanical-wings-cli dashboard --no-open
    docker compose run --rm mechanical-wings-cli devices list
    docker compose run --rm mechanical-wings-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm mechanical-wings-cli config set gateway.mode local
    docker compose run --rm mechanical-wings-cli config set gateway.bind lan
    docker compose run --rm mechanical-wings-cli devices list --url ws://127.0.0.1:18789
    ```
