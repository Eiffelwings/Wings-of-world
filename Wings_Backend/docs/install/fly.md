---
title: Fly.io
summary: "Step-by-step Fly.io deployment for Wings🪽 with persistent storage and HTTPS"
read_when:
  - Deploying Wings🪽 on Fly.io
  - Setting up Fly volumes, secrets, and first-run config
---

# Fly.io Deployment

**Goal:** Wings🪽 Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
<Steps>
  <Step title="Create the Fly app">
    ...
    fly apps create my-mechanical-wings

    # Create a persistent volume (1GB is usually enough)
    fly volumes create mechanical-wings_data --size 1 --region iad
    ```
  ...
  <Step title="Configure fly.toml">
    ...
    **Security note:** The default config exposes a public URL. For a hardened deployment with no public IP, see [Private Deployment](#private-deployment-hardened) or use `fly.private.toml`.
    ...
    ```toml
    app = "my-mechanical-wings"  # Your app name
    primary_region = "iad"
  ...
  <Step title="Set secrets">
    ...
    **Notes:**
    ...
    - **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `mechanical-wings.json` where they could be accidentally exposed or logged.
  ...
  <Step title="Create config file">
    ...
    ```bash
    mkdir -p /data
    cat > /data/mechanical-wings.json << 'EOF'
    {
      "agents": {
        "defaults": {
          "model": {
    ...
    **Note:** With `OPENCLAW_STATE_DIR=/data`, the config path is `/data/mechanical-wings.json`.
  ...
  <Step title="Access the Gateway">
    ...
    Or visit `https://my-mechanical-wings.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/mechanical-wings.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/mechanical-wings.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/mechanical-wings.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/mechanical-wings.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/mechanical-wings.json"
```
...
```bash
# List current IPs
fly ips list -a my-mechanical-wings

# Release public IPs
fly ips release <public-ipv4> -a my-mechanical-wings
fly ips release <public-ipv6> -a my-mechanical-wings
...
fly ips allocate-v6 --private -a my-mechanical-wings
```
...
fly proxy 3000:3000 -a my-mechanical-wings
...
```bash
fly ssh console -a my-mechanical-wings
```
...
- Keep Wings🪽 up to date: [Updating](/install/updating)
