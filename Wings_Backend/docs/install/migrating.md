---
summary: "Move (migrate) an Wings🪽 install from one machine to another"
read_when:
  - You are moving Wings🪽 to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating Wings🪽 to a New Machine

This guide moves an Wings🪽 gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.mechanical-wings/` by default) and your **workspace**, you preserve:

- **Config** -- `mechanical-wings.json` and all gateway settings
...
<Tip>
Run `mechanical-wings status` on the old machine to confirm your state directory path.
Custom profiles use `~/.mechanical-wings-<profile>/` or a path set via `OPENCLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf mechanical-wings-state.tgz .mechanical-wings
    ```

    If you use multiple profiles (e.g. `~/.mechanical-wings-work`), archive each separately.

  </Step>

  <Step title="Install Wings🪽 on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.mechanical-wings/` -- you will overwrite it next.
  </Step>
  ...
  <Step title="Copy state directory and workspace">
    ...
    ```bash
    cd ~
    tar -xzf mechanical-wings-state.tgz
    ```
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    ...
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `mechanical-wings doctor`.
  </Accordion>

  <Accordion title="Copying only mechanical-wings.json">
...
- [ ] `mechanical-wings status` shows the gateway running
