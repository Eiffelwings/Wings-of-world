...
Wings🪽 ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/mechanical-wings`, override with
...
mechanical-wings update --tag 2026.3.26
...
mechanical-wings update --tag mechanical-wings@2026.3.26
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `mechanical-wings update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  Wings🪽 prompts for confirmation (skip with `--yes`).

## Dry run

Preview what `mechanical-wings update` would do without making changes:

```bash
mechanical-wings update --dry-run
mechanical-wings update --channel beta --dry-run
mechanical-wings update --tag 2026.3.26 --dry-run
mechanical-wings update --dry-run --json
```
...
When you switch channels with `mechanical-wings update`, Wings🪽 also syncs plugin
sources:
