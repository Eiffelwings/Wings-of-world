---
summary: "Host Wings🪽 on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Wings🪽 on Oracle Cloud
  - Looking for free VPS hosting for Wings🪽
...
Run a persistent Wings🪽 Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@mechanical-wings`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=mechanical-wings --reset` to re-authenticate.

**Gateway will not start** -- Run `mechanical-wings doctor --non-interactive` and check logs with `journalctl --user -u mechanical-wings-gateway -n 50`.
...
- [Updating](/install/updating) -- keep Wings🪽 up to date
