...
- Jobs persist under `~/.mechanical-wings/cron/` so restarts don’t lose schedules.
...
- For upgrades, `mechanical-wings doctor --fix` can normalize legacy cron store fields before the scheduler touches them.
...
mechanical-wings cron list
mechanical-wings cron run <job-id>
mechanical-wings cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.mechanical-wings/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `mechanical-wings cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Wings🪽 applies a
...
If `delivery` is omitted for isolated jobs, Wings🪽 defaults to `announce`.
...
- CLI equivalents: `mechanical-wings cron add --light-context ...` and `mechanical-wings cron edit --light-context`.
...
- `schedule.at` accepts ISO 8601. Tool/API values without a timezone are treated as UTC; the CLI also accepts `mechanical-wings cron add|edit --at "<offset-less-iso>" --tz <iana>` for local wall-clock one-shots.
...
- Job store: `~/.mechanical-wings/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.mechanical-wings/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned by size and line count).
- Isolated cron run sessions in `sessions.json` are pruned by `cron.sessionRetention` (default `24h`; set `false` to disable).
- Override store path: `cron.store` in config.

## Retry policy

When a job fails, Wings🪽 classifies errors as **transient** (retryable) or **permanent** (disable immediately).
...
```json5
{
  cron: {
    ...
    store: "~/.mechanical-wings/cron/jobs.json",
...
- For removed run sessions no longer referenced by the session store, Wings🪽 archives transcript files and purges old deleted archives on the same retention window.
...
- review growth periodically with `mechanical-wings cron runs` and adjust retention before logs become large
...
mechanical-wings cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
mechanical-wings cron edit <jobId> --agent ops
mechanical-wings cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
mechanical-wings cron run <jobId>
mechanical-wings cron run <jobId> --due
```

`cron.run` now acknowledges once the manual run is queued, not after the job finishes. Successful queue responses look like `{ ok: true, enqueued: true, runId }`. If the job is already running or `--due` finds nothing due, the response stays `{ ok: true, ran: false, reason }`. Use `mechanical-wings cron runs --id <jobId>` or the `cron.runs` gateway method to inspect the eventual finished entry.

Edit an existing job (patch fields):

```bash
mechanical-wings cron edit <jobId> \
...
```bash
mechanical-wings cron edit <jobId> --exact
```
...
```bash
mechanical-wings system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`mechanical-wings system event`](/cli/system).
...
- Wings🪽 applies exponential retry backoff for recurring jobs after consecutive errors:
