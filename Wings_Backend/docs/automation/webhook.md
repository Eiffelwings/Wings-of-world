...
read_when:
  ...
  - Wiring external systems into Wings🪽
title: "Webhooks"
---
...
- `x-mechanical-wings-token: <token>`
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Wings🪽 config directory (typically `~/.mechanical-wings/hooks/transforms`).
...
- `mechanical-wings webhooks gmail setup` writes `hooks.gmail` config for `mechanical-wings webhooks gmail run`.
...
curl -X POST http://127.0.0.1:18789/hooks/agent \
  -H 'x-mechanical-wings-token: SECRET' \
  -H 'Content-Type: application/json' \
