...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be inspected with `mechanical-wings hooks`, while hook-pack installation and updates now go through `mechanical-wings plugins`.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in Wings🪽. See [Webhook Hooks](/automation/webhook) or use `mechanical-wings webhooks` for Gmail helper commands.

Hooks can also be bundled inside plugins; see [Plugin hooks](/plugins/architecture#provider-runtime-hooks). `mechanical-wings hooks list` shows both standalone hooks and plugin-managed hooks.
...
If you can write a small TypeScript function, you can write a hook. Managed and bundled hooks are trusted local code. Workspace hooks are discovered automatically, but Wings🪽 keeps them disabled until you explicitly enable them via the CLI or config.
...
- Extend Wings🪽's behavior without modifying core code

## Getting Started

### Bundled Hooks

Wings🪽 ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.mechanical-wings/workspace/memory/`) when you issue `/new` or `/reset`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.mechanical-wings/logs/commands.log`
- **🚀 boot-md**: Runs `BOOT.md` when the gateway starts (requires internal hooks enabled)

List available hooks:

```bash
mechanical-wings hooks list
```

Enable a hook:

```bash
mechanical-wings hooks enable session-memory
```

Check hook status:

```bash
mechanical-wings hooks check
```

Get detailed information:

```bash
mechanical-wings hooks info session-memory
```

### Onboarding

During onboarding (`mechanical-wings onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.

### Trust Boundary

Hooks run inside the Gateway process. Treat bundled hooks, managed hooks, and `hooks.internal.load.extraDirs` as trusted local code. Workspace hooks under `<workspace>/hooks/` are repo-local code, so Wings🪽 requires an explicit enable step before loading them.

## Hook Discovery

Hooks are automatically discovered from these directories, in order of increasing override precedence:

1. **Bundled hooks**: shipped with Wings🪽; located at `<mechanical-wings>/dist/hooks/bundled/` for npm installs (or a sibling `hooks/bundled/` for compiled binaries)
2. **Plugin hooks**: hooks bundled inside installed plugins (see [Plugin hooks](/plugins/architecture#provider-runtime-hooks))
3. **Managed hooks**: `~/.mechanical-wings/hooks/` (user-installed, shared across workspaces; can override bundled and plugin hooks). **Extra hook directories** configured via `hooks.internal.load.extraDirs` are also treated as managed hooks and share the same override precedence.
...
Hook packs are standard npm packages that export one or more hooks via `mechanical-wings.hooks` in
`package.json`. Install them with:

```bash
mechanical-wings plugins install <path-or-spec>
```
...
those to a prerelease, Wings🪽 stops and asks you to opt in explicitly with a
...
Hook packs can ship dependencies; they will be installed under `~/.mechanical-wings/hooks/<id>`.
Each `mechanical-wings.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `mechanical-wings plugins install` installs hook-pack dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.wings-of-world.ai/automation/hooks#my-hook
metadata:
  { "mechanical-wings": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.mechanical-wings` object supports:
...
```typescript
{
  ...
  cfg: Wings🪽Config            // Current gateway config
}
```
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before Wings🪽 persists them.
...
- **Managed hooks** (`~/.mechanical-wings/hooks/`): Shared across workspaces; can override bundled and plugin hooks

### 2. Create Directory Structure

```bash
mkdir -p ~/.mechanical-wings/hooks/my-hook
cd ~/.mechanical-wings/hooks/my-hook
```
...
metadata: { "mechanical-wings": { "emoji": "🎯", "events": ["command:new"] } }
---
...
# Enable it
mechanical-wings hooks enable my-hook
...
mechanical-wings hooks list --eligible

# Verbose output (show missing requirements)
mechanical-wings hooks list --verbose

# JSON output
mechanical-wings hooks list --json
```
...
# JSON output
mechanical-wings hooks info session-memory --json
```
...
# JSON output
mechanical-wings hooks check --json
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.mechanical-wings/workspace`)
...
```bash
mechanical-wings hooks enable bootstrap-extra-files
```
...
**Output**: `~/.mechanical-wings/logs/commands.log`
...
tail -n 20 ~/.mechanical-wings/logs/commands.log

# Pretty-print with jq
cat ~/.mechanical-wings/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.mechanical-wings/logs/commands.log | jq .
```

**Enable**:

```bash
mechanical-wings hooks enable command-logger
```
...
```bash
mechanical-wings hooks enable boot-md
```
...
```yaml
metadata: { "mechanical-wings": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "mechanical-wings": { "events": ["command"] } } # General - more overhead
```
...
```bash
mechanical-wings hooks info my-hook
```
...
# Other platforms
tail -f ~/.mechanical-wings/gateway.log
```
...
1. Check directory structure:
   ...
   ```bash
   ls -la ~/.mechanical-wings/hooks/my-hook/
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.mechanical-wings/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.mechanical-wings/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.mechanical-wings/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/mechanical-wings/mechanical-wings/tree/main/src/hooks/bundled)
