---
summary: "Gmail Pub/Sub push wired into Wings🪽 webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to Wings🪽
...
Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> Wings🪽 webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- Wings🪽 hooks enabled (see [Webhooks](/automation/webhook)).
...
under `~/.mechanical-wings/hooks/transforms` (see [Webhooks](/automation/webhook)).

## Wizard (recommended)

Use the Wings🪽 helper to wire everything together (installs deps on macOS via brew):
...
- Writes `hooks.gmail` config for `mechanical-wings webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, Wings🪽 automatically sets
...
```bash
mechanical-wings webhooks gmail run
```
...
- `--hook-url` points to Wings🪽 `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to Wings🪽.

Recommended: `mechanical-wings webhooks gmail run` wraps the same flow and auto-renews the watch.
