...
read_when:
  - You want to understand how Wings🪽 assembles model context
...
A **context engine** controls how Wings🪽 builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

Wings🪽 ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.mechanical-wings/mechanical-wings.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other Wings🪽 plugin. Install
...
mechanical-wings plugins install -l ./my-context-engine
```

Then enable the plugin and select it as the active engine in your config:

```json5
// mechanical-wings.json
{
  plugins: {
    slots: {
...
Every time Wings🪽 runs a model prompt, the context engine participates at
...
Wings🪽 currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. Wings🪽
...
The built-in `legacy` engine preserves Wings🪽's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. Wings🪽 uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. Wings🪽 disables Pi's built-in
...
`ownsCompaction: false` does **not** mean Wings🪽 automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `mechanical-wings/plugin-sdk/core` to use
  Wings🪽's built-in compaction behavior.
...
Wings🪽 resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to Wings🪽's built-in summarization. Plugin engines can implement
...
- Use `mechanical-wings doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, Wings🪽
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `mechanical-wings plugins install -l ./my-engine` to link a
