---
title: "Memory"
summary: "How Wings🪽 memory works (workspace files + automatic memory flush)"
read_when:
...
Wings🪽 memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, Wings🪽 loads both (deduplicated by realpath so symlinks pointing to the same file are not injected twice).
  - **Only load in the main, private session** (never in group contexts).

These files live under the workspace (`agents.defaults.workspace`, default
`~/.mechanical-wings/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

Wings🪽 exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, Wings🪽 triggers a **silent,
...
Wings🪽 can build a small vector index over `MEMORY.md` and `memory/*.md` so
