...
- `/usage cost` in chats: local cost summary aggregated from Wings🪽 session logs.
- CLI: `mechanical-wings status --usage` prints a full per-provider breakdown.
- CLI: `mechanical-wings channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
