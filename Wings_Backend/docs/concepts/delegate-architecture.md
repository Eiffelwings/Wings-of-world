---
summary: "Delegate architecture: running Wings🪽 as a named agent on behalf of an organization"
...
Goal: run Wings🪽 as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Wings🪽 agent that:
...
Wings🪽's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Wings🪽's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.mechanical-wings/workspace-delegate",
  tools: {
...
- Cron run history: `~/.mechanical-wings/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.mechanical-wings/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Wings🪽's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
mechanical-wings agents add delegate
```

This creates:

- Workspace: `~/.mechanical-wings/workspace-delegate`
- State: `~/.mechanical-wings/agents/delegate/agent`
- Sessions: `~/.mechanical-wings/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.mechanical-wings/workspace" },
      {
        id: "delegate",
        workspace: "~/.mechanical-wings/workspace-delegate",
        tools: {
...
~/.mechanical-wings/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.mechanical-wings/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.mechanical-wings/workspace-org",
        agentDir: "~/.mechanical-wings/agents/org-assistant/agent",
