---
summary: "Use Synthetic's Anthropic-compatible API in Wings🪽"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Wings🪽 registers it as the
...
```bash
mechanical-wings onboard --auth-choice synthetic-api-key
```
...
Note: Wings🪽's Anthropic client appends `/v1` to the base URL, so use
