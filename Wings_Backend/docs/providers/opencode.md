---
summary: "Use OpenCode Zen and Go catalogs with Wings🪽"
read_when:
...
OpenCode exposes two hosted catalogs in Wings🪽:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. Wings🪽 keeps the runtime provider ids
...
mechanical-wings onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
mechanical-wings onboard --auth-choice opencode-go
mechanical-wings onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
