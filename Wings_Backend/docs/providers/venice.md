---
summary: "Use Venice AI privacy-focused models in Wings🪽"
read_when:
  - You want privacy-focused inference in Wings🪽
...
```bash
mechanical-wings onboard --auth-choice venice-api-key
```
...
```bash
mechanical-wings agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, Wings🪽 shows all available Venice models. Pick based on your needs:
...
```bash
mechanical-wings models set venice/kimi-k2-5
mechanical-wings models set venice/claude-opus-4-6
```

List all available models:

```bash
mechanical-wings models list | grep venice
```

## Configure via `mechanical-wings configure`

1. Run `mechanical-wings configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Wings🪽 automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
mechanical-wings agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
mechanical-wings agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
mechanical-wings agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
mechanical-wings agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
mechanical-wings agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `mechanical-wings models list` to see currently available models. Some models may be temporarily offline.
