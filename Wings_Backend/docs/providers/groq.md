...
read_when:
  - You want to use Groq with Wings🪽
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. Wings🪽 connects
...
media-understanding provider, Wings🪽 uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.mechanical-wings/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `mechanical-wings models list | grep groq`
