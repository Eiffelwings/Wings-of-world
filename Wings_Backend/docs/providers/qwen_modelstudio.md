...
read_when:
  - You want to use Qwen (Alibaba Cloud Model Studio) with Wings🪽
...
```bash
# China endpoint
mechanical-wings onboard --auth-choice modelstudio-standard-api-key-cn

# Global/Intl endpoint
mechanical-wings onboard --auth-choice modelstudio-standard-api-key
```
...
```bash
# China endpoint
mechanical-wings onboard --auth-choice modelstudio-api-key-cn

# Global/Intl endpoint
mechanical-wings onboard --auth-choice modelstudio-api-key
```
...
`~/.mechanical-wings/.env` or via `env.shellEnv`).
