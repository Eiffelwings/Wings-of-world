...
This is the full reference for `mechanical-wings onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.mechanical-wings/mechanical-wings.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `mechanical-wings doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.mechanical-wings/credentials/oauth.json`; auth profiles live in `~/.mechanical-wings/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.mechanical-wings/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.mechanical-wings/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `mechanical-wings pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `mechanical-wings configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `mechanical-wings health`.
    - Tip: `mechanical-wings status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
- Stores it under `~/.mechanical-wings/tools/signal-cli/<version>/`.
...
Typical fields in `~/.mechanical-wings/mechanical-wings.json`:
...
`mechanical-wings agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.mechanical-wings/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.mechanical-wings/agents/<agentId>/sessions/`.
