---
summary: "How Wings🪽 builds prompt context and reports token usage + costs"
read_when:
...
Wings🪽 tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Wings🪽 assembles its own system prompt on every run. It includes:
...
For images, Wings🪽 downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Wings🪽 session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `mechanical-wings status --usage` and `mechanical-wings channels list` show
...
`cacheWrite`. If pricing is missing, Wings🪽 shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Wings🪽 can
...
Anthropic's 1M context window is currently beta-gated. Wings🪽 can inject the
...
Wings🪽 skips the `context-1m-*` beta header because Anthropic currently
