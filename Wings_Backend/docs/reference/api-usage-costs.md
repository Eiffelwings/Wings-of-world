...
Wings🪽 features that can generate provider usage or paid API calls.
...
- `mechanical-wings status --usage` and `mechanical-wings channels list` show provider **usage windows**
...
Wings🪽 can pick up credentials from:
...
- `mechanical-wings status --usage`
- `mechanical-wings models status --json`
...
`mechanical-wings models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
