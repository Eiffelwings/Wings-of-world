...
- With Anthropic API-key auth profiles, Wings🪽 seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, Wings🪽 injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
Wings🪽 exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.mechanical-wings/logs/cache-trace.jsonl" # optional
