...
Wings🪽 integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Wings🪽 owns lifecycle when `channels.signal.autoStart=true`.
...
- Wings🪽 spawns `imsg rpc` as a child process (legacy iMessage integration).
