...
- Same path: run the app from a fixed location (for Wings🪽, `dist/Wings🪽.app`).
...
```bash
sudo tccutil reset Accessibility ai.mechanical-wings.mac
sudo tccutil reset ScreenCapture ai.mechanical-wings.mac
...
Workaround: move files into the Wings🪽 workspace (`~/.mechanical-wings/workspace`) if you want to avoid per-folder grants.
