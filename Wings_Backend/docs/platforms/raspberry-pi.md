---
summary: "Wings🪽 on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Wings🪽 on a Raspberry Pi
  - Running Wings🪽 on ARM devices
...
Run a persistent, always-on Wings🪽 Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u mechanical-wings -f
```

## 9) Access the Wings🪽 Dashboard
...
```bash
ssh user@gateway-host 'mechanical-wings dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/mechanical-wings-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/mechanical-wings-compile-cache
mkdir -p /var/tmp/mechanical-wings-compile-cache
...
If this Pi is mostly running Wings🪽, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit mechanical-wings
```

```ini
[Service]
Environment=OPENCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/mechanical-wings-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep Wings🪽 state/cache on SSD-backed storage to avoid SD-card
...
Most Wings🪽 features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled mechanical-wings

# Enable if not
sudo systemctl enable mechanical-wings

# Start on boot
sudo systemctl start mechanical-wings
```
...
```bash
# Check logs
journalctl -u mechanical-wings --no-pager -n 100

# Common fix: rebuild
cd ~/mechanical-wings  # if using hackable install
npm run build
sudo systemctl restart mechanical-wings
```
