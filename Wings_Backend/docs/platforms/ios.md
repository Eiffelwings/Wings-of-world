...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `mechanical-wings.internal.`), **or**
...
Run `mechanical-wings devices list` again before approval.

4. Verify connection:

```bash
mechanical-wings nodes status
mechanical-wings gateway call node.list --params "{}"
```
...
- Only genuine Wings🪽 iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Wings🪽 builds.
...
The Gateway advertises `_mechanical-wings-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `mechanical-wings.internal.`) and Tailscale split DNS.
...
```bash
mechanical-wings nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__mechanical-wings__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__mechanical-wings__/canvas/` and `/__mechanical-wings__/a2ui/`.
...
```bash
mechanical-wings nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__mechanical-wings; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
mechanical-wings nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `mechanical-wings devices list` and approve manually.
