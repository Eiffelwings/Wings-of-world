...
read_when:
  - You need detailed behavior for mechanical-wings onboard
...
This page is the full reference for `mechanical-wings onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.mechanical-wings/mechanical-wings.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - CLI `--reset` defaults to `config+creds+sessions`; use `--reset-scope full` to also remove workspace.
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `mechanical-wings doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `mechanical-wings pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `mechanical-wings health`.
    - `mechanical-wings status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
- OAuth credentials: `~/.mechanical-wings/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.mechanical-wings/agents/<agentId>/agent/auth-profiles.json`
...
`~/.mechanical-wings/credentials/oauth.json` (or `$OPENCLAW_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`mechanical-wings onboard`](/cli/onboard)
