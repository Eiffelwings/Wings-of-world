...
CLI onboarding is the **recommended** way to set up Wings🪽 on macOS,
...
`mechanical-wings dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
mechanical-wings agents add <name>
```
...
`mechanical-wings configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.mechanical-wings/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `mechanical-wings doctor` first.
</Note>
...
Use `mechanical-wings agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.mechanical-wings/workspace-<agentId>`.
...
- CLI command reference: [`mechanical-wings onboard`](/cli/onboard)
