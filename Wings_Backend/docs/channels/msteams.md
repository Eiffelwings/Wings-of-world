...
```bash
mechanical-wings plugins install @mechanical-wings/msteams
```

Local checkout (when running from a git repo):

```bash
mechanical-wings plugins install ./extensions/msteams
```

If you choose Teams during setup and a git checkout is detected,
Wings🪽 will offer the local install path automatically.
...
3. Configure Wings🪽 with those credentials.
...
- Talk to Wings🪽 via Teams DMs, group chats, or channels.
...
- On startup, Wings🪽 resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.mechanical-wings/mechanical-wings.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Wings🪽, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `mechanical-wings-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `mechanical-wings plugins install @mechanical-wings/msteams`
   - From a local checkout: `mechanical-wings plugins install ./extensions/msteams`
...
```json5
{
  ...
  description: { short: "Wings🪽 in Teams", full: "Wings🪽 in Teams" },
...
Wings🪽 handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Wings🪽 only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/Wings🪽Shared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

Wings🪽 sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `mechanical-wings message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.mechanical-wings/msteams-polls.json`.
...
```bash
mechanical-wings message send --channel msteams \
...
mechanical-wings message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
mechanical-wings message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
mechanical-wings message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
mechanical-wings message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
