...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure Wings🪽 HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `mechanical-wings pairing approve slack <code>`.
...
- The interactive callback values are Wings🪽-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, Wings🪽 falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while Wings🪽 is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "Wings🪽",
    "description": "Slack connector for Wings🪽"
  },
  "features": {
    "bot_user": {
      "display_name": "Wings🪽",
    ...
    "slash_commands": [
      {
        ...
        "description": "Send a message to Wings🪽",
        "should_escape": false
      }
    ]
  },
...
```bash
mechanical-wings pairing list slack
```
...
Wings🪽 supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, Wings🪽 falls back to normal delivery for remaining payloads.
