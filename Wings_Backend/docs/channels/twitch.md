...
read_when:
  - Setting up Twitch chat integration for Wings🪽
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Wings🪽 connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
mechanical-wings plugins install @mechanical-wings/twitch
```

Local checkout (when running from a git repo):

```bash
mechanical-wings plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "mechanical-wings", // Bot's Twitch account
