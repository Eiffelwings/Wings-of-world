...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Wings🪽 to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current Wings🪽 releases, so no separate plugin install
is required.
...
```bash
mechanical-wings plugins install @mechanical-wings/feishu
```
...
If you just installed Wings🪽, run onboarding:
...
2. Configuring app credentials in Wings🪽
...
1. You already ran `mechanical-wings channels add` for Feishu
2. The gateway is running (`mechanical-wings gateway status`)
...
Edit `~/.mechanical-wings/mechanical-wings.json`:
...
```bash
mechanical-wings pairing approve feishu <CODE>
```
...
- **Approve pairing**:
  ...
  ```bash
  mechanical-wings pairing list feishu
  mechanical-wings pairing approve feishu <CODE>
  ```
...
2. Run `mechanical-wings logs --follow` and look for `chat_id`
...
2. Run `mechanical-wings logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `mechanical-wings gateway status`  | Show gateway status           |
| `mechanical-wings gateway install` | Install/start gateway service |
| `mechanical-wings gateway stop`    | Stop gateway service          |
| `mechanical-wings gateway restart` | Restart gateway service       |
| `mechanical-wings logs --follow`   | Tail gateway logs             |
...
4. Check logs: `mechanical-wings logs --follow`
...
5. Ensure the gateway is running: `mechanical-wings gateway status`
6. Check logs: `mechanical-wings logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.mechanical-wings/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.mechanical-wings/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
