---
summary: "Mattermost bot setup and Wings🪽 config"
read_when:
...
```bash
mechanical-wings plugins install @mechanical-wings/mattermost
```

Local checkout (when running from a git repo):

```bash
mechanical-wings plugins install ./extensions/mattermost
```
...
4. Configure Wings🪽 and start the gateway.
...
Native slash commands are opt-in. When enabled, Wings🪽 registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, Wings🪽 derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as Wings🪽.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to Wings🪽.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from Wings🪽, not `404`.
...
- Approve via:
  - `mechanical-wings pairing list mattermost`
  - `mechanical-wings pairing approve mattermost <CODE>`
...
Use these target formats with `mechanical-wings message send` or cron/webhooks:
...
Wings🪽 resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), Wings🪽 sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When Wings🪽 sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, Wings🪽 derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and Wings🪽 run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="mechanical-wings-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"mechanical-wings-mattermost-interactions",
...
`mechanical-wings message send` and cron/webhook deliveries.
