...
Tlon is a decentralized messenger built on Urbit. Wings🪽 connects to your Urbit ship and can
...
```bash
mechanical-wings plugins install @mechanical-wings/tlon
```

Local checkout (when running from a git repo):

```bash
mechanical-wings plugins install ./extensions/tlon
```
...
By default, Wings🪽 blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `mechanical-wings message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Wings🪽 replies in-thread.
