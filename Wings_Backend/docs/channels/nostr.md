...
read_when:
  - You want Wings🪽 to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Wings🪽 to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`mechanical-wings onboard`) and `mechanical-wings channels add` list optional channel plugins.
...
```bash
mechanical-wings plugins install @mechanical-wings/nostr
```

Use a local checkout (dev workflows):

```bash
mechanical-wings plugins install --link <path-to-mechanical-wings>/extensions/nostr
```
...
mechanical-wings channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "mechanical-wings@example.com",
        lud16: "mechanical-wings@example.com",
      },
    },
  },
}
