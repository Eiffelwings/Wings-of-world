---
summary: "Synology Chat webhook setup and Wings🪽 config"
read_when:
  - Setting up Synology Chat with Wings🪽
...
```bash
mechanical-wings plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `mechanical-wings onboard` now shows Synology Chat in the same channel setup list as `mechanical-wings channels add`.
   - Non-interactive setup: `mechanical-wings channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your Wings🪽 gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in Wings🪽.
   - Guided: `mechanical-wings onboard`
   - Direct: `mechanical-wings channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `mechanical-wings pairing list synology-chat`
  - `mechanical-wings pairing approve synology-chat <CODE>`
...
```bash
mechanical-wings message send --channel synology-chat --target 123456 --text "Hello from Wings🪽"
mechanical-wings message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. Wings🪽 now rejects duplicate exact paths
