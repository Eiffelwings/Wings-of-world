...
```
    ...
    Telegram does **not** use `mechanical-wings channels login telegram`; configure token in config/env, then start gateway.
...
mechanical-wings pairing approve telegram <CODE>
```
...
<Tabs>
  <Tab title="DM policy">
    ...
    If you upgraded and your config contains `@username` allowlist entries, run `mechanical-wings doctor --fix` to resolve them (best-effort; requires a Telegram bot token).
    If you previously relied on pairing-store allowlist files, `mechanical-wings doctor --fix` can recover entries into `channels.telegram.allowFrom` in allowlist flows (for example when `dmPolicy: "allowlist"` has no explicit IDs yet).
    ...
    1. DM your bot.
    2. Run `mechanical-wings logs --follow`.
    3. Read `from.id`.
...
```
    ...
    - or read `chat.id` from `mechanical-wings logs --follow`
...
- DM messages can carry `message_thread_id`; Wings🪽 routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Live stream preview (message edits)">
    Wings🪽 can stream partial replies in real time:
    ...
    - DM: Wings🪽 keeps the same preview message and performs a final edit in place (no second message)
    - group/topic: Wings🪽 keeps the same preview message and performs a final edit in place (no second message)

    For complex replies (for example media payloads), Wings🪽 falls back to normal final delivery and then cleans up the preview message.

    Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, Wings🪽 skips the preview stream to avoid double-streaming.

    If native draft transport is unavailable/rejected, Wings🪽 automatically falls back to `sendMessage` + `editMessageText`.
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, Wings🪽 retries as plain text.
...
```
  ...
  <Accordion title="Forum topics and thread behavior">
    ...
    **Thread-bound ACP spawn from chat**:
    ...
    - Wings🪽 pins the spawn confirmation message in-topic after a successful bind.
...
```
    ...
    - `~/.mechanical-wings/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, Wings🪽 enqueues system events like:
...
```bash
mechanical-wings message send --channel telegram --target 123456789 --message "hi"
mechanical-wings message send --channel telegram --target @name --message "hi"
```

    Telegram polls use `mechanical-wings message poll` and support forum topics:

```bash
mechanical-wings message poll --channel telegram --target 123456789 \
  --poll-question "Ship it?" --poll-option "Yes" --poll-option "No"
mechanical-wings message poll --channel telegram --target -1001234567890:topic:42 \
...
```
  ...
  <Accordion title="Exec approvals in Telegram">
    ...
    Channel delivery shows the command text in the chat, so only enable `channel` or `both` in trusted groups/topics. When the prompt lands in a forum topic, Wings🪽 preserves the topic for both the approval prompt and the post-approval follow-up.
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `mechanical-wings channels status` warns when config expects unmentioned group messages.
    - `mechanical-wings channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `mechanical-wings logs --follow` for skip reasons
  ...
  <Accordion title="Polling or network instability">
    ...
    - If logs include `TypeError: fetch failed` or `Network request for 'getUpdates' failed!`, Wings🪽 now retries these as recoverable network errors.
...
- `channels.telegram.allowFrom`: DM allowlist (numeric Telegram user IDs). `allowlist` requires at least one sender ID. `open` requires `"*"`. `mechanical-wings doctor --fix` can resolve legacy `@username` entries to IDs and can recover allowlist entries from pairing-store files in allowlist migration flows.
...
- `channels.telegram.groupAllowFrom`: group sender allowlist (numeric Telegram user IDs). `mechanical-wings doctor --fix` can resolve legacy `@username` entries to IDs. Non-numeric entries are ignored at auth time. Group auth does not use DM pairing-store fallback (`2026.2.25+`).
- Multi-account precedence:
  ...
  - If neither is set, Wings🪽 falls back to the first normalized account ID and `mechanical-wings doctor` warns.
