...
read_when:
  - Setting up Zalo Personal for Wings🪽
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Wings🪽.
...
- Install via CLI: `mechanical-wings plugins install @mechanical-wings/zalouser`
- Or from a source checkout: `mechanical-wings plugins install ./extensions/zalouser`
...
2. Login (QR, on the Gateway machine):
   - `mechanical-wings channels login --channel zalouser`
...
```bash
mechanical-wings directory self --channel zalouser
mechanical-wings directory peers list --channel zalouser --query "name"
mechanical-wings directory groups list --channel zalouser --query "work"
```
...
- `mechanical-wings pairing list zalouser`
- `mechanical-wings pairing approve zalouser <code>`
...
- On startup, Wings🪽 resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, Wings🪽 stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Wings🪽 state. Example:
...
- Wings🪽 sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Wings🪽 sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `mechanical-wings channels logout --channel zalouser && mechanical-wings channels login --channel zalouser`
...
- The channel now runs fully in Wings🪽 without external CLI binaries.
