---
summary: "Frequently asked questions about Wings🪽 setup, configuration, and usage"
read_when:
...
3. **Daemon + port state**
   ...
   ```bash
   mechanical-wings gateway status
   ```
...
5. **Tail the latest log**
   ...
   ```bash
   tail -f "$(ls -t /tmp/mechanical-wings/mechanical-wings-*.log | head -1)"
   ```
...
7. **Gateway snapshot**
   ...
   mechanical-wings health --verbose   # shows the target URL + config path on errors
   ```
...
<AccordionGroup>
  <Accordion title="I am stuck, fastest way to get unstuck">
    ...
    ```bash
    curl -fsSL https://wings-of-world.ai/install.sh | bash -s -- --install-method git
    ```

    This installs Wings🪽 **from a git checkout**, so the agent can read the code + docs and
    ...
    [https://github.com/mechanical-wings/mechanical-wings/issues](https://github.com/mechanical-wings/mechanical-wings/issues)
    [https://github.com/mechanical-wings/mechanical-wings/pulls](https://github.com/mechanical-wings/mechanical-wings/pulls)

    Start with these commands (share outputs when asking for help):

    ```bash
    mechanical-wings status
    mechanical-wings models status
    mechanical-wings doctor
    ```

    What they do:

    - `mechanical-wings status`: quick snapshot of gateway/agent health + basic config.
    - `mechanical-wings models status`: checks provider auth + model availability.
    - `mechanical-wings doctor`: validates and repairs common config/state issues.

    Other useful CLI checks: `mechanical-wings status --all`, `mechanical-wings logs --follow`,
    `mechanical-wings gateway status`, `mechanical-wings health --verbose`.
  ...
  <Accordion title="Recommended way to install and set up Wings🪽">
    The repo recommends running from source and using onboarding:

    ```bash
    curl -fsSL https://wings-of-world.ai/install.sh | bash
    mechanical-wings onboard --install-daemon
    ```
    ...
    ```bash
    git clone https://github.com/mechanical-wings/mechanical-wings.git
    cd mechanical-wings
    pnpm install
    pnpm build
    pnpm ui:build # auto-installs UI deps on first run
    mechanical-wings onboard
    ```

    If you don't have a global install yet, run it via `pnpm mechanical-wings onboard`.
  ...
  <Accordion title="How do I authenticate the dashboard (token) on localhost vs remote?">
    **Localhost (same machine):**
    ...
    - Retrieve it from the gateway host: `mechanical-wings config get gateway.auth.token` (or generate one: `mechanical-wings doctor --generate-gateway-token`).

    **Not on localhost:**

    - **Tailscale Serve** (recommended): keep bind loopback, run `mechanical-wings gateway --tailscale serve`, open `https://<magicdns>/`. If `gateway.auth.allowTailscale` is `true`, identity headers satisfy Control UI/WebSocket auth (no token, assumes trusted gateway host); HTTP APIs still require token/password.
    - **Tailnet bind**: run `mechanical-wings gateway --bind tailnet --token "<token>"`, open `http://<tailscale-ip>:18789/`, paste token in dashboard settings.
  ...
  <Accordion title="It is stuck on wake up my friend / onboarding will not hatch. What now?">
    ...
    ```bash
    mechanical-wings gateway restart
    ```
  ...
  <Accordion title="Can I migrate my setup to a new machine (Mac mini) without redoing onboarding?">
    ...
    1. Install Wings🪽 on the new machine.
    2. Copy `$OPENCLAW_STATE_DIR` (default: `~/.mechanical-wings`) from the old machine.
    3. Copy your workspace (default: `~/.mechanical-wings/workspace`).
    4. Run `mechanical-wings doctor` and restart the Gateway service.
    ...
    **Important:** if you only commit/push your workspace to GitHub, you're backing
    ...
    under `~/.mechanical-wings/` (for example `~/.mechanical-wings/agents/<agentId>/sessions/`).
  ...
  <Accordion title="Where do I see what is new in the latest version?">
    ...
    [https://github.com/mechanical-wings/mechanical-wings/blob/main/CHANGELOG.md](https://github.com/mechanical-wings/mechanical-wings/blob/main/CHANGELOG.md)
  ...
  <Accordion title="Cannot access docs.wings-of-world.ai (SSL error)">
    Some Comcast/Xfinity connections incorrectly block `docs.wings-of-world.ai` via Xfinity
    Advanced Security. Disable it or allowlist `docs.wings-of-world.ai`, then retry. More
    detail: [Troubleshooting](/help/faq#cannot-access-docsmechanical-wings-ai-ssl-error).
    Please help us unblock it by reporting here: [https://spa.xfinity.com/check_url_status](https://spa.xfinity.com/check_url_status).

    If you still can't reach the site, the docs are mirrored on GitHub:
    [https://github.com/mechanical-wings/mechanical-wings/tree/main/docs](https://github.com/mechanical-wings/mechanical-wings/tree/main/docs)
  ...
  <Accordion title="How do I install the beta version and what is the difference between beta and dev?">
    ...
    **Dev** is the moving head of `main` (git); when published, it uses the npm dist-tag `dev`.
    ...
    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash -s -- --beta
    ```

    ```bash
    curl -fsSL --proto '=https' --tlsv1.2 https://wings-of-world.ai/install.sh | bash -s -- --install-method git
    ```

    Windows installer (PowerShell):
    [https://wings-of-world.ai/install.ps1](https://wings-of-world.ai/install.ps1)
  ...
  <Accordion title="Installer stuck? How do I get more feedback?">
    ...
    ```bash
    curl -fsSL https://wings-of-world.ai/install.sh | bash -s -- --verbose
    ```

    Beta install with verbose:

    ```bash
    curl -fsSL https://wings-of-world.ai/install.sh | bash -s -- --beta --verbose
    ```

    For a hackable (git) install:

    ```bash
    curl -fsSL https://wings-of-world.ai/install.sh | bash -s -- --install-method git --verbose
    ```
    ...
    Set-PSDebug -Trace 1
    & ([scriptblock]::Create((iwr -useb https://wings-of-world.ai/install.ps1))) -NoOnboard
    Set-PSDebug -Trace 0
    ```
  ...
  <Accordion title="Windows install says git not found or mechanical-wings not recognized">
    ...
    **2) mechanical-wings is not recognized after install**
  ...
  <Accordion title="Windows exec output shows garbled Chinese text - what should I do?">
    ...
    If you still reproduce this on latest Wings🪽, track/report it in:

    - [Issue #30640](https://github.com/mechanical-wings/mechanical-wings/issues/30640)
  ...
  <Accordion title="How do I install Wings🪽 on Linux?">
  ...
  <Accordion title="How do I install Wings🪽 on a VPS?">
  ...
  <Accordion title="Can I ask Wings🪽 to update itself?">
    ...
    mechanical-wings update --channel stable|beta|dev
    mechanical-wings update --tag <dist-tag|version>
    ...
    ```bash
    mechanical-wings update --yes --no-restart
    mechanical-wings gateway restart
    ```
  ...
  <Accordion title="What does onboarding actually do?">
    `mechanical-wings onboard` is the recommended setup path. In **local mode** it walks you through:
  ...
  <Accordion title="Do I need a Claude or OpenAI subscription to run this?">
    No. You can run Wings🪽 with **API keys** (Anthropic/OpenAI/others) or with
    **local-only models** so your data stays on your device. Subscriptions (Claude
    ...
    OpenAI Codex OAuth is explicitly supported for external tools like Wings🪽.
  ...
  <Accordion title="How does Anthropic setup-token auth work?">
    `claude setup-token` generates a **token string** via the Claude Code CLI (it is not available in the web console). You can run it on **any machine**. Choose **Anthropic token (paste setup-token)** in onboarding or paste it with `mechanical-wings models auth paste-token --provider anthropic`. The token is stored as an auth profile for the **anthropic** provider and used like an API key (no auto-refresh). More detail: [OAuth](/concepts/oauth).
  </Accordion>
  ...
  <Accordion title="Where do I find an Anthropic setup-token?">
    ...
    Copy the token it prints, then choose **Anthropic token (paste setup-token)** in onboarding. If you want to run it on the gateway host, use `mechanical-wings models auth setup-token --provider anthropic`. If you ran `claude setup-token` elsewhere, paste it on the gateway host with `mechanical-wings models auth paste-token --provider anthropic`. See [Anthropic](/providers/anthropic).
  ...
  <Accordion title="Do you support Claude subscription auth (Claude Pro or Max)?">
    ...
    - use a **setup-token**
    - reuse a local **Claude CLI** login on the gateway host with `mechanical-wings models auth login --provider anthropic --method cli --set-default`
  ...
  <Accordion title="Why am I seeing HTTP 429 rate_limit_error from Anthropic?">
    ...
    Tip: set a **fallback model** so Wings🪽 can keep replying while a provider is rate-limited.
  ...
  <Accordion title="How does Codex auth work?">
    Wings🪽 supports **OpenAI Code (Codex)** via OAuth (ChatGPT sign-in). Onboarding can run the OAuth flow and will set the default model to `openai-codex/gpt-5.4` when appropriate. See [Model providers](/concepts/model-providers) and [Onboarding (CLI)](/start/wizard).
  </Accordion>

  <Accordion title="Do you support OpenAI subscription auth (Codex OAuth)?">
    Yes. Wings🪽 fully supports **OpenAI Code (Codex) subscription OAuth**.
    OpenAI explicitly allows subscription OAuth usage in external tools/workflows
    like Wings🪽. Onboarding can run the OAuth flow for you.
  ...
  <Accordion title="How do I set up Gemini CLI OAuth?">
    Gemini CLI uses a **plugin auth flow**, not a client id or secret in `mechanical-wings.json`.

    Steps:

    1. Enable the plugin: `mechanical-wings plugins enable google`
    2. Login: `mechanical-wings models auth login --provider google-gemini-cli --set-default`
  ...
  <Accordion title="Is a local model OK for casual chats?">
    Usually no. Wings🪽 needs large context + strong safety; small cards truncate and leak. If you must, run the **largest** MiniMax M2.5 build you can locally (LM Studio) and see [/gateway/local-models](/gateway/local-models). Smaller/quantized models increase prompt-injection risk - see [Security](/gateway/security).
  </Accordion>
  ...
  <Accordion title="Do I have to buy a Mac Mini to install this?">
    No. Wings🪽 runs on macOS or Linux (Windows via WSL2). A Mac mini is optional - some people
  ...
  <Accordion title="If I buy a Mac mini to run Wings🪽, can I connect it to my MacBook Pro?">
    ...
    **node** (companion device). Nodes don't run the Gateway - they provide extra
    ...
    - Use `mechanical-wings nodes status` / `mechanical-wings nodes list` to see it.
  ...
  <Accordion title="Telegram: what goes in allowFrom?">
    ...
    Onboarding accepts `@username` input and resolves it to a numeric ID, but Wings🪽 authorization uses numeric IDs only.

    Safer (no third-party bot):

    - DM your bot, then run `mechanical-wings logs --follow` and read `from.id`.
  ...
  <Accordion title="Can multiple people use one WhatsApp number with different Wings🪽 instances?">
  ...
  <Accordion title="Does Homebrew work on Linux?">
    ...
    If you run Wings🪽 via systemd, ensure the service PATH includes `/home/linuxbrew/.linuxbrew/bin` (or your brew prefix) so `brew`-installed tools resolve in non-login shells.
  ...
  <Accordion title="Can I switch between npm and git installs later?">
    ...
    This **does not delete your data** - it only changes the Wings🪽 code install. Your state
    (`~/.mechanical-wings`) and workspace (`~/.mechanical-wings/workspace`) stay untouched.
    ...
    ```bash
    npm install -g mechanical-wings@latest
    mechanical-wings doctor
    mechanical-wings gateway restart
    ```
  ...
  <Accordion title="Should I run the Gateway on my laptop or a VPS?">
    ...
    **Wings🪽-specific note:** WhatsApp/Telegram/Slack/Mattermost (plugin)/Discord all work fine from a VPS. The only real trade-off is **headless browser** vs a visible window. See [Browser](/tools/browser).

    **Recommended default:** VPS if you had gateway disconnects before. Local is great when you're actively using the Mac and want local file access or UI automation with a visible browser.

  </Accordion>

  <Accordion title="How important is it to run Wings🪽 on a dedicated machine?">
  ...
  <Accordion title="What are the minimum VPS requirements and recommended OS?">
    Wings🪽 is lightweight. For a basic Gateway + one chat channel:
  ...
  <Accordion title="Can I run Wings🪽 in a VM and what are the requirements?">
...
<AccordionGroup>
  <Accordion title="What is Wings🪽, in one paragraph?">
    Wings🪽 is a personal AI assistant you run on your own devices. It replies on the messaging surfaces you already use (WhatsApp, Telegram, Slack, Mattermost (plugin), Discord, Google Chat, Signal, iMessage, WebChat) and can also do voice + a live Canvas on supported platforms. The **Gateway** is the always-on control plane; the assistant is the product.
  </Accordion>

  <Accordion title="Value proposition">
    Wings🪽 is not "just a Claude wrapper." It's a **local-first control plane** that lets you run a
  ...
  <Accordion title="What are the top five everyday use cases for Wings🪽?">
  ...
  <Accordion title="Can Wings🪽 help with lead gen, outreach, ads, and blogs for a SaaS?">
    Yes for **research, qualification, and drafting**. It can scan sites, build shortlists,
    summarize prospects, and write outreach or ad copy drafts.

    For **outreach or ad runs**, keep a human in the loop. Avoid spam, follow local laws and
    platform policies, and review anything before it is sent. The safest pattern is to let
    Wings🪽 draft and you approve.
  ...
  <Accordion title="What are the advantages vs Claude Code for web development?">
    Wings🪽 is a **personal assistant** and coordination layer, not an IDE replacement. Use
    Claude Code or Codex for the fastest direct coding loop inside a repo. Use Wings🪽 when you
    ...
    Showcase: [https://wings-of-world.ai/showcase](https://wings-of-world.ai/showcase)
...
<AccordionGroup>
  <Accordion title="How do I customize skills without keeping the repo dirty?">
    Use managed overrides instead of editing the repo copy. Put your changes in `~/.mechanical-wings/skills/<name>/SKILL.md` (or add a folder via `skills.load.extraDirs` in `~/.mechanical-wings/mechanical-wings.json`). Precedence is `<workspace>/skills` > `~/.mechanical-wings/skills` > bundled, so managed overrides win without touching git. Only upstream-worthy edits should live in the repo and go out as PRs.
  </Accordion>

  <Accordion title="Can I load skills from a custom folder?">
    Yes. Add extra directories via `skills.load.extraDirs` in `~/.mechanical-wings/mechanical-wings.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.mechanical-wings/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Wings🪽 treats as `<workspace>/skills` on the next session.
  </Accordion>
  ...
  <Accordion title="Cron or reminders do not fire. What should I check?">
    ...
    ```bash
    mechanical-wings cron run <jobId> --force
    mechanical-wings cron runs --id <jobId> --limit 50
    ```
  ...
  <Accordion title="How do I install skills on Linux?">
    Use native `mechanical-wings skills` commands or drop skills into your workspace. The macOS Skills UI isn't available on Linux.
    Browse skills at [https://clawhub.com](https://clawhub.com).

    ```bash
    mechanical-wings skills search "calendar"
    mechanical-wings skills install <skill-slug>
  ...
  <Accordion title="Can Wings🪽 run tasks on a schedule or continuously in the background?">
  ...
  <Accordion title="Can I run Apple macOS-only skills from Linux?">
    Not directly. macOS skills are gated by `metadata.mechanical-wings.os` plus required binaries, and skills only appear in the system prompt when they are eligible on the **Gateway host**. On Linux, `darwin`-only skills (like `apple-notes`, `apple-reminders`, `things-mac`) will not load unless you override the gating.
    ...
    **Option B - use a macOS node (no SSH).**
    Run the Gateway on Linux, pair a macOS node (menubar app), and set **Node Run Commands** to "Always Ask" or "Always Allow" on the Mac. Wings🪽 can treat macOS-only skills as eligible when the required binaries exist on the node. The agent runs those skills via the `nodes` tool. If you choose "Always Ask", approving "Always Allow" in the prompt adds that command to the allowlist.
    ...
    **Option C - proxy macOS binaries over SSH (advanced).**
    ...
    3. Override the skill metadata (workspace or `~/.mechanical-wings/skills`) to allow Linux:

       ```markdown
       ---
       name: apple-notes
       description: Manage Apple Notes via the memo CLI on macOS.
       metadata: { "mechanical-wings": { "os": ["darwin", "linux"], "requires": { "bins": ["memo"] } } }
       ---
       ```
  ...
  <Accordion title="Do you have a Notion or HeyGen integration?">
    ...
    Native installs land in the active workspace `skills/` directory. For shared skills across agents, place them in `~/.mechanical-wings/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).

  </Accordion>

  <Accordion title="How do I use my existing signed-in Chrome with Wings🪽?">
    ...
    mechanical-wings browser --browser-profile user snapshot
    ```
...
<AccordionGroup>
  ...
  <Accordion title="How does memory work?">
    Wings🪽 memory is just Markdown files in the agent workspace:

    - Daily notes in `memory/YYYY-MM-DD.md`
    - Curated long-term notes in `MEMORY.md` (main/private sessions only)

    Wings🪽 also runs a **silent pre-compaction memory flush** to remind the model
  ...
  <Accordion title="Does semantic memory search require an OpenAI API key?">
    ...
    If you don't set a provider explicitly, Wings🪽 auto-selects a provider when it
...
<AccordionGroup>
  <Accordion title="Is all data used with Wings🪽 saved locally?">
    No - **Wings🪽's state is local**, but **external services still see what you send them**.

    - **Local by default:** sessions, memory files, config, and workspace live on the Gateway host
      (`~/.mechanical-wings` + your workspace directory).
  ...
  <Accordion title="Where does Wings🪽 store its data?">
    Everything lives under `$OPENCLAW_STATE_DIR` (default: `~/.mechanical-wings`):

    | Path                                                            | Purpose                                                            |
    | --------------------------------------------------------------- | ------------------------------------------------------------------ |
    | `$OPENCLAW_STATE_DIR/mechanical-wings.json`                             | Main config (JSON5)                                                |
    ...
    Legacy single-agent path: `~/.mechanical-wings/agent/*` (migrated by `mechanical-wings doctor`).

    Your **workspace** (AGENTS.md, memory files, skills, etc.) is separate and configured via `agents.defaults.workspace` (default: `~/.mechanical-wings/workspace`).

  </Accordion>

  <Accordion title="Where should AGENTS.md / SOUL.md / USER.md / MEMORY.md live?">
    These files live in the **agent workspace**, not `~/.mechanical-wings`.

    - **Workspace (per agent)**: `AGENTS.md`, `SOUL.md`, `IDENTITY.md`, `USER.md`,
      `MEMORY.md` (or legacy fallback `memory.md` when `MEMORY.md` is absent),
      `memory/YYYY-MM-DD.md`, optional `HEARTBEAT.md`.
    - **State dir (`~/.mechanical-wings`)**: config, credentials, auth profiles, sessions, logs,
      and shared skills (`~/.mechanical-wings/skills`).

    Default workspace is `~/.mechanical-wings/workspace`, configurable via:

    ```json5
    {
      agents: { defaults: { workspace: "~/.mechanical-wings/workspace" } },
    }
    ```
  ...
  <Accordion title="Recommended backup strategy">
    ...
    Do **not** commit anything under `~/.mechanical-wings` (credentials, sessions, tokens, or encrypted secrets payloads).
  ...
  <Accordion title="How do I completely uninstall Wings🪽?">
  ...
  <Accordion title="Can agents work outside the workspace?">
    ...
    `workspace` to the repo root. The Wings🪽 repo is just source code; keep the
...
<AccordionGroup>
  <Accordion title="What format is the config? Where is it?">
    Wings🪽 reads an optional **JSON5** config from `$OPENCLAW_CONFIG_PATH` (default: `~/.mechanical-wings/mechanical-wings.json`):

    ```
    $OPENCLAW_CONFIG_PATH
    ```

    If the file is missing, it uses safe-ish defaults (including a default workspace of `~/.mechanical-wings/workspace`).
  ...
  <Accordion title="Why do I need a token on localhost now?">
    Wings🪽 enforces token auth by default, including loopback. If no token is configured, gateway startup auto-generates one and saves it to `gateway.auth.token`, so **local WS clients must authenticate**. This blocks other local processes from calling the Gateway.

    If you **really** want open loopback, set `gateway.auth.mode: "none"` explicitly in your config. Doctor can generate a token for you any time: `mechanical-wings doctor --generate-gateway-token`.
  ...
  <Accordion title="How do I disable funny CLI taglines?">
    ...
    - `default`: uses `All your chats, one Wings🪽.` every time.
  ...
  <Accordion title="How do I enable web search (and web fetch)?">
    ...
    **Recommended:** run `mechanical-wings configure --section web` and choose a provider.
    ...
    - Daemons read env vars from `~/.mechanical-wings/.env` (or the service environment).
  ...
  <Accordion title="config.apply wiped my config. How do I recover and avoid this?">
    ...
    - Restore from backup (git or a copied `~/.mechanical-wings/mechanical-wings.json`).
    - If you have no backup, re-run `mechanical-wings doctor` and reconfigure channels/models.
    - If this was unexpected, file a bug and include your last known config or any backup.
    - A local coding agent can often reconstruct a working config from logs or history.

    Avoid it:

    - Use `mechanical-wings config set` for small changes.
    - Use `mechanical-wings configure` for interactive edits.
  ...
  <Accordion title="Can the Wings🪽 browser run headless?">
...
<AccordionGroup>
  ...
  <Accordion title="Tailscale is connected but I get no replies. What now?">
    ...
    - Gateway is running: `mechanical-wings gateway status`
    - Gateway health: `mechanical-wings status`
    - Channel health: `mechanical-wings channels status`
  ...
  <Accordion title="Can two Wings🪽 instances talk to each other (local + VPS)?">
    ...
    **CLI bridge (generic):** run a script that calls the other Gateway with
    `mechanical-wings agent --message ... --deliver`, targeting a chat where the other bot
    ...
    ```bash
    mechanical-wings agent --message "Hello from local bot" --deliver --channel telegram --reply-to <chat-id>
    ```
  ...
  <Accordion title="How do I set up Tailscale on a VPS and connect from my Mac?">
    ...
    ```bash
    mechanical-wings gateway --tailscale serve
    ```
...
<AccordionGroup>
  <Accordion title="How does Wings🪽 load environment variables?">
    Wings🪽 reads env vars from the parent process (shell, launchd/systemd, CI, etc.) and additionally loads:

    - `.env` from the current working directory
    - a global fallback `.env` from `~/.mechanical-wings/.env` (aka `$OPENCLAW_STATE_DIR/.env`)
  ...
  <Accordion title="I started the Gateway via the service and my env vars disappeared. What now?">
    ...
    1. Put the missing keys in `~/.mechanical-wings/.env` so they're picked up even when the service doesn't inherit your shell env.
  ...
  <Accordion title='I set COPILOT_GITHUB_TOKEN, but models status shows "Shell env: off." Why?'>
    `mechanical-wings models status` reports whether **shell env import** is enabled. "Shell env: off"
    does **not** mean your env vars are missing - it just means Wings🪽 won't load
    your login shell automatically.

    If the Gateway runs as a service (launchd/systemd), it won't inherit your shell
    environment. Fix by doing one of these:

    1. Put the token in `~/.mechanical-wings/.env`:
...
<AccordionGroup>
  ...
  <Accordion title="Is there a way to make a team of Wings🪽 instances (one CEO and many agents)?">
  ...
  <Accordion title="How do I completely reset Wings🪽 but keep it installed?">
    ...
    ```bash
    mechanical-wings reset --scope full --yes --non-interactive
    ```
    ...
    - If you used profiles (`--profile` / `OPENCLAW_PROFILE`), reset each state dir (defaults are `~/.mechanical-wings-<profile>`).
    - Dev reset: `mechanical-wings gateway --dev --reset` (dev-only; wipes dev config + credentials + sessions + workspace).
  ...
  <Accordion title="Why am I getting heartbeat messages every 30 minutes?">
    ...
    headers like `# Heading`), Wings🪽 skips the heartbeat run to save API calls.
  ...
  <Accordion title='Do I need to add a "bot account" to a WhatsApp group?'>
    No. Wings🪽 runs on **your own account**, so if you're in the group, Wings🪽 can see it.
  ...
  <Accordion title="How do I get the JID of a WhatsApp group?">
    ...
    ```bash
    mechanical-wings logs --follow --json
    ```
    ...
    ```bash
    mechanical-wings directory groups list --channel whatsapp
    ```
  ...
  <Accordion title="Why does Wings🪽 not reply in a group?">
    Two common causes:
  ...
  <Accordion title="How many workspaces and agents can I create?">
    ...
    - **Disk growth:** sessions + transcripts live under `~/.mechanical-wings/agents/<agentId>/sessions/`.
    ...
    - Use `mechanical-wings doctor` to spot stray workspaces and profile mismatches.
...
<AccordionGroup>
  <Accordion title='What is the "default model"?'>
    Wings🪽's default model is whatever you set as:

    ```
    agents.defaults.model.primary
    ```

    Models are referenced as `provider/model` (example: `anthropic/claude-opus-4-6`). If you omit the provider, Wings🪽 currently assumes `anthropic` as a temporary deprecation fallback - but you should still **explicitly** set `provider/model`.
  ...
  <Accordion title="How do I switch models without wiping my config?">
    ...
    - `mechanical-wings models set ...` (updates just model config)
    - `mechanical-wings configure --section model` (interactive)
    - edit `agents.defaults.model` in `~/.mechanical-wings/mechanical-wings.json`

    Avoid `config.apply` with a partial object unless you intend to replace the whole config.
    If you did overwrite config, restore from backup or re-run `mechanical-wings doctor` to repair.
  ...
  <Accordion title="Can I use self-hosted models (llama.cpp, vLLM, Ollama)?">
    ...
    4. Run `mechanical-wings onboard` and choose `Ollama`
    ...
    - for manual switching, use `mechanical-wings models list` and `mechanical-wings models set ollama/<model>`
  ...
  <Accordion title="What do Wings🪽, Flawd, and Krill use for models?">
    - These deployments can differ and may change over time; there is no fixed provider recommendation.
    - Check the current runtime setting on each gateway with `mechanical-wings models status`.
  ...
  <Accordion title='Why do I see "Unknown model: minimax/MiniMax-M2.7"?'>
    ...
    1. Upgrade to a current Wings🪽 release (or run from source `main`), then restart the gateway.
  ...
  <Accordion title="Are opus / sonnet / gpt built-in shortcuts?">
    Yes. Wings🪽 ships a few default shorthands (only applied when the model exists in `agents.defaults.models`):
  ...
  <Accordion title="How do I add models from other providers like OpenRouter or Z.AI?">
    ...
    **No API key found for provider after adding a new agent**
    ...
    ```
    ~/.mechanical-wings/agents/<agentId>/agent/auth-profiles.json
    ```

    Fix options:

    - Run `mechanical-wings agents add <id>` and configure auth during the wizard.
...
<AccordionGroup>
  <Accordion title="How does failover work?">
    ...
    Cooldowns apply to failing profiles (exponential backoff), so Wings🪽 can keep responding even when a provider is rate-limited or temporarily failing.
  ...
  <Accordion title='What does "No credentials found for profile anthropic:default" mean?'>
    ...
    **Fix checklist:**
    ...
    - **Confirm where auth profiles live** (new vs legacy paths)
      - Current: `~/.mechanical-wings/agents/<agentId>/agent/auth-profiles.json`
      - Legacy: `~/.mechanical-wings/agent/*` (migrated by `mechanical-wings doctor`)
    - **Confirm your env var is loaded by the Gateway**
      - If you set `ANTHROPIC_API_KEY` in your shell but run the Gateway via systemd/launchd, it may not inherit it. Put it in `~/.mechanical-wings/.env` or enable `env.shellEnv`.
    - **Make sure you're editing the correct agent**
      - Multi-agent setups mean there can be multiple `auth-profiles.json` files.
    - **Sanity-check model/auth status**
      - Use `mechanical-wings models status` to see configured models and whether providers are authenticated.
    ...
    **Fix checklist for "No credentials found for profile anthropic"**
    ...
    - **Use a setup-token**
      - Run `claude setup-token`, then paste it with `mechanical-wings models auth setup-token --provider anthropic`.
      - If the token was created on another machine, use `mechanical-wings models auth paste-token --provider anthropic`.
    - **If you want to use an API key instead**
      - Put `ANTHROPIC_API_KEY` in `~/.mechanical-wings/.env` on the **gateway host**.
      - Clear any pinned order that forces a missing profile:

        ```bash
        mechanical-wings models auth order clear --provider anthropic
        ```
  ...
  <Accordion title="Why did it also try Google Gemini and fail?">
    If your model config includes Google Gemini as a fallback (or you switched to a Gemini shorthand), Wings🪽 will try it during model fallback. If you haven't configured Google credentials, you'll see `No API key found for provider "google"`.
    ...
    **LLM request rejected: thinking signature required (Google Antigravity)**
    ...
    Fix: Wings🪽 now strips unsigned thinking blocks for Google Antigravity Claude. If it still appears, start a **new session** or set `/thinking off` for that agent.
...
<AccordionGroup>
  ...
  <Accordion title="What are typical profile IDs?">
    Wings🪽 uses provider-prefixed IDs like:
  ...
  <Accordion title="Can I control which auth profile is tried first?">
    ...
    Wings🪽 may temporarily skip a profile if it's in a short **cooldown** (rate limits/timeouts/auth failures) or a longer **disabled** state (billing/insufficient credits). To inspect this, run `mechanical-wings models status --json` and check `auth.unusableProfiles`. Tuning: `auth.cooldowns.billingBackoffHours*`.

    You can also set a **per-agent** order override (stored in that agent's `auth-profiles.json`) via the CLI:

    ```bash
    # Defaults to the configured default agent (omit --agent)
    mechanical-wings models auth order get --provider anthropic

    # Lock rotation to a single profile (only try this one)
    mechanical-wings models auth order set --provider anthropic anthropic:default

    # Or set an explicit order (fallback within provider)
    mechanical-wings models auth order set --provider anthropic anthropic:work anthropic:default
    ...
    ```bash
    mechanical-wings models auth order set --provider anthropic --agent main anthropic:default
    ```
...
<AccordionGroup>
  ...
  <Accordion title='Why does mechanical-wings gateway status say "Runtime: running" but "RPC probe: failed"?'>
    Because "running" is the **supervisor's** view (launchd/systemd/schtasks). The RPC probe is the CLI actually connecting to the gateway WebSocket and calling `status`.

    Use `mechanical-wings gateway status` and trust these lines:
  ...
  <Accordion title='Why does mechanical-wings gateway status show "Config (cli)" and "Config (service)" different?'>
    You're editing one config file while the service is running another (often a `--profile` / `OPENCLAW_STATE_DIR` mismatch).

    Fix:

    ```bash
    mechanical-wings gateway install --force
    ```
  ...
  <Accordion title='What does "another gateway instance is already listening" mean?'>
    Wings🪽 enforces a runtime lock by binding the WebSocket listener immediately on startup (default `ws://127.0.0.1:18789`). If the bind fails with `EADDRINUSE`, it throws `GatewayLockError` indicating another instance is already listening.

    Fix: stop the other instance, free the port, or run with `mechanical-wings gateway --port <port>`.

  </Accordion>

  <Accordion title="How do I run Wings🪽 in remote mode (client connects to a Gateway elsewhere)?">
    ...
    - `mechanical-wings gateway` only starts when `gateway.mode` is `local` (or you pass the override flag).
  ...
  <Accordion title='The Control UI says "unauthorized" (or keeps reconnecting). What now?'>
    ...
    - Fastest: `mechanical-wings dashboard` (prints + copies the dashboard URL, tries to open; shows SSH hint if headless).
    - If you don't have a token yet: `mechanical-wings doctor --generate-gateway-token`.
    ...
    - If mismatch persists after the one retry, rotate/re-approve the paired device token:
      - `mechanical-wings devices list`
      - `mechanical-wings devices rotate --device <id> --role operator`
    - Still stuck? Run `mechanical-wings status --all` and follow [Troubleshooting](/gateway/troubleshooting). See [Dashboard](/web/dashboard) for auth details.
  ...
  <Accordion title="Can I run multiple Gateways on the same host?">
    ...
    - Use `mechanical-wings --profile <name> ...` per instance (auto-creates `~/.mechanical-wings-<name>`).
    - Set a unique `gateway.port` in each profile config (or pass `--port` for manual runs).
    - Install a per-profile service: `mechanical-wings --profile <name> gateway install`.

    Profiles also suffix service names (`ai.mechanical-wings.<profile>`; legacy `com.mechanical-wings.*`, `mechanical-wings-gateway-<profile>.service`, `Wings🪽 Gateway (<profile>)`).
  ...
  <Accordion title='What does "invalid handshake" / code 1008 mean?'>
    ...
    ```
    mechanical-wings tui --url ws://<host>:18789 --token <token>
    ```
...
<AccordionGroup>
  <Accordion title="Where are logs?">
    ...
    - macOS: `$OPENCLAW_STATE_DIR/logs/gateway.log` and `gateway.err.log` (default: `~/.mechanical-wings/logs/...`; profiles use `~/.mechanical-wings-<profile>/logs/...`)
    - Linux: `journalctl --user -u mechanical-wings-gateway[-<profile>].service -n 200 --no-pager`
    - Windows: `schtasks /Query /TN "Wings🪽 Gateway (<profile>)" /V /FO LIST`
  ...
  <Accordion title="How do I start/stop/restart the Gateway service?">
    ...
    If you run the gateway manually, `mechanical-wings gateway --force` can reclaim the port. See [Gateway](/gateway).

  </Accordion>

  <Accordion title="I closed my terminal on Windows - how do I restart Wings🪽?">
  ...
  <Accordion title="The Gateway is up but replies never arrive. What should I check?">
    ...
    mechanical-wings status
    mechanical-wings models status
    mechanical-wings channels status
    mechanical-wings logs --follow
    ```
  ...
  <Accordion title='"Disconnected from gateway: no reason" - what now?'>
    ...
    1. Is the Gateway running? `mechanical-wings gateway status`
    2. Is the Gateway healthy? `mechanical-wings status`
    3. Does the UI have the right token? `mechanical-wings dashboard`
  ...
  <Accordion title="Telegram setMyCommands fails. What should I check?">
    ...
    ```bash
    mechanical-wings channels status
    mechanical-wings channels logs --channel telegram
    ```

    Then match the error:

    - `BOT_COMMANDS_TOO_MUCH`: the Telegram menu has too many entries. Wings🪽 already trims to the Telegram limit and retries with fewer commands, but some menu entries still need to be dropped. Reduce plugin/skill/custom commands, or disable `channels.telegram.commands.native` if you do not need the menu.
  ...
  <Accordion title="How do I completely stop then start the Gateway?">
    ...
    mechanical-wings gateway start
    ```
  ...
  <Accordion title="ELI5: mechanical-wings gateway restart vs mechanical-wings gateway">
    - `mechanical-wings gateway restart`: restarts the **background service** (launchd/systemd).
    - `mechanical-wings gateway`: runs the gateway **in the foreground** for this terminal session.

    If you installed the service, use the gateway commands. Use `mechanical-wings gateway` when
...
<AccordionGroup>
  <Accordion title="My skill generated an image/PDF, but nothing was sent">
    Outbound attachments from the agent must include a `MEDIA:<path-or-url>` line (on its own line). See [Wings🪽 assistant setup](/start/mechanical-wings) and [Agent send](/tools/agent-send).

    CLI sending:

    ```bash
    mechanical-wings message send --target +15555550123 --message "Here you go" --media /path/to/file.png
    ```
...
<AccordionGroup>
  <Accordion title="Is it safe to expose Wings🪽 to inbound DMs?">
    Treat inbound DMs as untrusted input. Defaults are designed to reduce risk:

    - Default behavior on DM-capable channels is **pairing**:
      - Unknown senders receive a pairing code; the bot does not process their message.
      - Approve with: `mechanical-wings pairing approve --channel <channel> [--account <id>] <code>`
      - Pending requests are capped at **3 per channel**; check `mechanical-wings pairing list --channel <channel> [--account <id>]` if a code didn't arrive.
    - Opening DMs publicly requires explicit opt-in (`dmPolicy: "open"` and allowlist `"*"`).

    Run `mechanical-wings doctor` to surface risky DM policies.
  ...
  <Accordion title="I ran /start in Telegram but did not get a pairing code">
    ...
    ```bash
    mechanical-wings pairing list telegram
    ```
  ...
  <Accordion title="WhatsApp: will it message my contacts? How does pairing work?">
    No. Default WhatsApp DM policy is **pairing**. Unknown senders only get a pairing code and their message is **not processed**. Wings🪽 only replies to chats it receives or to explicit sends you trigger.

    Approve pairing with:

    ```bash
    mechanical-wings pairing approve whatsapp <code>
    ```

    List pending requests:

    ```bash
    mechanical-wings pairing list whatsapp
    ```
...
<AccordionGroup>
  ...
  <Accordion title="How do I stop/cancel a running task?">
    ...
    stop agent
    stop the agent
    stop mechanical-wings
    mechanical-wings stop
  ...
  <Accordion title='How do I send a Discord message from Telegram? ("Cross-context messaging denied")'>
    Wings🪽 blocks **cross-provider** messaging by default. If a tool call is bound
