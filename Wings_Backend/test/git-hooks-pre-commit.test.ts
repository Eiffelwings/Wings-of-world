...
import { resolveUsableBash } from "./helpers/bash.js";

const BASH_BIN = resolveUsableBash();
const BASH_PATH_PREFIX = BASH_BIN ? path.dirname(BASH_BIN) : "";
...
function pathList(...segments: string[]): string {
  return segments.filter(Boolean).join(path.delimiter);
}

function linkOrCopyHook(targetPath: string): void {
  const sourcePath = path.join(process.cwd(), "git-hooks", "pre-commit");
  try {
...
describe("git-hooks/pre-commit (integration)", () => {
  const testWithBash = BASH_BIN ? it : it.skip;

  testWithBash("does not treat staged filenames as git-add flags (e.g. --all)", () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-pre-commit-"));
    ...
    linkOrCopyHook(path.join(dir, "git-hooks", "pre-commit"));
    writeFileSync(
    ...
    run(dir, BASH_BIN ?? "bash", ["git-hooks/pre-commit"], {
      PATH: pathList(fakeBinDir, BASH_PATH_PREFIX, process.env.PATH ?? ""),
    });
  ...
  testWithBash("skips pnpm format and pnpm check when FAST_COMMIT is enabled", () => {
    const dir = mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-pre-commit-yolo-"));
    run(dir, "git", ["init", "-q", "--initial-branch=main"]);
    ...
    const output = run(dir, BASH_BIN ?? "bash", ["git-hooks/pre-commit"], {
      PATH: pathList(fakeBinDir, BASH_PATH_PREFIX, process.env.PATH ?? ""),
      FAST_COMMIT: "1",
    });
