...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("mechanical-wings-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@mechanical-wings/whatsapp",
      version: "2026.3.23",
      description: "Wings🪽 WhatsApp channel plugin",
      "mechanical-wings": {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    writeJson(path.join(repoRoot, "extensions", "local-only", "package.json"), {
      name: "@mechanical-wings/local-only",
      "mechanical-wings": {
        channel: {
          id: "local-only",
          label: "Local Only",
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("mechanical-wings-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@mechanical-wings/whatsapp",
      "mechanical-wings": {
        channel: {
