...
import type { WingsConfig } from "../../../extensions/discord/src/runtime-api.js";
...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: WingsConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): WingsConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("mechanical-wings/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("mechanical-wings/plugin-sdk/acp-runtime")>(
    "mechanical-wings/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("mechanical-wings/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("mechanical-wings/plugin-sdk/command-auth")>(
...
vi.mock("mechanical-wings/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("mechanical-wings/plugin-sdk/reply-runtime")>(
    "mechanical-wings/plugin-sdk/reply-runtime",
  );
  return {
    ...actual,
...
vi.mock("mechanical-wings/plugin-sdk/config-runtime", async () => {
  const actual = await vi.importActual<typeof import("mechanical-wings/plugin-sdk/config-runtime")>(
...
vi.mock("mechanical-wings/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("mechanical-wings/plugin-sdk/runtime-env")>(
    "mechanical-wings/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("mechanical-wings/plugin-sdk/infra-runtime", async () => {
  const actual = await vi.importActual<typeof import("mechanical-wings/plugin-sdk/infra-runtime")>(
