...
function setTempHome(base: string) {
  ...
  process.env.OPENCLAW_STATE_DIR = path.join(base, ".mechanical-wings");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "mechanical-wings-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".mechanical-wings", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
