...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @mechanical-wings/zalo, @mechanical-wings/feishu  @mechanical-wings/zalo "),
    ).toEqual(["@mechanical-wings/feishu", "@mechanical-wings/zalo"]);
  });
});
...
describe("collectPublishablePluginPackageErrors", () => {
  ...
  it("flags invalid publishable plugin metadata", () => {
    expect(
      ...
      'package name must start with "@mechanical-wings/"; found "broken".',
      "package.json private must not be true.",
      'package.json version must match YYYY.M.D, YYYY.M.D-N, or YYYY.M.D-beta.N; found "latest".',
      "mechanical-wings.extensions must contain only non-empty strings.",
    ]);
  });
});
...
describe("resolveSelectedPublishablePluginPackages", () => {
  const publishablePlugins: PublishablePluginPackage[] = [
    ...
    },
    {
      extensionId: "zalo",
      packageDir: "extensions/zalo",
      packageName: "@mechanical-wings/zalo",
  ...
  it("filters by selected publishable package names", () => {
    expect(
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@mechanical-wings/zalo"],
      }),
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@mechanical-wings/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @mechanical-wings/missing.");
  });
});
