...
const BASE_PATH = [BASH_BIN ? path.dirname(BASH_BIN) : "", process.env.PATH ?? "/usr/bin:/bin"]
  .filter(Boolean)
  .join(path.delimiter);
...
function runScript(
  ...
  if (!BASH_BIN) {
    return { ok: false, stdout: "", stderr: "bash unavailable" };
  }
...
describe("scripts/ios-team-id.sh", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await mkdtemp(path.join(os.tmpdir(), "mechanical-wings-ios-team-id-"));
...
printf 'BBBBB22222\\t0\\tBeta Team\\r\\n'`,
  ...
  testWithBash("resolves a fallback team ID from Xcode team listings (smoke)", async () => {
  ...
  testWithBash("prints actionable guidance when Xcode account exists but no Team ID is resolvable", async () => {
