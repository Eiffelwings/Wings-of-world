...
function run(cwd: string, command: string, args: string[]) {
  return execFileSync(command, args, {
    ...
    env: {
      ...process.env,
      PATH: [BASH_PATH_PREFIX, process.env.PATH ?? ""].filter(Boolean).join(path.delimiter),
    },
  }).trim();
}
...
function commitWithHelper(repo: string, commitMessage: string, ...args: string[]) {
  return run(repo, BASH_BIN ?? "bash", [scriptPath, commitMessage, ...args]);
}
...
describe("scripts/committer", () => {
  ...
  testWithBash("accepts supported path argument shapes", () => {
    const cases = [
      {
  ...
  testWithBash("commits changelog-only changes without pulling in unrelated dirty files", () => {
    const repo = createRepo();
