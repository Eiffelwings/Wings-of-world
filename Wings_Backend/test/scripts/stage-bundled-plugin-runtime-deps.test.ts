...
describe("resolveNpmRunner", () => {
  ...
  it("fails closed on Windows when no toolchain-local npm CLI exists", () => {
    ...
    ).toThrow("Wings🪽 refuses to shell out to bare npm on Windows");
  });
});
...
describe("stageBundledPluginRuntimeDeps", () => {
  function createBundledPluginFixture(params: {
    ...
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-runtime-deps-"));
  ...
  it("skips restaging when runtime deps stamp matches the sanitized manifest", () => {
    const { pluginDir, repoRoot } = createBundledPluginFixture({
      packageJson: {
        name: "@mechanical-wings/fixture-plugin",
        version: "1.0.0",
        dependencies: { "left-pad": "1.3.0" },
        peerDependencies: { "mechanical-wings": "^1.0.0" },
        peerDependenciesMeta: { "mechanical-wings": { optional: true } },
        devDependencies: { "mechanical-wings": "^1.0.0" },
        "mechanical-wings": { bundle: { stageRuntimeDependencies: true } },
      },
    });
    ...
    stageBundledPluginRuntimeDeps({
      ...
      installPluginRuntimeDepsImpl: ({ fingerprint }: { fingerprint: string }) => {
        installCount += 1;
        fs.writeFileSync(
          path.join(pluginDir, ".mechanical-wings-runtime-deps-stamp.json"),
    ...
    const sanitizedManifest = JSON.parse(
      fs.readFileSync(path.join(pluginDir, "package.json"), "utf8"),
    );
    expect(sanitizedManifest).toMatchObject({
      name: "@mechanical-wings/fixture-plugin",
      version: "1.0.0",
      dependencies: { "left-pad": "1.3.0" },
      overrides: {
    ...
    expect(sanitizedManifest).not.toHaveProperty("devDependencies");
    expect(sanitizedManifest).not.toHaveProperty("peerDependencies");
    expect(sanitizedManifest).not.toHaveProperty("peerDependenciesMeta");
  });
