...
describe("cleanupSessionStateForTest", () => {
  ...
  it("waits for in-flight session store locks before clearing test state", async () => {
    const fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-session-cleanup-"));
    const storePath = path.join(fixtureRoot, "mechanical-wings-sessions.json");
