...
describe("env test utils", () => {
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "mechanical-wings-home");
    ...
    process.env.OPENCLAW_HOME = "/srv/mechanical-wings-home";
    process.env.OPENCLAW_STATE_DIR = "/srv/mechanical-wings-state";
    process.env.OPENCLAW_BUNDLED_PLUGINS_DIR = "/srv/mechanical-wings-bundled";
    ...
    try {
      ...
      restoreEnvKey("OPENCLAW_HOME", previousWingsHome);
    ...
    try {
      const seen = withPathResolutionEnv(
        ...
        (env) => ({
          ...
          processWingsHome: process.env.OPENCLAW_HOME,
      ...
      expect(seen).toEqual({
        ...
        processWingsHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.OPENCLAW_HOME).toBe("/srv/mechanical-wings-home");
    } finally {
      restoreEnvKey("OPENCLAW_HOME", previousWingsHome);
    }
  });
