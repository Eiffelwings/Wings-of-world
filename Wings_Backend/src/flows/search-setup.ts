...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<WingsConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<WingsConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: WingsConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: WingsConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: WingsConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: WingsConfig, result: WingsConfig): WingsConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: WingsConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as WingsConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with mechanical-wings configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Wings will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.wings-of-world.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Wings will store a reference instead of the API key.",
