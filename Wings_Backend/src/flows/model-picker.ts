...
import { resolveModelCostConfig, type ModelCostConfig } from "../utils/usage-format.js";
...
export type PromptDefaultModelResult = { model?: string; config?: WingsConfig };
...
function resolveConfiguredModelRaw(cfg: WingsConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: WingsConfig): string[] {
...
function formatModelCostHint(cost?: ModelCostConfig): string | undefined {
  if (!cost) {
    return undefined;
  }
  const parts: string[] = [];
  if (Number.isFinite(cost.input)) {
    parts.push(`$${cost.input}/M in`);
  }
  if (Number.isFinite(cost.output)) {
    parts.push(`$${cost.output}/M out`);
  }
  return parts.length > 0 ? parts.join(" · ") : undefined;
}

function getGeminiMenuCategory(entry: {
  provider: string;
  id: string;
  input?: Array<"text" | "image" | "document">;
}): string | undefined {
  if (normalizeProviderId(entry.provider) !== "google") {
    return undefined;
  }
  const id = entry.id.toLowerCase();
  if (id.includes("image-preview") || id.includes("flash-image")) {
    return "Gemini ภาพ";
  }
...
function buildModelOptionLabel(params: {
  key: string;
  entry: {
    provider: string;
    id: string;
  ...
  const geminiCategory = getGeminiMenuCategory(params.entry);
  return geminiCategory ? `[${geminiCategory}] ${params.key}` : params.key;
}
...
function addModelSelectOption(params: {
  ...
  const costHint = formatModelCostHint(
  ...
  params.options.push({
    value: key,
    label: buildModelOptionLabel({ key, entry: params.entry }),
...
export async function promptDefaultModel(
  ...
  for (const entry of filteredModels) {
    addModelSelectOption({ entry, options, seen, aliasIndex, hasAuth, cfg });
  }
...
export function applyModelAllowlist(cfg: WingsConfig, models: string[]): WingsConfig {
