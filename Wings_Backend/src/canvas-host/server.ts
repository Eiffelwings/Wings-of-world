...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.mechanicalWingsCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.mechanicalWingsCanvasA2UIAction &&
        typeof window.mechanicalWingsCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.mechanicalWingsSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("mechanical-wings:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Wings node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.mechanicalWingsSendUserAction === "function"
        ? window.mechanicalWingsSendUserAction
        : undefined;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Wings Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
