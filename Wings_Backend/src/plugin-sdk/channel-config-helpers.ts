...
type ChannelConfigAccessorParams<Config extends WingsConfig = WingsConfig> = {
  cfg: Config;
...
type MultiAccountChannelConfigAdapterParams<
  ...
  Config extends WingsConfig = WingsConfig,
> = {
  sectionKey: string;
...
export function adaptScopedAccountAccessor<Result, Config extends WingsConfig = WingsConfig>(
...
> {
  const base = {
    resolveAllowFrom: ({ cfg, accountId }: { cfg: WingsConfig; accountId?: string | null }) =>
      mapAllowFromEntries(
...
function setTopLevelChannelEnabledInConfigSection<Config extends WingsConfig>(params: {
  cfg: Config;
  sectionKey: string;
  enabled: boolean;
}): Config {
...
function removeTopLevelChannelConfigSection<Config extends WingsConfig>(params: {
  cfg: Config;
  sectionKey: string;
}): Config {
...
function clearTopLevelChannelConfigFields<Config extends WingsConfig>(params: {
  cfg: Config;
  sectionKey: string;
  clearBaseFields: string[];
}): Config {
