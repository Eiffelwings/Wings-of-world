...
} from "../infra/tmp-mechanical-wings-dir.js";
...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredWingsTmpDir() : POSIX_OPENCLAW_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "mechanical-wings.log")
    : `${POSIX_OPENCLAW_TMP_DIR}/mechanical-wings.log`;
}

export const DEFAULT_LOG_DIR = resolveDefaultLogDir();
export const DEFAULT_LOG_FILE = resolveDefaultLogFile(DEFAULT_LOG_DIR); // legacy single-file path

const LOG_PREFIX = "mechanical-wings";
const LOG_SUFFIX = ".log";
const MAX_LOG_AGE_MS = 24 * 60 * 60 * 1000; // 24h
const DEFAULT_MAX_LOG_FILE_BYTES = 500 * 1024 * 1024; // 500 MB

const requireConfig = resolveNodeRequireFromMeta(import.meta.url);
const requireLoggingConfig = resolveNodeRequireFromMeta(import.meta.url);

type LoggingConfigModule = {
  readLoggingConfig?: () => WingsConfig["logging"] | undefined;
  shouldSkipMutatingLoggingConfigRead?: (argv?: string[]) => boolean;
};

function loadLoggingConfigModule(): LoggingConfigModule | null {
  try {
    return (requireLoggingConfig?.("./config.js") as LoggingConfigModule | undefined) ?? null;
  } catch {
    return null;
  }
}

function readLoggingConfigLazy(): WingsConfig["logging"] | undefined {
  return loadLoggingConfigModule()?.readLoggingConfig?.();
}

function shouldSkipMutatingLoggingConfigReadLazy(argv?: string[]): boolean {
  const [primary, secondary] = getCommandPathWithRootOptions(argv ?? process.argv, 2);
...
function resolveSettings(): ResolvedSettings {
  ...
  let cfg: WingsConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfigLazy();
  if (!cfg && !shouldSkipMutatingLoggingConfigReadLazy()) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => WingsConfig;
          }
        | undefined;
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[mechanical-wings] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
...
export const __test__ = {
  shouldSkipMutatingLoggingConfigRead: shouldSkipMutatingLoggingConfigReadLazy,
};
