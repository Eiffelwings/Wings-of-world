...
let originalWingsLogLevel: string | undefined;
...
beforeEach(() => {
  ...
  originalWingsLogLevel = process.env.OPENCLAW_LOG_LEVEL;
...
afterEach(() => {
  ...
  if (originalWingsLogLevel === undefined) {
    delete process.env.OPENCLAW_LOG_LEVEL;
  } else {
    process.env.OPENCLAW_LOG_LEVEL = originalWingsLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
