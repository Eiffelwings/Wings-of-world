...
import { ensureWingsExecMarkerOnProcess } from "./infra/mechanical-wings-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "wings-of-world-backend.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "wings-of-world-backend.js", entryBasename: "entry.js" },
  { wrapperBasename: "mechanical-wings.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "mechanical-wings.js", entryBasename: "entry.js" },
] as const;

const LOG_PREFIX = "[wings-of-world-backend]";
...
if (
  ...
  process.title = "wings-of-world-backend";
  ...
  function ensureCliRespawnReady(): boolean {
    ...
    child.once("error", (error) => {
      console.error(
        `${LOG_PREFIX} Failed to respawn CLI:`,
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `Wings ${VERSION} (${commit})` : `Wings ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          `${LOG_PREFIX} Failed to resolve version:`,
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureCliRespawnReady()) {
    ...
    if (!parsedContainer.ok) {
      console.error(`${LOG_PREFIX} ${parsedContainer.error}`);
      process.exit(2);
    }
    ...
    if (containerTargetName && parsed.profile) {
      console.error(`${LOG_PREFIX} --container cannot be combined with --profile/--dev`);
      process.exit(2);
    }
...
export function tryHandleRootHelpFastPath(
  ...
  const handleError =
    ...
    ((error: unknown) => {
      console.error(
        `${LOG_PREFIX} Failed to display help:`,
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
...
function runMainOrRootHelp(argv: string[]): void {
  ...
  import("./cli/run-main.js")
    ...
    .catch((error) => {
      console.error(
        `${LOG_PREFIX} Failed to start CLI:`,
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
