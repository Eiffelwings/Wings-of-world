...
type ConfiguredBinding = NonNullable<WingsConfig["bindings"]>[number];
...
function resolveBindingRecord(cfg: WingsConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/mechanical-wings" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
