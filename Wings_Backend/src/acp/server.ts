...
function printHelp(): void {
  console.log(`Usage: wings-of-world-backend acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN.",
    );
  }
