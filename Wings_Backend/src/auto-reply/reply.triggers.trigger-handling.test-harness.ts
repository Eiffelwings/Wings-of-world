...
const piEmbeddedMocks = getSharedMocks("mechanical-wings.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("mechanical-wings.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("mechanical-wings.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("mechanical-wings.trigger-handling.web-session-mocks", () => ({
...
type TempHomeEnvSnapshot = {
  ...
  mechanicalWingsHome: string | undefined;
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    mechanicalWingsHome: process.env.OPENCLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("OPENCLAW_HOME", snapshot.mechanicalWingsHome);
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.OPENCLAW_STATE_DIR = join(home, ".mechanical-wings");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "mechanical-wings-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".mechanical-wings", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): WingsConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "mechanical-wings"),
