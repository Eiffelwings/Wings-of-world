...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(
    ...
    {
      env: {
        OPENCLAW_AGENT_DIR: (home) => path.join(home, ".mechanical-wings", "agent"),
        PI_CODING_AGENT_DIR: (home) => path.join(home, ".mechanical-wings", "agent"),
      },
      prefix: "mechanical-wings-reply-",
    },
  );
}
