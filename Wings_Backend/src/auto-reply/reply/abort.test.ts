...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop mechanical-wings!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@mechanical-wings_bot", { botUsername: "mechanical-wings_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@mechanical-wings_bot", { botUsername: "mechanical-wings_bot" })).toBe(true);
