...
import { scheduleGatewaySigusr1Restart, triggerWingsRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Wings in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerWingsRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Wings via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
