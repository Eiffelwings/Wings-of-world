...
export async function handleAcpDoctorAction(
  ...
  try {
    ...
    lines.push(`next: mechanical-wings config set plugins.entries.${backendId}.enabled true`);
...
export function handleAcpInstallAction(
  ...
  const lines = [
    ...
    `run: ${installHint}`,
    `then: mechanical-wings config set plugins.entries.${backendId}.enabled true`,
    "then: /acp doctor",
  ];
  return stopWithText(lines.join("\n"));
}
