...
const { createWingsCodingToolsMock } = vi.hoisted(() => ({
  createWingsCodingToolsMock: vi.fn(() => []),
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(async () => {
    vi.restoreAllMocks();
    vi.resetModules();
    createWingsCodingToolsMock.mockClear();
    createWingsCodingToolsMock.mockReturnValue([]);
    const piTools = await import("../../agents/pi-tools.js");
    vi.spyOn(piTools, "createWingsCodingTools").mockImplementation(
      createWingsCodingToolsMock,
    );
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    expect(createWingsCodingToolsMock).toHaveBeenCalledWith(
