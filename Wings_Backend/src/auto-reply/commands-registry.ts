...
export function isCommandEnabled(cfg: WingsConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: WingsConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: WingsConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: WingsConfig) {
