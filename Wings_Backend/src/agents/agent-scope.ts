...
type AgentEntry = NonNullable<NonNullable<WingsConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: WingsConfig): AgentEntry[] {
...
export function listAgentIds(cfg: WingsConfig): string[] {
...
export function resolveDefaultAgentId(cfg: WingsConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: WingsConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: WingsConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: WingsConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
