...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("mechanical-wings-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("mechanical-wings-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("mechanical-wings-sbx-");
