...
async function loadFreshWingsToolsModuleForTest() {
  vi.resetModules();
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshWingsToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-test-"));
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: mechanical-wings --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw =
      '{\n  agents: { defaults: { workspace: "~/mechanical-wings" } },\n  tools: { exec: { ask: "on-miss", security: "allowlist" } }\n}\n';
  ...
  it("rejects config.apply when protected exec settings are omitted", async () => {
    ...
    await expect(
      tool.execute("call-missing-protected", {
        action: "config.apply",
        raw: '{ agents: { defaults: { workspace: "~/mechanical-wings" } } }',
      }),
    ).rejects.toThrow(
