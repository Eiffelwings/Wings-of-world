...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseWingsManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyWingsManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveWingsMetadata(
  frontmatter: ParsedSkillFrontmatter,
): WingsSkillMetadata | undefined {
  const metadataObj = resolveWingsManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveWingsManifestRequires(metadataObj);
  const install = resolveWingsManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveWingsManifestOs(metadataObj);
  return {
