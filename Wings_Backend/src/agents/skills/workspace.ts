...
function resolveSkillsLimits(config?: WingsConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const extraSkills = mergedExtraDirs.flatMap((dir) => {
    ...
    return loadSkills({
      dir: resolved,
      source: "mechanical-wings-extra",
    });
  });
  const managedSkills = loadSkills({
    dir: managedSkillsDir,
    source: "mechanical-wings-managed",
  });
  ...
  const workspaceSkills = loadSkills({
    ...
    source: "mechanical-wings-workspace",
  });
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveWingsMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: WingsConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}${compact ? " (compact format, descriptions omitted)" : ""}. Run \`mechanical-wings skills check\` to audit.`
    : compact
      ? `⚠️ Skills catalog using compact format (descriptions omitted). Run \`mechanical-wings skills check\` to audit.`
      : "";
  const prompt = [
    remoteNote,
    truncationNote,
