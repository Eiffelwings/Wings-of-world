    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "mechanical-wings.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "mechanical-wings.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "mechanical-wings.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("mechanical-wings-");
  const acpxRoot = await tempDirs.make("mechanical-wings-acpx-plugin-");
  const helperRoot = await tempDirs.make("mechanical-wings-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("mechanical-wings-");
  const pluginRoot = await tempDirs.make("mechanical-wings-plugin-");
  const outsideDir = await tempDirs.make("mechanical-wings-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("mechanical-wings-");
    const pluginRoot = await tempDirs.make("mechanical-wings-claude-bundle-");
