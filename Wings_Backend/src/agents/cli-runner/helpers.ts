...
import { KeyedAsyncQueue } from "mechanical-wings/plugin-sdk/keyed-async-queue";
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-cli-images-"));
  const paths: string[] = [];
