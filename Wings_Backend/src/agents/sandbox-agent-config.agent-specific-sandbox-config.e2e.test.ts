...
async function resolveContext(config: WingsConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): WingsConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
    ...
    const cfg: WingsConfig = {
      agents: {
        ...
        list: [
          {
            id: "isolated",
            workspace: "~/mechanical-wings-isolated",
            sandbox: {
              mode: "all",
              scope: "agent",
