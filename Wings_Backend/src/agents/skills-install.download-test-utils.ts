...
export async function withTempWorkspace(
  ...
  const tempHome = await createTempHomeEnv("mechanical-wings-skills-install-home-");
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-skills-install-"));
  try {
