...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    "mechanical-wings": process.env.OPENCLAW_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot["mechanical-wings"] === undefined) {
      delete process.env.OPENCLAW_GATEWAY_TOKEN;
    } else {
      process.env.OPENCLAW_GATEWAY_TOKEN = envSnapshot["mechanical-wings"];
    }
  });
