...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("mechanical-wings-bundled-");
  const workspaceDir = await createTempDir("mechanical-wings-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: WingsConfig) {
