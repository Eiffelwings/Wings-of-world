...
import { resolvePreferredWingsTmpDir } from "../infra/tmp-mechanical-wings-dir.js";
...
async function withOutsideHardlinkInWingsTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside mechanical-wings tmp root",
      media: path.join(os.tmpdir(), "outside-mechanical-wings", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Wings tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Wings tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInWingsTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked Wings tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInWingsTmp(
