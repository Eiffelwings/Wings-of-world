...
function resolveAttachmentLimits(config: WingsConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".mechanical-wings", "attachments");
  const relDir = path.posix.join(".mechanical-wings", "attachments", attachmentId);
