...
async function createBundledRuntime(options?: { reservedToolNames?: string[] }) {
  const workspaceDir = await makeTempDir("mechanical-wings-bundle-mcp-tools-");
  const pluginRoot = path.join(workspaceDir, ".mechanical-wings", "extensions", "bundle-probe");
