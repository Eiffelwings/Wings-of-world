...
import { planWingsModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  config: WingsConfig;
  sourceConfigForSecrets: WingsConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: WingsConfig): {
  config: WingsConfig;
  sourceConfigForSecrets: WingsConfig;
} {
...
export async function ensureWingsModelsJson(
  config?: WingsConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveWingsAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planWingsModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
