 ...
 *
 * @see https://github.com/mechanical-wings/mechanical-wings/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern =
    /(?:wings-of-world|mechanical-wings)\.json|(?:wings-of-world-backend|mechanical-wings) gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumed = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied; do not re-modify wings-of-world.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
