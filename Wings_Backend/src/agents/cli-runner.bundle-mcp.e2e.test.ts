...
describe("runCliAgent bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-cli-bundle-mcp-"));
      ...
      const pluginRoot = path.join(tempHome, ".mechanical-wings", "extensions", "bundle-probe");
