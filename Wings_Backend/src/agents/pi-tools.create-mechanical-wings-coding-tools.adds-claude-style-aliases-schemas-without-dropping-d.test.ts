...
describe("createWingsCodingTools", () => {
  it("returns image metadata for images and text-only blocks for text files", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-read-"));
    try {
      ...
      const contents = "Hello from mechanical-wings read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createWingsCodingTools({ sandbox });
