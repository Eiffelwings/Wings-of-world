...
import { createWingsCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: WingsConfig) {
    return createWingsCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<WingsConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<WingsConfig["agents"]>["list"]>[number]["tools"];
  }): WingsConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createWingsCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createWingsCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createWingsCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createWingsCodingTools({
      config: cfg,
    ...
    const bobTools = createWingsCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createWingsCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createWingsCodingTools({
      config: cfg,
