...
export async function createSshSandboxSessionFromConfigText(params: {
  ...
  const configDir = await fs.mkdtemp(path.join(resolveSshTmpRoot(), "mechanical-wings-sandbox-ssh-"));
...
export async function createSshSandboxSessionFromSettings(
  ...
  try {
    ...
    const hostAlias = "mechanical-wings-sandbox";
    const configPath = path.join(configDir, "config");
    const lines = [
      `Host ${hostAlias}`,
...
export async function uploadDirectoryToSshTarget(params: {
  ...
  const remoteCommand = buildRemoteCommand([
    ...
    "mechanical-wings-sandbox-upload",
    params.remoteDir,
  ]);
...
function resolveSshTmpRoot(): string {
  return path.resolve(resolvePreferredWingsTmpDir() ?? os.tmpdir());
}
