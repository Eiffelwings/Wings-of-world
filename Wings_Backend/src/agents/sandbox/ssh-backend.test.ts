      ...
      defaults: {
        sandbox: {
          ...
          ssh: {
            ...
            command: "ssh",
            workspaceRoot: "/remote/mechanical-wings",
...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "mechanical-wings-test-ssh-config"),
    host: "mechanical-wings-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "mechanical-wings-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "mechanical-wings-ssh-worker-abcd1234",
    ...
    expect(sshMocks.runSshSandboxCommand).toHaveBeenCalledWith(
      expect.objectContaining({
        remoteCommand: expect.stringContaining("/remote/mechanical-wings/mechanical-wings-ssh-agent-worker"),
      }),
    );
  });
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const backend = await createSshSandboxBackend({
      ...
      cfg: {
        ...
        browser: {
          enabled: false,
          image: "mechanical-wings-browser",
          containerPrefix: "mechanical-wings-browser-",
          network: "bridge",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
    ...
    expect(execSpec.argv.at(-1)).toContain("/remote/mechanical-wings/mechanical-wings-ssh-agent-worker");
