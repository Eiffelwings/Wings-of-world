...
import { markWingsExecEnv } from "../../infra/mechanical-wings-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "mechanical-wings.sandbox=1");
  args.push("--label", `mechanical-wings.sessionKey=${params.scopeKey}`);
  args.push("--label", `mechanical-wings.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `mechanical-wings.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markWingsExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "mechanical-wings.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`mechanical-wings sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`mechanical-wings sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("mechanical-wings sandbox recreate --all");
}
