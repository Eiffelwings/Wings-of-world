    ...
    try {
      const result = await runSshSandboxCommand({
        ...
        remoteCommand: buildRemoteCommand([
          ...
          "mechanical-wings-sandbox-check",
    ...
    try {
      await runSshSandboxCommand({
        ...
        remoteCommand: buildRemoteCommand([
          ...
          "-c",
          'rm -rf -- "$1"',
          "mechanical-wings-sandbox-remove",
          runtimePaths.runtimeRootDir,
        ]),
        allowFailure: true,
      });
  ...
  private async replaceRemoteDirectoryFromLocal(
    ...
    await runSshSandboxCommand({
      ...
      remoteCommand: buildRemoteCommand([
        ...
        "mechanical-wings-sandbox-clear",
        remoteDir,
      ]),
    });
...
function buildSshSandboxRuntimeId(scopeKey: string): string {
  ...
  return `mechanical-wings-ssh-${safe || "session"}-${hash.toString(16).slice(0, 8)}`;
}
