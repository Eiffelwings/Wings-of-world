...
const defaultTools = createWingsCodingTools({ senderIsOwner: true });
...
describe("createWingsCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const disabledOpenAiTools = createWingsCodingTools({
    ...
    const anthropicTools = createWingsCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: WingsConfig = {
      tools: {
        exec: {
    ...
    const allowed = createWingsCodingTools({
    ...
    const denied = createWingsCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createWingsCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
