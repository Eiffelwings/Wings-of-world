...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("mechanical-wings-ws-", async (workspaceDir) => {
      await withTempDir("mechanical-wings-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createWingsCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("mechanical-wings-ws-", async (workspaceDir) => {
      await withTempDir("mechanical-wings-override-", async (overrideDir) => {
        const execTool = createExecTool(workspaceDir);
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("mechanical-wings-ws-", async (workspaceDir) => {
      const cfg: WingsConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createWingsCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-mechanical-wings.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("mechanical-wings-sandbox-", async (sandboxDir) => {
      await withTempDir("mechanical-wings-workspace-", async (workspaceDir) => {
        ...
        const tools = createWingsCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
