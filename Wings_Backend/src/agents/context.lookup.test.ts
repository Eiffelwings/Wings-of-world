...
function mockContextDeps(params: {
  ...
  const ensureWingsModelsJson = vi.fn(async () => {});
  vi.doMock("../config/config.js", () => ({
    loadConfig: params.loadConfig,
  }));
  vi.doMock("./models-config.js", () => ({
    ensureWingsModelsJson,
  }));
  vi.doMock("./agent-paths.js", () => ({
    resolveWingsAgentDir: () => "/tmp/mechanical-wings-agent",
  }));
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real mechanical-wings startup commands that need model metadata", async () => {
    const argvSnapshot = process.argv;
    try {
      for (const scenario of [
        {
          argv: ["node", "mechanical-wings", "chat"],
          expectedCalls: 1,
        },
        {
          argv: ["node", "mechanical-wings", "--profile", "--", "config", "validate"],
          expectedCalls: 0,
        },
        {
          argv: ["node", "mechanical-wings", "logs", "--limit", "5"],
          expectedCalls: 0,
        },
        {
        ...
        const { ensureWingsModelsJson } = mockContextModuleDeps(loadConfigMock);
        process.argv = scenario.argv;
        await import("./context.js");
        await flushAsyncWarmup();
        expect(loadConfigMock).toHaveBeenCalledTimes(scenario.expectedCalls);
        expect(ensureWingsModelsJson).toHaveBeenCalledTimes(scenario.expectedCalls);
      }
    } finally {
