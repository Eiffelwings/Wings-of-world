...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: WingsConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/mechanical-wings" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: WingsConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: WingsConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: WingsConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: WingsConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: WingsConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: WingsConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
    ...
    const cfg: WingsConfig = {
      agents: {
        list: [
          {
            id: "work",
            workspace: "~/mechanical-wings-work",
            sandbox: {
              mode: "all",
              scope: "agent",
              perSession: false,
    ...
    const cfg: WingsConfig = {
      agents: {
        list: [
          {
            id: "restricted",
            workspace: "~/mechanical-wings-restricted",
            tools: {
              allow: ["read"],
    ...
    const cfg: WingsConfig = {
      agents: {
        list: [
          {
            id: "family",
            workspace: "~/mechanical-wings-family",
            sandbox: {
              mode: "all",
              scope: "agent",
            },
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/mechanical-wings");
  });

  it("uses OPENCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "mechanical-wings-home");
    vi.stubEnv("OPENCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as WingsConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".mechanical-wings", "workspace"));
  });
  ...
  it("uses OPENCLAW_HOME for default agentDir", () => {
    ...
    const agentDir = resolveAgentDir({} as WingsConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".mechanical-wings", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/mechanical-wings-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/mechanical-wings-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-agent-scope-"));
