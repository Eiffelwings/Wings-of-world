...
import { resolveWingsAgentDir } from "./agent-paths.js";
...
let configuredConfig: WingsConfig | undefined;
...
function isLikelyWingsCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = path
    .basename(argv[1] ?? "")
    .trim()
    .toLowerCase();
  return (
    entryBasename === "mechanical-wings" ||
    entryBasename === "mechanical-wings.mjs" ||
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real Wings CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureWingsModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyWingsCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): WingsConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureWingsModelsJson(cfg);
    } catch {
