...
function findTool(name: string, config: WingsConfig) {
  const allTools = createWingsTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: WingsConfig; env?: NodeJS.ProcessEnv }) {
...
describe("mechanical-wings tools runtime web metadata wiring", () => {
