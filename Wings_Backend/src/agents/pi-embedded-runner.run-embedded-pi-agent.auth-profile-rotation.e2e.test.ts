...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async (importOriginal) => {
    ...
    return {
      ...mod,
      ensureWingsModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): WingsConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): WingsConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): WingsConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  try {
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `mechanical-wings-auth-rotation-${Date.now()}.log`),
    });
