...
import { OPENCLAW_CLI_ENV_VALUE } from "../infra/mechanical-wings-exec-env.js";
...
describe("buildSandboxCreateArgs", () => {
  ...
  it("includes hardening and resource flags", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "mechanical-wings-sbx-test",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      labels: { "mechanical-wings.sandboxBrowser": "1" },
    });
    ...
    expect(args).toEqual(
      expect.arrayContaining([
        "create",
        "--name",
        "mechanical-wings-sbx-test",
        "--label",
        "mechanical-wings.sandbox=1",
        "--label",
        "mechanical-wings.sessionKey=main",
        "--label",
        "mechanical-wings.createdAtMs=1700000000000",
        "--label",
        "mechanical-wings.sandboxBrowser=1",
        "--read-only",
        "--tmpfs",
        "/tmp",
        "--network",
        ...
        "--security-opt",
        "apparmor=mechanical-wings-sandbox",
        "--dns",
        "1.1.1.1",
        "--add-host",
  ...
  it("preserves the Wings exec marker when strict env sanitization is enabled", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "mechanical-wings-sbx-marker",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
  ...
  it("emits -v flags for safe custom binds", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "mechanical-wings-sbx-binds",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
  ...
  it("omits -v flags when binds is empty or undefined", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "mechanical-wings-sbx-no-binds",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
  ...
  it("blocks bind sources outside runtime allowlist roots", () => {
    ...
    expect(() =>
      buildSandboxCreateArgs({
        name: "mechanical-wings-sbx-outside-roots",
        cfg,
        scopeKey: "main",
        createdAtMs: 1700000000000,
  ...
  it("allows bind sources outside runtime allowlist with explicit override", () => {
    const cfg = createSandboxConfig({}, ["/opt/external:/data:rw"]);
    const args = buildSandboxCreateArgs({
      name: "mechanical-wings-sbx-outside-roots-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
      bindSourceRoots: ["/tmp/workspace", "/tmp/agent"],
  ...
  it("blocks reserved /workspace target bind mounts by default", () => {
    ...
    expectBuildToThrow("mechanical-wings-sbx-reserved-target", cfg, /reserved container path/);
  });

  it("allows reserved /workspace target bind mounts with explicit dangerous override", () => {
    const cfg = createSandboxConfig({}, ["/tmp/override:/workspace:rw"]);
    const args = buildSandboxCreateArgs({
      name: "mechanical-wings-sbx-reserved-target-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
  ...
  it("allows container namespace join with explicit dangerous override", () => {
    ...
    const args = buildSandboxCreateArgs({
      name: "mechanical-wings-sbx-container-network-override",
      cfg,
      scopeKey: "main",
      createdAtMs: 1700000000000,
    });
