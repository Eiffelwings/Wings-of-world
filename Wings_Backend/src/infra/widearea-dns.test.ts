...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  domain: "mechanical-wings.internal.",
  gatewayPort: 18789,
  displayName: "Mac Studio (Wings)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "mechanical-wings.internal", expected: "mechanical-wings.internal." },
    { value: "mechanical-wings.internal.", expected: "mechanical-wings.internal." },
    { value: "  mechanical-wings.internal  ", expected: "mechanical-wings.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("mechanical-wings.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "mechanical-wings.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    const txt = renderWideAreaGatewayZoneText({
      ...
      instanceLabel: "studio-london",
      sshPort: 22,
      cliPath: "/opt/homebrew/bin/mechanical-wings",
    });

    expect(txt).toContain(`$ORIGIN mechanical-wings.internal.`);
    ...
    expect(txt).toContain(`_mechanical-wings-gw._tcp IN PTR studio-london._mechanical-wings-gw._tcp`);
    expect(txt).toContain(`studio-london._mechanical-wings-gw._tcp IN SRV 0 0 18789 studio-london`);
    expect(txt).toContain(`displayName=Mac Studio (Wings)`);
    expect(txt).toContain(`gatewayPort=18789`);
    expect(txt).toContain(`sshPort=22`);
    expect(txt).toContain(`cliPath=/opt/homebrew/bin/mechanical-wings`);
  });
  ...
  it("includes gateway TLS TXT fields and trims display metadata", () => {
    ...
    expect(txt).toContain(`$ORIGIN mechanical-wings.internal.`);
    expect(txt).toContain(`studio-london IN A 100.123.224.76`);
    expect(txt).toContain(`studio-london._mechanical-wings-gw._tcp IN TXT`);
    expect(txt).toContain(`displayName=Mac Studio (Wings)`);
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("mechanical-wings.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("mechanical-wings.internal."),
