...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.mechanical-wings/.env as fallback without overriding CWD .env", async () => {
