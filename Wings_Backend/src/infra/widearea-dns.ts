...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*mechanical-wings-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "mechanical-wings";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "mechanical-wings");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "mechanical-wings-gw");
  ...
  records.push(`_mechanical-wings-gw._tcp IN PTR ${instanceLabel}._mechanical-wings-gw._tcp`);
  records.push(`${instanceLabel}._mechanical-wings-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._mechanical-wings-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; mechanical-wings-content-hash: ${contentHash}\n${contentBody}`;
}
