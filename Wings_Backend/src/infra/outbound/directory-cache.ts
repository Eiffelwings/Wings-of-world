...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: WingsConfig | null = null;
  ...
  get(key: string, cfg: WingsConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: WingsConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: WingsConfig): void {
