...
import { resolvePreferredWingsTmpDir } from "../tmp-mechanical-wings-dir.js";
...
const telegramChunkConfig: WingsConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: WingsConfig = {
  channels: { whatsapp: { textChunkLimit: 4000 } },
};

const expectedResolvedTmpRoot = path.resolve(resolvePreferredWingsTmpDir());
...
describe("deliverOutboundPayloads", () => {
  ...
  it("scopes media local roots to the active agent workspace when agentId is provided", async () => {
    ...
    expect(
      sendOpts?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".mechanical-wings", "workspace-work")),
      ),
    ).toBe(true);
  });

  it("includes Wings tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
  ...
  it("includes Wings tmp root in signal mediaLocalRoots", async () => {
  ...
  it("forwards audioAsVoice through generic plugin media delivery", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as WingsConfig,
      channel: "matrix",
  ...
  it("includes Wings tmp root in whatsapp mediaLocalRoots", async () => {
    const sendWhatsApp = vi.fn().mockResolvedValue({ messageId: "w1", toJid: "jid" });
  ...
  it("includes Wings tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: WingsConfig = { channels: { signal: { mediaMaxMb: 2 } } };
