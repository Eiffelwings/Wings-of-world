...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Wings tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideWings = path.join(os.tmpdir(), "outside-mechanical-wings", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
