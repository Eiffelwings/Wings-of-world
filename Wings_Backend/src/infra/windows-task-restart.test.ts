...
const resolvePreferredWingsTmpDirMock = vi.hoisted(() => vi.fn(() => os.tmpdir()));

vi.mock("node:child_process", () => ({
  spawn: (...args: unknown[]) => spawnMock(...args),
}));
vi.mock("./tmp-mechanical-wings-dir.js", () => ({
  resolvePreferredWingsTmpDir: () => resolvePreferredWingsTmpDirMock(),
}));
...
afterEach(() => {
  envSnapshot.restore();
  spawnMock.mockReset();
  resolvePreferredWingsTmpDirMock.mockReset();
  resolvePreferredWingsTmpDirMock.mockReturnValue(os.tmpdir());
...
describe("relaunchGatewayScheduledTask", () => {
  ...
  it("writes a detached schtasks relaunch helper", () => {
    ...
    expect(result).toMatchObject({
      ok: true,
      method: "schtasks",
      tried: expect.arrayContaining(['schtasks /Run /TN "Wings Of World Gateway (work)"']),
    });
    ...
    expect(script).toContain('schtasks /Run /TN "Wings Of World Gateway (work)" >nul 2>&1');
  ...
  it("prefers OPENCLAW_WINDOWS_TASK_NAME overrides", () => {
    ...
    const scriptPath = [...createdScriptPaths][0];
    const script = fs.readFileSync(scriptPath, "utf8");
    expect(script).toContain('schtasks /Run /TN "Wings Gateway (custom)" >nul 2>&1');
  });
  ...
  it("quotes the cmd /c script path when temp paths contain metacharacters", () => {
    const unref = vi.fn();
    const metacharTmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings&(restart)-"));
    createdTmpDirs.add(metacharTmpDir);
    resolvePreferredWingsTmpDirMock.mockReturnValue(metacharTmpDir);
