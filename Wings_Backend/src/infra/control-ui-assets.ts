...
import { resolveWingsPackageRoot, resolveWingsPackageRootSync } from "./mechanical-wings-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveWingsPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "mechanical-wings" + dist/control-ui/index.html
    ...
    for (let i = 0; i < 8; i++) {
      ...
      if (fs.existsSync(pkgJsonPath)) {
        try {
          ...
          if (parsed.name === "mechanical-wings") {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1Dir) {
    // mechanical-wings.mjs or dist/<bundle>.js
  ...
  if (argv1RealpathDir && argv1RealpathDir !== argv1Dir) {
    // Symlinked wrappers (e.g. ~/.bun/bin/mechanical-wings -> .../dist/index.js)
