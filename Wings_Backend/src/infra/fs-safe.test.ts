...
async function withOutsideHardlinkAlias(params: {
  ...
  const outside = await tempDirs.make("mechanical-wings-fs-safe-outside-");
...
async function setupSymlinkWriteRaceFixture(options?: { seedInsideTarget?: boolean }): Promise<{
  ...
  const root = await tempDirs.make("mechanical-wings-fs-safe-root-");
...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await tempDirs.make("mechanical-wings-fs-safe-");
  ...
  it("copies a file within root safely", async () => {
    const root = await tempDirs.make("mechanical-wings-fs-safe-root-");
    const sourceDir = await tempDirs.make("mechanical-wings-fs-safe-source-");
  ...
  it("writes a file within root from another local source path safely", async () => {
    const root = await tempDirs.make("mechanical-wings-fs-safe-root-");
    const outside = await tempDirs.make("mechanical-wings-fs-safe-src-");
...
describe("tilde expansion in file tools", () => {
  it("keeps tilde expansion behavior aligned", async () => {
    ...
    const originalWingsHome = process.env.OPENCLAW_HOME;
    ...
    try {
      ...
      process.env.OPENCLAW_HOME = originalWingsHome;
    }

    const root = await tempDirs.make("mechanical-wings-tilde-test-");
    ...
    const outsideRoot = await tempDirs.make("mechanical-wings-tilde-outside-");
    await expect(
      openFileWithinRoot({
        rootDir: outsideRoot,
        relativePath: "~/escape.txt",
