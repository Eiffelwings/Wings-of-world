 ...
 * returning control to the caller (typically `triggerWingsRestart` →
...
/**
 * Parse wings-of-world-backend gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("mechanical-wings")) {
 ...
 * Returns only PIDs that belong to wings-of-world-backend gateway processes (not the current process).
 */
