...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Wings";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Wings\)\s*$/i, "").trim() || normalized;
}
...
function isAnnouncedState(state: BonjourServiceState | "unknown") {
  return (state as string) === "announced";
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw = process.env.OPENCLAW_MDNS_HOSTNAME?.trim() || "mechanical-wings";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
      .trim() || "mechanical-wings";
  const instanceName =
