...
async function makeTempRoot() {
  const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-state-dir-"));
  tempRoot = root;
  return root;
}
...
describe("legacy state dir auto-migration", () => {
  it("follows legacy symlink when it points at another legacy dir (clawdbot -> moldbot)", async () => {
    ...
    const targetMarker = path.join(root, ".wings-of-world-backend", "marker.txt");
