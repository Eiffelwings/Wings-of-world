...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/mechanical-wings-state", "devices")).toEqual({
      dir: path.join("/tmp/mechanical-wings-state", "devices"),
      pendingPath: path.join("/tmp/mechanical-wings-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/mechanical-wings-state", "devices", "paired.json"),
    });
  });
