...
describe("checkDepsStatus", () => {
  it("reports unknown, missing, stale, and ok states from lockfile markers", async () => {
    const base = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-update-check-"));
...
describe("checkUpdateStatus", () => {
  ...
  it("detects package installs for non-git roots", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-update-check-"));
    await fs.writeFile(
