...
export type GatewayLockOptions = {
  ...
  readLinuxStartTime?: (pid: number) => number | null;
...
async function resolveGatewayOwnerStatus(
  ...
  readLinuxStartTimeFn = readLinuxStartTime,
  ...
  if (Number.isFinite(payloadStartTime)) {
    const currentStartTime = readLinuxStartTimeFn(pid);
...
export async function acquireGatewayLock(
  ...
  const readLinuxStartTimeFn = opts.readLinuxStartTime ?? readLinuxStartTime;
  const checkPortFreeFn = opts.checkPortFree ?? checkPortFree;
  const sleep =
  ...
  while (now() - startedAt < timeoutMs) {
    try {
      ...
      const startTime = platform === "linux" ? readLinuxStartTimeFn(process.pid) : null;
