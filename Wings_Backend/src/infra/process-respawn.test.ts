...
const triggerWingsRestartMock = vi.hoisted(() => vi.fn());
...
vi.mock("./restart.js", () => ({
  triggerWingsRestart: (...args: unknown[]) => triggerWingsRestartMock(...args),
}));
...
afterEach(() => {
  ...
  spawnMock.mockClear();
  triggerWingsRestartMock.mockClear();
...
function expectLaunchdSupervisedWithoutKickstart(params?: {
  ...
  process.env.OPENCLAW_LAUNCHD_LABEL = "ai.mechanical-wings.gateway";
  ...
  expect(triggerWingsRestartMock).not.toHaveBeenCalled();
...
describe("restartGatewayProcessWithFreshPid", () => {
  ...
  it("returns supervised when launchd hints are present on macOS (no kickstart)", () => {
    ...
    expectLaunchdSupervisedWithoutKickstart({
      launchJobLabel: "ai.mechanical-wings.gateway",
      detailContains: "launchd restart handoff",
    });
  });

  it("returns supervised on macOS when launchd label is set (no kickstart)", () => {
    expectLaunchdSupervisedWithoutKickstart({ launchJobLabel: "ai.mechanical-wings.gateway" });
  });

  it("launchd supervisor never returns failed regardless of triggerWingsRestart outcome", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.OPENCLAW_LAUNCHD_LABEL = "ai.mechanical-wings.gateway";
    // Even if triggerWingsRestart *would* fail, launchd path must not call it.
    triggerWingsRestartMock.mockReturnValue({
      ok: false,
      method: "launchctl",
  ...
  it("falls back to plain supervised exit when launchd handoff scheduling fails", () => {
    clearSupervisorHints();
    setPlatform("darwin");
    process.env.XPC_SERVICE_NAME = "ai.mechanical-wings.gateway";
  ...
  it("returns supervised when OPENCLAW_SYSTEMD_UNIT is set", () => {
    clearSupervisorHints();
    setPlatform("linux");
    process.env.OPENCLAW_SYSTEMD_UNIT = "mechanical-wings-gateway.service";
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(spawnMock).not.toHaveBeenCalled();
  });

  it("returns supervised when Wings gateway task markers are set on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.OPENCLAW_SERVICE_MARKER = "mechanical-wings";
    process.env.OPENCLAW_SERVICE_KIND = "gateway";
    triggerWingsRestartMock.mockReturnValue({ ok: true, method: "schtasks" });
    const result = restartGatewayProcessWithFreshPid();
    expect(result.mode).toBe("supervised");
    expect(triggerWingsRestartMock).toHaveBeenCalledOnce();
    expect(spawnMock).not.toHaveBeenCalled();
  });
  ...
  it("ignores node task script hints for gateway restart detection on Windows", () => {
    clearSupervisorHints();
    setPlatform("win32");
    process.env.OPENCLAW_TASK_SCRIPT = "C:\\mechanical-wings\\node.cmd";
