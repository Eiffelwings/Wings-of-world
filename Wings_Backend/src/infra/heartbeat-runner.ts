...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: WingsConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: WingsConfig) {
...
function resolveHeartbeatAgents(cfg: WingsConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: WingsConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: WingsConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: WingsConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
