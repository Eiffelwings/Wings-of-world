...
const providerRuntimeMocks = vi.hoisted(() => {
  const resolveProviderUsageAuthWithPlugin = vi.fn(async (..._args: unknown[]) => null);
  const createMock = () => ({
...
const resolveProviderUsageAuthWithPluginMock =
  providerRuntimeMocks.resolveProviderUsageAuthWithPlugin;

vi.mock("../plugins/provider-runtime.js", () => providerRuntimeMocks.createMock());

vi.mock("../plugins/provider-runtime.ts", () => providerRuntimeMocks.createMock());
...
describe("resolveProviderAuths key normalization", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-provider-auth-suite-"));
  });
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".mechanical-wings");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".mechanical-wings", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      OPENCLAW_STATE_DIR: path.join(home, ".mechanical-wings"),
      ...env,
    };
