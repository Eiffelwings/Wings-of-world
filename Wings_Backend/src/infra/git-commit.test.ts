...
async function makeTempDir(label: string): Promise<string> {
  return fs.mkdtemp(path.join(os.tmpdir(), `mechanical-wings-${label}-`));
}
...
describe("git commit resolution", () => {
  ...
  it("resolves commit metadata from the caller module root instead of the caller cwd", async () => {
    ...
    const packageRepo = path.join(temp, "package");
    const otherRepo = path.join(temp, "other");
    await makeFakeGitRepo(packageRepo, {
      head: "ref: refs/heads/main\n",
      refs: { "refs/heads/main": "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" },
    });
    await makeFakeGitRepo(otherRepo, {
      head: "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\n",
    });
    await fs.writeFile(
      path.join(packageRepo, "package.json"),
      JSON.stringify({ name: "wings-of-world-backend", version: "2026.3.26" }),
      "utf-8",
    );
    await fs.mkdir(path.join(packageRepo, "src"), { recursive: true });

    const entryModuleUrl = pathToFileURL(path.join(packageRepo, "src", "entry.ts")).href;
    vi.spyOn(process, "cwd").mockReturnValue(otherRepo);

    expect(resolveCommitHash({ moduleUrl: entryModuleUrl })).toBe("aaaaaaa");
    expect(resolveCommitHash({ moduleUrl: entryModuleUrl })).not.toBe("bbbbbbb");
  });

  it("prefers live git metadata over stale build info in a real checkout", async () => {
    const temp = await makeTempDir("git-commit-live-over-build-info");
    await makeFakeGitRepo(temp, {
      head: "ref: refs/heads/main\n",
    ...
    const entryModuleUrl = pathToFileURL(path.join(temp, "src", "entry.ts")).href;
  ...
  it("treats invalid moduleUrl inputs as a fallback hint instead of throwing", async () => {
    ...
    expect(() =>
      resolveCommitHash({ moduleUrl: "not-a-file-url", cwd: temp, env: {} }),
    ).not.toThrow();
    expect(resolveCommitHash({ moduleUrl: "not-a-file-url", cwd: temp, env: {} })).toBe(
      "ddddddd",
    );
  });

  it("does not walk out of the mechanical-wings package into a host repo", async () => {
    ...
    const packageRoot = path.join(hostRepo, "node_modules", "mechanical-wings");
    await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
    await fs.writeFile(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "mechanical-wings", version: "2026.3.10" }),
      "utf-8",
    );
