...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    if (!loaded) {
      ({ resolveWingsPackageRoot } = await import("./mechanical-wings-root.js"));
    ...
    vi.mocked(resolveWingsPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveWingsPackageRoot).mockResolvedValue("/opt/mechanical-wings");
    vi.mocked(checkUpdateStatus).mockResolvedValue({
      root: "/opt/mechanical-wings",
      installKind: "package",
      packageManager: "npm",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/mechanical-wings/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
    ...
    expect(runCommandWithTimeout).toHaveBeenCalledWith(
      [
        process.execPath,
        "/opt/mechanical-wings/dist/entry.js",
        "update",
        "--yes",
        "--channel",
        "beta",
