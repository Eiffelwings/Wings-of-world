...
function createWingsBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "mechanical-wings-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation(() => {
    call += 1;
    if (call === 1) {
      return createWingsBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses mechanical-wings-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "mechanical-wings-gateway" },
          { pid: process.pid, cmd: "mechanical-wings-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("excludes pids whose command does not include 'mechanical-wings'", () => {
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple mechanical-wings pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "mechanical-wings-gateway" },
          { pid: pid2, cmd: "mechanical-wings-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-mechanical-wings pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks mechanical-wings processes — non-mechanical-wings listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty mechanical-wings stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty mechanical-wings stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + mechanical-wings pid in stdout — container-restricted lsof reports partial results
          return createWingsBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "mechanical-wings-gateway" }]),
          stderr: "",
        };
      });
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'mechanical-wings'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("mechanical-wings")
      // inside the mid-loop flush: a non-mechanical-wings cmd between two entries must not
      // be pushed, but the following mechanical-wings entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-mechanical-wings entry first, then mechanical-wings entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid mechanical-wings entry must still be found.
      const stdout = `p0\ncmechanical-wings-gateway\np${stalePid}\ncmechanical-wings-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero mechanical-wings pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-mechanical-wings stdout (line 145)", () => {
    it("treats status 1 + non-mechanical-wings stdout as port-free (not an mechanical-wings process)", () => {
      // status 1 + non-empty stdout where no mechanical-wings pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill mechanical-wings pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-mechanical-wings output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // Should complete cleanly — no mechanical-wings pids in status-1 output → free
      expect(() => cleanStaleGatewayProcessesSync()).not.toThrow();
