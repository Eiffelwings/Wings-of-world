...
let ensureWingsCliOnPath: typeof import("./path-env.js").ensureWingsCliOnPath;

describe("ensureWingsCliOnPath", () => {
  const envKeys = [
    "PATH",
  ...
  beforeAll(async () => {
    ({ ensureWingsCliOnPath } = await import("./path-env.js"));
  });
  ...
  function setupAppCliRoot(name: string) {
    const tmp = abs(`/tmp/mechanical-wings-path/${name}`);
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "mechanical-wings");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  function bootstrapPath(params: {
    ...
    ensureWingsCliOnPath(params);
    return (process.env.PATH ?? "").split(path.delimiter);
  }

  it("prepends the bundled app bin dir when a sibling mechanical-wings exists", () => {
  ...
  it.each([
    ...
    ({ envValue, allowProjectLocalBin }) => {
      ...
      const localCli = path.join(localBinDir, "mechanical-wings");
  ...
  it("appends Linuxbrew dirs after system dirs", () => {
    const tmp = abs("/tmp/mechanical-wings-path/case-linuxbrew");
