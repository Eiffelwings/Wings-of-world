...
vi.mock("./install-source-utils.js", async (importOriginal) => {
  ...
  return {
    ...
    withTempDir: vi.fn(async (_prefix: string, fn: (tmpDir: string) => Promise<unknown>) => {
      return await fn("/tmp/mechanical-wings-npm-pack-install-test");
    }),
...
describe("installFromNpmSpecArchive", () => {
  const baseSpec = "@mechanical-wings/test@1.0.0";
  const baseArchivePath = "/tmp/mechanical-wings-test.tgz";
  ...
  const runInstall = async (overrides: {
    ...
    await installFromNpmSpecArchive({
      tempDirPrefix: "mechanical-wings-test-",
      spec: baseSpec,
      timeoutMs: 1000,
  ...
  it("returns pack errors without invoking installer", async () => {
    ...
    const result = await installFromNpmSpecArchive({
      tempDirPrefix: "mechanical-wings-test-",
      spec: "@mechanical-wings/test@1.0.0",
      timeoutMs: 1000,
      installFromArchive,
    });
    ...
    expect(withTempDir).toHaveBeenCalledWith("mechanical-wings-test-", expect.any(Function));
  });
  ...
  it("rejects unsupported npm specs before packing", async () => {
    ...
    const result = await installFromNpmSpecArchive({
      tempDirPrefix: "mechanical-wings-test-",
      spec: "file:/tmp/mechanical-wings.tgz",
      timeoutMs: 1000,
      installFromArchive,
    });
  ...
  it("returns resolution metadata and installer result on success", async () => {
    mockPackedSuccess({ name: "@mechanical-wings/test", version: "1.0.0" });
    ...
    expect(okResult.npmResolution.resolvedSpec).toBe("@mechanical-wings/test@1.0.0");
    expect(okResult.npmResolution.resolvedAt).toBeTruthy();
    expect(installFromArchive).toHaveBeenCalledWith({ archivePath: "/tmp/mechanical-wings-test.tgz" });
  });
  ...
  it("aborts when integrity drift callback rejects drift", async () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: "aborted: npm package integrity drift detected for @mechanical-wings/test@1.0.0",
    });
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("warns and proceeds on drift when no callback is configured", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "Integrity drift detected for @mechanical-wings/test@1.0.0: expected sha512-old, got sha512-new",
    );
  });
  ...
  it("rejects prerelease resolutions unless explicitly requested", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ...
      archivePath: baseArchivePath,
      metadata: {
        resolvedSpec: "@mechanical-wings/test@latest",
    ...
    const result = await installFromNpmSpecArchive({
      tempDirPrefix: "mechanical-wings-test-",
      spec: "@mechanical-wings/test@latest",
      timeoutMs: 1000,
      installFromArchive,
    });
  ...
  it("allows prerelease resolutions when explicitly requested by tag", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ...
      archivePath: baseArchivePath,
      metadata: {
        resolvedSpec: "@mechanical-wings/test@beta",
        integrity: "sha512-same",
        version: "1.1.0-beta.1",
      },
    });
    ...
    const result = await installFromNpmSpecArchive({
      tempDirPrefix: "mechanical-wings-test-",
      spec: "@mechanical-wings/test@beta",
      timeoutMs: 1000,
      installFromArchive,
    });
...
describe("installFromNpmSpecArchiveWithInstaller", () => {
  ...
  it("passes archive path and installer params to installFromArchive", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/mechanical-wings-plugin.tgz",
      metadata: {
        resolvedSpec: "@mechanical-wings/voice-call@1.0.0",
        integrity: "sha512-same",
      },
    });
    ...
    const result = await installFromNpmSpecArchiveWithInstaller({
      tempDirPrefix: "mechanical-wings-test-",
      spec: "@mechanical-wings/voice-call@1.0.0",
      timeoutMs: 1000,
      installFromArchive,
...
describe("finalizeNpmSpecArchiveInstall", () => {
  ...
  it("returns install errors unchanged", () => {
    const result = finalizeNpmSpecArchiveInstall<{ ok: true } | { ok: false; error: string }>({
      ...
      npmResolution: {
        resolvedSpec: "@mechanical-wings/test@1.0.0",
        integrity: "sha512-same",
