...
function candidateBinDirs(opts: EnsureWingsPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `mechanical-wings` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "mechanical-wings");
  ...
  if (allowProjectLocalBin) {
    ...
    if (isExecutable(path.join(localBinDir, "mechanical-wings"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `mechanical-wings`
...
/**
 * Best-effort PATH bootstrap so skills that require the `mechanical-wings` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureWingsCliOnPath(opts: EnsureWingsPathOpts = {}) {
