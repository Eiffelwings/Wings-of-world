...
vi.mock("./mechanical-wings-root.js", () => ({
  resolveWingsPackageRoot: vi.fn(async () => null),
  resolveWingsPackageRootSync: vi.fn(() => null),
}));
...
let mechanicalWingsRoot: typeof import("./mechanical-wings-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    mechanicalWingsRoot = await import("./mechanical-wings-root.js");
  });
  ...
  it("resolves dist control-ui index path for symlinked argv1 via realpath", async () => {
    const pkgRoot = abs("fixtures/bun-global/mechanical-wings");
    const wrapperArgv1 = abs("fixtures/bin/mechanical-wings");
  ...
  it("uses resolveWingsPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/mechanical-wings");
    (
      mechanicalWingsRoot.resolveWingsPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/mechanical-wings"))).resolves.toBe(
      path.join(pkgRoot, "dist", "control-ui", "index.html"),
    );
  });
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "mechanical-wings" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "mechanical-wings.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-mechanical-wings");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/mechanical-wings-bundle");
    (
      mechanicalWingsRoot.resolveWingsPackageRootSync as unknown as ReturnType<typeof vi.fn>
  ...
  it("prefers packaged app Control UI assets in Contents/Resources", () => {
    const execPath = abs("fixtures/Wings.app/Contents/MacOS/Wings");
    const bundledUiDir = abs("fixtures/Wings.app/Contents/Resources/control-ui");
  ...
  it("detects package-proven control-ui roots", () => {
    const pkgRoot = abs("fixtures/mechanical-wings-package-root");
    const uiDir = path.join(pkgRoot, "dist", "control-ui");
    setDir(uiDir);
    setFile(path.join(uiDir, "index.html"), "<html></html>\n");
    (
