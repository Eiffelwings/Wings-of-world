...
import { POSIX_OPENCLAW_TMP_DIR, resolvePreferredWingsTmpDir } from "./tmp-mechanical-wings-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredWingsTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `mechanical-wings-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredWingsTmpDir({
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredWingsTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredWingsTmpDir", () => {
  it("prefers /tmp/mechanical-wings when it already exists and is writable", () => {
  ...
  it("prefers /tmp/mechanical-wings when it does not exist but /tmp is writable", () => {
  ...
  it("falls back to os.tmpdir()/mechanical-wings when /tmp/mechanical-wings is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/mechanical-wings when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/mechanical-wings exists but is not writable", () => {
    const accessSync = vi.fn((target: string) => {
      if (target === POSIX_OPENCLAW_TMP_DIR) {
  ...
  it("falls back when /tmp/mechanical-wings is a symlink", () => {
    expectFallsBackToOsTmpDir({ lstatSync: symlinkTmpDirLstat() });
  });

  it("falls back when /tmp/mechanical-wings is not owned by the current user", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ uid: 0 })) });
  });

  it("falls back when /tmp/mechanical-wings is group/other writable", () => {
    expectFallsBackToOsTmpDir({ lstatSync: vi.fn(() => makeDirStat({ mode: 0o40777 })) });
  });

  it("repairs existing /tmp/mechanical-wings permissions when they are too broad", () => {
  ...
  it("repairs /tmp/mechanical-wings after create when the initial mode stays too broad", () => {
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Wings temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "mechanical-wings");
  ...
  it("throws when the fallback directory cannot be created", () => {
    ...
    ).toThrow(/Unable to create fallback Wings temp dir/);
  });
});
