...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevWingsHome = process.env.OPENCLAW_HOME;
    const prevHome = process.env.HOME;
    process.env.OPENCLAW_HOME = "/srv/mechanical-wings-home";
    process.env.HOME = "/home/other";
    const openClawHome = path.join(path.resolve("/srv/mechanical-wings-home"), "bin", "tool");
    ...
    try {
      ...
      if (prevWingsHome === undefined) {
        delete process.env.OPENCLAW_HOME;
      } else {
        process.env.OPENCLAW_HOME = prevWingsHome;
      }
