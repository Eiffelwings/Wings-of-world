...
const DEFAULT_PACKAGE_NAME = "mechanical-wings";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["mechanical-wings", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && !pkgRoot) {
    return {
      ...
      mode: "unknown",
      root: gitRoot,
      reason: "not-mechanical-wings-root",
      steps: [],
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "mechanical-wings.mjs");
    ...
    if (!doctorEntryExists) {
      steps.push({
        name: "wings-of-world-backend doctor entry",
    ...
    const doctorStep = await runStep(
      step("wings-of-world-backend doctor", doctorArgv, gitRoot, { OPENCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
