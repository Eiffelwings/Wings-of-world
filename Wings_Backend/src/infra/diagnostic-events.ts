...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __mechanicalWingsDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__mechanicalWingsDiagnosticEventsState) {
    globalStore.__mechanicalWingsDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__mechanicalWingsDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: WingsConfig): boolean {
