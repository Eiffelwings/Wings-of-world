...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDir("mechanical-wings-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", async () => {
    await withTempDir("mechanical-wings-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", async () => {
    await withTempDir("mechanical-wings-lid-a-", async (first) => {
      await withTempDir("mechanical-wings-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.mechanical-wings when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-config-dir-"));
    try {
  ...
  it("expands OPENCLAW_STATE_DIR using the provided env", () => {
    const env = {
      HOME: "/tmp/mechanical-wings-home",
      OPENCLAW_STATE_DIR: "~/state",
    } as NodeJS.ProcessEnv;

    expect(resolveConfigDir(env)).toBe(path.resolve("/tmp/mechanical-wings-home", "state"));
  });
});
...
describe("resolveHomeDir", () => {
  it("prefers OPENCLAW_HOME over HOME", () => {
    ...
    expect(resolveHomeDir()).toBe(path.resolve("/srv/mechanical-wings-home"));
...
describe("shortenHomePath", () => {
  it("uses $OPENCLAW_HOME prefix when OPENCLAW_HOME is set", () => {
    ...
    expect(shortenHomePath(`${path.resolve("/srv/mechanical-wings-home")}/.mechanical-wings/mechanical-wings.json`)).toBe(
      "$OPENCLAW_HOME/.mechanical-wings/mechanical-wings.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $OPENCLAW_HOME replacement when OPENCLAW_HOME is set", () => {
    ...
    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/mechanical-wings-home")}/.mechanical-wings/mechanical-wings.json`),
    ).toBe("config: $OPENCLAW_HOME/.mechanical-wings/mechanical-wings.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/mechanical-wings", {}, () => "/Users/thoffman")).toBe(
      path.resolve("/Users/thoffman", "mechanical-wings"),
    );
  });
  ...
  it("prefers OPENCLAW_HOME for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/mechanical-wings")).toBe(path.resolve("/srv/mechanical-wings-home", "mechanical-wings"));
  ...
  it("uses the provided env for tilde expansion", () => {
    ...
    expect(resolveUserPath("~/mechanical-wings", env)).toBe(path.resolve("/srv/mechanical-wings-home", "mechanical-wings"));
  });
