...
const loadWingsPluginsMock = vi.fn();

vi.mock("../plugins/loader.js", () => ({
  loadWingsPlugins: (...args: Parameters<typeof loadWingsPluginsMock>) =>
    loadWingsPluginsMock(...args),
}));
...
describe("speech provider registry", () => {
  beforeEach(async () => {
    ...
    loadWingsPluginsMock.mockReturnValue(createEmptyPluginRegistry());
    ({
      getSpeechProvider,
      listSpeechProviders,
  ...
  it("loads speech providers from plugins when config is provided", () => {
    loadWingsPluginsMock.mockReturnValue({
      ...createEmptyPluginRegistry(),
      speechProviders: [
        {
    ...
    expect(loadWingsPluginsMock).toHaveBeenCalledWith({ config: cfg });
  });
