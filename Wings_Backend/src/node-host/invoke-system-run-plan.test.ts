...
function withFakeRuntimeBin<T>(params: { binName: string; run: () => T }): T {
  return withFakeRuntimeBins({
    ...
    tmpPrefix: `mechanical-wings-${params.binName}-bin-`,
    run: params.run,
  });
}
...
function withFakeRuntimeBins<T>(params: {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), params.tmpPrefix ?? "mechanical-wings-runtime-bins-"));
...
const unsafeRuntimeInvocationCases: UnsafeRuntimeInvocationCase[] = [
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "mechanical-wings-ruby-require-",
  ...
  },
  {
    ...
    binName: "ruby",
    tmpPrefix: "mechanical-wings-ruby-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "mechanical-wings-perl-module-preload-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "mechanical-wings-perl-load-path-",
  ...
  },
  {
    ...
    binName: "perl",
    tmpPrefix: "mechanical-wings-perl-preload-load-path-",
  ...
  },
  {
    ...
    binName: "node",
    tmpPrefix: "mechanical-wings-inline-shell-node-",
...
describe("hardenApprovedExecutionPaths", () => {
  ...
  for (const testCase of cases) {
    it.runIf(process.platform !== "win32")(testCase.name, () => {
      const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-approval-hardening-"));
      const oldPath = process.env.PATH;
      ...
      withFakeRuntimeBins({
        ...
        run: () => {
          withScriptOperandPlanFixture(
            {
              tmpPrefix: "mechanical-wings-approval-script-plan-",
              fixture: runtimeCase,
  ...
  it("captures the real shell script operand after value-taking shell flags", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-shell-option-value-"));
    try {
