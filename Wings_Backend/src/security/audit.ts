...
import type { ConfigFileSnapshot, WingsConfig } from "../config/config.js";
import { resolveConfigPath, resolveStateDir } from "../config/paths.js";
import { hasConfiguredSecretInput } from "../config/types.secrets.js";
import { resolveGatewayAuth } from "../gateway/auth.js";
import { readGatewayPasswordEnv, readGatewayTokenEnv } from "../gateway/credential-planner.js";
...
function isFeishuDocToolEnabled(cfg: WingsConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your Wings state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your Wings state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(readGatewayTokenEnv(env));
  const envPasswordConfigured = hasNonEmptyString(readGatewayPasswordEnv(env));
...
function collectBrowserControlFindings(
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("mechanical-wings security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(readGatewayTokenEnv(env)) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(readGatewayPasswordEnv(env)) ||
...
function collectLoggingFindings(cfg: WingsConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: WingsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: WingsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: WingsConfig): string[] {
  const channels = asRecord(cfg.channels);
  if (!channels) {
    return [];
  }
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deep?.gateway?.attempted && !deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("mechanical-wings status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("mechanical-wings security audit --deep")}".`,
    });
  }
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN/WINGS_OF_WORLD_BACKEND_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("mechanical-wings security audit --deep")}".`,
    });
  }
