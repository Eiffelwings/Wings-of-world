...
import { clearPluginDiscoveryCache, discoverWingsPlugins } from "./discovery.js";
import {
...
function makeTempDir() {
  return makeTrackedTempDir("mechanical-wings-plugins", tempDirs);
}
...
function buildDiscoveryEnv(stateDir: string): NodeJS.ProcessEnv {
  const bundledDir = path.join(stateDir, "empty-bundled-plugins");
  fs.mkdirSync(bundledDir, { recursive: true });
  return {
    OPENCLAW_STATE_DIR: stateDir,
    OPENCLAW_HOME: undefined,
    WINGS_OF_WORLD_BACKEND_BUNDLED_PLUGINS_DIR: bundledDir,
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverWingsPlugins>[0],
) {
  return discoverWingsPlugins({ ...params, env: buildDiscoveryEnv(stateDir) });
}
...
function writePluginPackageManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      "mechanical-wings": { extensions: params.extensions },
    }),
    "utf-8",
  );
}
...
describe("discoverWingsPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".mechanical-wings", "extensions");
  ...
  it("resolves tilde workspace dirs against the provided env", () => {
    ...
    const workspaceExt = path.join(workspaceRoot, ".mechanical-wings", "extensions");
    mkdirSafe(workspaceExt);
    fs.writeFileSync(path.join(workspaceExt, "tilde-workspace.ts"), "export default {}", "utf-8");

    const result = discoverWingsPlugins({
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@mechanical-wings/voice-call",
  ...
  it("strips provider suffixes from package-derived ids", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@mechanical-wings/ollama-provider",
      extensions: ["./src/index.ts"],
    });
    fs.writeFileSync(
      path.join(globalExt, "src", "index.ts"),
  ...
  it("normalizes bundled speech package ids to canonical plugin ids", async () => {
    ...
    writePluginPackageManifest({
      ...
      packageName: "@mechanical-wings/elevenlabs-speech",
      extensions: ["./src/index.ts"],
    });
    writePluginPackageManifest({
      packageDir: microsoftDir,
      packageName: "@mechanical-wings/microsoft-speech",
      extensions: ["./src/index.ts"],
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    writePluginPackageManifest({
      packageDir: packDir,
      packageName: "@mechanical-wings/demo-plugin-dir",
      extensions: ["./index.js"],
    });
  ...
  it("falls back to legacy index discovery when a scanned bundle sidecar is malformed", async () => {
    ...
    expect(legacy?.format).toBe("mechanical-wings");
  ...
  it("blocks extension entries that escape package directory", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@mechanical-wings/escape-pack",
  ...
  it("skips missing package extension entries without escape diagnostics", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@mechanical-wings/missing-entry-pack",
  ...
  it("rejects package extension entries that escape via symlink", async () => {
    ...
    writePluginPackageManifest({
      packageDir: globalExt,
      packageName: "@mechanical-wings/pack",
  ...
  it("ignores package manifests that are hardlinked aliases", async () => {
    ...
    fs.writeFileSync(
      outsideManifest,
      JSON.stringify({
        name: "@mechanical-wings/pack",
        "mechanical-wings": { extensions: ["./entry.ts"] },
      }),
      "utf-8",
    );
    try {
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = discoverWingsPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
    ...
    const second = discoverWingsPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPENCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
    ...
    const third = discoverWingsPlugins({
      env: {
        ...buildDiscoveryEnv(stateDir),
        OPENCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
