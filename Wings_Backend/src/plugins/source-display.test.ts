...
function createPluginSourceRoots() {
  ...
  const globalRoot = path.resolve(path.sep, "Users", "x", ".mechanical-wings", "extensions");
  const workspaceRoot = path.resolve(path.sep, "Users", "x", "ws", ".mechanical-wings", "extensions");
  return {
    stock: stockRoot,
    global: globalRoot,
...
describe("formatPluginSourceForTable", () => {
  ...
  it("resolves source roots from an explicit env override", () => {
    const homeDir = path.resolve(path.sep, "tmp", "mechanical-wings-home");
    ...
    expect(roots).toEqual({
      ...
      workspace: path.join(homeDir, "ws", ".mechanical-wings", "extensions"),
    });
  });
});
