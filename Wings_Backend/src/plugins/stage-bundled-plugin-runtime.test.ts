...
import { discoverWingsPlugins } from "./discovery.js";
...
function canCreateFileSymlink(): boolean {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-file-symlink-probe-"));
  try {
...
function expectLinkedOrCopiedFile(filePath: string): void {
...
describe("stageBundledPluginRuntime", () => {
  it("stages bundled dist plugins as runtime wrappers and links staged dist node_modules", () => {
    const repoRoot = makeRepoRoot("mechanical-wings-stage-bundled-runtime-");
  ...
  it("writes wrappers that forward plugin entry imports into canonical dist files", async () => {
    const repoRoot = makeRepoRoot("mechanical-wings-stage-bundled-runtime-chunks-");
  ...
  it("stages root runtime sidecars that bundled plugin boundaries resolve directly", () => {
    const repoRoot = makeRepoRoot("mechanical-wings-stage-bundled-runtime-sidecars-");
  ...
  it("keeps plugin command registration on the canonical dist graph when loaded from dist-runtime", async () => {
    const repoRoot = makeRepoRoot("mechanical-wings-stage-bundled-runtime-commands-");
    ...
    fs.writeFileSync(
      ...
      [
        "const registry = globalThis.__mechanicalWingsTestPluginCommands ??= new Map();",
  ...
  it("copies package metadata files and links or copies other non-js plugin artifacts into the runtime overlay", () => {
    const repoRoot = makeRepoRoot("mechanical-wings-stage-bundled-runtime-assets-");
    const distPluginDir = path.join(repoRoot, "dist", "extensions", "diffs");
    fs.mkdirSync(path.join(distPluginDir, "assets"), { recursive: true });
    fs.writeFileSync(
      path.join(distPluginDir, "package.json"),
      JSON.stringify(
        { name: "@mechanical-wings/diffs", "mechanical-wings": { extensions: ["./index.js"] } },
        null,
        2,
      ),
      "utf8",
    );
    fs.writeFileSync(path.join(distPluginDir, "mechanical-wings.plugin.json"), "{}\n", "utf8");
    ...
    expectLinkedOrCopiedFile(runtimeAssetPath);
  ...
  it("preserves package metadata needed for bundled plugin discovery from dist-runtime", () => {
    const repoRoot = makeRepoRoot("mechanical-wings-stage-bundled-runtime-discovery-");
    const distPluginDir = path.join(repoRoot, "dist", "extensions", "demo");
    ...
    fs.writeFileSync(
      path.join(distPluginDir, "mechanical-wings.plugin.json"),
      JSON.stringify(
        {
          id: "demo",
          channels: ["demo"],
    ...
    const discovery = discoverWingsPlugins({
      env,
      cache: false,
    });
  ...
  it("removes stale runtime plugin directories that are no longer in dist", () => {
    const repoRoot = makeRepoRoot("mechanical-wings-stage-bundled-runtime-stale-");
  ...
  it("removes dist-runtime when the built bundled plugin tree is absent", () => {
    const repoRoot = makeRepoRoot("mechanical-wings-stage-bundled-runtime-missing-");
  ...
  it.runIf(canCreateFileSymlink())(
    "tolerates EEXIST when an identical runtime symlink is materialized concurrently",
    () => {
      const repoRoot = makeRepoRoot("mechanical-wings-stage-bundled-runtime-eexist-");
