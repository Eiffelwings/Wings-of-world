...
function createBundledWhatsAppRuntimeFixture() {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-whatsapp-boundary-"));
  ...
  fs.writeFileSync(path.join(rootDir, "mechanical-wings.mjs"), "export {};\n", "utf8");
  ...
  fs.writeFileSync(
    ...
    [
      'const key = Symbol.for("mechanical-wings.whatsapp.activeListenerState");',
      "const g = globalThis;",
      "if (!g[key]) {",
