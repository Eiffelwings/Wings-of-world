...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const override =
    env.WINGS_OF_WORLD_BACKEND_BUNDLED_PLUGINS_DIR?.trim() ||
    env.WINGS_OF_WORLD_BUNDLED_PLUGINS_DIR?.trim() ||
    env.OPENCLAW_BUNDLED_PLUGINS_DIR?.trim();
  if (override) {
    ...
    try {
      const argvPackageRoot = resolveWingsPackageRootSync({ argv1: process.argv[1] });
  ...
  try {
    const packageRoots = [
      resolveWingsPackageRootSync({ argv1: process.argv[1] }),
