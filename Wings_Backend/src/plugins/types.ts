 ...
 */
export type WingsPluginConfigSchema = {
...
export type WingsPluginToolContext = {
  config?: WingsConfig;
  workspaceDir?: string;
  agentDir?: string;
  agentId?: string;
...
export type WingsPluginToolFactory = (
  ctx: WingsPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type WingsPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type WingsPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<WingsConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Wings uses this to expose
 ...
 *
 * Runs after Wings resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Wings auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Wings wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Wings builds its generic
...
/**
 * Provider-owned stream wrapper hook after Wings applies its generic
 ...
 *
 * Return `true` or `false` to override Wings's built-in provider cache TTL
 ...
 *
 * Runs only after Wings exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Wings should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Wings loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Wings falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Wings calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic Wings wrappers.
   *
   ...
   *
   * Called after Wings resolves the raw configured credential but before the
   ...
   * hint than Wings's generic auth-store guidance.
   */
   ...
   * Wings's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Wings should surface a provider-specific hint for
   ...
   * merging. Wings deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * Wings uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `wings-of-world-backend doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `wings-of-world-backend doctor` should offer.
   *
   ...
   *
   * Wings calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep Wings's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
   ...
   * when Wings has no config-aware runtime context yet.
   *
  ...
  getConfiguredCredentialValue?: (config?: WingsConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: WingsConfig, value: unknown) => void;
  applySelectionConfig?: (config: WingsConfig) => WingsConfig;
...
export type WingsPluginGatewayMethod = {
  method: string;
 ...
 */
export type WingsPluginCommandDefinition = {
...
export type WingsPluginHttpRouteAuth = "gateway" | "plugin";
export type WingsPluginHttpRouteMatch = "exact" | "prefix";

export type WingsPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type WingsPluginHttpRouteParams = {
  path: string;
  handler: WingsPluginHttpRouteHandler;
  auth: WingsPluginHttpRouteAuth;
  match?: WingsPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type WingsPluginCliContext = {
  program: Command;
  config: WingsConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type WingsPluginCliRegistrar = (ctx: WingsPluginCliContext) => void | Promise<void>;

export type WingsPluginCliCommandDescriptor = {
  name: string;
  description: string;
...
export type WingsPluginServiceContext = {
  config: WingsConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type WingsPluginService = {
  id: string;
  start: (ctx: WingsPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: WingsPluginServiceContext) => void | Promise<void>;
};
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Wings should inject bundle MCP config for this backend.
   *
...
export type WingsPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type WingsPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  ...
  register?: (api: WingsPluginApi) => void | Promise<void>;
  activate?: (api: WingsPluginApi) => void | Promise<void>;
};

export type WingsPluginModule =
  | WingsPluginDefinition
  | ((api: WingsPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

/** Main registration API injected into native plugin entry files. */
export type WingsPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | WingsPluginToolFactory,
    opts?: WingsPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: WingsPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: WingsPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: WingsPluginChannelRegistration | ChannelPlugin) => void;
  ...
  registerCli: (
    registrar: WingsPluginCliRegistrar,
    opts?: {
      commands?: string[];
      descriptors?: WingsPluginCliCommandDescriptor[];
    },
  ) => void;
  registerService: (service: WingsPluginService) => void;
   ...
   */
  registerCommand: (command: WingsPluginCommandDefinition) => void;
...
export type PluginFormat = "mechanical-wings" | "bundle";
...
export type PluginHookBeforeToolCallResult = {
  ...
  requireApproval?: {
     ...
     * Wings does not await this callback before allowing or denying the tool call.
     */
