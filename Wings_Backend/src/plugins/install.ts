...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing mechanical-wings.extensions; update the plugin package to include mechanical-wings.extensions (for example ["./dist/index.js"]). See https://docs.wings-of-world.ai/help/troubleshooting#plugin-install-fails-with-missing-mechanical-wings-extensions';
const PLUGIN_ARCHIVE_ROOT_MARKERS = [
  "package.json",
  "mechanical-wings.plugin.json",
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_OPENCLAW_EXTENSIONS: "missing_mechanical-wings_extensions",
  EMPTY_OPENCLAW_EXTENSIONS: "empty_mechanical-wings_extensions",
...
function ensureWingsExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json mechanical-wings.extensions is empty",
...
async function installBundleFromSourceDir(
  ...
  try {
    ...
    logger.warn?.(
      `Bundle "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "mechanical-wings security audit --deep" after install.`,
    );
  }
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureWingsExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureWingsExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from mechanical-wings.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-mechanical-wings"), the plugin registry
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json mechanical-wings.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires Wings >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set OPENCLAW_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires Wings >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade Wings and retry.`,
  ...
  try {
    ...
    logger.warn?.(
      `Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "mechanical-wings security audit --deep" after install.`,
    );
  }
...
export async function installPluginFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "mechanical-wings-plugin-",
    timeoutMs,
    logger,
...
export async function installPluginFromNpmSpec(params: {
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "mechanical-wings-npm-pack-",
    spec,
    timeoutMs,
