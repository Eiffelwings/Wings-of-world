...
function collectConfiguredActivationIds(config: WingsConfig): Set<string> {
...
export function resolveGatewayStartupPluginIds(params: {
  ...
  return manifestRegistry.plugins
    .filter((plugin) => {
      ...
      const activatedBundledProvider =
      ...
      if (
        !enableState.enabled &&
        !(activatedBundledProvider && enableState.reason === "bundled (disabled by default)")
      ) {
        return false;
      }
