...
let loadWingsPluginsMock: ReturnType<typeof vi.fn>;
...
describe("resolvePluginWebSearchProviders", () => {
  ...
  beforeEach(() => {
    ...
    loadPluginManifestRegistryMock = vi
      ...
      .mockReturnValue({
        plugins: [
          {
            ...
            manifestPath: "/tmp/brave/mechanical-wings.plugin.json",
            channels: [],
            providers: [],
            skills: [],
            hooks: [],
          ...
          },
          {
            ...
            manifestPath: "/tmp/noise/mechanical-wings.plugin.json",
            channels: [],
            providers: [],
            skills: [],
            hooks: [],
        ...
        ? R
        : never);
    loadWingsPluginsMock = vi
      .spyOn(loaderModule, "loadWingsPlugins")
  ...
  it("loads bundled providers through the plugin loader in alphabetical order", () => {
    ...
    expect(loadWingsPluginsMock).toHaveBeenCalledTimes(1);
  });
  ...
  it("scopes plugin loading to manifest-declared web-search candidates", () => {
    ...
    expect(loadWingsPluginsMock).toHaveBeenCalledWith(
  ...
  it("memoizes snapshot provider resolution for the same config and env", () => {
    ...
    const env = { OPENCLAW_HOME: "/tmp/mechanical-wings-home" } as NodeJS.ProcessEnv;
  ...
  it("invalidates the snapshot cache when config or env contents change in place", () => {
    ...
    const env = {
      OPENCLAW_HOME: "/tmp/mechanical-wings-home-a",
    ...
    env.OPENCLAW_HOME = "/tmp/mechanical-wings-home-b";
    resolvePluginWebSearchProviders({
      config,
      env,
      bundledAllowlistCompat: true,
    ...
    expect(loadWingsPluginsMock).toHaveBeenCalledTimes(2);
  });
  ...
  it("skips web-search snapshot memoization when plugin cache opt-outs are set", () => {
    ...
    const env = {
      OPENCLAW_HOME: "/tmp/mechanical-wings-home",
  ...
  it("prefers the active plugin registry for runtime resolution", () => {
    ...
    expect(loadWingsPluginsMock).not.toHaveBeenCalled();
  });
});
