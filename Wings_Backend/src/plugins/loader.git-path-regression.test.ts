...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-plugin-loader-"));
  tempRoots.push(dir);
  return dir;
}
...
`,
    ...
    const script = `
      ...
      const withAlias = createJiti(${JSON.stringify(jitiBaseFile)}, {
        ...
        extensions: [".ts", ".tsx", ".mts", ".cts", ".mtsx", ".ctsx", ".js", ".mjs", ".cjs", ".json"],
        alias: {
          "mechanical-wings/plugin-sdk/infra-runtime": ${JSON.stringify(copiedChannelRuntimeShim)},
        },
      });
