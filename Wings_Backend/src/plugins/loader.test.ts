...
import { __testing, clearPluginLoaderCache, loadWingsPlugins } from "./loader.js";
...
type PluginLoadConfig = NonNullable<Parameters<typeof loadWingsPlugins>[0]>["config"];
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "mechanical-wings-plugin-"));
let tempDirIndex = 0;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "mechanical-wings.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
      ...
      JSON.stringify(
        {
          ...
          "mechanical-wings": { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function expectTelegramLoaded(registry: ReturnType<typeof loadWingsPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadWingsPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadWingsPlugins>[0], "cache" | "config">,
) {
  return loadWingsPlugins({
    cache: false,
    ...options,
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "mechanical-wings.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
        configSchema: EMPTY_PLUGIN_SCHEMA,
...
function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".mechanical-wings", "extensions", params.pluginId);
...
function expectNoUnwiredBundleDiagnostic(
  registry: ReturnType<typeof loadWingsPlugins>,
  pluginId: string,
) {
  expect(
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadWingsPlugins>;
  loadSecond: () => ReturnType<typeof loadWingsPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadWingsPlugins>;
  loadVariant: () => ReturnType<typeof loadWingsPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const mechanicalWingsHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(mechanicalWingsHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  ...
  const env = {
    ...process.env,
    OPENCLAW_HOME: mechanicalWingsHome,
    HOME: ignoredHome,
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadWingsPlugins({
    cache: false,
    config: {
      plugins: {
...
describe("bundle plugins", () => {
  it("reports Codex bundles as loaded bundle plugins without importing runtime code", () => {
    ...
    const workspaceDir = makeTempDir();
    const stateDir = makeTempDir();
    const bundleRoot = path.join(workspaceDir, ".mechanical-wings", "extensions", "sample-bundle");
...
describe("loadWingsPlugins", () => {
    ...
    const cases = [
      {
        ...
        assert: (registry: ReturnType<typeof loadWingsPlugins>) => {
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    const registry = loadBundledMemoryPluginRegistry({
      packageMeta: {
        name: "@mechanical-wings/memory-core",
        version: "1.2.3",
...
module.exports = { id: "skipped-scoped-only", register() { throw new Error("skipped plugin should not load"); } };`,
      ...
      },
      {
        ...
        run: () => {
          ...
          const full = loadWingsPlugins(options);
          const scoped = loadWingsPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
          const scopedAgain = loadWingsPlugins({
            ...options,
            onlyPluginIds: ["allowed-cache-scope"],
          });
  ...
  it("throws when activate:false is used without cache:false", () => {
    expect(() => loadWingsPlugins({ activate: false })).toThrow(
      "activate:false requires cache:false",
    );
    expect(() => loadWingsPlugins({ activate: false, cache: true })).toThrow(
      "activate:false requires cache:false",
    );
  });
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadWingsPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadWingsPlugins(options);
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(mechanicalWingsHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadWingsPlugins(options),
          loadVariant: () =>
            loadWingsPlugins({
              ...options,
              runtimeOptions: {
  ...
  it("prefers OPENCLAW_HOME over HOME for env-expanded load paths", () => {
    ...
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "mechanical-wings-home-demo",
      dir: path.join(mechanicalWingsHome, "plugins", "mechanical-wings-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "mechanical-wings-home-demo", register() {} };`,
    });
    ...
    const registry = loadWingsPlugins({
      ...
      config: {
        plugins: {
          allow: ["mechanical-wings-home-demo"],
          entries: {
            "mechanical-wings-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/mechanical-wings-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "mechanical-wings-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadWingsPlugins>) =>
  ...
  api.registerCli(() => {}, { commands: ["shared-cli"] });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadWingsPlugins>) => {
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "mechanical-wings.plugin.json"),
      JSON.stringify(
        {
          id: "lazy-channel",
          configSchema: EMPTY_PLUGIN_SCHEMA,
    ...
    const setupRegistry = loadWingsPlugins({
      cache: false,
      config,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@mechanical-wings/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@mechanical-wings/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@mechanical-wings/setup-runtime-preferred-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@mechanical-wings/setup-runtime-not-preferred-test",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "mechanical-wings.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "mechanical-wings.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          const workspaceDir = makeTempDir();
          const workspaceExtDir = path.join(workspaceDir, ".mechanical-wings", "extensions", "shadowed");
          mkdirSafe(workspaceExtDir);
          writePlugin({
            id: "shadowed",
            body: `module.exports = { id: "shadowed", register() {} };`,
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("mechanical-wings/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__mechanicalWingsLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("mechanical-wings/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("mechanical-wings/plugin-sdk");
