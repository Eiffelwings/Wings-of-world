...
import type { WingsPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("mechanical-wings-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "mechanical-wings.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  format?: "mechanical-wings" | "bundle";
  bundleFormat?: "codex" | "claude" | "cursor";
  packageManifest?: WingsPackageManifest;
  packageDir?: string;
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "mechanical-wings.plugin.json");
  const linkedManifest = path.join(rootDir, "mechanical-wings.plugin.json");
  ...
  return loadPluginManifestRegistry({
    ...
    candidates: [
      createPluginCandidate({
        ...
        packageManifest: {
          install: {
            npmSpec: "@mechanical-wings/synology-chat",
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "mechanical-wings.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it("skips plugins whose minHostVersion is newer than the current host", () => {
    ...
    expect(
      registry.diagnostics.some((diag) =>
        diag.message.includes("plugin requires Wings >=2026.3.22, but this host is 2026.3.21"),
      ),
    ).toBe(true);
  });
  ...
  it("rejects invalid minHostVersion metadata", () => {
    ...
    expect(
      registry.diagnostics.some((diag) =>
        diag.message.includes("plugin manifest invalid | mechanical-wings.install.minHostVersion must use"),
      ),
    ).toBe(true);
  });
