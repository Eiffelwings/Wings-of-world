...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("mechanical-wings-bundle-commands-home-");
      const workspaceDir = await tempHarness.createTempDir("mechanical-wings-bundle-commands-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.OPENCLAW_HOME;
      delete process.env.OPENCLAW_STATE_DIR;

      const pluginRoot = path.join(homeDir, ".mechanical-wings", "extensions", "compound-bundle");
