...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: WingsPluginDefinition;
  register?: WingsPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as WingsPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as WingsPluginDefinition;
...
function createPluginRecord(params: {
  ...
  return {
    ...
    format: params.format ?? "mechanical-wings",
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverWingsPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverWingsPlugins>["candidates"][number];
  right: ReturnType<typeof discoverWingsPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadWingsPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  if (options.activate === false && options.cache !== false) {
    throw new Error(
      "loadWingsPlugins: activate:false requires cache:false to prevent command registry divergence",
    );
  }
  ...
  const getJiti = (modulePath: string) => {
    ...
    // Pass loader's moduleUrl so the mechanical-wings root can always be resolved even when
    // loading external plugins from outside the installation directory (e.g. ~/.mechanical-wings/extensions/).
  ...
  for (const candidate of orderedCandidates) {
    ...
    if (record.format === "bundle") {
      ...
      for (const capability of unsupportedCapabilities) {
        registry.diagnostics.push({
          ...
          pluginId: record.id,
          source: record.source,
          message: `bundle capability detected but not wired into Wings yet: ${capability}`,
        });
      }
      if (
        enableState.enabled &&
    ...
    let mod: WingsPluginModule | null = null;
    try {
      mod = getJiti(safeSource)(safeSource) as WingsPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
