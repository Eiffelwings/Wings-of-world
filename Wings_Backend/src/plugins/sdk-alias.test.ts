...
const fixtureRoot = makeTrackedTempDir("mechanical-wings-sdk-alias-root", fixtureTempDirs);
let tempDirIndex = 0;
...
function createPluginSdkAliasFixture(params?: {
  ...
  const packageJson: Record<string, unknown> = {
    name: params?.packageName ?? "mechanical-wings",
    type: "module",
  };
  if (trustedRootIndicatorMode === "bin+marker") {
    packageJson.bin = {
      "mechanical-wings": "mechanical-wings.mjs",
    };
  }
  ...
  if (trustedRootIndicatorMode === "bin+marker") {
    fs.writeFileSync(path.join(root, "mechanical-wings.mjs"), "export {};\n", "utf-8");
  }
...
function createExtensionApiAliasFixture(params?: { srcBody?: string; distBody?: string }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "mechanical-wings", type: "module" }, null, 2),
    "utf-8",
  );
  fs.writeFileSync(path.join(root, "mechanical-wings.mjs"), "export {};\n", "utf-8");
  fs.writeFileSync(srcFile, params?.srcBody ?? "export {};\n", "utf-8");
...
function createUserInstalledPluginSdkAliasFixture() {
  ...
  const externalPluginRoot = path.join(makeTempDir(), ".mechanical-wings", "extensions", "demo");
...
describe("plugin sdk alias helpers", () => {
  it.each([
    ...
    },
    {
      ...
      modulePath: () => "/tmp/tsx-cache/mechanical-wings-loader.js",
      argv1: (root: string) => path.join(root, "mechanical-wings.mjs"),
      srcFile: "index.ts",
      distFile: "index.js",
      env: { NODE_ENV: undefined },
      expected: "src" as const,
  ...
  it("does not derive plugin-sdk subpaths from cwd fallback when package root is not an Wings root", () => {
    const fixture = createPluginSdkAliasFixture({
      packageName: "moltbot",
  ...
  it("builds plugin-sdk aliases from the module being loaded, not the loader location", () => {
    ...
    expect(fs.realpathSync(sourceAliases["mechanical-wings/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(sourceAliases["mechanical-wings/plugin-sdk/channel-runtime"] ?? "")).toBe(
    ...
    expect(fs.realpathSync(distAliases["mechanical-wings/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(distRootAlias),
    );
    expect(fs.realpathSync(distAliases["mechanical-wings/plugin-sdk/channel-runtime"] ?? "")).toBe(
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via the running mechanical-wings argv hint", () => {
    ...
    const aliases = withCwd(externalPluginRoot, () =>
      withEnv({ NODE_ENV: undefined }, () =>
        buildPluginLoaderAliasMap(externalPluginEntry, path.join(fixture.root, "mechanical-wings.mjs")),
      ),
    );

    expect(fs.realpathSync(aliases["mechanical-wings/plugin-sdk"] ?? "")).toBe(
      fs.realpathSync(sourceRootAlias),
    );
    expect(fs.realpathSync(aliases["mechanical-wings/plugin-sdk/channel-runtime"] ?? "")).toBe(
      fs.realpathSync(path.join(fixture.root, "src", "plugin-sdk", "channel-runtime.ts")),
    );
  });
  ...
  it("resolves plugin-sdk aliases for user-installed plugins via moduleUrl hint", () => {
    ...
    // This covers installations where argv1 does not resolve to the mechanical-wings root
    // (e.g. single-binary distributions or custom process launchers).
    // Use mechanical-wings.mjs which is created by createPluginSdkAliasFixture (bin+marker mode).
    // Use fixture.root as cwd so process.cwd() fallback also resolves to fixture, not the
    // real mechanical-wings repo root in the test runner environment.
    const loaderModuleUrl = pathToFileURL(path.join(fixture.root, "mechanical-wings.mjs")).href;
    ...
    // runner binary, inside the mechanical-wings repo), which resolves before moduleUrl is checked.
    // An empty string is falsy so resolveTrustedWingsRootFromArgvHint returns null,
  ...
  it("does not resolve plugin-sdk alias files from cwd fallback when package root is not an Wings root", () => {
    const fixture = createPluginSdkAliasFixture({
      srcFile: "channel-runtime.ts",
      distFile: "channel-runtime.js",
      packageName: "moltbot",
  ...
  it("loads source runtime shims through the non-native Jiti boundary", async () => {
    ...
    fs.writeFileSync(
      ...
      `import { resolveOutboundSendDep } from "mechanical-wings/plugin-sdk/infra-runtime";
...
`,
    ...
    const withAlias = createJiti(jitiBaseUrl, {
      ...buildPluginLoaderJitiOptions({
        "mechanical-wings/plugin-sdk/infra-runtime": copiedChannelRuntimeShim,
      }),
      tryNative: false,
    });
