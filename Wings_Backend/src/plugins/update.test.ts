...
describe("updateNpmInstalledPlugins", () => {
  ...
  it("skips integrity drift checks for unpinned npm specs during dry-run updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "opik-mechanical-wings",
      targetDir: "/tmp/opik-mechanical-wings",
      version: "0.2.6",
      extensions: ["index.ts"],
    });
    ...
    await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "opik-mechanical-wings": {
              source: "npm",
              spec: "@opik/opik-mechanical-wings",
              integrity: "sha512-old",
              installPath: "/tmp/opik-mechanical-wings",
            },
          },
        },
      },
      pluginIds: ["opik-mechanical-wings"],
      dryRun: true,
    });
      ...
      config: {
        plugins: {
          installs: {
            "opik-mechanical-wings": {
              source: "npm",
              spec: "@opik/opik-mechanical-wings@0.2.5",
              integrity: "sha512-old",
              installPath: "/tmp/opik-mechanical-wings",
            },
          },
  ...
  it("formats package-not-found updates with a stable message", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      error: "Package not found on npm: @mechanical-wings/missing.",
    });
    ...
    expect(result.outcomes).toEqual([
      {
        pluginId: "missing",
        status: "error",
        message: "Failed to check missing: npm package not found for @mechanical-wings/missing.",
      },
    ]);
  });
  ...
  it("reuses a recorded npm dist-tag spec for id-based updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "mechanical-wings-codex-app-server",
      targetDir: "/tmp/mechanical-wings-codex-app-server",
    ...
    const result = await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "mechanical-wings-codex-app-server": {
              source: "npm",
              spec: "mechanical-wings-codex-app-server@beta",
              installPath: "/tmp/mechanical-wings-codex-app-server",
              resolvedName: "mechanical-wings-codex-app-server",
              resolvedSpec: "mechanical-wings-codex-app-server@0.2.0-beta.3",
            },
          },
        },
      },
      pluginIds: ["mechanical-wings-codex-app-server"],
    });

    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "mechanical-wings-codex-app-server@beta",
        expectedPluginId: "mechanical-wings-codex-app-server",
      }),
    );
    expect(result.config.plugins?.installs?.["mechanical-wings-codex-app-server"]).toMatchObject({
      source: "npm",
      spec: "mechanical-wings-codex-app-server@beta",
      installPath: "/tmp/mechanical-wings-codex-app-server",
      version: "0.2.0-beta.4",
  ...
  it("uses and persists an explicit npm spec override during updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ...
      extensions: ["index.ts"],
      npmResolution: {
        name: "mechanical-wings-codex-app-server",
        version: "0.2.0-beta.4",
        resolvedSpec: "mechanical-wings-codex-app-server@0.2.0-beta.4",
      },
    });
    ...
    const result = await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "mechanical-wings-codex-app-server": {
              source: "npm",
              spec: "mechanical-wings-codex-app-server",
              installPath: "/tmp/mechanical-wings-codex-app-server",
            },
          },
        },
      ...
      pluginIds: ["mechanical-wings-codex-app-server"],
      specOverrides: {
        "mechanical-wings-codex-app-server": "mechanical-wings-codex-app-server@beta",
      },
    });
  ...
  it("skips recorded integrity checks when an explicit npm version override changes the spec", async () => {
    ...
    await updateNpmInstalledPlugins({
      config: {
        plugins: {
          installs: {
            "mechanical-wings-codex-app-server": {
              source: "npm",
              spec: "mechanical-wings-codex-app-server@0.2.0-beta.3",
              integrity: "sha512-old",
              installPath: "/tmp/mechanical-wings-codex-app-server",
            },
          },
      ...
      pluginIds: ["mechanical-wings-codex-app-server"],
      specOverrides: {
        "mechanical-wings-codex-app-server": "mechanical-wings-codex-app-server@0.2.0-beta.4",
      },
    });

    expect(installPluginFromNpmSpecMock).toHaveBeenCalledWith(
      expect.objectContaining({
        spec: "mechanical-wings-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      }),
    );
  });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@mechanical-wings/voice-call",
      targetDir: "/tmp/mechanical-wings-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@mechanical-wings/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@mechanical-wings/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@mechanical-wings/voice-call");
    expect(result.config.plugins?.entries?.["@mechanical-wings/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@mechanical-wings/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@mechanical-wings/voice-call",
      installPath: "/tmp/mechanical-wings-voice-call",
      version: "0.0.2",
    });
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { OPENCLAW_HOME: "/srv/mechanical-wings-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/mechanical-wings-home";
    resolveBundledPluginSourcesMock.mockReturnValue(
      new Map([
        [
          "feishu",
