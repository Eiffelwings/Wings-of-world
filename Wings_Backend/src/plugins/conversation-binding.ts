...
const APPROVALS_PATH = "~/.mechanical-wings/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "mechanical-wings-app-server:thread:",
  "mechanical-wings-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("mechanical-wings.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("mechanical-wings.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to Wings instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
