...
import type {
  ...
  WingsPluginApi,
  WingsPluginChannelRegistration,
  WingsPluginCliCommandDescriptor,
  WingsPluginCliRegistrar,
  WingsPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  WingsPluginHttpRouteAuth,
  WingsPluginHttpRouteMatch,
  WingsPluginHttpRouteHandler,
  WingsPluginHttpRouteParams,
  WingsPluginHookOptions,
...
export type PluginToolRegistration = {
  pluginId: string;
  pluginName?: string;
  factory: WingsPluginToolFactory;
  names: string[];
  optional: boolean;
  source: string;
  rootDir?: string;
...
export type PluginCliRegistration = {
  pluginId: string;
  pluginName?: string;
  register: WingsPluginCliRegistrar;
  commands: string[];
...
export type PluginCommandRegistration = {
  pluginId: string;
  pluginName?: string;
  command: WingsPluginCommandDefinition;
  source: string;
  rootDir?: string;
};
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerTool = (
    ...
    const factory: WingsPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: WingsPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: WingsPluginHookOptions | undefined,
    config: WingsPluginApi["config"],
  ) => {
    ...
    const hookEntry: HookEntry = entry
      ? {
          ...
          hook: {
            ...
            name,
            description,
            source: "mechanical-wings-plugin",
            pluginId: record.id,
          },
          metadata: {
            ...entry.metadata,
  ...
  const registerHttpRoute = (record: PluginRecord, params: WingsPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: WingsPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const normalized =
      typeof (registration as WingsPluginChannelRegistration).plugin === "object"
        ? (registration as WingsPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: WingsPluginCliCommandDescriptor[] },
  ) => {
  ...
  const registerService = (record: PluginRecord, service: WingsPluginService) => {
  ...
  const registerCommand = (record: PluginRecord, command: WingsPluginCommandDefinition) => {
