...
const DYNAMIC_ARCHIVE_TEMPLATE_PRESETS = [
  {
    ...
    packageJson: {
      name: "@evil/..",
      version: "0.0.1",
      "mechanical-wings": { extensions: ["./dist/index.js"] },
...
function ensureSuiteTempRoot() {
  ...
  suiteTempRoot = fs.mkdtempSync(path.join(bundleTempRoot, "mechanical-wings-plugin-install-"));
  return suiteTempRoot;
}
...
function setupDualFormatInstallFixture(params: { bundleFormat: "codex" | "claude" }) {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@mechanical-wings/native-dual",
      version: "0.0.1",
      "mechanical-wings": { extensions: ["./dist/index.js"] },
...
beforeAll(async () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@mechanical-wings/test-plugin",
      version: "0.0.1",
      "mechanical-wings": { extensions: ["./dist/index.js"] },
      dependencies: { "left-pad": "1.3.0" },
    }),
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@mechanical-wings/cognee-mechanical-wings",
      version: "0.0.1",
      "mechanical-wings": { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  ...
  fs.writeFileSync(
    path.join(manifestInstallTemplateDir, "mechanical-wings.plugin.json"),
    JSON.stringify({
...
describe("installPluginFromArchive", () => {
  it("installs scoped archives, rejects duplicate installs, and allows updates", async () => {
    ...
    expectSuccessfulArchiveInstall({ result: first, stateDir, pluginId: "@mechanical-wings/voice-call" });
  ...
  it("installs from a zip archive", async () => {
    ...
    expectSuccessfulArchiveInstall({ result, stateDir, pluginId: "@mechanical-wings/zipper" });
  });

  it("installs flat-root plugin archives from ClawHub-style downloads", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: {
        name: "@mechanical-wings/rootless",
        version: "0.0.1",
        "mechanical-wings": { extensions: ["./dist/index.js"] },
      },
  ...
  it("rejects packages without mechanical-wings.extensions", async () => {
    const result = await installArchivePackageAndReturnResult({
      packageJson: { name: "@mechanical-wings/nope", version: "0.0.1" },
      outName: "bad.tgz",
    });
    expect(result.ok).toBe(false);
    if (result.ok) {
    ...
    expect(result.error).toContain("mechanical-wings.extensions");
    expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_OPENCLAW_EXTENSIONS);
  });

  it("rejects legacy plugin package shape when mechanical-wings.extensions is missing", async () => {
    const { pluginDir, extensionsDir } = setupPluginInstallDirs();
    fs.writeFileSync(
      path.join(pluginDir, "package.json"),
      JSON.stringify({
        name: "@mechanical-wings/legacy-entry-fallback",
        version: "0.0.1",
      }),
      "utf-8",
    );
    ...
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain("package.json missing mechanical-wings.extensions");
      expect(result.error).toContain("update the plugin package");
      expect(result.code).toBe(PLUGIN_INSTALL_ERROR_CODE.MISSING_OPENCLAW_EXTENSIONS);
      return;
    }
    expect.unreachable("expected install to fail without mechanicalWings.extensions");
  });
  ...
  it("warns when plugin contains dangerous code patterns", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        "mechanical-wings": { extensions: ["index.js"] },
      }),
    );
    fs.writeFileSync(
  ...
  it("scans extension entry files in hidden directories", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        version: "1.0.0",
        "mechanical-wings": { extensions: [".hidden/index.js"] },
      }),
    );
    fs.writeFileSync(
...
describe("installPluginFromDir", () => {
  ...
  it("strips workspace devDependencies before npm install", async () => {
    ...
    expect(manifest.devDependencies?.["mechanical-wings"]).toBeUndefined();
  ...
  it("rejects plugins whose minHostVersion is newer than the current host", async () => {
    ...
    const packageJsonPath = path.join(pluginDir, "package.json");
    const manifest = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as {
      "mechanical-wings"?: { install?: Record<string, unknown> };
    };
    manifest["mechanical-wings"] = {
      ...manifest["mechanical-wings"],
      install: {
        ...manifest["mechanical-wings"]?.install,
    ...
    expect(result.error).toContain("requires Wings >=2026.3.22, but this host is 2026.3.21");
  ...
  it("rejects plugins with invalid minHostVersion metadata", async () => {
    ...
    expect(result.error).toContain("invalid package.json mechanical-wings.install.minHostVersion");
    expect(vi.mocked(runCommandWithTimeout)).not.toHaveBeenCalled();
  });
  ...
  it("uses mechanicalWings.plugin.json id as install key when it differs from package name", async () => {
    ...
    expect(
      infoMessages.some((msg) =>
        msg.includes(
          'Plugin manifest id "memory-cognee" differs from npm package name "@mechanical-wings/cognee-mechanical-wings"',
        ),
      ),
    ).toBe(true);
  });
  ...
  it("keeps scoped install ids aligned across manifest and package-name cases", async () => {
    const scenarios = [
      ...
      },
      {
        ...
        expectedPluginId: "@mechanical-wings/test-plugin",
        install: (pluginDir: string, extensionsDir: string) =>
          installPluginFromDir({
            dirPath: pluginDir,
            extensionsDir,
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@mechanical-wings/voice-call@0.0.1",
              name: "@mechanical-wings/voice-call",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@mechanical-wings/voice-call@0.0.1",
      extensionsDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@mechanical-wings/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@mechanical-wings/voice-call@0.0.1",
    });
  ...
  it("classifies npm package-not-found errors with a stable error code", async () => {
    ...
    const result = await installPluginFromNpmSpec({
      spec: "@mechanical-wings/not-found",
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@mechanical-wings/voice-call@0.0.2-beta.1",
      name: "@mechanical-wings/voice-call",
...

    {
      ...
      const result = await installPluginFromNpmSpec({
        spec: "@mechanical-wings/voice-call",
        logger: { info: () => {}, warn: () => {} },
      });
      expect(result.ok).toBe(false);
      if (!result.ok) {
        expect(result.error).toContain("prerelease version 0.0.2-beta.1");
        expect(result.error).toContain('"@mechanical-wings/voice-call@beta"');
      }
    }
...

    {
      ...
      const result = await installPluginFromNpmSpec({
        spec: "@mechanical-wings/voice-call@beta",
        extensionsDir,
        logger: { info: () => {}, warn: () => {} },
      });
      expect(result.ok).toBe(true);
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@mechanical-wings/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls,
        expectedSpec: "@mechanical-wings/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
