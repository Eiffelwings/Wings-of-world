...
import { resolveWingsPackageRootSync } from "../infra/mechanical-wings-root.js";
...
function hasTrustedWingsRootIndicator(params: {
  packageRoot: string;
  ...
  const hasWingsBin =
    (typeof params.packageJson.bin === "string" &&
      params.packageJson.bin.toLowerCase().includes("mechanical-wings")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin["mechanical-wings"] === "string");
  const hasWingsEntrypoint = fs.existsSync(path.join(params.packageRoot, "mechanical-wings.mjs"));
  return hasCliEntryExport || hasWingsBin || hasWingsEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedWingsRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedWingsRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  const packageRoot = resolveWingsPackageRootSync({
    cwd: params.cwd,
    argv1: params.argv1,
  });
  if (!packageRoot) {
  ...
  return hasTrustedWingsRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveWingsPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedWingsRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
      ? resolveWingsPackageRootSync({
          cwd,
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (fs.existsSync(candidate)) {
        aliasMap[`mechanical-wings/plugin-sdk/${subpath}`] = candidate;
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  return {
    ...(extensionApiAlias ? { "mechanical-wings/extension-api": extensionApiAlias } : {}),
    ...(pluginSdkAlias ? { "mechanical-wings/plugin-sdk": pluginSdkAlias } : {}),
