...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @mechanical-wings/voice-call -> voice-call).
    ...
    addCandidate({
      ...
      manifest: {
        ...
        ...(!packageManifest?.description && entry.packageDescription
          ...
          : {}),
        ...(!getPackageManifestMetadata(packageManifest ?? undefined) && entry.packageManifest
          ? { "wings-of-world": entry.packageManifest }
          : {}),
      },
      packageDir: rootDir,
      bundledManifest: entry.manifest,
      bundledManifestPath: path.join(rootDir, "mechanical-wings.plugin.json"),
    });
  }
...
export function discoverWingsPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;
