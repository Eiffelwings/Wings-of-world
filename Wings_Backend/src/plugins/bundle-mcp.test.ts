...
describe("loadEnabledBundleMcpConfig", () => {
  it("loads enabled Claude bundle MCP config and absolutizes relative args", async () => {
    ...
    try {
      const homeDir = await tempHarness.createTempDir("mechanical-wings-bundle-mcp-home-");
      const workspaceDir = await tempHarness.createTempDir("mechanical-wings-bundle-mcp-workspace-");
  ...
  it("merges inline bundle MCP servers and skips disabled bundles", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "OPENCLAW_HOME", "OPENCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("mechanical-wings-bundle-inline-home-");
      const workspaceDir = await tempHarness.createTempDir("mechanical-wings-bundle-inline-workspace-");
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.OPENCLAW_HOME;
      delete process.env.OPENCLAW_STATE_DIR;

      const enabledRoot = path.join(homeDir, ".mechanical-wings", "extensions", "inline-enabled");
      const disabledRoot = path.join(homeDir, ".mechanical-wings", "extensions", "inline-disabled");
  ...
  it("resolves inline Claude MCP paths from the plugin root and expands CLAUDE_PLUGIN_ROOT", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "OPENCLAW_HOME", "OPENCLAW_STATE_DIR"]);
    try {
      const homeDir = await tempHarness.createTempDir("mechanical-wings-bundle-inline-placeholder-home-");
      const workspaceDir = await tempHarness.createTempDir(
        "mechanical-wings-bundle-inline-placeholder-workspace-",
      );
      process.env.HOME = homeDir;
      process.env.USERPROFILE = homeDir;
      delete process.env.OPENCLAW_HOME;
      ...
      const pluginRoot = path.join(homeDir, ".mechanical-wings", "extensions", "inline-claude");
