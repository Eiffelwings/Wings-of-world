  ...
  it.each([
    {
      ...
      expected: path.join("C:\\Users\\test", ".wings-of-world-backend", "gateway.cmd"),
    },
    {
      ...
      expected: path.join("C:\\Users\\test", ".wings-of-world-backend-jbphoenix", "gateway.cmd"),
    },
    {
      ...
      env: {
        ...
        OPENCLAW_STATE_DIR: "C:\\State\\mechanical-wings",
      },
      expected: path.join("C:\\State\\mechanical-wings", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", OPENCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".wings-of-world-backend", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Wings Gateway",
          "cd /d C:\\Projects\\mechanical-wings",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\mechanical-wings",
          environment: {
            NODE_ENV: "production",
            OPENCLAW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\mechanical-wings\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\mechanical-wings\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Wings Share\\node.exe" "\\\\fileserver\\Wings Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Wings Share\\node.exe",
            "\\\\fileserver\\Wings Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
