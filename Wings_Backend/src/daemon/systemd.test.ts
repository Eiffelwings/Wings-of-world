...
const TEST_MANAGED_HOME = "/tmp/mechanical-wings-test-home";
const GATEWAY_SERVICE = "wings-of-world-backend-gateway.service";
...
async function expectExecStartWithoutEnvironment(envFileLine: string) {
  mockReadGatewayServiceFile(["[Service]", "ExecStart=/usr/bin/mechanical-wings gateway run", envFileLine]);

  const command = await readSystemdServiceExecStart({ HOME: TEST_SERVICE_HOME });
  expect(command?.programArguments).toEqual(["/usr/bin/mechanical-wings", "gateway", "run"]);
...
describe("isSystemdServiceEnabled", () => {
  ...
  it("returns false without calling systemctl when the managed unit file is missing", async () => {
    ...
    const result = await isSystemdServiceEnabled({ env: { HOME: "/tmp/mechanical-wings-test-home" } });
  ...
  it("throws when systemctl is-enabled fails for non-state errors", async () => {
    ...
    execFileMock
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        expect(args).toEqual(["--user", "is-enabled", GATEWAY_SERVICE]);
      ...
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        ...
        expect(args.slice(2)).toEqual(["--user", "is-enabled", GATEWAY_SERVICE]);
        const err = new Error("permission denied") as Error & { code?: number };
        err.code = 1;
        cb(err, "", "permission denied");
      });
    await expect(
      isSystemdServiceEnabled({ env: { HOME: "/tmp/mechanical-wings-test-home" } }),
...
describe("isNonFatalSystemdInstallProbeError", () => {
  it("matches wrapper-only WSL install probe failures", () => {
    expect(
      isNonFatalSystemdInstallProbeError(
        new Error(`Command failed: systemctl --user is-enabled ${GATEWAY_SERVICE}`),
      ),
    ).toBe(true);
  });
  ...
  it.each([
    {
      ...
      env: { HOME: "/home/test" },
      expected: "/home/test/.config/systemd/user/wings-of-world-backend-gateway.service",
    },
    {
      ...
      expected: "/home/test/.config/systemd/user/wings-of-world-backend-gateway-jbphoenix.service",
    },
    {
...
describe("splitArgsPreservingQuotes", () => {
  it("splits on whitespace outside quotes", () => {
    expect(splitArgsPreservingQuotes('/usr/bin/mechanical-wings gateway start --name "My Bot"')).toEqual([
      "/usr/bin/mechanical-wings",
      "gateway",
      "start",
      "--name",
      "My Bot",
  ...
  it("supports systemd-style backslash escaping", () => {
    expect(
      splitArgsPreservingQuotes('mechanical-wings --name "My \\"Bot\\"" --foo bar', {
        escapeMode: "backslash",
      }),
    ).toEqual(["mechanical-wings", "--name", 'My "Bot"', "--foo", "bar"]);
  });

  it("supports schtasks-style escaped quotes while preserving other backslashes", () => {
    expect(
      splitArgsPreservingQuotes('mechanical-wings --path "C:\\\\Program Files\\\\Wings"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["mechanical-wings", "--path", "C:\\\\Program Files\\\\Wings"]);

    expect(
      splitArgsPreservingQuotes('mechanical-wings --label "My \\"Quoted\\" Name"', {
        escapeMode: "backslash-quote-only",
      }),
    ).toEqual(["mechanical-wings", "--label", 'My "Quoted" Name']);
  });
});

describe("parseSystemdExecStart", () => {
  it("preserves quoted arguments", () => {
    const execStart = '/usr/bin/mechanical-wings gateway start --name "My Bot"';
...
describe("readSystemdServiceExecStart", () => {
  ...
  it("loads OPENCLAW_GATEWAY_TOKEN from EnvironmentFile", async () => {
    const readFileSpy = mockReadGatewayServiceFile(
      ["[Service]", "ExecStart=/usr/bin/mechanical-wings gateway run", "EnvironmentFile=%h/.mechanical-wings/.env"],
      { [`${TEST_SERVICE_HOME}/.mechanical-wings/.env`]: "OPENCLAW_GATEWAY_TOKEN=env-file-token\n" },
    );
  ...
  it("lets EnvironmentFile override inline Environment values", async () => {
    mockReadGatewayServiceFile(
      [
        "[Service]",
        "ExecStart=/usr/bin/mechanical-wings gateway run",
        "EnvironmentFile=%h/.mechanical-wings/.env",
  ...
  it("ignores missing optional EnvironmentFile entries", async () => {
    await expectExecStartWithoutEnvironment("EnvironmentFile=-%h/.mechanical-wings/missing.env");
  });
    ...
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith(`/${GATEWAY_SERVICE}`)) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/mechanical-wings gateway run",
          'EnvironmentFile=%h/.mechanical-wings/first.env "%h/.mechanical-wings/second env.env"',
        ].join("\n");
      }
      if (pathValue === "/home/test/.mechanical-wings/first.env") {
        return "OPENCLAW_GATEWAY_TOKEN=first-token\n"; // pragma: allowlist secret
      }
      if (pathValue === "/home/test/.mechanical-wings/second env.env") {
    ...
    vi.spyOn(fs, "readFile").mockImplementation(async (pathname) => {
      ...
      if (pathValue.endsWith(`/${GATEWAY_SERVICE}`)) {
        return [
          "[Service]",
          "ExecStart=/usr/bin/mechanical-wings gateway run",
          "EnvironmentFile=%h/.mechanical-wings/gateway.env",
        ].join("\n");
      }
      if (pathValue === "/home/test/.mechanical-wings/gateway.env") {
        return [
          "# comment",
          "; another comment",
...
describe("systemd service control", () => {
  ...
  it("restarts a profile-specific user unit", async () => {
    execFileMock
      .mockImplementationOnce((_cmd, _args, _opts, cb) => cb(null, "", ""))
      .mockImplementationOnce((_cmd, args, _opts, cb) => {
        assertUserSystemctlArgs(args, "restart", "wings-of-world-backend-gateway-work.service");
        cb(null, "", "");
      });
