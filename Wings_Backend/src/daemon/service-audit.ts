...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message:
      "Gateway service embeds WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `wings-of-world-backend gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN does not match gateway.auth.token in wings-of-world.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (
    command.environmentValueSources?.WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN === "file" ||
    command.environmentValueSources?.OPENCLAW_GATEWAY_TOKEN === "file"
  ) {
    return undefined;
  }
  return (
    command.environment?.WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN?.trim() ||
    command.environment?.WINGS_OF_WORLD_GATEWAY_TOKEN?.trim() ||
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `wings-of-world-backend gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
