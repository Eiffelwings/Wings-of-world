...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.OPENCLAW_SERVICE_MARKER).toBe("wings-of-world-backend");
    expect(env.OPENCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.OPENCLAW_SERVICE_VERSION).toBe("string");
    expect(env.OPENCLAW_SYSTEMD_UNIT).toBe("wings-of-world-backend-gateway.service");
    expect(env.OPENCLAW_WINDOWS_TASK_NAME).toBe("Wings Of World Gateway");
    if (process.platform === "darwin") {
      expect(env.OPENCLAW_LAUNCHD_LABEL).toBe("ai.wings-of-world.backend.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    ...
    expect(env.OPENCLAW_SYSTEMD_UNIT).toBe("wings-of-world-backend-gateway-work.service");
    expect(env.OPENCLAW_WINDOWS_TASK_NAME).toBe("Wings Of World Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.OPENCLAW_LAUNCHD_LABEL).toBe("ai.wings-of-world.backend.work");
    }
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".wings-of-world-backend"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", OPENCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(
      path.join("/Users/test", ".wings-of-world-backend-rescue"),
    );
  });
  ...
  it("uses OPENCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", OPENCLAW_STATE_DIR: "/var/lib/mechanical-wings" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/mechanical-wings"));
  });

  it("expands ~ in OPENCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", OPENCLAW_STATE_DIR: "~/mechanical-wings-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/mechanical-wings-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { OPENCLAW_STATE_DIR: "C:\\State\\mechanical-wings" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\mechanical-wings");
  });
});
