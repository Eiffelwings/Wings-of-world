...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "mechanical-wings" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["mechanical-wings", "clawdbot", "moltbot"] as const;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["mechanical-wings_service_marker"];
  const kindKeys = ["mechanical-wings_service_kind"];
...
function isWingsGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.mechanical-wings.");
}

function isWingsGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("mechanical-wings-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isWingsGatewayTaskName(name: string): boolean {
  ...
  return (
    ...
    normalized.startsWith("wings-of-world-backend gateway") ||
    normalized.startsWith("mechanical-wings gateway")
  );
}
...
async function scanLaunchdDir(params: {
  ...
  for (const { name: labelFromName, fullPath, contents } of candidates) {
    ...
    if (marker === "mechanical-wings" && isWingsGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "mechanical-wings" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const { entry, name, fullPath, contents } of candidates) {
    ...
    if (marker === "mechanical-wings" && isWingsGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "mechanical-wings",
    });
  }
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isWingsGatewayTaskName(name)) {
        continue;
      }
