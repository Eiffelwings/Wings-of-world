...
const TEMP_MEDIA_PREFIX = "mechanical-wings-media-";
...
function createGroqAudioConfig(): WingsConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): WingsConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): WingsConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: WingsConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredWingsTmpDir();
