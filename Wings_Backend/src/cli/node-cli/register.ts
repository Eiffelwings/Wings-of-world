    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "mechanical-wings node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["mechanical-wings node status", "Check node host service status."],
          ["mechanical-wings node install", "Install the node host service."],
          ["mechanical-wings node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.wings-of-world.ai/cli/node")}\n`,
    );
