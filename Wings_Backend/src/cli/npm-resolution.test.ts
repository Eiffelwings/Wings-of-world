...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@mechanical-wings/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@mechanical-wings/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@mechanical-wings/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@mechanical-wings/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @mechanical-wings/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      mapNpmResolutionMetadata({
        name: "@mechanical-wings/plugin-alpha",
        version: "1.2.3",
        resolvedSpec: "@mechanical-wings/plugin-alpha@1.2.3",
      ...
      resolvedName: "@mechanical-wings/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@mechanical-wings/plugin-alpha@1.2.3",
        installPath: "/tmp/mechanical-wings/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@mechanical-wings/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @mechanical-wings/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@mechanical-wings/plugin-alpha@latest",
      true,
      "/tmp/mechanical-wings/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@mechanical-wings/plugin-alpha@latest",
      installPath: "/tmp/mechanical-wings/extensions/alpha",
      version: "1.2.3",
