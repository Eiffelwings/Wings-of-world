...
describe("cron cli", () => {
  ...
  it("creates an isolated autonomous agent cron job", { timeout: CRON_CLI_TEST_TIMEOUT_MS }, async () => {
    await runCronCommand([
      "cron",
      "agent",
      "--name",
      ...
      "0 9 * * 1-5",
      "--message",
      "Check Wings Of World readiness and create evidence.",
      "--model",
      "gpt-5.4",
      "--announce",
    ]);
    ...
    expect(params.payload?.message).toContain("readiness");
    expect(params.payload?.model).toBe("gpt-5.4");
    expect(params.payload?.thinking).toBe("low");
