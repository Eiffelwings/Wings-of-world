...
function createEnabledPluginConfig(pluginId: string): WingsConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createClawHubInstallResult(params: {
  ...
  return {
    ...
    targetDir: `/tmp/mechanical-wings-state/extensions/${params.pluginId}`,
...
describe("plugins cli install", () => {
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/mechanical-wings-config.json5",
      exists: true,
  ...
  it("installs marketplace plugins and persists config", async () => {
    ...
    const installedCfg = {
      ...
      plugins: {
        ...
        installs: {
          alpha: {
            source: "marketplace",
            installPath: "/tmp/mechanical-wings-state/extensions/alpha",
          },
        },
      },
    } as WingsConfig;
    ...
    installPluginFromMarketplace.mockResolvedValue({
      ok: true,
      pluginId: "alpha",
      targetDir: "/tmp/mechanical-wings-state/extensions/alpha",
      version: "1.2.3",
  ...
  it("installs ClawHub plugins and persists source metadata", async () => {
    ...
    const installedCfg = createClawHubInstalledConfig({
      ...
      install: {
        ...
        installPath: "/tmp/mechanical-wings-state/extensions/demo",
        clawhubPackage: "demo",
        clawhubFamily: "code-plugin",
  ...
  it("falls back to npm when ClawHub does not have the package", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "demo",
      targetDir: "/tmp/mechanical-wings-state/extensions/demo",
      version: "1.2.3",
      npmResolution: {
        packageName: "demo",
  ...
  it("does not fall back to npm when ClawHub rejects a real package", async () => {
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "mechanical-wings skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "mechanical-wings skills install demo" instead.');
  });
  ...
  it("falls back to installing hook packs from npm specs", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing mechanical-wings.plugin.json",
    });
