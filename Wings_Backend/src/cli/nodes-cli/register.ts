    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mechanical-wings nodes status", "List known nodes with live status."],
          ["mechanical-wings nodes pairing pending", "Show pending node pairing requests."],
          ['mechanical-wings nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["mechanical-wings nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.wings-of-world.ai/cli/nodes")}\n`,
    );
