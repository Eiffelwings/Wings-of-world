...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "mechanical-wings", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "mechanical-wings", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "mechanical-wings", "status"]);
  });
