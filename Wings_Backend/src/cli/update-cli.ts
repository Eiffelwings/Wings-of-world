...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update Wings and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["mechanical-wings update", "Update a source checkout (git)"],
        ["mechanical-wings update --channel beta", "Switch to beta channel (git + npm)"],
        ["mechanical-wings update --channel dev", "Switch to dev channel (git + npm)"],
        ["mechanical-wings update --tag beta", "One-off update to a dist-tag or version"],
        ["mechanical-wings update --tag main", "One-off package install from GitHub main"],
        ["mechanical-wings update --dry-run", "Preview actions without changing anything"],
        ["mechanical-wings update --no-restart", "Update without restarting the service"],
        ["mechanical-wings update --json", "Output result as JSON"],
        ["mechanical-wings update --yes", "Non-interactive (accept downgrade prompts)"],
        ["mechanical-wings update wizard", "Interactive update wizard"],
        ["mechanical-wings --update", "Shorthand for mechanical-wings update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run mechanical-wings update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.wings-of-world.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mechanical-wings update status", "Show channel + version status."],
          ["mechanical-wings update status --json", "JSON output."],
          ["mechanical-wings update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.wings-of-world.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
