...
import type { ConfigFileSnapshot } from "../config/types.mechanical-wings.js";

const loadConfigMock = vi.fn<() => WingsConfig>();
...
vi.mock("../commands/doctor/providers/matrix.js", () => ({
  cleanStaleMatrixPluginConfig: (cfg: WingsConfig) => cleanStaleMatrixPluginConfigMock(cfg),
}));
...
function makeSnapshot(overrides: Partial<ConfigFileSnapshot> = {}): ConfigFileSnapshot {
  return {
    ...
    raw: '{ "plugins": {} }',
    parsed: { plugins: {} },
    resolved: { plugins: {} } as WingsConfig,
    valid: false,
    config: { plugins: {} } as WingsConfig,
    hash: "abc",
...
describe("loadConfigForInstall", () => {
  const matrixNpmRequest = {
    rawSpec: "@mechanical-wings/matrix",
    normalizedSpec: "@mechanical-wings/matrix",
  };
  ...
  beforeEach(() => {
    ...
    cleanStaleMatrixPluginConfigMock.mockImplementation((cfg: WingsConfig) => ({
      config: cfg,
      changes: [],
    }));
  });
  ...
  it("returns the config directly when loadConfig succeeds", async () => {
    const cfg = { plugins: { entries: { matrix: { enabled: true } } } } as WingsConfig;
  ...
  it("does not run stale Matrix cleanup on the happy path", async () => {
    const cfg = { plugins: {} } as WingsConfig;
    loadConfigMock.mockReturnValue(cfg);
  ...
  it("allows explicit repo-checkout Matrix reinstall recovery", async () => {
    ...
    const snapshotCfg = { plugins: {} } as WingsConfig;
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        config: snapshotCfg,
  ...
  it("rejects non-Matrix install requests when config is invalid", async () => {
    ...
    await expect(
      ...
      "Config invalid; run `wings-of-world-backend doctor --fix` before installing plugins.",
    );
    expect(readConfigFileSnapshotMock).not.toHaveBeenCalled();
  });
  ...
  it("throws when loadConfig fails with INVALID_CONFIG and snapshot parsed is empty", async () => {
    ...
    readConfigFileSnapshotMock.mockResolvedValue(
      makeSnapshot({
        parsed: {},
        config: {} as WingsConfig,
      }),
    );

    await expect(loadConfigForInstall(matrixNpmRequest)).rejects.toThrow(
      "Config file could not be parsed; run `wings-of-world-backend doctor` to repair it.",
    );
  });
