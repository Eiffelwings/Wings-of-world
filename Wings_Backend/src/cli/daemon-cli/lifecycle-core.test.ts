...
describe("runServiceRestart token drift", () => {
  ...
  it("prints the container restart hint when restart is requested for a not-loaded service", async () => {
    ...
    vi.stubEnv("OPENCLAW_CONTAINER_HINT", "mechanical-wings-demo-container");
    ...
    expect(runtimeLogs).toContain(
      "Start with: Restart the container or the service that manages it for mechanical-wings-demo-container.",
    );
  });
  ...
  it("falls back to not-loaded hints when start finds no install artifacts", async () => {
    ...
    await runServiceStart({
      serviceNoun: "Gateway",
      service,
      renderStartHints: () => ["mechanical-wings gateway install"],
      opts: { json: true },
    });
    ...
    expect(payload.hints).toEqual(expect.arrayContaining(["mechanical-wings gateway install"]));
    expect(payload.hintItems).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          kind: "install",
          text: "mechanical-wings gateway install",
        }),
      ]),
    );
    expect(service.restart).not.toHaveBeenCalled();
