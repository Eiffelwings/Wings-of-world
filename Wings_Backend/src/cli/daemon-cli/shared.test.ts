...
describe("renderGatewayServiceStartHints", () => {
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        OPENCLAW_CONTAINER: "mechanical-wings-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("mechanical-wings-demo-container");
    expect(
      resolveDaemonContainerContext({
        OPENCLAW_CONTAINER_HINT: "mechanical-wings-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("mechanical-wings-demo-container");
  });
  ...
  it("prepends a single container restart hint when OPENCLAW_CONTAINER is set", () => {
    expect(
      ...
      expect.arrayContaining([
        "Restart the container or the service that manages it for mechanical-wings-demo-container.",
      ]),
    );
  });
    ...
    expect(
      filterContainerGenericHints(
        [
          ...
          "If you're in a container, run the gateway in the foreground instead of `mechanical-wings gateway`.",
        ],
        { OPENCLAW_CONTAINER: "mechanical-wings-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toEqual([]);
  });
  ...
  it("drops the generic container foreground hint when OPENCLAW_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        ...
        { OPENCLAW_CONTAINER_HINT: "mechanical-wings-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toEqual([]);
  });
});
