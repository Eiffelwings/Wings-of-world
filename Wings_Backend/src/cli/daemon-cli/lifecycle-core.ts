...
export async function runServiceStart(params: {
  ...
  // to prevent launching from invalid config in any start path.
  {
    ...
    if (configError) {
      fail(
        `${params.serviceNoun} aborted: config is invalid.\n${configError}\nFix the config and retry, or run "wings-of-world-backend doctor" to repair.`,
      );
      return;
    }
  }
...
export async function runServiceRestart(params: {
  ...
  if (loaded && params.checkTokenDrift) {
    ...
    try {
      ...
      const serviceToken =
        command?.environment?.WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN ||
        command?.environment?.WINGS_OF_WORLD_GATEWAY_TOKEN ||
        command?.environment?.OPENCLAW_GATEWAY_TOKEN;
