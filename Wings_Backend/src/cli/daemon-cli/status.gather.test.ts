...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    OPENCLAW_STATE_DIR: "/tmp/mechanical-wings-daemon",
    OPENCLAW_CONFIG_PATH: "/tmp/mechanical-wings-daemon/mechanical-wings.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.OPENCLAW_STATE_DIR ?? "/tmp/mechanical-wings-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.OPENCLAW_CONFIG_PATH ?? `${stateDir}/mechanical-wings.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/mechanical-wings-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.OPENCLAW_STATE_DIR = "/tmp/mechanical-wings-cli";
    process.env.OPENCLAW_CONFIG_PATH = "/tmp/mechanical-wings-cli/mechanical-wings.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/mechanical-wings-daemon/mechanical-wings.json",
      }),
    );
  });
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "mechanical-wings-gateway" }],
        hints: [],
      },
      healthy: false,
