...
async function assertUnmanagedGatewayRestartEnabled(port: number): Promise<void> {
  ...
  const probe = await probeGateway({
    ...
    auth: {
      token:
        process.env.WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN?.trim() ||
        process.env.WINGS_OF_WORLD_GATEWAY_TOKEN?.trim() ||
        process.env.OPENCLAW_GATEWAY_TOKEN?.trim() ||
        undefined,
      password:
        process.env.WINGS_OF_WORLD_BACKEND_GATEWAY_PASSWORD?.trim() ||
        process.env.WINGS_OF_WORLD_GATEWAY_PASSWORD?.trim() ||
...
async function restartGatewayWithoutServiceManager(port: number) {
  ...
  if (pids.length > 1) {
    throw new Error(
      `multiple gateway processes are listening on port ${port}: ${formatGatewayPidList(pids)}; use "wings-of-world-backend gateway status --deep" before retrying restart`,
    );
  }
    ...
    postRestartCheck: async ({ warnings, fail, stdout }) => {
      if (restartedWithoutServiceManager) {
        ...
        fail(`Gateway restart timed out after ${restartWaitSeconds}s waiting for health checks.`, [
          formatCliCommand("wings-of-world-backend gateway status --deep"),
          formatCliCommand("wings-of-world-backend doctor"),
        ]);
      }
