...
describe("parseCliContainerArgs", () => {
  it("extracts a root --container flag before the command", () => {
    expect(
      parseCliContainerArgs(["node", "mechanical-wings", "--container", "demo", "status", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "mechanical-wings", "status", "--deep"],
    });
  });

  it("accepts the equals form", () => {
    expect(parseCliContainerArgs(["node", "mechanical-wings", "--container=demo", "health"])).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "mechanical-wings", "health"],
    });
  });

  it("rejects a missing container value", () => {
    expect(parseCliContainerArgs(["node", "mechanical-wings", "--container"])).toEqual({
      ok: false,
  ...
  it("does not consume an adjacent flag as the container value", () => {
    expect(
      parseCliContainerArgs(["node", "mechanical-wings", "--container", "--no-color", "status"]),
    ).toEqual({
      ok: false,
      error: "--container requires a value",
    });
  ...
  it("leaves argv unchanged when the flag is absent", () => {
    expect(parseCliContainerArgs(["node", "mechanical-wings", "status"])).toEqual({
      ok: true,
      container: null,
  ...
  it("extracts --container after the command like other root options", () => {
    expect(
      parseCliContainerArgs(["node", "mechanical-wings", "status", "--container", "demo", "--deep"]),
    ).toEqual({
      ok: true,
      container: "demo",
      argv: ["node", "mechanical-wings", "status", "--deep"],
...
describe("resolveCliContainerTarget", () => {
  it("uses argv first and falls back to OPENCLAW_CONTAINER", () => {
    expect(
      resolveCliContainerTarget(["node", "mechanical-wings", "--container", "demo", "status"], {}),
    ).toBe("demo");
    expect(resolveCliContainerTarget(["node", "mechanical-wings", "status"], {})).toBeNull();
    expect(
      resolveCliContainerTarget(["node", "mechanical-wings", "status"], {
...
describe("maybeRunCliInContainer", () => {
  it("passes through when no container target is provided", () => {
    expect(maybeRunCliInContainer(["node", "mechanical-wings", "status"], { env: {} })).toEqual({
      handled: false,
      argv: ["node", "mechanical-wings", "status"],
    });
  });
  ...
  it("uses OPENCLAW_CONTAINER when the flag is absent", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "mechanical-wings", "status"], {
  ...
  it("executes through podman when the named container is running", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "mechanical-wings", "--container", "demo", "status"], {
        env: {},
        spawnSync,
      }),
    ).toEqual({
  ...
  it("falls back to docker when podman does not have the container", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "mechanical-wings", "--container", "demo", "health"], {
        env: { USER: "mechanical-wings" } as NodeJS.ProcessEnv,
        spawnSync,
      }),
    ).toEqual({
      handled: true,
    ...
    expect(spawnSync).toHaveBeenNthCalledWith(
      ...
      {
        stdio: "inherit",
        env: { USER: "mechanical-wings", OPENCLAW_CONTAINER: "" },
      },
    );
  });
  ...
  it("allocates a tty for interactive terminal sessions", () => {
    ...
    maybeRunCliInContainer(["node", "mechanical-wings", "--container", "demo", "setup"], {
      env: {},
      spawnSync,
      stdinIsTTY: true,
      stdoutIsTTY: true,
  ...
  it("prefers --container over OPENCLAW_CONTAINER", () => {
    ...
    expect(
      maybeRunCliInContainer(["node", "mechanical-wings", "--container", "flag-demo", "health"], {
  ...
  it("skips recursion when the bypass env is set", () => {
    expect(
      ...
      handled: false,
      argv: ["node", "mechanical-wings", "--container", "demo", "status"],
    });
  });
  ...
  it("blocks updater commands from running inside the container", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "mechanical-wings", "--container", "demo", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
      "mechanical-wings update is not supported with --container; rebuild or restart the container image instead.",
    );
  ...
  it("blocks update after interleaved root flags", () => {
    ...
    expect(() =>
      maybeRunCliInContainer(["node", "mechanical-wings", "--container", "demo", "--no-color", "update"], {
        env: {},
        spawnSync,
      }),
    ).toThrow(
