...
export function registerCronCli(program: Command) {
  const cron = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/cron", "docs.wings-of-world.ai/cli/cron")}\n${theme.muted("Upgrade tip:")} run \`wings-of-world-backend doctor --fix\` to normalize legacy cron job storage.\n`,
    );
