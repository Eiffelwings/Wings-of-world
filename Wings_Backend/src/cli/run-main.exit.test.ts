...
vi.mock("../infra/path-env.js", () => ({
  ensureWingsCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "mechanical-wings", "status"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    await runCli(["node", "mechanical-wings", "--help"]);

    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "mechanical-wings", "--help"]);
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "mechanical-wings", "--container", "demo", "status"]);
