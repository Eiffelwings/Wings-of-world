...
export function applyCliProfileEnv(params: {
  ...
  env.WINGS_OF_WORLD_BACKEND_PROFILE = profile;

  const stateDir =
    env.WINGS_OF_WORLD_BACKEND_STATE_DIR?.trim() ||
    env.WINGS_OF_WORLD_STATE_DIR?.trim() ||
    env.OPENCLAW_STATE_DIR?.trim() ||
    resolveProfileStateDir(profile, env, homedir);
  if (!env.WINGS_OF_WORLD_BACKEND_STATE_DIR?.trim()) {
    env.WINGS_OF_WORLD_BACKEND_STATE_DIR = stateDir;
  }
  ...
  const configPath =
    env.WINGS_OF_WORLD_BACKEND_CONFIG_PATH?.trim() ||
    env.WINGS_OF_WORLD_CONFIG_PATH?.trim() ||
    env.OPENCLAW_CONFIG_PATH?.trim() ||
    path.join(stateDir, "wings-of-world.json");
  if (!env.WINGS_OF_WORLD_BACKEND_CONFIG_PATH?.trim()) {
    env.WINGS_OF_WORLD_BACKEND_CONFIG_PATH = configPath;
  }
  if (!env.OPENCLAW_CONFIG_PATH?.trim()) {
    env.OPENCLAW_CONFIG_PATH = configPath;
  }

  if (profile === "dev" && !env.WINGS_OF_WORLD_BACKEND_GATEWAY_PORT?.trim()) {
    env.WINGS_OF_WORLD_BACKEND_GATEWAY_PORT = "19001";
  }
