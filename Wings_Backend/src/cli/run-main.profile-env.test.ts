...
describe("runCli profile env bootstrap", () => {
  ...
  it("applies --profile before dotenv loading", async () => {
    await runCli(["node", "mechanical-wings", "--profile", "rawdog", "status"]);
  ...
  it("rejects --container combined with --profile", async () => {
    await expect(
      runCli(["node", "mechanical-wings", "--container", "demo", "--profile", "rawdog", "status"]),
  ...
  it("rejects --container combined with interleaved --profile", async () => {
    await expect(
      runCli(["node", "mechanical-wings", "status", "--container", "demo", "--profile", "rawdog"]),
    ).rejects.toThrow("--container cannot be combined with --profile/--dev");
  });

  it("rejects --container combined with interleaved --dev", async () => {
    await expect(
      runCli(["node", "mechanical-wings", "status", "--container", "demo", "--dev"]),
    ).rejects.toThrow("--container cannot be combined with --profile/--dev");
  });
  ...
  it("does not let dotenv change container target resolution", async () => {
    ...
    await runCli(["node", "mechanical-wings", "status"]);

    expect(dotenvState.loadDotEnv).toHaveBeenCalledOnce();
    expect(process.env.OPENCLAW_CONTAINER).toBe("demo");
    expect(dotenvState.state.containerAtDotenvLoad).toBe("demo");
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "mechanical-wings", "status"]);
  ...
  it("allows container mode when OPENCLAW_PROFILE is already set in env", async () => {
    ...
    await expect(
      runCli(["node", "mechanical-wings", "--container", "demo", "status"]),
  ...
  it("allows container mode when only OPENCLAW_STATE_DIR is set in env", async () => {
    process.env.OPENCLAW_STATE_DIR = "/tmp/mechanical-wings-host-state";
  ...
  it("allows container mode when only OPENCLAW_CONFIG_PATH is set in env", async () => {
    process.env.OPENCLAW_CONFIG_PATH = "/tmp/mechanical-wings-host-state/mechanical-wings.json";
