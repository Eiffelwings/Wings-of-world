...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mechanical-wings security audit", "Run a local security audit."],
          ["mechanical-wings security audit --deep", "Include best-effort live Gateway probe checks."],
          ["mechanical-wings security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "mechanical-wings security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["mechanical-wings security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["mechanical-wings security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.wings-of-world.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("Wings security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("mechanical-wings security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("mechanical-wings security audit --fix")}`));
        if (!fixResult) {
