...
const STEP_LABELS: Record<string, string> = {
  ...
  "wings-of-world-backend doctor entry": "Checking doctor entrypoint",
  "wings-of-world-backend doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (failedStep.name.startsWith("global update") && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g mechanical-wings@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g mechanical-wings@latest --omit=optional");
  }
