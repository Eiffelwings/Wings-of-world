  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain(
        "systemctl --user restart 'wings-of-world-backend-gateway.service'",
      );
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain(
        "launchctl kickstart -k 'gui/501/ai.wings-of-world.backend.gateway'",
      );
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.wings-of-world.backend.gateway'");
    ...
    it("uses OPENCLAW_LAUNCHD_LABEL override on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        OPENCLAW_PROFILE: "default",
        OPENCLAW_LAUNCHD_LABEL: "com.custom.mechanical-wings",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.mechanical-wings'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Wings Of World Gateway"');
      expect(content).toContain('schtasks /Run /TN "Wings Of World Gateway"');
    ...
    it("uses OPENCLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        OPENCLAW_PROFILE: "default",
        OPENCLAW_WINDOWS_TASK_NAME: "Wings Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "Wings Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "Wings Gateway (custom)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      ...
      expect(content).toContain("wings-of-world-backend-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      expect(content).toContain("gui/502/ai.wings-of-world.backend.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "Wings Of World Gateway (production)"');
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        OPENCLAW_LAUNCHD_LABEL: "ai.mechanical-wings.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.mechanical-wings.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
