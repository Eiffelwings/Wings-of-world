...
import type { WingsConfig, ConfigFileSnapshot } from "../config/types.mechanical-wings.js";
...
vi.mock("../infra/mechanical-wings-root.js", () => ({
  resolveWingsPackageRoot: vi.fn(),
}));
...
const { resolveWingsPackageRoot } = await import("../infra/mechanical-wings-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/mechanical-wings-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as WingsConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/mechanical-wings-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveWingsPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("mechanical-wings-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("mechanical-wings-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveWingsPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("mechanical-wings");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/mechanical-wings-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "mechanical-wings-gateway" }],
      hints: [],
    });
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "mechanical-wings", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as WingsConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "mechanical-wings@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    mockPackageInstallStatus(createCaseDir("mechanical-wings-update"));
    vi.mocked(fetchNpmPackageTargetStatus).mockResolvedValue({
      target: "latest",
      version: "2026.3.23-2",
      nodeEngine: ">=22.14.0",
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g mechanical-wings` can silently install an older compatible release.",
    );
  });
    ...
    for (const scenario of [
      {
        ...
        expectedSpec: "mechanical-wings@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:mechanical-wings/mechanical-wings#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("mechanical-wings-update"));
          await updateCommand({ yes: true, tag: "github:mechanical-wings/mechanical-wings#main" });
        },
        expectedSpec: "github:mechanical-wings/mechanical-wings#main",
      },
      {
        ...
        expectedSpec: "http://10.211.55.2:8138/mechanical-wings-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("mechanical-wings");
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "mechanical-wings", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("mechanical-wings-update");
    const localAppData = createCaseDir("mechanical-wings-localappdata");
        ...
        await withEnvAsync(
          {
            ...
            OPENCLAW_CONFIG_PATH: "./config/mechanical-wings.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            ...
            OPENCLAW_CONFIG_PATH: path.resolve("./config/mechanical-wings.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("mechanical-wings-update-wizard");
  ...
  it("uses ~/mechanical-wings as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ OPENCLAW_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "mechanical-wings"));
    });
