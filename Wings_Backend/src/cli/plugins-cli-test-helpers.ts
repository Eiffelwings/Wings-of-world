...
export const loadConfig = vi.fn<() => WingsConfig>(() => ({}) as WingsConfig);
export const readConfigFileSnapshot = vi.fn();
export const writeConfigFile = vi.fn<(config: WingsConfig) => Promise<void>>(
  async () => undefined,
);
export const resolveStateDir = vi.fn(() => "/tmp/mechanical-wings-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: (config: WingsConfig) => writeConfigFile(config),
}));
...
export function resetPluginsCliTestState() {
  ...
  loadConfig.mockReturnValue({} as WingsConfig);
  ...
  resolveStateDir.mockReturnValue("/tmp/mechanical-wings-state");
  resolveMarketplaceInstallShortcut.mockResolvedValue(null);
  installPluginFromMarketplace.mockResolvedValue({
    ok: false,
    error: "marketplace install failed",
  });
  enablePluginInConfig.mockImplementation((cfg: WingsConfig) => ({ config: cfg }));
  recordPluginInstall.mockImplementation((cfg: WingsConfig) => cfg);
  buildPluginStatusReport.mockReturnValue({
    plugins: [],
    diagnostics: [],
  });
  applyExclusiveSlotSelection.mockImplementation(({ config }: { config: WingsConfig }) => ({
    config,
    warnings: [],
  }));
  ...
  recordHookInstall.mockImplementation((cfg: WingsConfig) => cfg);
}
