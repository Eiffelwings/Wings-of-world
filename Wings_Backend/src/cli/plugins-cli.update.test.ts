...
describe("plugins cli update", () => {
  ...
  it("maps an explicit unscoped npm dist-tag update to the tracked plugin id", async () => {
    ...
    await runPluginsCommand(["plugins", "update", "mechanical-wings-codex-app-server@beta"]);
  ...
  it("maps an explicit scoped npm dist-tag update to the tracked plugin id", async () => {
      ...
      plugins: {
        installs: {
          "voice-call": {
            ...
            resolvedName: "@mechanical-wings/voice-call",
          },
        },
      },
    } as WingsConfig;
    ...
    await runPluginsCommand(["plugins", "update", "@mechanical-wings/voice-call@beta"]);
    ...
    expect(updateNpmInstalledPlugins).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        specOverrides: {
          "voice-call": "@mechanical-wings/voice-call@beta",
        },
      }),
    );
  });
  ...
  it("maps an explicit npm version update to the tracked plugin id", async () => {
    ...
    await runPluginsCommand(["plugins", "update", "mechanical-wings-codex-app-server@0.2.0-beta.4"]);
  ...
  it("keeps using the recorded npm tag when update is invoked by plugin id", async () => {
    ...
    await runPluginsCommand(["plugins", "update", "mechanical-wings-codex-app-server"]);
    ...
    const cfg = {
      plugins: {
        installs: {
          alpha: {
            source: "npm",
            spec: "@mechanical-wings/alpha@1.0.0",
          },
        },
      },
    } as WingsConfig;
    const nextConfig = {
      plugins: {
        installs: {
          alpha: {
            source: "npm",
            spec: "@mechanical-wings/alpha@1.1.0",
          },
        },
      },
    } as WingsConfig;
