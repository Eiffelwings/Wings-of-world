...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Wings Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(mechanical-wings completion --shell zsh)
  return (
 ...
 * Returns true if profile has `source <(mechanical-wings completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/completion", "docs.wings-of-world.ai/cli/completion")}\n`,
    )
    .addOption(
...
export async function installCompletion(shell: string, yes: boolean, binName = "mechanical-wings") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
