...
export function formatPairingApproveHint(channelId: string): string {
  const listCmd = formatCliCommand(`mechanical-wings pairing list ${channelId}`);
  const approveCmd = formatCliCommand(`mechanical-wings pairing approve ${channelId} <code>`);
