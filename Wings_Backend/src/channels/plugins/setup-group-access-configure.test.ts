...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: WingsConfig, policy: ChannelAccessPolicy) => WingsConfig;
  resolveAllowlist: (params: { cfg: WingsConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: WingsConfig; resolved: TResolved }) => WingsConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: WingsConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: WingsConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: WingsConfig, policy: ChannelAccessPolicy): WingsConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: WingsConfig, policy: ChannelAccessPolicy): WingsConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: WingsConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: WingsConfig; resolved: string[] }) => {
      calls.push("apply");
