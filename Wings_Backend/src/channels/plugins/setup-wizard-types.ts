...
export type ChannelOnboardingPostWriteHook = {
  channel: ChannelId;
  accountId: string;
  run: (ctx: { cfg: WingsConfig; runtime: RuntimeEnv }) => Promise<void> | void;
};
...
export type ChannelSetupDmPolicy = {
  ...
  getCurrent: (cfg: WingsConfig, accountId?: string) => DmPolicy;
  setPolicy: (cfg: WingsConfig, policy: DmPolicy, accountId?: string) => WingsConfig;
