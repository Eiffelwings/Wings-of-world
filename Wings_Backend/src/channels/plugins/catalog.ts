...
import { discoverWingsPlugins } from "../../plugins/discovery.js";
import { getPackageManifestMetadata, loadPluginManifest } from "../../plugins/manifest.js";
import type { WingsPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, WingsPackageManifest>>;
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = [
    resolveWingsPackageRootSync({ cwd: process.cwd() }),
    resolveWingsPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<WingsPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
