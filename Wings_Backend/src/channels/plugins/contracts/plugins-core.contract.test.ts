...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@mechanical-wings/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-catalog-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@mechanical-wings/demo-channel",
            "mechanical-wings": {
              channel: {
                id: "demo-channel",
                label: "Demo Channel",
              ...
              install: {
                npmSpec: "@mechanical-wings/demo-channel",
              },
            },
          },
        ],
  ...
  it("preserves plugin ids when they differ from channel ids", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-channel-catalog-state-"));
  ...
  it("uses the provided env for external catalog path resolution", () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-catalog-home-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@mechanical-wings/env-demo-channel",
            "mechanical-wings": {
              channel: {
                id: "env-demo-channel",
              ...
              install: {
                npmSpec: "@mechanical-wings/env-demo-channel",
              },
            },
          },
        ],
  ...
  it("uses the provided env for default catalog paths", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-catalog-state-"));
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@mechanical-wings/default-env-demo",
            "mechanical-wings": {
              channel: {
                id: "default-env-demo",
              ...
              install: {
                npmSpec: "@mechanical-wings/default-env-demo",
              },
            },
          },
        ],
  ...
  it("includes bundled metadata-only channel entries even when the runtime entrypoint is omitted", () => {
    const packageRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-bundled-catalog-"));
    const bundledDir = path.join(packageRoot, "dist", "extensions", "whatsapp");
    fs.mkdirSync(bundledDir, { recursive: true });
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "mechanical-wings" }),
      "utf8",
    );
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        "mechanical-wings": {
          ...
          install: {
            npmSpec: "@mechanical-wings/whatsapp",
            defaultChoice: "npm",
          },
        },
      }),
    ...
    fs.writeFileSync(
      path.join(bundledDir, "mechanical-wings.plugin.json"),
    ...
    expect(entry?.install.npmSpec).toBe("@mechanical-wings/whatsapp");
    expect(entry?.pluginId).toBe("whatsapp");
  });

  it("includes shipped official channel catalog entries when bundled metadata is omitted", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-official-catalog-"));
  ...
  it("lets external catalogs override shipped fallback channel metadata", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-fallback-catalog-"));
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: WingsConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
