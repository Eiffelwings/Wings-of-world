...
function collectConfigEnvVarsByTarget(cfg?: WingsConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: WingsConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: WingsConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: WingsConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
