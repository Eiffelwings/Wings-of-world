...
import { parseWingsVersion } from "./version.js";

async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  home: string,
  dirname: ".mechanical-wings",
  port: number,
  filename: string = "mechanical-wings.json",
) {
...
async function expectNoNewerVersionWarning(touchedVersion: string) {
  await withTempHome(async (home) => {
    ...
    const configPath = path.join(configDir, "mechanical-wings.json");
    await fs.writeFile(
      configPath,
    ...
    expect(logger.warn).not.toHaveBeenCalledWith(
      expect.stringContaining("Config was last written by a newer Wings"),
    );
...
describe("config io paths", () => {
  it("uses ~/.mechanical-wings/mechanical-wings.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".mechanical-wings", 19001);
  ...
  it("defaults to ~/.wings-of-world-backend/wings-of-world.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(
        path.join(home, ".wings-of-world-backend", "wings-of-world.json"),
      );
    });
  });
  ...
  it("uses OPENCLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      ...
      expect(io.configPath).toBe(
        path.join(home, "svc-home", ".wings-of-world-backend", "wings-of-world.json"),
      );
    });
  });
  ...
  it("honors explicit OPENCLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".mechanical-wings", 20002, "custom.json");
  ...
  it("does not warn when config was last touched by a same-base correction publish", async () => {
    const parsedVersion = parseWingsVersion(VERSION);
    if (!parsedVersion) {
