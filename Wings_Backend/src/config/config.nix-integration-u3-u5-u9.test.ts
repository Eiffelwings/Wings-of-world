...
describe("Nix integration (U3, U5, U9)", () => {
  ...
  describe("U5: CONFIG_PATH and STATE_DIR env var overrides", () => {
    it("STATE_DIR defaults to ~/.wings-of-world-backend when env not set", () => {
      expect(resolveStateDir(envWith({ OPENCLAW_STATE_DIR: undefined }))).toMatch(
    ...
    it("STATE_DIR respects OPENCLAW_HOME when state override is unset", () => {
      ...
      ).toBe(path.join(path.resolve(customHome), ".wings-of-world-backend"));
    });

    it("CONFIG_PATH defaults to OPENCLAW_HOME/.wings-of-world-backend/wings-of-world.json", () => {
      const customHome = path.join(path.sep, "custom", "home");
      expect(
        resolveConfigPathCandidate(
          envWith({
        ...
        path.join(path.resolve(customHome), ".wings-of-world-backend", "wings-of-world.json"),
      );
    });

    it("CONFIG_PATH defaults to ~/.wings-of-world-backend/wings-of-world.json when env not set", () => {
      expect(
        resolveConfigPathCandidate(
          envWith({ OPENCLAW_CONFIG_PATH: undefined, OPENCLAW_STATE_DIR: undefined }),
        ),
      ).toMatch(/\.wings-of-world-backend[\\/]wings-of-world\.json$/);
    });

    it("CONFIG_PATH respects OPENCLAW_CONFIG_PATH override", () => {
      expect(
        resolveConfigPathCandidate(
          envWith({ OPENCLAW_CONFIG_PATH: "/nix/store/abc/mechanical-wings.json" }),
        ),
      ).toBe(path.resolve("/nix/store/abc/mechanical-wings.json"));
    });
    ...
    it("CONFIG_PATH expands ~ in OPENCLAW_CONFIG_PATH override", async () => {
      await withTempHome(async (home) => {
        expect(
          resolveConfigPathCandidate(
            envWith({ OPENCLAW_HOME: home, OPENCLAW_CONFIG_PATH: "~/.mechanical-wings/custom.json" }),
            () => home,
          ),
        ).toBe(path.join(home, ".mechanical-wings", "custom.json"));
      });
    });
    ...
    it("CONFIG_PATH uses STATE_DIR when only state dir is overridden", () => {
      expect(
        resolveConfigPathCandidate(
          ...
          () => path.join(path.sep, "tmp", "mechanical-wings-config-home"),
        ),
      ).toBe(path.join(path.resolve("/custom/state"), "wings-of-world.json"));
    });
  });
  ...
  describe("U5b: tilde expansion for config paths", () => {
    it("expands ~ in common path-ish config fields", async () => {
      await withTempHome(async (home) => {
        const configDir = path.join(home, ".mechanical-wings");
        ...
        await fs.writeFile(
          path.join(configDir, "mechanical-wings.json"),
          JSON.stringify(
            {
              plugins: {
                load: {
                ...
                list: [
                  {
                    id: "main",
                    workspace: "~/ws-agent",
                    agentDir: "~/.mechanical-wings/agents/main",
              ...
              channels: {
                whatsapp: {
                  accounts: {
                    personal: {
                      authDir: "~/.mechanical-wings/credentials/wa-personal",
                    },
                  },
                },
              },
        ...
        expect(cfg.agents?.list?.[0]?.agentDir).toBe(
          path.join(home, ".mechanical-wings", "agents", "main"),
        );
        expect(cfg.agents?.list?.[0]?.sandbox?.workspaceRoot).toBe(path.join(home, "sandbox-root"));
        expect(cfg.channels?.whatsapp?.accounts?.personal?.authDir).toBe(
          path.join(home, ".mechanical-wings", "credentials", "wa-personal"),
        );
      });
    });
  });
