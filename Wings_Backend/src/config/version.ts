...
export function parseWingsVersion(raw: string | null | undefined): WingsVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeWingsVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseWingsVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameWingsStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseWingsVersion(a);
  const parsedB = parseWingsVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareWingsVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseWingsVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseWingsVersion(current);
  const parsedTouched = parseWingsVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameWingsStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareWingsVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: WingsVersion): number {
