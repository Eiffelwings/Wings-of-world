...
function makeTempDir() {
  return makeTrackedTempDir("mechanical-wings-plugin-auto-enable", tempDirs);
}

function writePluginManifestFixture(params: { rootDir: string; id: string; channels: string[] }) {
  mkdirSafeDir(params.rootDir);
  fs.writeFileSync(
    path.join(params.rootDir, "mechanical-wings.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function makeRegistry(
  ...
  return {
    plugins: plugins.map((p) => ({
      ...
      manifestPath: `/fake/${p.id}/mechanical-wings.plugin.json`,
    })),
    diagnostics: [],
  };
}
...
describe("applyPluginAutoEnable", () => {
  ...
  it("uses env-scoped catalog metadata for preferOver auto-enable decisions", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        entries: [
          {
            name: "@mechanical-wings/env-secondary",
            "mechanical-wings": {
              channel: {
                id: "env-secondary",
                label: "Env Secondary",
              ...
              install: {
                npmSpec: "@mechanical-wings/env-secondary",
              },
            },
          },
        ],
  ...
  describe("third-party channel plugins (pluginId ≠ channelId)", () => {
    it("uses the plugin manifest id, not the channel id, for plugins.entries", () => {
      // Reproduces: https://github.com/mechanical-wings/mechanical-wings/issues/25261
