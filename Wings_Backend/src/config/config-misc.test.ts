...
import { buildWebSearchProviderConfig, withTempHome, writeWingsConfig } from "./test-helpers.js";
import { WingsSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = WingsSchema.safeParse({
      $schema: "https://wings-of-world.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://wings-of-world.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = WingsSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = WingsSchema.safeParse({
      cron: {
        enabled: true,
