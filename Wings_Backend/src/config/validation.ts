...
import type { WingsConfig, ConfigValidationIssue } from "./types.js";
...
function validateChannelAccessPolicy(params: {
  channelId: string;
  inheritedAllowFrom?: unknown;
  issues: ConfigValidationIssue[];
  path: string;
  value: unknown;
}) {
  const { channelId, inheritedAllowFrom, issues, path, value } = params;
  if (!isRecord(value)) {
    return;
  }
  ...
  const effectiveAllowFrom = Object.prototype.hasOwnProperty.call(value, "allowFrom")
    ? value.allowFrom
    : inheritedAllowFrom;
  const allowFromPath = `${path}.allowFrom`;

  if (dmPolicy === "allowlist" && !hasNonEmptyStringArray(effectiveAllowFrom)) {
    issues.push({
      path: allowFromPath,
      message: `${channelId} allowFrom must include at least one sender when dmPolicy=allowlist`,
    });
    return;
  }

  if (dmPolicy === "open" && !hasWildcardAllowFrom(effectiveAllowFrom)) {
    issues.push({
      path: allowFromPath,
      message: `${channelId} allowFrom must include "*" when dmPolicy=open`,
    });
  }
}
...
function validateIdentityAvatar(config: WingsConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: WingsConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: WingsConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = WingsSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = validated.data as WingsConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; Wings mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: WingsConfig | null | undefined;

  const ensureCompatConfig = (): WingsConfig => {
  ...
  if (config.channels && isRecord(config.channels)) {
    for (const key of Object.keys(config.channels)) {
      ...
      validateChannelAccessPolicy({
        channelId: trimmed,
        issues,
        path: `channels.${trimmed}`,
        value: result.value,
      });
      if (isRecord(result.value) && isRecord(result.value.accounts)) {
        for (const [accountId, accountConfig] of Object.entries(result.value.accounts)) {
          validateChannelAccessPolicy({
            channelId: trimmed,
            inheritedAllowFrom: result.value.allowFrom,
            issues,
            path: `channels.${trimmed}.accounts.${accountId}`,
            value: accountConfig,
          });
        }
      }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Wings config schema.
