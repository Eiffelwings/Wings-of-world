  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.mechanical-wings/logs/mechanical-wings.log" },
        hooks: {
          path: "~/.mechanical-wings/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.mechanical-wings/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".mechanical-wings", "logs", "mechanical-wings.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".mechanical-wings", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".mechanical-wings", "telegram.token"),
      );
      ...
      expect(cfg.agents?.list?.[0]?.agentDir).toBe(path.join(home, ".mechanical-wings", "agents", "main"));
