...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("mechanical-wings-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "mechanical-wings-sandbox-browser-global" },
      agentBrowser: { network: "mechanical-wings-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("mechanical-wings-sandbox-browser-agent");
  });
