...
async function writePluginFixture(params: {
  ...
  await fs.writeFile(
    path.join(params.dir, "mechanical-wings.plugin.json"),
...
describe("config plugin validation", () => {
  ...
  const suiteEnv = () =>
    ({
      ...
      OPENCLAW_STATE_DIR: path.join(suiteHome, ".mechanical-wings"),
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-config-plugin-validation-"));
