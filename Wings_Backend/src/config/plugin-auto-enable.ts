...
function collectModelRefs(cfg: WingsConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: WingsConfig, providerId: string): boolean {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry["mechanical-wings"]) || !isRecord(entry["mechanical-wings"].channel)) {
      continue;
    }
    const channel = entry["mechanical-wings"].channel;
...
function collectCandidateChannelIds(cfg: WingsConfig): string[] {
...
function configMayNeedPluginManifestRegistry(cfg: WingsConfig): boolean {
  const configuredChannels = cfg.channels as Record<string, unknown> | undefined;
  if (!configuredChannels || typeof configuredChannels !== "object") {
    return false;
  }
...
function isPluginExplicitlyDisabled(cfg: WingsConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: WingsConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: WingsConfig, pluginId: string): WingsConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
