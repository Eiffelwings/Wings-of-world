...
describe("multi-agent agentDir validation", () => {
  it("rejects shared agents.list agentDir", async () => {
    const shared = path.join(tmpdir(), "mechanical-wings-shared-agentdir");
    ...
    await withTempHomeConfig(
      {
        agents: {
          list: [
            { id: "a", agentDir: "~/.mechanical-wings/agents/shared/agent" },
            { id: "b", agentDir: "~/.mechanical-wings/agents/shared/agent" },
          ],
        },
