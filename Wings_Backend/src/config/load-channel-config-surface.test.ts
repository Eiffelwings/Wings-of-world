...
describe("loadChannelConfigSurfaceModule", () => {
  it("retries from an isolated package copy when extension-local node_modules is broken", async () => {
    const repoRoot = makeTempRoot("mechanical-wings-config-surface-");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "@mechanical-wings/demo", type: "module" }, null, 2),
      "utf8",
    );
    fs.writeFileSync(
      modulePath,
    ...
    const localZodPath = path.join(packageRoot, "node_modules", "zod");
    try {
      fs.symlinkSync("../../../node_modules/.pnpm/zod@0.0.0/node_modules/zod", localZodPath, "dir");
    } catch (error) {
      if (!isSymlinkUnavailable(error)) {
        throw error;
      }
      fs.mkdirSync(localZodPath, { recursive: true });
      fs.writeFileSync(
        path.join(localZodPath, "package.json"),
        JSON.stringify({
          name: "zod",
          type: "module",
          exports: { ".": "./missing.js" },
