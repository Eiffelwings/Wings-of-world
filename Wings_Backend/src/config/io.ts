...
import type { WingsConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN",
  "WINGS_OF_WORLD_BACKEND_GATEWAY_PASSWORD",
  "WINGS_OF_WORLD_GATEWAY_TOKEN",
  "WINGS_OF_WORLD_GATEWAY_PASSWORD",
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  mechanical-wings config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  mechanical-wings config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: WingsConfig,
  pathSegments: string[],
): { changed: boolean; next: WingsConfig } {
...
function coerceConfig(value: unknown): WingsConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as WingsConfig;
}
...
function stampConfigVersion(cfg: WingsConfig): WingsConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: WingsConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer Wings (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as WingsConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): WingsConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as WingsConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: WingsConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as WingsConfig)
        : cfgToWrite;
...
// module scope. `WINGS_OF_WORLD_BACKEND_CONFIG_PATH` (and legacy friends) are expected to work even
...
let runtimeConfigSnapshot: WingsConfig | null = null;
let runtimeConfigSourceSnapshot: WingsConfig | null = null;
...
export function getRuntimeConfigSnapshot(): WingsConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): WingsConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: WingsConfig;
  candidate: WingsConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: WingsConfig): WingsConfig {
...
export async function readBestEffortConfig(): Promise<WingsConfig> {
...
export async function writeConfigFile(
  ...
  // edits to mechanical-wings.json remain visible (no stale snapshot).
}
