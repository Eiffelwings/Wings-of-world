...
describe("config io observe", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-config-observe-"));
  });
  ...
  async function makeIo(home: string, warn = vi.fn(), error = vi.fn()) {
    ...
    const auditPath = path.join(path.dirname(configPath), "logs", "config-audit.jsonl");
