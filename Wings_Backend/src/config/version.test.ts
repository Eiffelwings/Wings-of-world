...
import {
  compareWingsVersions,
  isSameWingsStableFamily,
  parseWingsVersion,
...
describe("parseWingsVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseWingsVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseWingsVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseWingsVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseWingsVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseWingsVersion("2026.3")).toBeNull();
    expect(parseWingsVersion("latest")).toBeNull();
  });
});

describe("compareWingsVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareWingsVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareWingsVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareWingsVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareWingsVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareWingsVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareWingsVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameWingsStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameWingsStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameWingsStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameWingsStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameWingsStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
