...
describe("usage-format", () => {
  ...
  beforeEach(async () => {
    agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-usage-format-"));
  ...
  it("prefers models.json pricing over mechanical-wings config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          openai: {
  ...
  it("falls back to mechanical-wings config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
          anthropic: {
