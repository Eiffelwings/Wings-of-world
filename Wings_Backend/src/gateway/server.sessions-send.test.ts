...
const { createWingsTools } = await import("../agents/mechanical-wings-tools.js");
...
type SessionSendTool = ReturnType<typeof createWingsTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createWingsTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
