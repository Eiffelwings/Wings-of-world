...
export function resolveHooksConfig(cfg: WingsConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: WingsConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-mechanical-wings-token"] === "string"
      ? req.headers["x-mechanical-wings-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-mechanical-wings-idempotency-key"]) ||
