  ...
  return [
    ...
    JSON.stringify({
      ...
      message: {
        role: "user",
        content:
          'Sender (untrusted metadata):\n```json\n{"label":"mechanical-wings-control-ui"}\n```\n\n[Thu 2026-03-26 16:29 GMT] hi',
      },
    }),
    JSON.stringify({
      type: "assistant",
...
async function withClaudeProjectsDir<T>(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-claude-history-"));
  ...
  it("augments chat history when a session has a claude-cli binding", async () => {
    await withClaudeProjectsDir(async ({ homeDir, sessionId }) => {
      const messages = augmentChatHistoryWithCliSessionImports({
        entry: {
          sessionId: "mechanical-wings-session",
          updatedAt: Date.now(),
          cliSessionBindings: {
            "claude-cli": {
              sessionId,
      ...
      expect(messages[0]).toMatchObject({
        role: "user",
        "__mechanical-wings": { cliSessionId: sessionId },
      });
    });
  });
