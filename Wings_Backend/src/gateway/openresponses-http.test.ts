  ...
  it("handles OpenResponses request parsing and validation", async () => {
    ...
    try {
      ...
      const resMissingAuth = await fetch(`http://127.0.0.1:${port}/v1/responses`, {
        ...
        body: JSON.stringify({ model: "mechanical-wings", input: "hi" }),
      });
      ...
      const resHeader = await postResponses(
        port,
        { model: "mechanical-wings", input: "hi" },
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resModel = await postResponses(port, { model: "mechanical-wings/beta", input: "hi" });
      ...
      mockAgentOnce([{ text: "hello" }]);
      const resDefaultAlias = await postResponses(port, { model: "mechanical-wings/default", input: "hi" });
      ...
      const resChannelHeader = await postResponses(
        port,
        { model: "mechanical-wings", input: "hi" },
        { "x-mechanical-wings-message-channel": "custom-client-channel" },
      );
      ...
      expect(invalidOverrideJson.error?.message).toBe("Invalid `x-mechanical-wings-model`.");
      expect(agentCommand).toHaveBeenCalledTimes(0);
