...
let lastCreateWingsToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/mechanical-wings-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateWingsToolsContext?.agentTo,
          agentThreadId: lastCreateWingsToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createWingsTools: (ctx: Record<string, unknown>) => {
      lastCreateWingsToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateWingsToolsContext = undefined;
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateWingsToolsContext?.allowMediaInvokeCommands).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateWingsToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-mechanical-wings-message-to": "channel:24514",
        "x-mechanical-wings-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
