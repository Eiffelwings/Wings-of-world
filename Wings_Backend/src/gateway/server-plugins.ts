...
const FALLBACK_GATEWAY_CONTEXT_STATE_KEY: unique symbol = Symbol.for(
  "mechanical-wings.fallbackGatewayContextState",
);
...
const PLUGIN_SUBAGENT_POLICY_STATE_KEY: unique symbol = Symbol.for(
  "mechanical-wings.pluginSubagentOverridePolicyState",
);
  ...
  if (!policy?.allowModelOverride) {
    return {
      ...
      reason:
        ...
        "See https://docs.wings-of-world.ai/tools/plugin#runtime-helpers and search for: " +
...
export function loadGatewayPlugins(params: {
  ...
  const pluginRegistry = loadWingsPlugins({
    config: params.cfg,
