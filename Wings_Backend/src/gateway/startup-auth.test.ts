...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: WingsConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: WingsConfig) {
  ...
  async function expectNoTokenGeneration(cfg: WingsConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  function createMissingGatewayTokenSecretRefConfig(): WingsConfig {
    return {
      gateway: {
        auth: {
          mode: "token",
