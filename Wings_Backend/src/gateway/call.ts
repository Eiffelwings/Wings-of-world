...
function resolveGatewayPortValue(config?: WingsConfig, env?: NodeJS.ProcessEnv): number {
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set WINGS_OF_WORLD_BACKEND_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: wings-of-world-backend doctor --fix",
        "Docs: see Wings Of World backend README",
      ].join("\n"),
    );
  }
