...
function cfg(input: Partial<WingsConfig>): WingsConfig {
  return input as WingsConfig;
}
...
async function expectFailClosedOnUnresolvedLocalAuth(config: WingsConfig, path: string) {
  await expect(
    resolveGatewayConnectionAuth({
      config,
