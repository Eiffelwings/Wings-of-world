...
import { type WingsConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: WingsConfig): string[] {
...
export function listAgentsForGateway(cfg: WingsConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: WingsConfig): string {
...
function resolveSessionStoreAgentId(cfg: WingsConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: WingsConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: WingsConfig): GatewaySessionsDefaults {
