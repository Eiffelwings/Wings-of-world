...
import { ensureWingsCliOnPath } from "../infra/path-env.js";
import {
...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("wings-of-world-backend doctor")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `mechanical-wings config set gateway.auth.mode token` and `mechanical-wings config set gateway.auth.token <token>`.",
      );
    }
  }
    ...
    transcriptUnsub = minimalTestGateway
      ...
      : onSessionTranscriptUpdate((update) => {
          ...
          const sessionSnapshot = sessionRow
            ...
            : {};
          const message = attachWingsTranscriptMeta(update.message, {
