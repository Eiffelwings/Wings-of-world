...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.OPENCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.OPENCLAW_STATE_DIR, "mechanical-wings.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.OPENCLAW_STATE_DIR = path.join(tempHome, ".mechanical-wings");
...
function applyGatewaySkipEnv() {
  ...
  process.env.OPENCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "mechanical-wings-test-no-bundled-extensions")
    : "mechanical-wings-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".mechanical-wings-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".mechanical-wings-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".mechanical-wings-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>mechanical-wings-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__mechanicalWingsTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__mechanicalWingsTestConnectChallengeTracked";
