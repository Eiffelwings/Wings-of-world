...
import { createWingsTools } from "../agents/mechanical-wings-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-mechanical-wings-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-mechanical-wings-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-mechanical-wings-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-mechanical-wings-thread-id")?.trim() || undefined;
  ...
  const allTools = createWingsTools({
