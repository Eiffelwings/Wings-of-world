    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "mechanical-wings", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-mechanical-wings-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "mechanical-wings", messages: [{ role: "user", content: "hi" }] },
          {
            "x-mechanical-wings-agent-id": "beta",
            "x-mechanical-wings-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
...

      {
        ...
        expect(json.error?.message).toBe(
          "Invalid `model`. Use `mechanical-wings` or `mechanical-wings/<agentId>`.",
        );
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }

      {
        ...
        const res = await postChatCompletions(
          ...
          { "x-mechanical-wings-model": "openai/" },
        );
        expect(res.status).toBe(400);
        const json = (await res.json()) as { error?: { type?: string; message?: string } };
        expect(json.error?.type).toBe("invalid_request_error");
        expect(json.error?.message).toBe("Invalid `x-mechanical-wings-model`.");
        expect(agentCommand).toHaveBeenCalledTimes(0);
      }
...

      {
        ...
        const choice0 = (json.choices as Array<Record<string, unknown>>)[0] ?? {};
        const msg = (choice0.message as Record<string, unknown> | undefined) ?? {};
        expect(msg.content).toBe("No response from Wings.");
      }
