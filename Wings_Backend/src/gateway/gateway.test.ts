...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "mechanical-wings");
      await fs.mkdir(workspaceDir, { recursive: true });

      const configDir = path.join(tempHome, ".mechanical-wings");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-wizard-home-"));
      process.env.HOME = tempHome;
