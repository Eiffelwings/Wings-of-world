...
export function attachWingsTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record["__mechanical-wings"] && typeof record["__mechanical-wings"] === "object" && !Array.isArray(record["__mechanical-wings"])
      ? (record["__mechanical-wings"] as Record<string, unknown>)
      : {};
  return {
    ...record,
    "__mechanical-wings": {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachWingsTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "mechanical-wings" && model === "delivery-mirror";
