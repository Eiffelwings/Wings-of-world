...
export const OPENCLAW_MODEL_ID = "mechanical-wings";
export const OPENCLAW_DEFAULT_MODEL_ID = "mechanical-wings/default";
...
export function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    getHeader(req, "x-mechanical-wings-agent-id")?.trim() ||
    getHeader(req, "x-mechanical-wings-agent")?.trim() ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  const m =
    raw.match(/^mechanical-wings[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `mechanical-wings` or `mechanical-wings/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-mechanical-wings-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-mechanical-wings-model`." };
  }
...
export function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-mechanical-wings-session-key")?.trim();
  if (explicit) {
    return explicit;
  }
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-mechanical-wings-message-channel")) ??
