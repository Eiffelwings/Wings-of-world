...
function listAgents(cfg: WingsConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: WingsConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<WingsConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: WingsConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: WingsConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: WingsConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<WingsConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  WingsConfig,
  {
    bindingsRef: WingsConfig["bindings"];
    agentsRef: WingsConfig["agents"];
    sessionRef: WingsConfig["session"];
...
function resolveRouteCacheForConfig(cfg: WingsConfig): Map<string, ResolvedAgentRoute> {
