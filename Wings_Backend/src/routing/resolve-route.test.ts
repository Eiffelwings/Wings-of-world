...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: WingsConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: WingsConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/mechanical-wings-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<WingsConfig["bindings"]>[number];
