...
function asConfig(value: unknown): WingsConfig {
...
function createOpenAIFileRuntimeConfig(secretFile: string): WingsConfig {
  return asConfig({
    secrets: {
      providers: {
        default: {
...
describe("secrets runtime snapshot integration", () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          ...
          agentDirs: ["/tmp/mechanical-wings-agent-main"],
          loadAuthStore: () =>
        ...
        expect(
          ensureAuthProfileStore("/tmp/mechanical-wings-agent-main").profiles["openai:default"],
        ).toMatchObject({
          type: "api_key",
          key: "sk-runtime",
        });
  ...
  it("keeps active secrets runtime snapshots resolved after config writes", async () => {
    ...
    await withTempHome("mechanical-wings-secrets-runtime-write-", async (home) => {
  ...
  it("keeps last-known-good runtime snapshot active when refresh fails after a write", async () => {
    ...
    await withTempHome("mechanical-wings-secrets-runtime-refresh-fail-", async (home) => {
      const { secretFile, agentDir } = await createOpenAIFileRuntimeFixture(home);
  ...
  it(
    ...
    async () => {
      await withTempHome("mechanical-wings-secrets-runtime-web-reload-lkg-", async (home) => {
        const prepared = await prepareSecretsRuntimeSnapshot({
          config: asConfig({
            tools: {
              web: {
        ...
        const persistedConfigPath = resolveConfigPathCandidate(process.env, () => home);
        const persistedConfig = JSON.parse(
          await fs.readFile(persistedConfigPath, "utf8"),
        ) as WingsConfig;
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("mechanical-wings-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".mechanical-wings", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".mechanical-wings", "agents", "ops", "agent");
