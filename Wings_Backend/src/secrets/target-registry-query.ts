...
const WINGS_CONFIG_COMPILED_SECRET_TARGETS = COMPILED_SECRET_TARGET_REGISTRY.filter(
  (entry) => entry.configFile === "wings-of-world.json" || entry.configFile === "mechanical-wings.json",
);
...
function buildConfigTargetIdIndex(): Map<string, CompiledTargetRegistryEntry[]> {
  ...
  for (const entry of WINGS_CONFIG_COMPILED_SECRET_TARGETS) {
...
export function discoverConfigSecretTargetsByIds(
  ...
  const discoveryEntries = resolveDiscoveryEntries({
    allowedTargetIds,
    defaultEntries: WINGS_CONFIG_COMPILED_SECRET_TARGETS,
