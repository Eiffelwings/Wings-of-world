...
async function createAuditFixture(): Promise<AuditFixture> {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-secrets-audit-"));
  const stateDir = path.join(rootDir, ".mechanical-wings");
...
describe("secrets audit", () => {
  ...
  it("does not flag non-sensitive routing headers in mechanical-wings config", async () => {
    await writeJsonFile(fixture.configPath, {
      models: {
        providers: {
          openai: {
