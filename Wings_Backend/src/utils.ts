...
export function resolveConfigDir(
  ...
  const newDir = path.join(home, ".wings-of-world-backend");
  const legacyDir = path.join(home, ".mechanical-wings");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome =
    process.env.WINGS_OF_WORLD_BACKEND_HOME?.trim() ||
    process.env.WINGS_OF_WORLD_HOME?.trim() ||
    process.env.OPENCLAW_HOME?.trim();
  if (explicitHome) {
    const prefix = process.env.WINGS_OF_WORLD_BACKEND_HOME?.trim()
      ? "$WINGS_OF_WORLD_BACKEND_HOME"
      : process.env.WINGS_OF_WORLD_HOME?.trim()
        ? "$WINGS_OF_WORLD_HOME"
        : "$OPENCLAW_HOME";
    return { home, prefix };
...
// Configuration root; can be overridden via WINGS_OF_WORLD_BACKEND_STATE_DIR
// or the legacy OPENCLAW_STATE_DIR compatibility variable.
