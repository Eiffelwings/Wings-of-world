...
describe("resolveInitialTuiAgentId", () => {
  const cfg: WingsConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/mechanical-wings" },
        { id: "ops", workspace: "/tmp/mechanical-wings/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/mechanical-wings/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run mechanical-wings devices list");
    expect(state.pairingHint).toContain("mechanical-wings devices list");
  });
