...
export async function finalizeSetupWizard(
    ...
    if (gatewayProbe.ok) {
      try {
        ...
        await prompter.note(
          [
            "Docs:",
            "https://docs.wings-of-world.ai/gateway/health",
            "https://docs.wings-of-world.ai/gateway/troubleshooting",
          ].join("\n"),
          "Health check help",
        );
      }
      ...
      await prompter.note(
        [
          ...
          `Start now: ${formatCliCommand("wings-of-world-backend gateway run")}`,
          `Or rerun with: ${formatCliCommand("mechanical-wings onboard --install-daemon")}`,
          `Or skip this probe next time: ${formatCliCommand("mechanical-wings onboard --skip-health")}`,
        ].join("\n"),
        "Gateway",
      );
    }
  ...
  await prompter.note(
    [
      ...
      gatewayStatusLine,
      "Docs: https://docs.wings-of-world.ai/web/control-ui",
    ]
      .filter(Boolean)
      .join("\n"),
    "Control UI",
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.wings-of-world-backend/wings-of-world.json (gateway.auth.token) or WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN.",
        `View token: ${formatCliCommand("mechanical-wings config get gateway.auth.token")}`,
        `Generate token: ${formatCliCommand("wings-of-world-backend doctor --generate-gateway-token")}`,
        "Web UI keeps dashboard URL tokens in memory for the current tab and strips them from the URL after load.",
        `Open the dashboard anytime: ${formatCliCommand("mechanical-wings dashboard --no-open")}`,
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control Wings."
            : "Copy/paste this URL in a browser on this machine to control Wings.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("mechanical-wings dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    [
      ...
      "Docs: https://docs.wings-of-world.ai/concepts/agent-workspace",
    ].join("\n"),
    "Workspace backup",
  );

  await prompter.note(
    "Running agents on your computer is risky — harden your setup: https://docs.wings-of-world.ai/security",
    "Security",
  );
    ...
    if (!entry) {
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("mechanical-wings configure --section web")}`,
          "",
          "Docs: https://docs.wings-of-world.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `  ${formatCliCommand("mechanical-wings configure --section web")}`,
          "",
          `Get your key at: ${entry?.signupUrl ?? "https://docs.wings-of-world.ai/tools/web"}`,
          "Docs: https://docs.wings-of-world.ai/tools/web",
        ].join("\n"),
        "Web search",
      );
      ...
      await prompter.note(
        [
          ...
          `Re-enable: ${formatCliCommand("mechanical-wings configure --section web")}`,
          "",
          "Docs: https://docs.wings-of-world.ai/tools/web",
        ].join("\n"),
        "Web search",
  ...
  await prompter.note(
    'What now: https://wings-of-world.ai/showcase ("What People Are Building").',
    "What now",
  );

  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control Wings."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control Wings.",
  );
