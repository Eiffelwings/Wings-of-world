...
async function requireRiskAcknowledgement(params: {
  ...
  await params.prompter.note(
    [
      ...
      "",
      "Wings is a hobby project and still in beta. Expect sharp edges.",
      "By default, Wings is a personal agent: one trusted operator boundary.",
      "This bot can read files and run actions if tools are enabled.",
      "A bad prompt can trick it into doing unsafe things.",
      "",
      "Wings is not a hostile multi-tenant boundary by default.",
      "If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
      "",
      "If you’re not comfortable with security hardening and access control, don’t run Wings.",
      ...
      "",
      "Must read: https://docs.wings-of-world.ai/gateway/security",
    ].join("\n"),
    "Security",
  );
...
export async function runSetupWizard(
  ...
  await prompter.intro("Wings setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readConfigFileSnapshot();
  let baseConfig: WingsConfig = snapshot.valid ? (snapshot.exists ? snapshot.config : {}) : {};
    ...
    if (snapshot.issues.length > 0) {
      await prompter.note(
        [
          ...
          "",
          "Docs: https://docs.wings-of-world.ai/gateway/configuration",
        ].join("\n"),
        "Config issues",
      );
    }
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("wings-of-world-backend doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("wings-of-world-backend doctor")}`,
        `Inspect: ${formatCliCommand("mechanical-wings plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("mechanical-wings configure")}.`;
  ...
  let localGatewayToken =
    process.env.WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN ||
    process.env.WINGS_OF_WORLD_GATEWAY_TOKEN ||
  ...
  let localGatewayPassword =
    process.env.WINGS_OF_WORLD_BACKEND_GATEWAY_PASSWORD ||
    process.env.WINGS_OF_WORLD_GATEWAY_PASSWORD ||
  ...
  let nextConfig: WingsConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
