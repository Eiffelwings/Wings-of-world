...
async function ensureWingsHooks(manifest: HookPackageManifest) {
  const manifestKeys = [MANIFEST_KEY, ...LEGACY_MANIFEST_KEYS] as const;
  for (const manifestKey of manifestKeys) {
    const hooks = manifest[manifestKey]?.hooks;
    if (!Array.isArray(hooks)) {
      continue;
    }
    const list = hooks.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
    if (list.length === 0) {
      throw new Error(`package.json ${manifestKey}.hooks is empty`);
    }
    return { hooks: list, manifestKey };
  }
  throw new Error(`package.json missing ${MANIFEST_KEY}.hooks`);
}

function formatHookManifestPath(manifestKey: string): string {
  return `${manifestKey}.hooks`;
}

function resolveHookEntries(params: {
  entry: string;
  manifestKey: string;
  packageDir: string;
}): { ok: true; hookDir: string } | { ok: false; error: string } {
  const hookDir = path.resolve(params.packageDir, params.entry);
  if (!path.relative(params.packageDir, hookDir) || hookDir === params.packageDir) {
    return { ok: true, hookDir };
  }
  if (!path.relative(params.packageDir, hookDir).startsWith("..")) {
    return { ok: true, hookDir };
  }
  return {
    ok: false,
    error: `${formatHookManifestPath(params.manifestKey)} entry escapes package directory: ${params.entry}`,
  };
}

function formatHookEntryOutsidePackageError(manifestKey: string, entry: string): string {
  return `${formatHookManifestPath(manifestKey)} entry resolves outside package directory: ${entry}`;
}

async function validateManifestHooks(manifest: HookPackageManifest) {
  const resolved = await ensureWingsHooks(manifest);
  const list = resolved.hooks;
  if (list.length === 0) {
    throw new Error(`package.json ${resolved.manifestKey}.hooks is empty`);
  }
  return resolved;
}
...
async function installHookPackageFromDir(
  ...
  try {
    const resolved = await validateManifestHooks(manifest);
    hookEntries = resolved.hooks;
  ...
  for (const entry of hookEntries) {
    const resolvedEntry = resolveHookEntries({
      entry,
      manifestKey,
    ...
    const hookDir = resolvedEntry.hookDir;
    ...
    if (
      ...
      return {
        ok: false,
        error: formatHookEntryOutsidePackageError(manifestKey, entry),
      };
    }
...
export async function installHooksFromArchive(
  ...
  return await runtime.withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "mechanical-wings-hook-",
    timeoutMs,
    logger,
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await runtime.installFromValidatedNpmSpecArchive({
    tempDirPrefix: "mechanical-wings-hook-pack-",
    spec,
    timeoutMs,
