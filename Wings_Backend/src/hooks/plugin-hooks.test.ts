...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-plugin-hooks-"));
  });
  ...
  function createConfig(enabled: boolean): WingsConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entries[0]?.hook.source).toBe("mechanical-wings-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Wings hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".mechanical-wings", "extensions", "claude-bundle");
