...
This directory contains hooks that ship with Wings🪽. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.wings-of-world-backend/workspace`)
...
**Output**: `~/.wings-of-world-backend/logs/commands.log`
...
- **Managed hooks**: `~/.wings-of-world-backend/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.wings-of-world-backend/wings-of-world.json`:
...
2. Restart gateway: `pkill -9 -f 'wings-of-world-backend.*gateway' && pnpm wings-of-world-backend gateway`
3. Enable the hook: `mechanical-wings hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.wings-of-world.ai/automation/hooks
