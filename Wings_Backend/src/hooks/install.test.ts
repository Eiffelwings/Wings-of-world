...
const fixtureRoot = path.join(process.cwd(), ".tmp", `mechanical-wings-hook-install-${randomUUID()}`);
...
function writeHookPackManifest(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      name: "@mechanical-wings/test-hooks",
      version: "0.0.1",
      "wings-of-world": { hooks: params.hooks },
    ...
    const cases = [
      {
        hooks: ["../outside"],
        setupLink: false,
        expected: "wings-of-world.hooks entry escapes package directory",
      },
      {
        hooks: ["./linked"],
        setupLink: true,
        expected: "wings-of-world.hooks entry resolves outside package directory",
      },
    ] as const;
...
describe("installHooksFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
      ...
      if (argv[0] === "npm" && argv[1] === "pack") {
        ...
        return {
          ...
          stdout: JSON.stringify([
            {
              id: "@mechanical-wings/test-hooks@0.0.1",
              name: "@mechanical-wings/test-hooks",
              version: "0.0.1",
              filename: packedName,
    ...
    const result = await installHooksFromNpmSpec({
      spec: "@mechanical-wings/test-hooks@0.0.1",
      hooksDir,
      logger: { info: () => {}, warn: () => {} },
    });
    expect(result.ok).toBe(true);
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@mechanical-wings/test-hooks@0.0.1");
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@mechanical-wings/test-hooks@0.0.1",
    });
  ...
  it("rejects invalid npm spec shapes", async () => {
    ...
    const run = vi.mocked(runCommandWithTimeout);
    mockNpmPackMetadataResult(run, {
      id: "@mechanical-wings/test-hooks@0.0.2-beta.1",
      name: "@mechanical-wings/test-hooks",
    ...
    if (!result.ok) {
      ...
      expect(result.error).toContain('"@mechanical-wings/test-hooks@beta"');
    }
  });
});
