...
import { type WingsConfig, readConfigFileSnapshot } from "../config/config.js";
...
export async function requireValidConfigSnapshot(
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    runtime.error(`Fix the config or run ${formatCliCommand("wings-of-world-backend doctor")}.`);
    runtime.exit(1);
    return null;
  }
