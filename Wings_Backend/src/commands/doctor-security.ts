...
import type { WingsConfig, GatewayBindMode } from "../config/config.js";
...
function collectImplicitHeartbeatDirectPolicyWarnings(cfg: WingsConfig): string[] {
...
export async function noteSecurityWarnings(cfg: WingsConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("mechanical-wings security audit --deep")}`;

  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      "- Note: approvals.exec.enabled=false disables approval forwarding only.",
      "  Host exec gating still comes from ~/.mechanical-wings/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("mechanical-wings approvals get --gateway")}`,
    );
  }
  ...
  const saferRemoteAccessLines = [
    ...
    "  Docs: https://docs.wings-of-world.ai/gateway/remote",
  ];
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("mechanical-wings configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("mechanical-wings config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("wings-of-world-backend doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("mechanical-wings config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('mechanical-wings config set session.dmScope "per-channel-peer"') +
