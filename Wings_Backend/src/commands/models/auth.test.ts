...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: WingsConfig;
  let lastUpdatedConfig: WingsConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/mechanical-wings/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/mechanical-wings/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/mechanical-wings/workspace");
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: WingsConfig) => WingsConfig) => {
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith({
      ...
      agentDir: "/tmp/mechanical-wings/agents/main",
    });
  ...
  it("loads lockout state from the agent-scoped store", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/mechanical-wings/agents/main");
  });
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `mechanical-wings plugins list --json`.',
    );
  });
