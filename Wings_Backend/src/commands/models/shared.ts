...
export async function loadValidConfigOrThrow(): Promise<WingsConfig> {
...
export async function updateConfig(
  mutator: (cfg: WingsConfig) => WingsConfig,
): Promise<WingsConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: WingsConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: WingsConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("wings-of-world-backend agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
