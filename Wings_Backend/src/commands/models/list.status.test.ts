...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveWingsAgentDir: vi.fn().mockReturnValue("/tmp/mechanical-wings-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/mechanical-wings-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/mechanical-wings-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/mechanical-wings-dev/mechanical-wings.json",
    }),
...
async function loadFreshModelsStatusCommandModuleForTest() {
  ...
  vi.doMock("../../agents/agent-paths.js", () => ({
    resolveWingsAgentDir: mocks.resolveWingsAgentDir,
  }));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/mechanical-wings-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveWingsAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.configPath).toBe("/tmp/mechanical-wings-dev/mechanical-wings.json");
    expect(payload.auth.storePath).toBe("/tmp/mechanical-wings-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/mechanical-wings-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/mechanical-wings-agent-custom");
