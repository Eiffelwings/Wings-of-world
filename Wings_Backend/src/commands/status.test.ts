...
vi.mock("../infra/mechanical-wings-root.js", () => ({
  resolveWingsPackageRoot: vi.fn().mockResolvedValue("/tmp/mechanical-wings"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/mechanical-wings/pnpm-lock.yaml",
      markerPath: "/tmp/mechanical-wings/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.mechanical-wings.gateway.plist",
    }),
  }),
...
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.mechanical-wings.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("mechanical-wings status --all") ||
          line.includes("mechanical-wings --profile isolated status --all"),
      ),
    ).toBe(true);
  });
