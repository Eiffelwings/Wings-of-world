...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".mechanical-wings", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: WingsConfig): boolean {
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  const tokenEntries = [
    ["WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN", await getenv("WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN")],
    ["WINGS_OF_WORLD_GATEWAY_TOKEN", await getenv("WINGS_OF_WORLD_GATEWAY_TOKEN")],
    ["OPENCLAW_GATEWAY_TOKEN", await getenv("OPENCLAW_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    [
      "WINGS_OF_WORLD_BACKEND_GATEWAY_PASSWORD",
      await getenv("WINGS_OF_WORLD_BACKEND_GATEWAY_PASSWORD"),
    ],
    ["WINGS_OF_WORLD_GATEWAY_PASSWORD", await getenv("WINGS_OF_WORLD_GATEWAY_PASSWORD")],
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "WINGS_OF_WORLD_BACKEND_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
