...
import { WingsSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: WingsConfig): {
  config: WingsConfig;
  removed: string[];
} {
  const parsed = WingsSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: WingsConfig): void {
