...
export function createStatusScanSharedMocks(configPathLabel: string) {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/mechanical-wings-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = WingsConfig>(
  overrides: T = {} as T,
): WingsConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): WingsConfig {
