...
describe("detectLinuxSdBackedStateDir", () => {
  it("detects state dir on mmc-backed mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/pi/.mechanical-wings", {
      platform: "linux",
      mountInfo,
    });

    expect(result).toEqual({
      path: "/home/pi/.mechanical-wings",
      mountPoint: "/",
      fsType: "ext4",
  ...
  it("returns null for non-mmc devices", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/home/user/.mechanical-wings", {
      platform: "linux",
      mountInfo,
    });
  ...
  it("resolves /dev/disk aliases to mmc devices", () => {
    ...
    expect(result).toEqual({
      path: "/home/user/.mechanical-wings",
      mountPoint: "/",
      fsType: "ext4",
  ...
  it("uses resolved state path to select mount", () => {
    ...
    const result = detectLinuxSdBackedStateDir("/tmp/mechanical-wings-state", {
      platform: "linux",
      mountInfo,
      resolveRealPath: () => "/mnt/slow/mechanical-wings/.mechanical-wings",
    });

    expect(result).toEqual({
      path: "/mnt/slow/mechanical-wings/.mechanical-wings",
      mountPoint: "/mnt/slow",
      fsType: "ext4",
  ...
  it("returns null outside linux", () => {
    ...
    const result = detectLinuxSdBackedStateDir(path.join("/Users", "tester", ".mechanical-wings"), {
      platform: "darwin",
      mountInfo,
    });
  ...
  it("escapes decoded mountinfo control characters in warning output", () => {
    ...
    const stateDir = `${mountRoot}/.mechanical-wings`;
