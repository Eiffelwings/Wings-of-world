...
describe("readServiceStatusSummary", () => {
  it("marks Wings-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["mechanical-wings", "gateway", "run"] })),
    ...
    expect(summary.managedByWings).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByWings).toBe(false);
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["mechanical-wings", "gateway", "run", "--port", "18789"],
