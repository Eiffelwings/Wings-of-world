...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "Wings", ".mechanical-wings");
  ...
  it("detects cloud-synced target when state dir resolves via symlink", () => {
    const symlinkPath = "/tmp/mechanical-wings-state";
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".mechanical-wings");
  ...
  it("anchors cloud detection to OS homedir when OPENCLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".mechanical-wings");
    const originalWingsHome = process.env.OPENCLAW_HOME;
    process.env.OPENCLAW_HOME = "/tmp/mechanical-wings-home-override";
    ...
    try {
      ...
      if (originalWingsHome === undefined) {
