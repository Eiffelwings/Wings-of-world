...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".mechanical-wings"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for WINGS_OF_WORLD_BACKEND_STATE_DIR.",
  ].join("\n");
}
...
function shouldRequireOAuthDir(cfg: WingsConfig, env: NodeJS.ProcessEnv): boolean {
  if (
    env.WINGS_OF_WORLD_BACKEND_OAUTH_DIR?.trim() ||
    env.WINGS_OF_WORLD_OAUTH_DIR?.trim() ||
...
export async function noteStateIntegrity(
  ...
  const defaultStateDir = path.join(homedir(), ".mechanical-wings");
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.wings-of-world-backend).",
        `  Set locally: WINGS_OF_WORLD_BACKEND_STATE_DIR=~/.wings-of-world-backend ${formatCliCommand("wings-of-world-backend doctor")}`,
      ].join("\n"),
    );
  }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`mechanical-wings sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`mechanical-wings sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`mechanical-wings sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
...
export function noteWorkspaceBackupTip(workspaceDir: string) {
  ...
  note(
    [
      ...
      "- Keep ~/.mechanical-wings out of git; it contains credentials and session history.",
