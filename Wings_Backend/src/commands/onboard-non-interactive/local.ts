...
export async function runNonInteractiveLocalSetup(params: {
        ...
        hints:
          daemonInstall.skippedReason === "systemd-user-unavailable"
            ? [
                ...
                "If your auth profile uses env-backed refs, keep those env vars set in the shell that runs `wings-of-world-backend gateway run` or `wings-of-world-backend agent --local`.",
              ]
            : [`Run \`${formatCliCommand("wings-of-world-backend gateway status --deep")}\` for more detail.`],
      });
      runtime.exit(1);
      return;
    }
      ...
      logNonInteractiveOnboardingFailure({
        ...
        hints: !opts.installDaemon
          ? [
              ...
              `Fix: start \`${formatCliCommand("wings-of-world-backend gateway run")}\`, re-run with \`--install-daemon\`, or use \`--skip-health\`.`,
  ...
  if (!opts.json) {
    runtime.log(
      `Tip: run \`${formatCliCommand("mechanical-wings configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.wings-of-world.ai/tools/web`,
    );
  }
}
