...
const SEARCH_TOOL = "https://docs.wings-of-world.ai/mcp.SearchWings";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.wings-of-world.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('wings-of-world-backend docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.wings-of-world.ai/");
      runtime.log(`Search: ${formatCliCommand('wings-of-world-backend docs "your query"')}`);
    }
    return;
  }
