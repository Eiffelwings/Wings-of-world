...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN",
          placeholder: "WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like WINGS_OF_WORLD_BACKEND_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. Wings will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: Wings trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.wings-of-world.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
