...
describe("doctor telegram provider warnings", () => {
  beforeEach(() => {
    ...
    listTelegramAccountIdsMock.mockReset().mockImplementation((cfg: WingsConfig) => {
    ...
    inspectTelegramAccountMock
      .mockReset()
      .mockImplementation((_params: { cfg: WingsConfig; accountId: string }) => ({
        enabled: true,
        tokenStatus: "configured",
      }));
    resolveTelegramAccountMock
      .mockReset()
      .mockImplementation((params: { cfg: WingsConfig; accountId?: string | null }) => {
