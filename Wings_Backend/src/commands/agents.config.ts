...
function resolveAgentName(cfg: WingsConfig, agentId: string) {
...
function resolveAgentModel(cfg: WingsConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: WingsConfig): AgentSummary[] {
