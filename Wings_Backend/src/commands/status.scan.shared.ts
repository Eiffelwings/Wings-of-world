...
export function hasExplicitMemorySearchConfig(cfg: WingsConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: WingsConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: WingsConfig, agentId: string) => { store: { path: string } } | null;
