...
function createPerplexityConfig(apiKey: string, enabled?: boolean): WingsConfig {
  return {
    tools: {
      web: {
        search: {
...
function pluginWebSearchApiKey(config: WingsConfig, pluginId: string): unknown {
  const entry = (
...
function createDisabledFirecrawlConfig(apiKey?: string): WingsConfig {
  return {
    tools: {
      web: {
        search: {
...
function readFirecrawlPluginApiKey(config: WingsConfig): string | undefined {
...
async function runQuickstartPerplexitySetup(
  apiKey: string,
  enabled?: boolean,
): Promise<{ result: WingsConfig; prompter: WizardPrompter }> {
  const cfg = createPerplexityConfig(apiKey, enabled);
