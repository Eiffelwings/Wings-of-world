...
const TEST_ARCHIVE_ROOT = "2026-03-09T00-00-00.000Z-mechanical-wings-backup";
...
function createBackupManifest(assetArchivePath: string) {
  return {
    ...
    assets: [
      {
        kind: "state",
        sourcePath: "/tmp/.mechanical-wings",
...
describe("backupVerifyCommand", () => {
  ...
  async function resetTempHome() {
    ...
    await fs.mkdir(path.join(tempHome.home, ".mechanical-wings"), { recursive: true });
    delete process.env.OPENCLAW_CONFIG_PATH;
  }

  beforeAll(async () => {
    tempHome = await createTempHomeEnv("mechanical-wings-backup-verify-test-");
  });
  ...
  it("verifies an archive created by backup create", async () => {
    const stateDir = path.join(tempHome.home, ".mechanical-wings");
    const archiveDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-backup-verify-out-"));
    try {
      await fs.writeFile(path.join(stateDir, "mechanical-wings.json"), JSON.stringify({}), "utf8");
  ...
  it("fails when the archive does not contain a manifest", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-backup-no-manifest-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
  ...
  it("fails when the manifest references a missing asset payload", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-backup-missing-asset-"));
    const archivePath = path.join(tempDir, "broken.tar.gz");
    try {
      const rootName = "2026-03-09T00-00-00.000Z-mechanical-wings-backup";
        ...
        assets: [
          {
            kind: "state",
            sourcePath: "/tmp/.mechanical-wings",
            archivePath: `${rootName}/payload/posix/tmp/.mechanical-wings`,
          },
        ],
      };
      await fs.writeFile(
  ...
  it("fails when archive paths contain traversal segments", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "mechanical-wings-backup-traversal-",
  ...
  it("fails when archive paths contain backslashes", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "mechanical-wings-backup-backslash-",
  ...
  it("ignores payload manifest.json files when locating the backup manifest", async () => {
    const stateDir = path.join(tempHome.home, ".mechanical-wings");
    const externalWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-workspace-"));
  ...
  it("fails when the archive contains duplicate root manifest entries", async () => {
    const payloadArchivePath = `${TEST_ARCHIVE_ROOT}/payload/posix/tmp/.mechanical-wings/payload.txt`;
    await withBrokenArchiveFixture(
      {
        tempPrefix: "mechanical-wings-backup-duplicate-manifest-",
  ...
  it("fails when the archive contains duplicate payload entries", async () => {
    ...
    await withBrokenArchiveFixture(
      {
        tempPrefix: "mechanical-wings-backup-duplicate-payload-",
        manifestAssetArchivePath: payloadArchivePath,
        payloads: [
