...
import { readConfigFileSnapshot, writeConfigFile, type WingsConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("mechanical-wings config set gateway.auth.mode token")} or ${formatCliCommand("mechanical-wings config set gateway.auth.mode password")}.`,
  ].join(" ");
}
