...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByWings ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByWings ? "installed · " : "";
  ...
  runtime.log(theme.heading("Wings status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`mechanical-wings devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
    runtime.log(theme.muted(`Fallback: ${formatCliCommand("mechanical-wings devices approve --latest")}`));
    runtime.log(theme.muted(`Inspect: ${formatCliCommand("mechanical-wings devices list")}`));
  }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("mechanical-wings security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("mechanical-wings security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.wings-of-world.ai/faq");
  runtime.log("Troubleshooting: https://docs.wings-of-world.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("mechanical-wings status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("mechanical-wings logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("mechanical-wings status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("wings-of-world-backend gateway probe")}`);
  }
}
