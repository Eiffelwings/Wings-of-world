...
const ensureWingsModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveWingsAgentDir = vi.fn().mockReturnValue("/tmp/mechanical-wings-agent");
...
vi.mock("../config/config.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    CONFIG_PATH: "/tmp/mechanical-wings.json",
    STATE_DIR: "/tmp/mechanical-wings-state",
    loadConfig,
...
beforeEach(() => {
  ...
  ensureWingsModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureWingsModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureWingsModelsJson).toHaveBeenCalled();
    expect(ensureWingsModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
