 ...
 * Runs as part of `wings-of-world-backend doctor` — config-only, no network calls.
 */
  ...
  if (resolved.provider !== "auto") {
        ...
        if (opts?.gatewayMemoryProbe?.checked && !opts.gatewayMemoryProbe.ready) {
          ...
          note(
            [
              ...
              "",
              `Verify: ${formatCliCommand("mechanical-wings memory status --deep")}`,
            ]
              .filter(Boolean)
              .join("\n"),
            "Memory search",
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`mechanical-wings config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("mechanical-wings configure --section model")}`,
        `- To disable: ${formatCliCommand("mechanical-wings config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("mechanical-wings memory status --deep")}`,
      ].join("\n"),
      "Memory search",
