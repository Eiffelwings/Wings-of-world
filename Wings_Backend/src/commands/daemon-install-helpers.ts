...
export async function buildGatewayInstallPlan(params: {
  ...
  //   1. ~/.wings-of-world-backend/.env file vars (lowest - user secrets / fallback keys)
  //   2. Config env vars (wings-of-world.json env.vars + inline keys)
  //   3. Auth-profile env refs (credential store -> env var lookups)
  //   4. Service environment (HOME, PATH, WINGS_OF_WORLD_* / OPENCLAW_* - highest)
  return {
    programArguments,
    workingDirectory,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("wings-of-world-backend gateway install")}\` after fixing the error.`;
}
