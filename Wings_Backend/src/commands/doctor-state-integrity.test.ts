...
function setupSessionState(cfg: WingsConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
async function runStateIntegrity(cfg: WingsConfig) {
...
async function runStateIntegrityText(cfg: WingsConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-doctor-state-integrity-"));
  ...
  it("prints mechanical-wings-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: WingsConfig = {};
    ...
    expect(text).toMatch(/mechanical-wings sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/mechanical-wings sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /mechanical-wings sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
