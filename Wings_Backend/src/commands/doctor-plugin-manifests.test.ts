...
function makeTempDir() {
  return makeTrackedTempDir("mechanical-wings-doctor-plugin-manifests", tempDirs);
}
...
describe("doctor plugin manifest legacy contract repair", () => {
  ...
  it("rewrites legacy top-level capability keys into contracts", async () => {
    ...
    const next = JSON.parse(fs.readFileSync(path.join(root, "mechanical-wings.plugin.json"), "utf-8")) as {
