...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    "mechanical-wings": process.env.OPENCLAW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.OPENCLAW_STATE_DIR).toBe(snapshot["mechanical-wings"]);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips OPENCLAW_STATE_DIR", () => {
    ...
    setStateDirEnv("/tmp/mechanical-wings-state-dir-test");
    expect(process.env.OPENCLAW_STATE_DIR).toBe("/tmp/mechanical-wings-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("mechanical-wings-state-dir-env-", async ({ tempRoot, stateDir }) => {
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("mechanical-wings-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
