...
This document describes how the Wings🪽 ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`mechanical-wings acp` exposes an ACP agent over stdio and forwards prompts to a running
Wings🪽 Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing Wings🪽 Gateway
...
- ACP client spawns `mechanical-wings acp` and speaks ACP messages over stdio.
...
mechanical-wings acp --reset-session
```
...
`mechanical-wings acp` resolves the Gateway URL and auth from CLI flags or config:
