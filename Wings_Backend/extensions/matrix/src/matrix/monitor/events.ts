...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another Wings runtime is using the same account. " +
    `Check 'mechanical-wings matrix verify status --verbose --account ${accountId}' and 'mechanical-wings matrix devices list --account ${accountId}'.`
  );
}
