...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleWingsDevices.length > 0) {
      params.logger.warn(
        `matrix: stale Wings devices detected for ${params.auth.userId}: ${deviceHealth.staleWingsDevices.map((device) => device.deviceId).join(", ")}. Run 'mechanical-wings matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
