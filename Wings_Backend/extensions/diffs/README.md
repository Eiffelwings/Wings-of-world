# @mechanical-wings/diffs

Read-only diff viewer plugin for **Wings🪽** agents.
...
Set plugin-wide defaults in `~/.mechanical-wings/mechanical-wings.json`:
...
Before:
Wings🪽 supports plugins.

After:
Wings🪽 supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/mechanical-wings-diffs`).
