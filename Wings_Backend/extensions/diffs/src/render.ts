...
function renderDiffCard(payload: DiffViewerPayload): string {
  return `<section class="oc-diff-card">
    <diffs-container class="oc-diff-host" data-mechanical-wings-diff-host>
      <template shadowrootmode="open">${payload.prerenderedHTML}</template>
    </diffs-container>
    <script type="application/json" data-mechanical-wings-diff-payload>${escapeJsonScript(payload)}</script>
  </section>`;
}
...
<html lang="en">
  <head>
    ...
    <style>
      * {
      ...
      [data-mechanical-wings-diff-root] {
        display: grid;
        gap: 18px;
      }
  ...
  <body data-theme="${params.theme}">
    <main class="oc-frame" data-render-mode="${params.runtimeMode}">
      <div data-mechanical-wings-diff-root>
        ${params.bodyHtml}
      </div>
    </main>
