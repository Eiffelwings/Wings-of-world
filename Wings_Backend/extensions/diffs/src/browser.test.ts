...
import type { WingsPluginApi, WingsPluginToolContext } from "../api.js";
...
describe("PlaywrightDiffScreenshotter", () => {
  ...
  beforeEach(async () => {
    vi.useFakeTimers();
    ({ rootDir, cleanup: cleanupRootDir } = await createTempDiffRoot("mechanical-wings-diffs-browser-"));
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<WingsPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: WingsPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<WingsPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as WingsPluginApi);
