...
import type { ChannelSetupDmPolicy } from "mechanical-wings/plugin-sdk/setup";
import type { ChannelSetupWizard } from "mechanical-wings/plugin-sdk/setup";
import { formatDocsLink } from "mechanical-wings/plugin-sdk/setup";
import type { WizardPrompter } from "mechanical-wings/plugin-sdk/setup";
...
export const ircSetupWizard: ChannelSetupWizard = {
  ...
  textInputs: [
    ...
    },
    {
      ...
      initialValue: ({ cfg, accountId }) =>
        resolveIrcAccount({ cfg: cfg as CoreConfig, accountId }).config.realname || "Wings",
      validate: ({ value }) => (String(value ?? "").trim() ? undefined : "Required"),
      normalizeValue: ({ value }) => String(value).trim(),
      applySet: async ({ cfg, accountId, value }) =>
        updateIrcAccountConfig(cfg as CoreConfig, accountId, {
    ...
    },
    {
      ...
      placeholder: "#mechanical-wings, #ops",
      required: false,
  ...
  completionNote: {
    ...
    lines: [
      ...
      "Command: mechanical-wings channels status --probe",
      `Docs: ${formatDocsLink("/channels/irc", "channels/irc")}`,
    ],
  },
