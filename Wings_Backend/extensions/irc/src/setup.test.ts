...
function buildAccount(): ResolvedIrcAccount {
  return {
    ...
    port: 6697,
    tls: true,
    nick: "mechanical-wings",
    username: "mechanical-wings",
    realname: "Wings",
    password: "",
    passwordSource: "none",
...
describe("irc setup", () => {
  ...
  it("normalizes allowlist groups and handles non-allowlist policies", () => {
    ...
    expect(
      setIrcGroupAccess(
        ...
        "default",
        "allowlist",
        ["mechanical-wings", "#ops", "mechanical-wings", "*"],
        (raw) => {
  ...
  it("validates required input and applies normalized account config", () => {
    ...
    expect(
      validateInput!({
        input: { host: "", nick: "mechanical-wings" },
      } as never),
    ...
    expect(
      validateInput!({
        input: { host: "irc.libera.chat", nick: "mechanical-wings" },
      } as never),
    ).toBeNull();
  ...
  it("configures host and nick via setup prompts", async () => {
    ...
    expect(result.cfg.channels?.irc?.nick).toBe("mechanical-wings-bot");
    expect(result.cfg.channels?.irc?.tls).toBe(true);
    expect(result.cfg.channels?.irc?.channels).toEqual(["#mechanical-wings", "#ops"]);
    expect(result.cfg.channels?.irc?.groupPolicy).toBe("allowlist");
    expect(Object.keys(result.cfg.channels?.irc?.groups ?? {})).toEqual(["#mechanical-wings", "#ops"]);
  });
