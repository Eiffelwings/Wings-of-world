...
describe("mattermost monitor slash", () => {
  ...
  it("registers deduped default and native skill commands across teams", async () => {
    ...
    resolveCallbackUrl.mockReturnValue("https://mechanical-wings.test/slash");
    ...
    expect(registerSlashCommands.mock.calls[0]?.[0]).toMatchObject({
      ...
      callbackUrl: "https://mechanical-wings.test/slash",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "mattermost: slash commands registered (2 commands across 2 teams, callback=https://mechanical-wings.test/slash)",
    );
  });
