...
import type { WingsPluginApi, WingsPluginToolContext } from "../runtime-api.js";
import {
...
function fakeApi(overrides: Partial<WingsPluginApi> = {}): WingsPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<WingsPluginToolContext> = {}): WingsPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  beforeAll(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-lobster-plugin-"));
  });
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: WingsPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
...
describe("resolveWindowsLobsterSpawn", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-lobster-win-spawn-"));
    setProcessPlatform("win32");
  });
