...
import {
  ...
  resolveStateDir,
  type WingsConfig,
} from "mechanical-wings/plugin-sdk/memory-core-host-runtime-core";
import {
  listMemoryFiles,
  normalizeExtraMemoryPaths,
} from "mechanical-wings/plugin-sdk/memory-core-host-runtime-files";
...
function resolveAgent(cfg: WingsConfig, agent?: string) {
...
function resolveAgentIds(cfg: WingsConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
