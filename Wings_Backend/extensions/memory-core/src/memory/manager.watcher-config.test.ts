...
import type {
  MemorySearchConfig,
  WingsConfig,
} from "mechanical-wings/plugin-sdk/memory-core-host-engine-foundation";
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): WingsConfig {
    const defaults: NonNullable<NonNullable<WingsConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: WingsConfig) {
