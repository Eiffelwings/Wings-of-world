...
import {
  ...
  formatHelpExamples,
  theme,
} from "mechanical-wings/plugin-sdk/memory-core-host-runtime-cli";
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mechanical-wings memory status", "Show index and provider status."],
          ["mechanical-wings memory status --deep", "Probe embedding provider readiness."],
          ["mechanical-wings memory index --force", "Force a full reindex."],
          ['mechanical-wings memory search "meeting notes"', "Quick search using positional query."],
          [
            'mechanical-wings memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          ["mechanical-wings memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.wings-of-world.ai/cli/memory")}\n`,
    );
