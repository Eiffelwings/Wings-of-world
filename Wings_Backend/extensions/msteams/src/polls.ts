...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["mechanicalWingsPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["mechanical-wings", "pollId"]) ??
    readNestedString(value, ["mechanical-wings", "poll", "id"]) ??
    readNestedString(value, ["data", "mechanicalWingsPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "mechanical-wings", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          mechanicalWingsPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "mechanical-wings poll vote",
          displayText: "Vote recorded",
          value: { mechanicalWingsPollId: pollId, pollId },
        },
      },
    ],
  };
