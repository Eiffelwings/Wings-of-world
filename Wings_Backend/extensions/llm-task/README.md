...
`mechanical-wings.invoke --each`) without adding new Wings🪽 code per workflow.
...
This extension depends on Wings🪽 internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Wings🪽 extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.mechanical-wings/extensions` as a standalone plugin directory.
