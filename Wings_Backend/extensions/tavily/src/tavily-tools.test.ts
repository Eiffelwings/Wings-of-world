...
import type { WingsPluginApi } from "mechanical-wings/plugin-sdk/plugin-runtime";
...
function fakeApi(): WingsPluginApi {
  return {
    config: {},
  } as WingsPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.wings-of-world.ai", "", "wings-of-world.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.wings-of-world.ai", "wings-of-world.ai"],
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as WingsConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
