...
import type { WingsPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/mechanical-wings-diagnostics-otel-test";
...
function createTraceOnlyContext(endpoint: string): WingsPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("mechanical-wings.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("mechanical-wings.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("mechanical-wings.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("mechanical-wings.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("mechanical-wings.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("mechanical-wings.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("mechanical-wings.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("mechanical-wings.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("mechanical-wings.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("mechanical-wings.webhook.processed");
    expect(spanNames).toContain("mechanical-wings.message.processed");
    expect(spanNames).toContain("mechanical-wings.session.stuck");
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        "mechanical-wings.reason": expect.stringContaining("…"),
      }),
    );
