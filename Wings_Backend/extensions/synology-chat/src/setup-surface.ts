...
function getChannelConfig(cfg: WingsConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: WingsConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: WingsConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: WingsConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL Wings uses to send replies back to Chat.",
      ],
