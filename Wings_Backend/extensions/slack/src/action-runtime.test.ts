...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): WingsConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as WingsConfig;
  ...
  async function resolveReadToken(cfg: WingsConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: WingsConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
