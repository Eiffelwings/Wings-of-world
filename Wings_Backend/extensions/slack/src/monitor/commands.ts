import type { SlackSlashCommandConfig } from "mechanical-wings/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "mechanical-wings");
  const name = normalizedName || "mechanical-wings";
  return {
