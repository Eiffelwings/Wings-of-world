...
describe("buildSlackSlashCommandMatcher", () => {
  it("matches with or without a leading slash", () => {
    const matcher = buildSlackSlashCommandMatcher("mechanical-wings");

    expect(matcher.test("mechanical-wings")).toBe(true);
    expect(matcher.test("/mechanical-wings")).toBe(true);
  });
  ...
  it("does not match similar names", () => {
    ...
    expect(matcher.test("/mechanical-wings-bot")).toBe(false);
    expect(matcher.test("mechanical-wings-bot")).toBe(false);
  });
});
