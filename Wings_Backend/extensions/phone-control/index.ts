...
function normalizeDenyList(cfg: WingsPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: WingsPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: WingsPluginApi) {
    ...
    const timerService: WingsPluginService = {
