...
RUN echo "mechanical-wings-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && OPENCLAW_OPENSHELL_E2E)(
    ...
    async () => {
      ...
      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-openshell-e2e-"));
      ...
      const gatewayName = `mechanical-wings-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `mechanical-wings-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/mechanical-wings-sandboxes"),
      ...
      try {
        ...
        expect(stdout).toContain("mechanical-wings-openshell-e2e");
