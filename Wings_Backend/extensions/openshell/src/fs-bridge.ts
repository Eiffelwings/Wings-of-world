...
import { createWritableRenameTargetResolver } from "mechanical-wings/plugin-sdk/sandbox";
...
class OpenShellFsBridge implements SandboxFsBridge {
  ...
  async writeFile(params: {
    ...
    const tempPath = path.join(
      parentDir,
      `.mechanical-wings-openshell-write-${path.basename(hostPath)}-${process.pid}-${Date.now()}`,
    );
