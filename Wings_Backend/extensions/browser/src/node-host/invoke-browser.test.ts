...
describe("runBrowserProxyCommand", () => {
    ...
    const result = expect(
      runBrowserProxyCommand(
        JSON.stringify({
          method: "GET",
          path: "/snapshot",
          profile: "mechanical-wings",
          timeoutMs: 5,
        }),
      ),
    ).rejects.toThrow(
      /browser proxy timed out for GET \/snapshot after 5ms; ws-backed browser action; profile=mechanical-wings; status\(running=true, cdpHttp=true, cdpReady=false, cdpUrl=http:\/\/127\.0\.0\.1:18792\)/,
    );
  ...
  it("rejects unauthorized query.profile when allowProfiles is configured", async () => {
    configMocks.loadConfig.mockReturnValue({
      browser: {},
      nodeHost: { browserProxy: { enabled: true, allowProfiles: ["mechanical-wings"] } },
    });
    ...
    await expect(
      runBrowserProxyCommand(
        JSON.stringify({
          method: "POST",
          path: "/reset-profile",
          body: { profile: "mechanical-wings", name: "mechanical-wings" },
          timeoutMs: 50,
        }),
      ),
    ).rejects.toThrow(
  ...
  it("canonicalizes an allowlisted body profile into the dispatched query", async () => {
    ...
    expect(dispatcherMocks.dispatch).toHaveBeenCalledWith(
      expect.objectContaining({
        path: "/stop",
        query: { profile: "mechanical-wings" },
      }),
    );
  });
