...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "mechanical-wings";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "mechanical-wings"}`,
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "mechanical-wings" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (mechanical-wings|existing-session). Default: mechanical-wings")
    .action(
      async (
        opts: {
          name: string;
