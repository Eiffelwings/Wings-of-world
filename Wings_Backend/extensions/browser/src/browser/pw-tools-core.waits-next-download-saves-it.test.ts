...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredWingsTmpDir: vi.fn(() => "/tmp/mechanical-wings"),
}));
vi.mock("../infra/tmp-mechanical-wings-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredWingsTmpDir.mockReturnValue("/tmp/mechanical-wings");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".mechanical-wings-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredWingsTmpDir.mockReturnValue("/tmp/mechanical-wings-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "mechanical-wings-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "mechanical-wings-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredWingsTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "mechanical-wings-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "mechanical-wings-preferred", "downloads")}${path.sep}`),
    );
  });
