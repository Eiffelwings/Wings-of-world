...
type StopChromeTarget = Parameters<typeof stopWingsChrome>[0];
...
describe("browser chrome profile decoration", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-chrome-suite-"));
  });
  ...
  it("writes expected name + signed ARGB seed to Chrome prefs", async () => {
    ...
    decorateWingsProfile(userDataDir, { color: DEFAULT_OPENCLAW_BROWSER_COLOR });
    ...
    const marker = await fsp.readFile(
      path.join(userDataDir, ".mechanical-wings-profile-decorated"),
      "utf-8",
    );
  ...
  it("best-effort writes name when color is invalid", async () => {
    const userDataDir = await createUserDataDir();
    decorateWingsProfile(userDataDir, { color: "lobster-orange" });
    const def = await readDefaultProfileFromLocalState(userDataDir);
...
describe("browser chrome helpers", () => {
  ...
  it("stopWingsChrome no-ops when process is already killed", async () => {
    const proc = makeChromeTestProc({ killed: true });
    await stopChromeWithProc(proc, 10);
    expect(proc.kill).not.toHaveBeenCalled();
  });

  it("stopWingsChrome sends SIGTERM and returns once CDP is down", async () => {
  ...
  it("stopWingsChrome escalates to SIGKILL when CDP stays reachable", async () => {
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ok: true,
