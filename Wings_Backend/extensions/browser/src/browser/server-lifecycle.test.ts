...
const { stopWingsChromeMock } = vi.hoisted(() => ({
  stopWingsChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopWingsChrome: stopWingsChromeMock,
}));
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["mechanical-wings", "user"]);
    const stopMap: Record<string, ReturnType<typeof vi.fn>> = {
      "mechanical-wings": vi.fn(async () => {}),
      user: vi.fn(async () => {
    ...
    expect(stopMap["mechanical-wings"]).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopWingsChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("mechanical-wings browser stop failed: Error: oops");
  });
});
