...
vi.mock("mechanical-wings/plugin-sdk/browser-support", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/browser-support")>();
  return {
    ...actual,
...
vi.mock("./config.js", () => ({
  resolveBrowserConfig: vi.fn(() => ({
    enabled: true,
    controlPort: 18791,
    profiles: { "mechanical-wings": { cdpPort: 18800 } },
  })),
}));
