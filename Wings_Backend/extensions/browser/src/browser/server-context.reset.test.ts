...
function localWingsProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    ...
    cdpPort: 18800,
    color: "#f60",
    driver: "mechanical-wings",
    attachOnly: false,
  };
}

function createLocalWingsResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localWingsProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveWingsUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-reset-"));
    const profileDir = path.join(tempRoot, "mechanical-wings");
    ...
    const ops = createLocalWingsResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveWingsUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-reset-no-own-"));
    const profileDir = path.join(tempRoot, "mechanical-wings");
    fs.mkdirSync(profileDir, { recursive: true });
