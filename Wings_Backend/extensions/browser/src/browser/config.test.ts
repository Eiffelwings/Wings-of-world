...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(profile?.name).toBe("mechanical-wings");
    expect(profile?.driver).toBe("mechanical-wings");
    expect(profile?.cdpPort).toBe(18800);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:18800");

    const mechanicalWings = resolveProfile(resolved, "mechanical-wings");
    expect(mechanicalWings?.driver).toBe("mechanical-wings");
    expect(mechanicalWings?.cdpPort).toBe(18800);
    expect(mechanicalWings?.cdpUrl).toBe("http://127.0.0.1:18800");
  ...
  it("derives default ports from OPENCLAW_GATEWAY_PORT when unset", () => {
    withEnv({ OPENCLAW_GATEWAY_PORT: "19001" }, () => {
      ...
      const mechanicalWings = resolveProfile(resolved, "mechanical-wings");
      expect(mechanicalWings?.cdpPort).toBe(19012);
      expect(mechanicalWings?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ OPENCLAW_GATEWAY_PORT: undefined }, () => {
      ...
      const mechanicalWings = resolveProfile(resolved, "mechanical-wings");
      expect(mechanicalWings?.cdpPort).toBe(19022);
      expect(mechanicalWings?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("supports overriding the local CDP auto-allocation range start", () => {
    ...
    expect(mechanicalWings?.cdpPort).toBe(19000);
    expect(mechanicalWings?.cdpUrl).toBe("http://127.0.0.1:19000");
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "mechanical-wings");
  ...
  it("sets usesChromeMcp only for existing-session profiles", () => {
    ...
    const managed = resolveProfile(resolved, "mechanical-wings")!;
  ...
  describe("default profile preference", () => {
    it("defaults to mechanicalWings profile when defaultProfile is not configured", () => {
      const resolved = resolveBrowserConfig({
        headless: false,
        noSandbox: false,
      });
      expect(resolved.defaultProfile).toBe("mechanical-wings");
    });

    it("keeps mechanicalWings default when headless=true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
      });
      expect(resolved.defaultProfile).toBe("mechanical-wings");
    ...
    it("keeps mechanicalWings default when noSandbox=true", () => {
      const resolved = resolveBrowserConfig({
        noSandbox: true,
      });
      expect(resolved.defaultProfile).toBe("mechanical-wings");
    ...
    it("keeps mechanicalWings default when both headless and noSandbox are true", () => {
      const resolved = resolveBrowserConfig({
        headless: true,
        noSandbox: true,
      });
