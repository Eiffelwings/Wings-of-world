...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    ...
    const createDuplicate = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "mechanical-wings" }),
    });
    ...
    const explicitUserDataDir = "/tmp/mechanical-wings-brave-profile";
    ...
    const deleteDefault = await realFetch(`${base}/profiles/mechanical-wings`, {
      method: "DELETE",
    });
