...
describe("server-context hot-reload profiles", () => {
  ...
  it("forProfile hot-reloads newly added profiles from config", async () => {
    // Start with only mechanical-wings profile
    ...
    // 2. Simulate adding a new profile to config (like user editing mechanical-wings.json)
  ...
  it("forProfile refreshes existing profile config after loadConfig cache updates", async () => {
    ...
    cfgProfiles["mechanical-wings"] = { cdpPort: 19999, color: "#FF4500" };
    cachedConfig = null;
    ...
    expect(state.resolved.profiles["mechanical-wings"]?.cdpPort).toBe(19999);
  });
  ...
  it("marks existing runtime state for reconcile when profile invariants change", async () => {
    const cfg = loadConfig();
    const resolved = resolveBrowserConfig(cfg.browser, cfg);
    const mechanicalWingsProfile = resolveProfile(resolved, "mechanical-wings");
    expect(mechanicalWingsProfile).toBeTruthy();
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "mechanical-wings",
          {
            profile: mechanicalWingsProfile!,
    ...
    const runtime = state.profiles.get("mechanical-wings");
