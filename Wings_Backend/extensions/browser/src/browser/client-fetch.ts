...
type LoopbackBrowserAuthDeps = {
  ...
  ensureBrowserControlAuth: typeof ensureBrowserControlAuth;
...
async function withLoopbackBrowserAuthImpl(
  url: string,
  init: (RequestInit & { timeoutMs?: number }) | undefined,
  deps: LoopbackBrowserAuthDeps,
): Promise<RequestInit & { timeoutMs?: number }> {
  const headers = new Headers(init?.headers ?? {});
  if (headers.has("authorization") || headers.has("x-mechanical-wings-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    const ensured = await deps.ensureBrowserControlAuth({ cfg });
    const auth = ensured.auth.token || ensured.auth.password
      ? ensured.auth
      : deps.resolveBrowserControlAuth(cfg);
    if (auth.token) {
      headers.set("Authorization", `Bearer ${auth.token}`);
      return { ...init, headers };
    }
    if (auth.password) {
      headers.set("x-mechanical-wings-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-mechanical-wings-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
...
function resolveBrowserFetchOperatorHint(url: string): string {
  ...
  return isLocal
    ? `Restart the Wings gateway (Wings.app menubar, or \`${formatCliCommand("mechanical-wings gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      appendBrowserToolModelHint(
        `Can't reach the Wings browser control service (timed out after ${timeoutMs}ms). ${operatorHint}`,
      ),
    );
  }
  return new Error(
    appendBrowserToolModelHint(
      `Can't reach the Wings browser control service. ${operatorHint} (${msg})`,
    ),
  );
}
