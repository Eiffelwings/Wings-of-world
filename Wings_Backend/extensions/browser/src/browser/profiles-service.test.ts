...
import { resolveWingsUserDataDir } from "./chrome.js";
...
vi.mock("./chrome.js", () => ({
  resolveWingsUserDataDir: vi.fn(() => "/tmp/mechanical-wings-test/mechanical-wings/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "mechanical-wings-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveWingsUserDataDir).mockReturnValue(userDataDir);
