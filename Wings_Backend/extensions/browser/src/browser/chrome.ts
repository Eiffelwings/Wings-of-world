...
export function resolveWingsUserDataDir(profileName = DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME) {
...
export function buildWingsChromeLaunchArgs(params: {
...
export async function launchWingsChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  ...
  const userDataDir = resolveWingsUserDataDir(profile.name);
  ...
  const spawnOnce = () => {
    const args = buildWingsChromeLaunchArgs({
      resolved,
      profile,
      userDataDir,
    });
  ...
  if (needsDecorate) {
    try {
      decorateWingsProfile(userDataDir, {
        name: profile.name,
        color: profile.color,
      });
      log.info(`🪽 mechanical-wings browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`mechanical-wings browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`mechanical-wings browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🪽 mechanical-wings browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
...
export async function stopWingsChrome(
