...
type HarnessState = {
  ...
  cfgProfiles: Record<
    string,
    {
      ...
      cdpUrl?: string;
      color: string;
      driver?: "mechanical-wings" | "existing-session";
      attachOnly?: boolean;
    }
  >;
...
const state: HarnessState = {
  ...
  cfgDefaultProfile: "mechanical-wings",
  cfgProfiles: {},
  createTargetId: null,
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "mechanical-wings",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/mechanical-wings" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    "mechanical-wings": { cdpPort: testPort + 9, color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchWingsChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "mechanical-wings";
