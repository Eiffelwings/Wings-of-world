...
import { createSubsystemLogger, loadConfig } from "mechanical-wings/plugin-sdk/browser-support";
...
export async function startBrowserControlServerFromConfig(): Promise<BrowserServerState | null> {
  ...
  const server = await new Promise<Server>((resolve, reject) => {
    ...
    logServer.error(`mechanical-wings browser server failed to bind 127.0.0.1:${port}: ${String(err)}`);
    return null;
  });
