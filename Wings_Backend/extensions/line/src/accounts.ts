...
} from "mechanical-wings/plugin-sdk/account-id";
import type { WingsConfig } from "mechanical-wings/plugin-sdk/account-resolution";
import { resolveAccountEntry } from "mechanical-wings/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: WingsConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: WingsConfig): string {
