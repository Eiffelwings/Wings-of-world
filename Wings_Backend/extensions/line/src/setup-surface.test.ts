...
import type { WingsConfig, PluginRuntime, ResolvedLineAccount } from "../api.js";
...
function collectRuntimeApiOverlapExports(params: {
  ...
  for (const statement of runtimeApiFile.statements) {
    ...
    if (moduleSpecifier === "mechanical-wings/plugin-sdk/line-runtime") {
  ...
  it("loads through Jiti without duplicate export errors", () => {
    ...
    expect(
      loadRuntimeApiExportTypesViaJiti({
        ...
        realPluginSdkSpecifiers: ["mechanical-wings/plugin-sdk/line-runtime"],
      }),
    ).toEqual({
