...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): WingsConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as WingsConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): WingsConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as WingsConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: WingsConfig, accountId = "media") {
    await handleTelegramAction(
