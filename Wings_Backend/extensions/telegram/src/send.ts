...
import { loadConfig } from "mechanical-wings/plugin-sdk/config-runtime";
import { resolveMarkdownTableMode } from "mechanical-wings/plugin-sdk/config-runtime";
import { recordChannelActivity } from "mechanical-wings/plugin-sdk/infra-runtime";
import { isDiagnosticFlagEnabled } from "mechanical-wings/plugin-sdk/infra-runtime";
import { formatErrorMessage, formatUncaughtError } from "mechanical-wings/plugin-sdk/infra-runtime";
import { createTelegramRetryRunner } from "mechanical-wings/plugin-sdk/infra-runtime";
import type { RetryConfig } from "mechanical-wings/plugin-sdk/infra-runtime";
import type { MediaKind } from "mechanical-wings/plugin-sdk/media-runtime";
import { buildOutboundMediaLoadOptions } from "mechanical-wings/plugin-sdk/media-runtime";
import { getImageMetadata } from "mechanical-wings/plugin-sdk/media-runtime";
import { isGifMedia, kindFromMime } from "mechanical-wings/plugin-sdk/media-runtime";
import { normalizePollInput, type PollInput } from "mechanical-wings/plugin-sdk/media-runtime";
import { logVerbose } from "mechanical-wings/plugin-sdk/runtime-env";
import { createSubsystemLogger } from "mechanical-wings/plugin-sdk/runtime-env";
import { redactSensitiveText } from "mechanical-wings/plugin-sdk/text-runtime";
import { loadWebMedia } from "mechanical-wings/plugin-sdk/web-media";
...
type TelegramSendOpts = {
  ...
  verbose?: boolean;
  mediaUrl?: string;
  /** Array of media URLs for sending as a media group (album). */
  mediaUrls?: string[];
 ...
 * @param mediaUrls - Array of media URLs to send
 * @param text - Optional caption for the first media item
...
export async function sendMediaGroupTelegram(
  ...
  if (!mediaUrls?.length) {
    throw new Error("Telegram media group requires at least one media URL");
  }
  if (mediaUrls.length > 10) {
    throw new Error("Telegram media group supports at most 10 items");
  }
  ...
  for (let i = 0; i < mediaUrls.length; i++) {
    ...
    const fileName = media.fileName ?? inferFilename(kind ?? "document");
    const file = new InputFileCtor(media.buffer, fileName);

    // Apply caption only to the first item (Telegram convention)
    let caption: string | undefined;
    if (i === 0 && text.trim()) {
      const split = splitTelegramCaption(text);
      caption = renderHtmlText(split.caption || text);
    }
  ...
  const results = await withTelegramThreadFallback(
    mediaGroupParams,
    "mediaGroup",
    opts.verbose,
    async (effectiveParams, label) =>
      requestWithChatNotFound(
        () => api.sendMediaGroup(chatId, inputMedia, effectiveParams as any),
        label,
      ),
  );

  const messageIds = (results as TelegramMessageLike[]).map((res) => {
    const id = resolveTelegramMessageIdOrThrow(res, "media group send");
    recordSentMessage(chatId, id);
    return String(id);
  });
  ...
  return { messageIds, chatId: String((results[0] as any)?.chat?.id ?? chatId) };
}
...
export async function sendMessageTelegram(
  ...
  if (opts.mediaUrls && opts.mediaUrls.length > 1) {
    const groupResult = await sendMediaGroupTelegram(to, opts.mediaUrls, text, opts);
    return {
      messageId: groupResult.messageIds[groupResult.messageIds.length - 1],
