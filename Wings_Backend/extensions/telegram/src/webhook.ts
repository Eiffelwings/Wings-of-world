...
import { isDiagnosticsEnabled } from "mechanical-wings/plugin-sdk/infra-runtime";
import {
  createTelegramRetryRunner,
  formatErrorMessage,
} from "mechanical-wings/plugin-sdk/infra-runtime";
import { readJsonBodyWithLimit } from "mechanical-wings/plugin-sdk/infra-runtime";
import type { RuntimeEnv } from "mechanical-wings/plugin-sdk/runtime-env";
import { defaultRuntime } from "mechanical-wings/plugin-sdk/runtime-env";
import {
  logWebhookError,
  logWebhookProcessed,
  logWebhookReceived,
...
} from "mechanical-wings/plugin-sdk/text-runtime";
import {
...
async function initializeTelegramWebhookBot(params: {
  ...
  const retry = createTelegramRetryRunner({
    verbose: true,
    shouldRetry: (err) => isRecoverableTelegramNetworkError(err, { context: "webhook" }),
  });
...
function resolveTelegramWebhookClientIp(req: IncomingMessage, config?: WingsConfig): string {
          ...
          fn: () =>
            bot.api.setWebhook(publicUrl, {
              ...
              certificate: opts.webhookCertPath
                ? new InputFileCtor(opts.webhookCertPath)
                : undefined,
            }),
        }),
      "setWebhook",
