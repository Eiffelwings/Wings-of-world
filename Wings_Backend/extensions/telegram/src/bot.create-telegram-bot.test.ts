...
async function withIsolatedStateDirAsync<T>(fn: () => Promise<T>): Promise<T> {
  const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-telegram-state-"));
...
async function withConfigPathAsync<T>(cfg: unknown, fn: () => Promise<T>): Promise<T> {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-telegram-cfg-"));
  const configPath = path.join(dir, "mechanical-wings.json");
...
describe("createTelegramBot", () => {
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    await withIsolatedStateDirAsync(async () => {
      for (const [index, testCase] of cases.entries()) {
        ...
        expect(pairingText, testCase.name).toContain("mechanical-wings pairing approve telegram");
    ...
    await withIsolatedStateDirAsync(async () => {
      ...
      await handler({
        message: {
          ...
          message_id: 1884,
          date: 1736380800,
          from: { id: 7, is_bot: true, first_name: "Wings", username: "mechanical-wings_bot" },
          pinned_message: {
            message_id: 1883,
            date: 1736380799,
            chat: { id: 1234, type: "private", first_name: "Harold" },
            from: { id: 7, is_bot: true, first_name: "Wings", username: "mechanical-wings_bot" },
            text: "Binding: Review pull request 54118 (mechanical-wings)",
          },
        },
        me: { id: 7, is_bot: true, first_name: "Wings", username: "mechanical-wings_bot" },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
  ...
  const groupPolicyCases: Array<{
  ...
  }> = [
    {
      ...
      message: {
        ...
        text: "@mechanical-wings_bot hello",
        date: 1736380800,
      },
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "mechanical-wings_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("honors routed group activation from session store", async () => {
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "mechanical-wings-telegram-"));
