...
vi.mock("mechanical-wings/plugin-sdk/media-runtime", () => ({
  AUTO_IMAGE_KEY_PROVIDERS: ["openai"],
  DEFAULT_IMAGE_MODELS: { openai: "gpt-4.1-mini" },
  resolveAutoImageModel: vi.fn(async () => null),
}));

vi.mock("mechanical-wings/plugin-sdk/media-understanding-runtime", () => ({
  describeImageFileWithModel: vi.fn(),
}));

const TEST_CACHE_DIR = "/tmp/mechanical-wings-test-sticker-cache/telegram";
...
describe("sticker-cache", () => {
  beforeEach(async () => {
    process.env.OPENCLAW_STATE_DIR = "/tmp/mechanical-wings-test-sticker-cache";
    fs.rmSync("/tmp/mechanical-wings-test-sticker-cache", { recursive: true, force: true });
