import { resolveDefaultModelForAgent } from "mechanical-wings/plugin-sdk/agent-runtime";
import type { WingsConfig } from "mechanical-wings/plugin-sdk/config-runtime";
import { resetInboundDedupe } from "mechanical-wings/plugin-sdk/reply-runtime";
import type { MsgContext } from "mechanical-wings/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "mechanical-wings/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "mechanical-wings/plugin-sdk/reply-runtime";
import type { MockFn } from "mechanical-wings/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("mechanical-wings/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("mechanical-wings/plugin-sdk/config-runtime").resolveStorePath;
type TelegramBotRuntimeForTest = NonNullable<
  Parameters<typeof import("./bot.js").setTelegramBotRuntimeForTest>[0]
>;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("mechanical-wings/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/mechanical-wings-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("mechanical-wings/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("mechanical-wings/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("mechanical-wings/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("mechanical-wings/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("mechanical-wings/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: WingsConfig): {
...
vi.doMock("mechanical-wings/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("mechanical-wings/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("mechanical-wings/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("mechanical-wings/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("mechanical-wings/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: WingsConfig = {
  agents: {
    defaults: {
...
export function makeTelegramMessageCtx(params: {
  ...
  return {
    ...
    me: { username: "mechanical-wings_bot" },
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: WingsConfig) => {
