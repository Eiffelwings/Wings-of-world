...
function createAgentRuntime(payloads: Array<Record<string, unknown>>) {
  ...
  const runtime = {
    ...
    resolveAgentDir: () => "/tmp/mechanical-wings/agents/main",
    resolveAgentWorkspaceDir: () => "/tmp/mechanical-wings/workspace/main",
    ...
    runEmbeddedPiAgent,
    session: {
      resolveStorePath: () => "/tmp/mechanical-wings/sessions.json",
      loadSessionStore: () => ({}),
      saveSessionStore: async () => {},
      resolveSessionFilePath: () => "/tmp/mechanical-wings/sessions/session.jsonl",
    },
