...
import type { WingsPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredWingsTmpDir: vi.fn(() => path.join(os.tmpdir(), "mechanical-wings-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredWingsTmpDir: pluginApiMocks.resolvePreferredWingsTmpDir,
...
function createApi(params?: {
  runtime?: WingsPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as WingsPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: WingsPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as WingsPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredWingsTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredWingsTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Wings iOS app:");
    expect(text).toContain("![Wings pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Wings iOS app:");
