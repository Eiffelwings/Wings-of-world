...
vi.mock("mechanical-wings/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("mechanical-wings/plugin-sdk/media-runtime")>(
    "mechanical-wings/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `mechanical-wings-inbound-media-${crypto.randomUUID()}`);
