...
import { VERSION } from "mechanical-wings/plugin-sdk/cli-runtime";
import { danger, success } from "mechanical-wings/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "mechanical-wings/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "mechanical-wings/plugin-sdk/text-runtime";
...
export async function createWaSocket(
  ...
  const sock = makeWASocket({
    ...
    browser: ["mechanical-wings", "cli", VERSION],
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("mechanical-wings channels login")}`,
              ),
            );
          }
        }
