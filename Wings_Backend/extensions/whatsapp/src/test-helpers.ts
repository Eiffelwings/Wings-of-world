...
const CONFIG_KEY = Symbol.for("mechanical-wings:testConfigMock");
...
vi.mock("mechanical-wings/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/media-runtime")>();
  const mockModule = Object.create(null) as Record<string, unknown>;
  Object.defineProperties(mockModule, Object.getOwnPropertyDescriptors(actual));
...
vi.mock("mechanical-wings/plugin-sdk/state-paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/state-paths")>();
  return {
    ...actual,
    resolveOAuthDir: () => "/tmp/mechanical-wings-oauth",
  };
});
...
vi.mock("@whiskeysockets/baileys", async (importOriginal) => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("mechanical-wings:lastSocket")] =
    created.lastSocket;
  return {
    ...actual,
    ...created.mod,
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("mechanical-wings:lastSocket")];
  if (typeof getter === "function") {
