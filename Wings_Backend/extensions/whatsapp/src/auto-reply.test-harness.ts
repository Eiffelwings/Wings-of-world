...
import * as ssrf from "mechanical-wings/plugin-sdk/infra-runtime";
import { resetInboundDedupe } from "mechanical-wings/plugin-sdk/reply-runtime";
import { resetLogger, setLoggerOverride } from "mechanical-wings/plugin-sdk/runtime-env";
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mechanical-wings-session-"));
