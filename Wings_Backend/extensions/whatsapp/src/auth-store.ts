...
import { info, success } from "mechanical-wings/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "mechanical-wings/plugin-sdk/state-paths";
import type { WebChannel } from "mechanical-wings/plugin-sdk/text-runtime";
import { resolveUserPath } from "mechanical-wings/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("mechanical-wings channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
