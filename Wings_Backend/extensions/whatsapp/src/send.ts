...
import { generateSecureUuid } from "mechanical-wings/plugin-sdk/infra-runtime";
...
import { getChildLogger } from "mechanical-wings/plugin-sdk/text-runtime";
...
import { markdownToWhatsApp } from "mechanical-wings/plugin-sdk/text-runtime";
import { toWhatsappJid } from "mechanical-wings/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: WingsConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
