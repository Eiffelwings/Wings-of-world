...
const hoisted = vi.hoisted(() => ({
  ...
  resolveWhatsAppAuthDir: vi.fn(() => ({
    authDir: "/tmp/mechanical-wings-whatsapp-test",
  })),
}));
...
vi.mock("mechanical-wings/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<typeof import("mechanical-wings/plugin-sdk/setup")>(
    "mechanical-wings/plugin-sdk/setup",
  );
  return {
    ...actual,
...
describe("whatsapp setup wizard", () => {
  ...
  beforeEach(() => {
    ...
    hoisted.resolveWhatsAppAuthDir.mockReturnValue({ authDir: "/tmp/mechanical-wings-whatsapp-test" });
  });
  ...
  it("skips relink note when already linked and relink is declined", async () => {
    ...
    expect(harness.note).not.toHaveBeenCalledWith(
      expect.stringContaining("mechanical-wings channels login"),
      "WhatsApp",
    );
  });
