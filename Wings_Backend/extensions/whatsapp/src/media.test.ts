...
import { resolvePreferredWingsTmpDir } from "../../../src/infra/tmp-mechanical-wings-dir.js";
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredWingsTmpDir(), "mechanical-wings-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    ...
    // Also keep it outside the Wings temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["OPENCLAW_STATE_DIR"]);
    process.env.OPENCLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "mechanical-wings-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredWingsTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default Wings state workspace and sandbox roots", async () => {
  ...
  it("rejects default Wings state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
