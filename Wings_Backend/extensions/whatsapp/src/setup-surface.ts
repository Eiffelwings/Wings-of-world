...
import type { ChannelSetupWizard } from "mechanical-wings/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "mechanical-wings/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: WingsConfig,
  patch: Partial<NonNullable<NonNullable<WingsConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: WingsConfig, dmPolicy: DmPolicy): WingsConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: WingsConfig, allowFrom?: string[]): WingsConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: WingsConfig, selfChatMode: boolean): WingsConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: WingsConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so Wings can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for Wings" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("mechanical-wings channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
