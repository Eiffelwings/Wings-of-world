import { resolveInboundDebounceMs } from "mechanical-wings/plugin-sdk/channel-inbound";
import { formatCliCommand } from "mechanical-wings/plugin-sdk/cli-runtime";
import { waitForever } from "mechanical-wings/plugin-sdk/cli-runtime";
...
import { DEFAULT_GROUP_HISTORY_LIMIT } from "mechanical-wings/plugin-sdk/reply-history";
import { getReplyFromConfig } from "mechanical-wings/plugin-sdk/reply-runtime";
...
import { registerUnhandledRejectionHandler } from "mechanical-wings/plugin-sdk/runtime-env";
import { getChildLogger } from "mechanical-wings/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "mechanical-wings/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("mechanical-wings channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("mechanical-wings channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
