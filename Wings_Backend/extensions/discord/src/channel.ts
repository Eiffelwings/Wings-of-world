...
} from "mechanical-wings/plugin-sdk/allowlist-config-edit";
import { createScopedDmSecurityResolver } from "mechanical-wings/plugin-sdk/channel-config-helpers";
import { createPairingPrefixStripper } from "mechanical-wings/plugin-sdk/channel-pairing";
import { createOpenProviderConfiguredRouteWarningCollector } from "mechanical-wings/plugin-sdk/channel-policy";
import { resolveTargetsWithOptionalToken } from "mechanical-wings/plugin-sdk/channel-targets";
...
import { normalizeMessageChannel } from "mechanical-wings/plugin-sdk/routing";
import {
...
function hasDiscordExecApprovalDmRoute(cfg: WingsConfig): boolean {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Wings adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
