...
const STORE_PATH = path.join(os.tmpdir(), "mechanical-wings-exec-approvals-test.json");
...
vi.mock("mechanical-wings/plugin-sdk/gateway-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("mechanical-wings/plugin-sdk/gateway-runtime")>();
