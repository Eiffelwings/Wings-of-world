...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/mechanical-wings-discord-process-test-sessions.json",
  ),
}));
...
const replyRuntimeModule = await import("mechanical-wings/plugin-sdk/reply-runtime");
...
const conversationRuntimeModule = await import("mechanical-wings/plugin-sdk/conversation-runtime");
...
const configRuntimeModule = await import("mechanical-wings/plugin-sdk/config-runtime");
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/mechanical-wings-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createBaseContext({
      cfg: {
        ...
        session: { store: "/tmp/mechanical-wings-discord-process-test-sessions.json" },
      },
    });
