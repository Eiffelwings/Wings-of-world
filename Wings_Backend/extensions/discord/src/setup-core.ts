import { DEFAULT_ACCOUNT_ID } from "mechanical-wings/plugin-sdk/account-id";
import type { DiscordGuildEntry } from "mechanical-wings/plugin-sdk/config-runtime";
import type { WingsConfig } from "mechanical-wings/plugin-sdk/config-runtime";
import { createEnvPatchedAccountSetupAdapter } from "mechanical-wings/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "mechanical-wings/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "mechanical-wings/plugin-sdk/setup-runtime";
import { formatDocsLink } from "mechanical-wings/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: WingsConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: WingsConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: WingsConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: WingsConfig; accountId: string }) =>
    ...
    disable: (cfg: WingsConfig) => setSetupChannelEnabled(cfg, channel, false),
