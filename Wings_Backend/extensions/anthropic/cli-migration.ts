import type { WingsConfig, ProviderAuthResult } from "mechanical-wings/plugin-sdk/provider-auth";
import { readClaudeCliCredentialsCached } from "mechanical-wings/plugin-sdk/provider-auth";

const DEFAULT_CLAUDE_CLI_MODEL = "claude-cli/claude-sonnet-4-6";
type AgentDefaultsModel = NonNullable<NonNullable<WingsConfig["agents"]>["defaults"]>["model"];
type AgentDefaultsModels = NonNullable<NonNullable<WingsConfig["agents"]>["defaults"]>["models"];
...
export function buildAnthropicCliMigrationResult(config: WingsConfig): ProviderAuthResult {
