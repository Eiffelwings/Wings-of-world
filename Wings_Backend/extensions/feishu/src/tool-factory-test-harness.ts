import type { AnyAgentTool, WingsPluginApi } from "../runtime-api.js";
...
export function createToolFactoryHarness(cfg: WingsPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<WingsPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
