...
metadata: { "mechanical-wings": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Wings🪽 `sessions_spawn`
- **File I/O** == Wings🪽 `read`/`write`
- **Remote fetch** == Wings🪽 `web_fetch` (or `exec` with curl when POST is required)
