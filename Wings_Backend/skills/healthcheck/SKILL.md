---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Wings🪽 deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Wings🪽 cron scheduling for periodic checks, or version status checks on a machine running Wings🪽 (laptop, workstation, Pi, VPS).
---

# Wings🪽 Host Hardening

## Overview

Assess and harden the host running Wings🪽, then align it to a user-defined risk tolerance without breaking access. Use Wings🪽 security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Wings🪽 changes the host firewall, SSH, or OS updates; it does not.
...
5. Wings🪽 gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Wings🪽 can access sensitive data.
...
### 2) Run Wings🪽 security audits (read-only)

As part of the default read-only checks, run `mechanical-wings security audit --deep`. Only offer alternatives if the user requests them:

1. `mechanical-wings security audit` (faster, non-probing)
2. `mechanical-wings security audit --json` (structured output)

Offer to apply Wings🪽 safe defaults (numbered):

1. `mechanical-wings security audit --fix`

Be explicit that `--fix` only tightens Wings🪽 defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Wings🪽 version/update status (read-only)

As part of the default read-only checks, run `mechanical-wings update status`.
...
- Credential hygiene notes (location of Wings🪽 creds, prefer disk encryption)
...
After Wings🪽 install or first hardening pass, run at least one baseline audit and version check:

- `mechanical-wings security audit`
- `mechanical-wings security audit --deep`
- `mechanical-wings update status`

Ongoing monitoring is recommended. Use the Wings🪽 cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `mechanical-wings cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `mechanical-wings update status`
...
Before creating, `mechanical-wings cron list` and match on exact `name`. If found, `mechanical-wings cron edit <id> ...`.
If not found, `mechanical-wings cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `mechanical-wings update status` (preferred for source checkouts and channels)
2. `npm view mechanical-wings version` (published npm version)

## Wings🪽 command accuracy

Use only supported commands and flags:

- `mechanical-wings security audit [--deep] [--fix] [--json]`
- `mechanical-wings status` / `mechanical-wings status --deep`
- `mechanical-wings health --json`
- `mechanical-wings update status`
- `mechanical-wings cron add|list|runs|run`

Do not invent CLI flags or imply Wings🪽 enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Wings🪽 compaction:
