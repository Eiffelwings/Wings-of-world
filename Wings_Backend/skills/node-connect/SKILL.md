---
name: node-connect
description: Diagnose Wings🪽 node connection and pairing failures for Android, iOS, and macOS companion apps. Use when QR/setup code/manual connect fails, local Wi-Fi works but VPS/tailnet does not, or errors mention pairing required, unauthorized, bootstrap token invalid or expired, gateway.bind, gateway.remote.url, Tailscale, or plugins.entries.device-pair.config.publicUrl.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Wings🪽 is advertising that route, then fix pairing/auth.
...
- whether `mechanical-wings devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `mechanical-wings qr --json`. It uses the same setup-code payload Android scans.

```bash
mechanical-wings config get gateway.mode
mechanical-wings config get gateway.bind
mechanical-wings config get gateway.tailscale.mode
mechanical-wings config get gateway.remote.url
mechanical-wings config get gateway.auth.mode
mechanical-wings config get gateway.auth.allowTailscale
mechanical-wings config get plugins.entries.device-pair.config.publicUrl
mechanical-wings qr --json
mechanical-wings devices list
mechanical-wings nodes status
```

If this Wings🪽 instance is pointed at a remote gateway, also run:

```bash
mechanical-wings qr --remote --json
```
...
`mechanical-wings qr --json` success means:
...
If `mechanical-wings qr --json` says `Gateway is only bound to loopback`:
...
- `mechanical-wings devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run mechanical-wings qr again, rescan, then approve the pending device pairing.`
