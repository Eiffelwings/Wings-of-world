...
export async function handleWhatsAppStart(host: WingsApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: WingsApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: WingsApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: WingsApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: WingsApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: WingsApp): string {
...
function resolveGatewayHttpAuthHeader(host: WingsApp): string | null {
...
function buildGatewayHttpHeaders(host: WingsApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: WingsApp) {
...
export function handleNostrProfileToggleAdvanced(host: WingsApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: WingsApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: WingsApp) {
  const state = host.nostrProfileFormState;
