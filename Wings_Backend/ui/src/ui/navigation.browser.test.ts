...
function expectConfirmedGatewayChange(app: ReturnType<typeof mountApp>) {
  expect(app.settings.gatewayUrl).toBe("wss://other-gateway.example/mechanical-wings");
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/mechanical-wings/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/mechanical-wings");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/mechanical-wings/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__OPENCLAW_CONTROL_UI_BASE_PATH__ = "/mechanical-wings";
    const app = mountApp("/mechanical-wings/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/mechanical-wings");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/mechanical-wings/sessions");
  });
  ...
  it("hydrates token from query params and strips them", async () => {
    ...
    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("mechanical-wings.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("hydrates token from URL hash when settings already set", async () => {
    localStorage.setItem(
      "mechanical-wings.control.settings.v1",
      JSON.stringify({ token: "existing-token", gatewayUrl: "wss://gateway.example/mechanical-wings" }),
    );
    const app = mountApp("/ui/overview#token=abc123");
    await app.updateComplete;

    expect(app.settings.token).toBe("abc123");
    expect(JSON.parse(localStorage.getItem("mechanical-wings.control.settings.v1") ?? "{}")).toMatchObject({
      gatewayUrl: "wss://gateway.example/mechanical-wings",
    });
    expect(JSON.parse(localStorage.getItem("mechanical-wings.control.settings.v1") ?? "{}").token).toBe(
      undefined,
    );
  ...
  it("clears the current token when the gateway URL changes", async () => {
    ...
    gatewayUrlInput!.value = "wss://other-gateway.example/mechanical-wings";
  ...
  it("keeps a hash token pending until the gateway URL change is confirmed", async () => {
    const app = mountApp(
      "/ui/overview?gatewayUrl=wss://other-gateway.example/mechanical-wings#token=abc123",
    );
    await app.updateComplete;

    expect(app.settings.gatewayUrl).not.toBe("wss://other-gateway.example/mechanical-wings");
    expect(app.settings.token).toBe("");
