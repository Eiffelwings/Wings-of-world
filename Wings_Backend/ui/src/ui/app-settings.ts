...
import type { WingsApp } from "./app.ts";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") {
    await loadPresence(host as unknown as WingsApp);
  }
  if (host.tab === "usage") {
    await loadUsage(host as unknown as WingsApp);
  }
  if (host.tab === "sessions") {
    await loadSessions(host as unknown as WingsApp);
  }
  if (host.tab === "cron") {
    await loadCron(host);
  }
  if (host.tab === "skills") {
    await loadSkills(host as unknown as WingsApp);
  }
  if (host.tab === "agents") {
    await loadAgents(host as unknown as WingsApp);
    await loadConfig(host as unknown as WingsApp);
    const agentIds = host.agentsList?.agents?.map((entry) => entry.id) ?? [];
    if (agentIds.length > 0) {
      void loadAgentIdentities(host as unknown as WingsApp, agentIds);
    }
    const agentId =
      host.agentsSelectedId ?? host.agentsList?.defaultId ?? host.agentsList?.agents?.[0]?.id;
    if (agentId) {
      void loadAgentIdentity(host as unknown as WingsApp, agentId);
      if (host.agentsPanel === "skills") {
        void loadAgentSkills(host as unknown as WingsApp, agentId);
      }
      if (host.agentsPanel === "channels") {
        void loadChannels(host as unknown as WingsApp, false);
      }
  ...
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as WingsApp);
    await loadDevices(host as unknown as WingsApp);
    await loadConfig(host as unknown as WingsApp);
    await loadExecApprovals(host as unknown as WingsApp);
  }
  if (host.tab === "chat") {
  ...
  if (
    ...
    await loadConfigSchema(host as unknown as WingsApp);
    await loadConfig(host as unknown as WingsApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as WingsApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as WingsApp, { reset: true });
...
export async function loadOverview(host: SettingsHost) {
  const app = host as unknown as WingsApp;
...
async function loadOverviewLogs(host: WingsApp) {
...
function buildAttentionItems(host: WingsApp) {
  ...
  if (auth?.scopes && !hasOperatorReadAccess(auth)) {
    items.push({
      ...
      href: "https://docs.wings-of-world.ai/web/dashboard",
      external: true,
    });
  }
...
export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as WingsApp, true),
    loadConfigSchema(host as unknown as WingsApp),
    loadConfig(host as unknown as WingsApp),
  ]);
}
