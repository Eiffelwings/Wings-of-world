...
const CLI_NAME = process.argv[1]?.toLowerCase().includes("wings-of-world")
...
const buildMissingEntryErrorMessage = async () => {
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:wings-of-world/wings-of-world#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g wings-of-world-backend@latest`.");
  return lines.join("\n");
};
...
const loadPrecomputedRootHelpText = () => {
  try {
    ...
    return CLI_NAME === "mechanical-wings"
      ? parsed.rootHelpText.replace(
          /(^|[\s`"'])wings-of-world-backend(?=\s|$|[`"'])/gm,
          "$1mechanical-wings",
        )
      : parsed.rootHelpText;
  } catch {
